    ...
    <el-table v-if="isDisplay === true"
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('wyyz')">
                <el-table-column show-overflow-tooltip :prop="'wyyz'" :label="$t('entities.pimlanguageability.main_grid.columns.wyyz')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimlanguageability.main_grid.columns.wyyz')}}
                      </span>
                    </template>
            ...
            <template v-if="getColumnState('wydj')">
                <el-table-column show-overflow-tooltip :prop="'wydj'" :label="$t('entities.pimlanguageability.main_grid.columns.wydj')" :width="300"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimlanguageability.main_grid.columns.wydj')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template v-if="actualIsOpenEdit">
            ...
            <template v-if="getColumnState('wydjhqsj')">
                <el-table-column show-overflow-tooltip :prop="'wydjhqsj'" :label="$t('entities.pimlanguageability.main_grid.columns.wydjhqsj')" :width="200"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimlanguageability.main_grid.columns.wydjhqsj')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template v-if="actualIsOpenEdit">
            ...
            <template v-if="getColumnState('fj')">
                <el-table-column show-overflow-tooltip :prop="'fj'" :label="$t('entities.pimlanguageability.main_grid.columns.fj')" :width="200"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimlanguageability.main_grid.columns.fj')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template v-if="actualIsOpenEdit">
...
export default class MainBase extends Vue implements ControlInterface {
    ...
    */
    ...
    public allColumns: any[] = [
        {
            name: 'wyyz',
            label: '语种',
            langtag: 'entities.pimlanguageability.main_grid.columns.wyyz',
            show: true,
            util: 'PX'
        },
        {
            name: 'wydj',
            label: '外语等级',
            langtag: 'entities.pimlanguageability.main_grid.columns.wydj',
            show: true,
            util: 'PX'
        },
        {
            name: 'wydjhqsj',
            label: '外语等级获取时间',
            langtag: 'entities.pimlanguageability.main_grid.columns.wydjhqsj',
            show: true,
            util: 'PX'
        },
        {
            name: 'fj',
            label: '附件',
            langtag: 'entities.pimlanguageability.main_grid.columns.fj',
            show: true,
            util: 'PX'
        },
    ]
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'PIMLANGUAGEABILITYYYNLGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'PIMLANGUAGEABILITYYYNLGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('pimlanguageability_main_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('pimlanguageability_main_grid', JSON.stringify(this.allColumns));
    }
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: '错误', desc: 'PIMLANGUAGEABILITYYYNLGridView视图表格createAction参数未配置' });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: '错误', desc: 'PIMLANGUAGEABILITYYYNLGridView视图表格updateAction参数未配置' });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'PIMLANGUAGEABILITYYYNLGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
