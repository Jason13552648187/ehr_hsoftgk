        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
                ...
                <el-table-column show-overflow-tooltip :prop="'ordervalue'" :label="$t('entities.ormorgsector.bmxmjc_grid.columns.ordervalue')" :width="100"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.ormorgsector.bmxmjc_grid.columns.ordervalue')}}
                      </span>
                    </template>
            ...
            <template v-if="getColumnState('orgsectorname')">
                <el-table-column show-overflow-tooltip :prop="'orgsectorname'" :label="$t('entities.ormorgsector.bmxmjc_grid.columns.orgsectorname')" :width="400"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.ormorgsector.bmxmjc_grid.columns.orgsectorname')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('orgcode')">
                <el-table-column show-overflow-tooltip :prop="'orgcode'" :label="$t('entities.ormorgsector.bmxmjc_grid.columns.orgcode')" :width="120"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.ormorgsector.bmxmjc_grid.columns.orgcode')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('shortname')">
                <el-table-column show-overflow-tooltip :prop="'shortname'" :label="$t('entities.ormorgsector.bmxmjc_grid.columns.shortname')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.ormorgsector.bmxmjc_grid.columns.shortname')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('orgname')">
                <el-table-column show-overflow-tooltip :prop="'orgname'" :label="$t('entities.ormorgsector.bmxmjc_grid.columns.orgname')" :width="250"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.ormorgsector.bmxmjc_grid.columns.orgname')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('belongregion')">
                <el-table-column show-overflow-tooltip :prop="'belongregion'" :label="$t('entities.ormorgsector.bmxmjc_grid.columns.belongregion')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.ormorgsector.bmxmjc_grid.columns.belongregion')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
            ...
            <template v-if="getColumnState('qy')">
                <el-table-column show-overflow-tooltip :prop="'qy'" :label="$t('entities.ormorgsector.bmxmjc_grid.columns.qy')" :width="130"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.ormorgsector.bmxmjc_grid.columns.qy')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
...
export default class BMXMJCBase extends Vue implements ControlInterface {
    ...
    */
    ...
    public allColumns: any[] = [
        {
            name: 'ordervalue',
            label: '排序',
            langtag: 'entities.ormorgsector.bmxmjc_grid.columns.ordervalue',
            show: true,
            util: 'PX'
        },
        {
            name: 'orgsectorname',
            label: '部门/项目',
            langtag: 'entities.ormorgsector.bmxmjc_grid.columns.orgsectorname',
            show: true,
            util: 'PX'
        },
        {
            name: 'orgcode',
            label: '编号',
            langtag: 'entities.ormorgsector.bmxmjc_grid.columns.orgcode',
            show: true,
            util: 'PX'
        },
        {
            name: 'shortname',
            label: '简称',
            langtag: 'entities.ormorgsector.bmxmjc_grid.columns.shortname',
            show: true,
            util: 'PX'
        },
        {
            name: 'orgname',
            label: '所属组织',
            langtag: 'entities.ormorgsector.bmxmjc_grid.columns.orgname',
            show: true,
            util: 'PX'
        },
        {
            name: 'belongregion',
            label: '所属区域',
            langtag: 'entities.ormorgsector.bmxmjc_grid.columns.belongregion',
            show: true,
            util: 'PX'
        },
        {
            name: 'qy',
            label: '补贴标准',
            langtag: 'entities.ormorgsector.bmxmjc_grid.columns.qy',
            show: true,
            util: 'PX'
        },
    ]
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'ORMORGSECTORFPXXPickupGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'ORMORGSECTORFPXXPickupGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: '错误', desc: 'ORMORGSECTORFPXXPickupGridView视图表格createAction参数未配置' });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: '错误', desc: 'ORMORGSECTORFPXXPickupGridView视图表格updateAction参数未配置' });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'ORMORGSECTORFPXXPickupGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
