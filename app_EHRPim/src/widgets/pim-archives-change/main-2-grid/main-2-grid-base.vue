    ...
    <el-table v-if="isDisplay === true"
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('ygbh')">
                <el-table-column show-overflow-tooltip :prop="'ygbh'" :label="$t('entities.pimarchiveschange.main_2_grid.columns.ygbh')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimarchiveschange.main_2_grid.columns.ygbh')}}
                      </span>
                    </template>
            ...
            <template v-if="getColumnState('pimpersonname')">
                <el-table-column show-overflow-tooltip :prop="'pimpersonname'" :label="$t('entities.pimarchiveschange.main_2_grid.columns.pimpersonname')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimarchiveschange.main_2_grid.columns.pimpersonname')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('ormorgname')">
                <el-table-column show-overflow-tooltip :prop="'ormorgname'" :label="$t('entities.pimarchiveschange.main_2_grid.columns.ormorgname')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimarchiveschange.main_2_grid.columns.ormorgname')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('ormorgname2')">
                <el-table-column show-overflow-tooltip :prop="'ormorgname2'" :label="$t('entities.pimarchiveschange.main_2_grid.columns.ormorgname2')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimarchiveschange.main_2_grid.columns.ormorgname2')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('jlcssj')">
                <el-table-column show-overflow-tooltip :prop="'jlcssj'" :label="$t('entities.pimarchiveschange.main_2_grid.columns.jlcssj')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimarchiveschange.main_2_grid.columns.jlcssj')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
...
export default class Main_2Base extends Vue implements ControlInterface {
     ...
     * @memberof Main_2Base
     */
    public appEntityService: PimArchivesChangeService = new PimArchivesChangeService({ $store: this.$store });
     ...
     * @memberof Main_2Base
     */
    public minorSortPSDEF: string = 'daddrq';
    ...
    */
    ...
    public allColumns: any[] = [
        {
            name: 'ygbh',
            label: '员工编号',
            langtag: 'entities.pimarchiveschange.main_2_grid.columns.ygbh',
            show: true,
            util: 'px'
        },
        {
            name: 'pimpersonname',
            label: '员工姓名',
            langtag: 'entities.pimarchiveschange.main_2_grid.columns.pimpersonname',
            show: true,
            util: 'px'
        },
        {
            name: 'ormorgname',
            label: '调出单位',
            langtag: 'entities.pimarchiveschange.main_2_grid.columns.ormorgname',
            show: true,
            util: 'px'
        },
        {
            name: 'ormorgname2',
            label: '调入单位',
            langtag: 'entities.pimarchiveschange.main_2_grid.columns.ormorgname2',
            show: true,
            util: 'px'
        },
        {
            name: 'jlcssj',
            label: '调动时间',
            langtag: 'entities.pimarchiveschange.main_2_grid.columns.jlcssj',
            show: true,
            util: 'PX'
        },
    ]
    ...
    public rules: any = {
        ...
        srfkey: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '档案归档地变更记录标识 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '档案归档地变更记录标识 值不能为空', trigger: 'blur' },
        ],
    }
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'PIMARCHIVESCHANGEDADCJLGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'PIMARCHIVESCHANGEDADCJLGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        _datas.forEach((record: any, index: number) => {
            let srfmajortext = record.pimarchiveschangename;
            if (index < 5) {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ pimarchiveschange: _keys }),Object.assign({ pimarchiveschange: _keys },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    public rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.pimarchiveschange,$event.pimarchiveschange);
        });
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('pimarchiveschange_main_2_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('pimarchiveschange_main_2_grid', JSON.stringify(this.allColumns));
    }
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: '错误', desc: 'PIMARCHIVESCHANGEDADCJLGridView视图表格createAction参数未配置' });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: '错误', desc: 'PIMARCHIVESCHANGEDADCJLGridView视图表格updateAction参数未配置' });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
                        if(item.pimarchiveschange){
                            Object.assign(this.context,{pimarchiveschange:item.pimarchiveschange});
                        }
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'PIMARCHIVESCHANGEDADCJLGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
    ...
    public getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.pimarchiveschange,args.row.pimarchiveschange);
        });
