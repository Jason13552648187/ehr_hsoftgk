        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
                ...
                <el-table-column show-overflow-tooltip :prop="'ybrgx'" :label="$t('entities.pimfaminfo.main_grid.columns.ybrgx')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimfaminfo.main_grid.columns.ybrgx')}}
                      </span>
                    </template>
            ...
            <template v-if="getColumnState('pimfaminfoname')">
                <el-table-column show-overflow-tooltip :prop="'pimfaminfoname'" :label="$t('entities.pimfaminfo.main_grid.columns.pimfaminfoname')" :width="110"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimfaminfo.main_grid.columns.pimfaminfoname')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('xb')">
                <el-table-column show-overflow-tooltip :prop="'xb'" :label="$t('entities.pimfaminfo.main_grid.columns.xb')" :width="80"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimfaminfo.main_grid.columns.xb')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
            ...
            <template v-if="getColumnState('nl')">
                <el-table-column show-overflow-tooltip :prop="'nl'" :label="$t('entities.pimfaminfo.main_grid.columns.nl')" :width="80"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimfaminfo.main_grid.columns.nl')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.nl}}</span>
            ...
            <template v-if="getColumnState('gzdw')">
                <el-table-column show-overflow-tooltip :prop="'gzdw'" :label="$t('entities.pimfaminfo.main_grid.columns.gzdw')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimfaminfo.main_grid.columns.gzdw')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.gzdw}}</span>
            ...
            <template v-if="getColumnState('zw')">
                <el-table-column show-overflow-tooltip :prop="'zw'" :label="$t('entities.pimfaminfo.main_grid.columns.zw')" :width="100"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimfaminfo.main_grid.columns.zw')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.zw}}</span>
            ...
            <template v-if="getColumnState('zzmm')">
                <el-table-column show-overflow-tooltip :prop="'zzmm'" :label="$t('entities.pimfaminfo.main_grid.columns.zzmm')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimfaminfo.main_grid.columns.zzmm')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
            ...
            <template v-if="getColumnState('zjh')">
                <el-table-column show-overflow-tooltip :prop="'zjh'" :label="$t('entities.pimfaminfo.main_grid.columns.zjh')" :width="200"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimfaminfo.main_grid.columns.zjh')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.zjh}}</span>
            ...
            <template v-if="getColumnState('csrq')">
                <el-table-column show-overflow-tooltip :prop="'csrq'" :label="$t('entities.pimfaminfo.main_grid.columns.csrq')" :width="120"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimfaminfo.main_grid.columns.csrq')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('sruvivors')">
                <el-table-column show-overflow-tooltip :prop="'sruvivors'" :label="$t('entities.pimfaminfo.main_grid.columns.sruvivors')" :width="120"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimfaminfo.main_grid.columns.sruvivors')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
            <codelist :value="row.sruvivors" tag='EhrCodeList0401' codelistType='STATIC' renderMode="STR" valueSeparator=";" textSeparator="、" ></codelist>
                        </template>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('sfjjllr')">
                <el-table-column show-overflow-tooltip :prop="'sfjjllr'" :label="$t('entities.pimfaminfo.main_grid.columns.sfjjllr')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimfaminfo.main_grid.columns.sfjjllr')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
...
export default class MainBase extends Vue implements ControlInterface {
    ...
    */
    ...
    public allColumns: any[] = [
        {
            name: 'ybrgx',
            label: '与本人关系',
            langtag: 'entities.pimfaminfo.main_grid.columns.ybrgx',
            show: true,
            util: 'PX'
        },
        {
            name: 'pimfaminfoname',
            label: '姓名',
            langtag: 'entities.pimfaminfo.main_grid.columns.pimfaminfoname',
            show: true,
            util: 'PX'
        },
        {
            name: 'xb',
            label: '性别',
            langtag: 'entities.pimfaminfo.main_grid.columns.xb',
            show: true,
            util: 'PX'
        },
        {
            name: 'nl',
            label: '年龄',
            langtag: 'entities.pimfaminfo.main_grid.columns.nl',
            show: true,
            util: 'PX'
        },
        {
            name: 'gzdw',
            label: '工作单位',
            langtag: 'entities.pimfaminfo.main_grid.columns.gzdw',
            show: true,
            util: 'px'
        },
        {
            name: 'zw',
            label: '职务',
            langtag: 'entities.pimfaminfo.main_grid.columns.zw',
            show: true,
            util: 'PX'
        },
        {
            name: 'zzmm',
            label: '政治面貌',
            langtag: 'entities.pimfaminfo.main_grid.columns.zzmm',
            show: true,
            util: 'px'
        },
        {
            name: 'zjh',
            label: '证件号',
            langtag: 'entities.pimfaminfo.main_grid.columns.zjh',
            show: true,
            util: 'PX'
        },
        {
            name: 'csrq',
            label: '出生日期',
            langtag: 'entities.pimfaminfo.main_grid.columns.csrq',
            show: true,
            util: 'PX'
        },
        {
            name: 'sruvivors',
            label: '是否遗属',
            langtag: 'entities.pimfaminfo.main_grid.columns.sruvivors',
            show: true,
            util: 'PX'
        },
        {
            name: 'sfjjllr',
            label: '是否紧急联络人',
            langtag: 'entities.pimfaminfo.main_grid.columns.sfjjllr',
            show: true,
            util: 'PX'
        },
    ]
    ...
    public rules: any = {
        sruvivors: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '是否遗属 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '是否遗属 值不能为空', trigger: 'blur' },
        ],
        srfkey: [
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'PIMFAMINFOPimfimGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'PIMFAMINFOPimfimGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('pimfaminfo_main_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('pimfaminfo_main_grid', JSON.stringify(this.allColumns));
    }
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: '错误', desc: 'PIMFAMINFOPimfimGridView视图表格createAction参数未配置' });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: '错误', desc: 'PIMFAMINFOPimfimGridView视图表格updateAction参数未配置' });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
                        if(item.pimfaminfo){
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'PIMFAMINFOPimfimGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
