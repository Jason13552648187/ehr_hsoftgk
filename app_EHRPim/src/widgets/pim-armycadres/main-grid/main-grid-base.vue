    ...
    <el-table v-if="isDisplay === true"
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('trooptype')">
                <el-table-column show-overflow-tooltip :prop="'trooptype'" :label="$t('entities.pimarmycadres.main_grid.columns.trooptype')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimarmycadres.main_grid.columns.trooptype')}}
                      </span>
                    </template>
             ...
             <dropdown-list 
              ...
              @change="($event)=>{gridEditItemChange(row, column.property, $event, $index)}">
                        ...
                        <template v-if="!actualIsOpenEdit">
            <codelist :value="row.trooptype" tag='EhrCodeList0035' codelistType='DYNAMIC' ></codelist>
                        </template>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('leveltype')">
                <el-table-column show-overflow-tooltip :prop="'leveltype'" :label="$t('entities.pimarmycadres.main_grid.columns.leveltype')" :width="250"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimarmycadres.main_grid.columns.leveltype')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template v-if="actualIsOpenEdit">
             ...
             <dropdown-list 
              ...
              @change="($event)=>{gridEditItemChange(row, column.property, $event, $index)}">
                        ...
                        <template v-if="!actualIsOpenEdit">
            <codelist :value="row.leveltype" tag='EhrCodeList0198' codelistType='DYNAMIC' renderMode="STR" valueSeparator=";" textSeparator="、" ></codelist>
                        </template>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('fssj')">
                <el-table-column show-overflow-tooltip :prop="'fssj'" :label="$t('entities.pimarmycadres.main_grid.columns.fssj')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimarmycadres.main_grid.columns.fssj')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template v-if="actualIsOpenEdit">
            ...
            <template v-if="getColumnState('fj')">
                <el-table-column show-overflow-tooltip :prop="'fj'" :label="$t('entities.pimarmycadres.main_grid.columns.fj')" :width="200"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimarmycadres.main_grid.columns.fj')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template v-if="actualIsOpenEdit">
...
export default class MainBase extends Vue implements ControlInterface {
    ...
    */
    ...
    public allColumns: any[] = [
        {
            name: 'trooptype',
            label: '类型(*)',
            langtag: 'entities.pimarmycadres.main_grid.columns.trooptype',
            show: true,
            util: 'PX'
        },
        {
            name: 'leveltype',
            label: '级别(*)',
            langtag: 'entities.pimarmycadres.main_grid.columns.leveltype',
            show: true,
            util: 'PX'
        },
        {
            name: 'fssj',
            label: '发生时间(*)',
            langtag: 'entities.pimarmycadres.main_grid.columns.fssj',
            show: true,
            util: 'PX'
        },
        {
            name: 'fj',
            label: '附件',
            langtag: 'entities.pimarmycadres.main_grid.columns.fj',
            show: true,
            util: 'PX'
        },
    ]
    ...
    public getGridRowModel(){
        return {
          trooptype: new FormItemModel(),
          jlczz: new FormItemModel(),
          fj: new FormItemModel(),
          pimpersonid: new FormItemModel(),
          fssj: new FormItemModel(),
          srfkey: new FormItemModel(),
          leveltype: new FormItemModel(),
        }
    }
    ...
    public rules: any = {
        trooptype: [
             { required: true, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '类型(*) 值不能为空', trigger: 'change' },
            { required: true, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '类型(*) 值不能为空', trigger: 'blur' },
        ],
        jlczz: [
        ...
        fssj: [
             { required: true, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '发生时间(*) 值不能为空', trigger: 'change' },
            { required: true, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '发生时间(*) 值不能为空', trigger: 'blur' },
        ],
        srfkey: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '军转干部标识 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '军转干部标识 值不能为空', trigger: 'blur' },
        ],
        leveltype: [
             { required: true, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '级别(*) 值不能为空', trigger: 'change' },
            { required: true, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '级别(*) 值不能为空', trigger: 'blur' },
        ],
    }
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'PIMARMYCADRESARMYGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'PIMARMYCADRESARMYGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        _datas.forEach((record: any, index: number) => {
            let srfmajortext = record.pimarmycadresname;
            if (index < 5) {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ pimarmycadres: _keys }),Object.assign({ pimarmycadres: _keys },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    public rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.pimarmycadres,$event.pimarmycadres);
        });
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('pimarmycadres_main_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('pimarmycadres_main_grid', JSON.stringify(this.allColumns));
    }
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: '错误', desc: 'PIMARMYCADRESARMYGridView视图表格createAction参数未配置' });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: '错误', desc: 'PIMARMYCADRESARMYGridView视图表格updateAction参数未配置' });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
                        if(item.pimarmycadres){
                            Object.assign(this.context,{pimarmycadres:item.pimarmycadres});
                        }
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'PIMARMYCADRESARMYGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
    ...
    public getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.pimarmycadres,args.row.pimarmycadres);
        });
