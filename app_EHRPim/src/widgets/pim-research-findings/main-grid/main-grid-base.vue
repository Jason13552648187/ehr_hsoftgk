        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
                ...
                <el-table-column show-overflow-tooltip :prop="'pimresearchfindingsname'" :label="$t('entities.pimresearchfindings.main_grid.columns.pimresearchfindingsname')" :width="200"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimresearchfindings.main_grid.columns.pimresearchfindingsname')}}
                      </span>
                    </template>
            ...
            <template v-if="getColumnState('hqsj')">
                <el-table-column show-overflow-tooltip :prop="'hqsj'" :label="$t('entities.pimresearchfindings.main_grid.columns.hqsj')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimresearchfindings.main_grid.columns.hqsj')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template v-if="actualIsOpenEdit">
            ...
            <template v-if="getColumnState('fj')">
                <el-table-column show-overflow-tooltip :prop="'fj'" :label="$t('entities.pimresearchfindings.main_grid.columns.fj')" :width="200"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimresearchfindings.main_grid.columns.fj')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template v-if="actualIsOpenEdit">
...
export default class MainBase extends Vue implements ControlInterface {
    ...
    */
    ...
    public allColumns: any[] = [
        {
            ...
            label: '科研成果名称(*)',
            langtag: 'entities.pimresearchfindings.main_grid.columns.pimresearchfindingsname',
            show: true,
            util: 'PX'
        },
        {
            name: 'hqsj',
            label: '获取时间(*)',
            langtag: 'entities.pimresearchfindings.main_grid.columns.hqsj',
            show: true,
            util: 'PX'
        },
        {
            name: 'fj',
            label: '附件',
            langtag: 'entities.pimresearchfindings.main_grid.columns.fj',
            show: true,
            util: 'PX'
        },
    ]
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'PIMRESEARCHFINDINGSRESEARCHGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'PIMRESEARCHFINDINGSRESEARCHGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('pimresearchfindings_main_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('pimresearchfindings_main_grid', JSON.stringify(this.allColumns));
    }
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: '错误', desc: 'PIMRESEARCHFINDINGSRESEARCHGridView视图表格createAction参数未配置' });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: '错误', desc: 'PIMRESEARCHFINDINGSRESEARCHGridView视图表格updateAction参数未配置' });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'PIMRESEARCHFINDINGSRESEARCHGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
