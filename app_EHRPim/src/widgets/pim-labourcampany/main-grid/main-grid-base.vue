        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
                ...
                <el-table-column show-overflow-tooltip :prop="'pimlabourcampanyname'" :label="$t('entities.pimlabourcampany.main_grid.columns.pimlabourcampanyname')" :width="300"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimlabourcampany.main_grid.columns.pimlabourcampanyname')}}
                      </span>
                    </template>
            ...
            <template v-if="getColumnState('zzdzs')">
                <el-table-column show-overflow-tooltip :prop="'zzdzs'" :label="$t('entities.pimlabourcampany.main_grid.columns.zzdzs')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimlabourcampany.main_grid.columns.zzdzs')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.zzdzs}}</span>
            ...
            <template v-if="getColumnState('lxdz')">
                <el-table-column show-overflow-tooltip :prop="'lxdz'" :label="$t('entities.pimlabourcampany.main_grid.columns.lxdz')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimlabourcampany.main_grid.columns.lxdz')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.lxdz}}</span>
            ...
            <template v-if="getColumnState('lxr')">
                <el-table-column show-overflow-tooltip :prop="'lxr'" :label="$t('entities.pimlabourcampany.main_grid.columns.lxr')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimlabourcampany.main_grid.columns.lxr')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.lxr}}</span>
            ...
            <template v-if="getColumnState('lxfs')">
                <el-table-column show-overflow-tooltip :prop="'lxfs'" :label="$t('entities.pimlabourcampany.main_grid.columns.lxfs')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimlabourcampany.main_grid.columns.lxfs')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.lxfs}}</span>
            ...
            <template v-if="getColumnState('regcapital')">
                <el-table-column show-overflow-tooltip :prop="'regcapital'" :label="$t('entities.pimlabourcampany.main_grid.columns.regcapital')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimlabourcampany.main_grid.columns.regcapital')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('legalperosn')">
                <el-table-column show-overflow-tooltip :prop="'legalperosn'" :label="$t('entities.pimlabourcampany.main_grid.columns.legalperosn')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimlabourcampany.main_grid.columns.legalperosn')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('updatedate')">
                <el-table-column show-overflow-tooltip :prop="'updatedate'" :label="$t('entities.pimlabourcampany.main_grid.columns.updatedate')" :width="200"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimlabourcampany.main_grid.columns.updatedate')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
...
export default class MainBase extends Vue implements ControlInterface {
     ...
     * @memberof MainBase
     */
    public appEntityService: PimLabourcampanyService = new PimLabourcampanyService({ $store: this.$store });
    ...
    */
    ...
    public allColumns: any[] = [
        {
            ...
            label: '公司名称',
            langtag: 'entities.pimlabourcampany.main_grid.columns.pimlabourcampanyname',
            show: true,
            util: 'PX'
        },
        {
            name: 'zzdzs',
            label: '所属单位',
            langtag: 'entities.pimlabourcampany.main_grid.columns.zzdzs',
            show: true,
            util: 'px'
        },
        {
            name: 'lxdz',
            label: '联系地址',
            langtag: 'entities.pimlabourcampany.main_grid.columns.lxdz',
            show: true,
            util: 'px'
        },
        {
            name: 'lxr',
            label: '联系人',
            langtag: 'entities.pimlabourcampany.main_grid.columns.lxr',
            show: true,
            util: 'px'
        },
        {
            name: 'lxfs',
            label: '联系电话',
            langtag: 'entities.pimlabourcampany.main_grid.columns.lxfs',
            show: true,
            util: 'px'
        },
        {
            name: 'regcapital',
            label: '注册资本金',
            langtag: 'entities.pimlabourcampany.main_grid.columns.regcapital',
            show: true,
            util: 'px'
        },
        {
            name: 'legalperosn',
            label: '法人',
            langtag: 'entities.pimlabourcampany.main_grid.columns.legalperosn',
            show: true,
            util: 'px'
        },
        {
            name: 'updatedate',
            label: '更新时间',
            langtag: 'entities.pimlabourcampany.main_grid.columns.updatedate',
            show: true,
            util: 'PX'
        },
    ]
    ...
    public rules: any = {
        srfkey: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '劳务派遣公司管理标识 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '劳务派遣公司管理标识 值不能为空', trigger: 'blur' },
        ],
    }
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'PIMLABOURCAMPANYGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'PIMLABOURCAMPANYGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        _datas.forEach((record: any, index: number) => {
            let srfmajortext = record.pimlabourcampanyname;
            if (index < 5) {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ pimlabourcampany: _keys }),Object.assign({ pimlabourcampany: _keys },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    public rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.pimlabourcampany,$event.pimlabourcampany);
        });
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('pimlabourcampany_main_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('pimlabourcampany_main_grid', JSON.stringify(this.allColumns));
    }
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: '错误', desc: 'PIMLABOURCAMPANYGridView视图表格createAction参数未配置' });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: '错误', desc: 'PIMLABOURCAMPANYGridView视图表格updateAction参数未配置' });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
                        if(item.pimlabourcampany){
                            Object.assign(this.context,{pimlabourcampany:item.pimlabourcampany});
                        }
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'PIMLABOURCAMPANYGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
    ...
    public getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.pimlabourcampany,args.row.pimlabourcampany);
        });
