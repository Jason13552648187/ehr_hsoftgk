    ...
    <el-table v-if="isDisplay === true"
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('typecode')">
                <el-table-column show-overflow-tooltip :prop="'typecode'" :label="$t('entities.pimtypecontract.main_grid.columns.typecode')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimtypecontract.main_grid.columns.typecode')}}
                      </span>
                    </template>
            ...
            <template v-if="getColumnState('pimtypecontractname')">
                <el-table-column show-overflow-tooltip :prop="'pimtypecontractname'" :label="$t('entities.pimtypecontract.main_grid.columns.pimtypecontractname')" :width="250"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimtypecontract.main_grid.columns.pimtypecontractname')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template v-if="actualIsOpenEdit">
              ...
              @change="($event)=>{gridEditItemChange(row, column.property, $event, $index)}">
                        ...
                        <template v-if="!actualIsOpenEdit">
                                <app-span name='pimtypecontractname' editorType="TEXTBOX" :value="row.pimtypecontractname"></app-span>
                        </template>
                    </template>
                </el-table-column>
            </template>
...
export default class MainBase extends Vue implements ControlInterface {
     ...
     * @memberof MainBase
     */
    public appEntityService: PimTypeContractService = new PimTypeContractService({ $store: this.$store });
    ...
    */
    ...
    public allColumns: any[] = [
        {
            name: 'typecode',
            label: '合同类型代码',
            langtag: 'entities.pimtypecontract.main_grid.columns.typecode',
            show: true,
            util: 'PX'
        },
        {
            ...
            label: '合同类型名称',
            langtag: 'entities.pimtypecontract.main_grid.columns.pimtypecontractname',
            show: true,
            util: 'PX'
        },
    ]
    ...
    public getGridRowModel(){
        return {
          pimtypecontractname: new FormItemModel(),
    ...
    public rules: any = {
        pimtypecontractname: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '合同类型名称 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '合同类型名称 值不能为空', trigger: 'blur' },
        ],
        typecode: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '合同类型代码 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '合同类型代码 值不能为空', trigger: 'blur' },
        ],
        srfkey: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '合同类型标识 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '合同类型标识 值不能为空', trigger: 'blur' },
        ],
    }
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'PIMTYPECONTRACTGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'PIMTYPECONTRACTGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        _datas.forEach((record: any, index: number) => {
            let srfmajortext = record.pimtypecontractname;
            if (index < 5) {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ pimtypecontract: _keys }),Object.assign({ pimtypecontract: _keys },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    public rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.pimtypecontract,$event.pimtypecontract);
        });
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('pimtypecontract_main_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('pimtypecontract_main_grid', JSON.stringify(this.allColumns));
    }
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: '错误', desc: 'PIMTYPECONTRACTGridView视图表格createAction参数未配置' });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: '错误', desc: 'PIMTYPECONTRACTGridView视图表格updateAction参数未配置' });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
                        if(item.pimtypecontract){
                            Object.assign(this.context,{pimtypecontract:item.pimtypecontract});
                        }
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'PIMTYPECONTRACTGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
    ...
    public getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.pimtypecontract,args.row.pimtypecontract);
        });
