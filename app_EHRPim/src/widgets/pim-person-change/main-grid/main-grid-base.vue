        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
                ...
                <el-table-column show-overflow-tooltip :prop="'ygbh'" :label="$t('entities.pimpersonchange.main_grid.columns.ygbh')" :width="130"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimpersonchange.main_grid.columns.ygbh')}}
                      </span>
                    </template>
            ...
            <template v-if="getColumnState('pimpersonname')">
                <el-table-column show-overflow-tooltip :prop="'pimpersonname'" :label="$t('entities.pimpersonchange.main_grid.columns.pimpersonname')" :width="130"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimpersonchange.main_grid.columns.pimpersonname')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('bgnr')">
                <el-table-column show-overflow-tooltip :prop="'bgnr'" :label="$t('entities.pimpersonchange.main_grid.columns.bgnr')" :min-width="800"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimpersonchange.main_grid.columns.bgnr')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.bgnr}}</span>
            ...
            <template v-if="getColumnState('fj')">
                <el-table-column show-overflow-tooltip :prop="'fj'" :label="$t('entities.pimpersonchange.main_grid.columns.fj')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimpersonchange.main_grid.columns.fj')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.fj}}</span>
            ...
            <template v-if="getColumnState('bglx')">
                <el-table-column show-overflow-tooltip :prop="'bglx'" :label="$t('entities.pimpersonchange.main_grid.columns.bglx')" :width="130"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimpersonchange.main_grid.columns.bglx')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
            ...
            <template v-if="getColumnState('sqsj')">
                <el-table-column show-overflow-tooltip :prop="'sqsj'" :label="$t('entities.pimpersonchange.main_grid.columns.sqsj')" :width="130"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimpersonchange.main_grid.columns.sqsj')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('uagridcolumn1')">
                <el-table-column :column-key="'uagridcolumn1'" :label="$t('entities.pimpersonchange.main_grid.columns.uagridcolumn1')" :width="100"  :align="'center'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimpersonchange.main_grid.columns.uagridcolumn1')}}
                      </span>
                    </template>
...
export default class MainBase extends Vue implements ControlInterface {
     ...
     * @memberof 
     */
    public grid_uagridcolumn1_ub39742b_click(params: any = {}, tag?: any, $event?: any) {
        // 取数
        let datas: any[] = [];
        let xData: any = null;
        // _this 指向容器对象
        ...
        curUIService.PimPersonChange_Refuse(datas,contextJO, paramJO,  $event, xData,this,"PimPersonChange");
    }
     ...
     * @memberof MainBase
     */
    public minorSortPSDEF: string = 'sqsj';
    ...
    */
    ...
    public allColumns: any[] = [
        {
            name: 'ygbh',
            label: '员工编号',
            langtag: 'entities.pimpersonchange.main_grid.columns.ygbh',
            show: true,
            util: 'PX'
        },
        {
            name: 'pimpersonname',
            label: '员工姓名',
            langtag: 'entities.pimpersonchange.main_grid.columns.pimpersonname',
            show: true,
            util: 'PX'
        },
        {
            name: 'bgnr',
            label: '变更内容',
            langtag: 'entities.pimpersonchange.main_grid.columns.bgnr',
            show: true,
            util: 'STAR'
        },
        {
            name: 'fj',
            label: '附件信息',
            langtag: 'entities.pimpersonchange.main_grid.columns.fj',
            show: true,
            util: 'PX'
        },
        {
            name: 'bglx',
            label: '变更类型',
            langtag: 'entities.pimpersonchange.main_grid.columns.bglx',
            show: true,
            util: 'PX'
        },
        {
            name: 'sqsj',
            label: '申请时间',
            langtag: 'entities.pimpersonchange.main_grid.columns.sqsj',
            show: true,
            util: 'PX'
        },
        {
            name: 'uagridcolumn1',
            label: '操作列',
            langtag: 'entities.pimpersonchange.main_grid.columns.uagridcolumn1',
            show: true,
            util: 'PX'
        },
    ]
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'PIMPERSONCHANGEGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'PIMPERSONCHANGEGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('pimpersonchange_main_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('pimpersonchange_main_grid', JSON.stringify(this.allColumns));
    }
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: '错误', desc: 'PIMPERSONCHANGEGridView视图表格createAction参数未配置' });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: '错误', desc: 'PIMPERSONCHANGEGridView视图表格updateAction参数未配置' });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'PIMPERSONCHANGEGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
