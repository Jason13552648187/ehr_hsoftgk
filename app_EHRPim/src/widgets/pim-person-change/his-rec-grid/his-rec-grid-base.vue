    ...
    <el-table v-if="isDisplay === true"
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('ygbh')">
                <el-table-column show-overflow-tooltip :prop="'ygbh'" :label="$t('entities.pimpersonchange.hisrec_grid.columns.ygbh')" :width="130"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimpersonchange.hisrec_grid.columns.ygbh')}}
                      </span>
                    </template>
            ...
            <template v-if="getColumnState('pimpersonname')">
                <el-table-column show-overflow-tooltip :prop="'pimpersonname'" :label="$t('entities.pimpersonchange.hisrec_grid.columns.pimpersonname')" :width="130"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimpersonchange.hisrec_grid.columns.pimpersonname')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('bgnr')">
                <el-table-column show-overflow-tooltip :prop="'bgnr'" :label="$t('entities.pimpersonchange.hisrec_grid.columns.bgnr')" :min-width="400"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimpersonchange.hisrec_grid.columns.bgnr')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.bgnr}}</span>
            ...
            <template v-if="getColumnState('fj')">
                <el-table-column show-overflow-tooltip :prop="'fj'" :label="$t('entities.pimpersonchange.hisrec_grid.columns.fj')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimpersonchange.hisrec_grid.columns.fj')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.fj}}</span>
            ...
            <template v-if="getColumnState('sqsj')">
                <el-table-column show-overflow-tooltip :prop="'sqsj'" :label="$t('entities.pimpersonchange.hisrec_grid.columns.sqsj')" :width="130"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimpersonchange.hisrec_grid.columns.sqsj')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('bglx')">
                <el-table-column show-overflow-tooltip :prop="'bglx'" :label="$t('entities.pimpersonchange.hisrec_grid.columns.bglx')" :width="130"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimpersonchange.hisrec_grid.columns.bglx')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
            <codelist :value="row.bglx" tag='EhrCodeList0232' codelistType='STATIC' ></codelist>
                        </template>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('reason')">
                <el-table-column show-overflow-tooltip :prop="'reason'" :label="$t('entities.pimpersonchange.hisrec_grid.columns.reason')" :width="200"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimpersonchange.hisrec_grid.columns.reason')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('zt')">
                <el-table-column show-overflow-tooltip :prop="'zt'" :label="$t('entities.pimpersonchange.hisrec_grid.columns.zt')" :width="120"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimpersonchange.hisrec_grid.columns.zt')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
            <codelist :value="row.zt" tag='EhrCodeList0127' codelistType='STATIC' ></codelist>
                        </template>
                    </template>
                </el-table-column>
            </template>
...
import HisRecService from './his-rec-grid-service';
...
export default class HisRecBase extends Vue implements ControlInterface {
    ...
    */
    ...
    public allColumns: any[] = [
        {
            name: 'ygbh',
            label: '员工编号',
            langtag: 'entities.pimpersonchange.hisrec_grid.columns.ygbh',
            show: true,
            util: 'PX'
        },
        {
            name: 'pimpersonname',
            label: '员工姓名',
            langtag: 'entities.pimpersonchange.hisrec_grid.columns.pimpersonname',
            show: true,
            util: 'PX'
        },
        {
            name: 'bgnr',
            label: '变更内容',
            langtag: 'entities.pimpersonchange.hisrec_grid.columns.bgnr',
            show: true,
            util: 'STAR'
        },
        {
            name: 'fj',
            label: '附件信息',
            langtag: 'entities.pimpersonchange.hisrec_grid.columns.fj',
            show: true,
            util: 'PX'
        },
        {
            name: 'sqsj',
            label: '申请时间',
            langtag: 'entities.pimpersonchange.hisrec_grid.columns.sqsj',
            show: true,
            util: 'PX'
        },
        {
            name: 'bglx',
            label: '变更类型',
            langtag: 'entities.pimpersonchange.hisrec_grid.columns.bglx',
            show: true,
            util: 'PX'
        },
        {
            name: 'reason',
            label: '拒绝原因',
            langtag: 'entities.pimpersonchange.hisrec_grid.columns.reason',
            show: true,
            util: 'PX'
        },
        {
            name: 'zt',
            label: '审批状态',
            langtag: 'entities.pimpersonchange.hisrec_grid.columns.zt',
            show: true,
            util: 'PX'
        },
    ]
    ...
    public async remove(datas: any[]): Promise<any> {
        ...
        _datas.forEach((record: any, index: number) => {
            let srfmajortext = record.pimpersonchangename;
            if (index < 5) {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ pimpersonchange: _keys }),Object.assign({ pimpersonchange: _keys },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    public rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.pimpersonchange,$event.pimpersonchange);
        });
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: '错误', desc: 'PIMPERSONCHANGEHisRecGridView视图表格createAction参数未配置' });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: '错误', desc: 'PIMPERSONCHANGEHisRecGridView视图表格updateAction参数未配置' });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
                        if(item.pimpersonchange){
                            Object.assign(this.context,{pimpersonchange:item.pimpersonchange});
                        }
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'PIMPERSONCHANGEHisRecGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
