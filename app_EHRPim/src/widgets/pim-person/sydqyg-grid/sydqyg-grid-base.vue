    ...
    <el-table v-if="isDisplay === true"
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('ormorgsectorname')">
                <el-table-column show-overflow-tooltip :prop="'ormorgsectorname'" :label="$t('entities.pimperson.sydqyg_grid.columns.ormorgsectorname')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimperson.sydqyg_grid.columns.ormorgsectorname')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('zw')">
                <el-table-column show-overflow-tooltip :prop="'zw'" :label="$t('entities.pimperson.sydqyg_grid.columns.zw')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimperson.sydqyg_grid.columns.zw')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
            ...
            <template v-if="getColumnState('gw')">
                <el-table-column show-overflow-tooltip :prop="'gw'" :label="$t('entities.pimperson.sydqyg_grid.columns.gw')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimperson.sydqyg_grid.columns.gw')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
            ...
            <template v-if="getColumnState('dbdwsj')">
                <el-table-column show-overflow-tooltip :prop="'dbdwsj'" :label="$t('entities.pimperson.sydqyg_grid.columns.dbdwsj')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimperson.sydqyg_grid.columns.dbdwsj')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('sydq')">
                <el-table-column show-overflow-tooltip :prop="'sydq'" :label="$t('entities.pimperson.sydqyg_grid.columns.sydq')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimperson.sydqyg_grid.columns.sydq')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
...
export default class SYDQYGBase extends Vue implements ControlInterface {
    ...
    */
    ...
    public allColumns: any[] = [
        {
            name: 'ygbh',
            label: '员工编号',
            langtag: 'entities.pimperson.sydqyg_grid.columns.ygbh',
            show: true,
            util: 'px'
        },
        {
            name: 'pimpersonname',
            label: '员工姓名',
            langtag: 'entities.pimperson.sydqyg_grid.columns.pimpersonname',
            show: true,
            util: 'px'
        },
        {
        ...
        },
        {
            name: 'zw',
            label: '职务',
            langtag: 'entities.pimperson.sydqyg_grid.columns.zw',
            show: true,
            util: 'px'
        },
        {
            name: 'gw',
            label: '岗位',
            langtag: 'entities.pimperson.sydqyg_grid.columns.gw',
            show: true,
            util: 'px'
        },
        {
            name: 'dbdwsj',
            label: '到本单位时间',
            langtag: 'entities.pimperson.sydqyg_grid.columns.dbdwsj',
            show: true,
            util: 'PX'
        },
        {
            name: 'sydq',
            label: '试用到期时间',
            langtag: 'entities.pimperson.sydqyg_grid.columns.sydq',
            show: true,
            util: 'PX'
        },
    ]
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'PIMPERSONSYQYGGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'PIMPERSONSYQYGGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: '错误', desc: 'PIMPERSONSYQYGGridView视图表格createAction参数未配置' });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: '错误', desc: 'PIMPERSONSYQYGGridView视图表格updateAction参数未配置' });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
                        if(item.pimperson){
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'PIMPERSONSYQYGGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
