        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
                ...
                <el-table-column show-overflow-tooltip :prop="'ygbh'" :label="$t('entities.pimperson.gzdq_grid.columns.ygbh')" :width="120"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimperson.gzdq_grid.columns.ygbh')}}
                      </span>
                    </template>
            ...
            <template v-if="getColumnState('pimpersonname')">
                <el-table-column show-overflow-tooltip :prop="'pimpersonname'" :label="$t('entities.pimperson.gzdq_grid.columns.pimpersonname')" :width="120"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimperson.gzdq_grid.columns.pimpersonname')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('ygzt')">
                <el-table-column show-overflow-tooltip :prop="'ygzt'" :label="$t('entities.pimperson.gzdq_grid.columns.ygzt')" :width="120"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimperson.gzdq_grid.columns.ygzt')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
            ...
            <template v-if="getColumnState('gzzt')">
                <el-table-column show-overflow-tooltip :prop="'gzzt'" :label="$t('entities.pimperson.gzdq_grid.columns.gzzt')" :width="120"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimperson.gzdq_grid.columns.gzzt')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
            <codelist :value="row.gzzt" tag='EhrCodeList0228' codelistType='STATIC' ></codelist>
                        </template>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('gzzz')">
                <el-table-column show-overflow-tooltip :prop="'gzzz'" :label="$t('entities.pimperson.gzdq_grid.columns.gzzz')" :width="250"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimperson.gzdq_grid.columns.gzzz')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.gzzz}}</span>
            ...
            <template v-if="getColumnState('gzbm')">
                <el-table-column show-overflow-tooltip :prop="'gzbm'" :label="$t('entities.pimperson.gzdq_grid.columns.gzbm')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimperson.gzdq_grid.columns.gzbm')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.gzbm}}</span>
            ...
            <template v-if="getColumnState('gzkssj')">
                <el-table-column show-overflow-tooltip :prop="'gzkssj'" :label="$t('entities.pimperson.gzdq_grid.columns.gzkssj')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimperson.gzdq_grid.columns.gzkssj')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('gzjssj')">
                <el-table-column show-overflow-tooltip :prop="'gzjssj'" :label="$t('entities.pimperson.gzdq_grid.columns.gzjssj')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimperson.gzdq_grid.columns.gzjssj')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
...
import GZDQService from './gzdq-grid-service';
...
export default class GZDQBase extends Vue implements ControlInterface {
     ...
     * @memberof GZDQBase
     */
    public service: GZDQService = new GZDQService({ $store: this.$store });
     ...
     * @memberof GZDQBase
     */
    public minorSortPSDEF: string = 'gzjssj';
    ...
    */
    ...
    public allColumns: any[] = [
        {
            name: 'ygbh',
            label: '员工编号',
            langtag: 'entities.pimperson.gzdq_grid.columns.ygbh',
            show: true,
            util: 'PX'
        },
        {
            name: 'pimpersonname',
            label: '员工姓名',
            langtag: 'entities.pimperson.gzdq_grid.columns.pimpersonname',
            show: true,
            util: 'PX'
        },
        {
            name: 'ygzt',
            label: '员工状态',
            langtag: 'entities.pimperson.gzdq_grid.columns.ygzt',
            show: true,
            util: 'PX'
        },
        {
            name: 'gzzt',
            label: '挂职状态',
            langtag: 'entities.pimperson.gzdq_grid.columns.gzzt',
            show: true,
            util: 'PX'
        },
        {
            name: 'gzzz',
            label: '挂职组织',
            langtag: 'entities.pimperson.gzdq_grid.columns.gzzz',
            show: true,
            util: 'PX'
        },
        {
            name: 'gzbm',
            label: '挂职部门',
            langtag: 'entities.pimperson.gzdq_grid.columns.gzbm',
            show: true,
            util: 'px'
        },
        {
            name: 'gzkssj',
            label: '挂职开始时间',
            langtag: 'entities.pimperson.gzdq_grid.columns.gzkssj',
            show: true,
            util: 'px'
        },
        {
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'PIMPERSONGZRYHMCGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'PIMPERSONGZRYHMCGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('pimperson_gzdq_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('pimperson_gzdq_grid', JSON.stringify(this.allColumns));
    }
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: '错误', desc: 'PIMPERSONGZRYHMCGridView视图表格createAction参数未配置' });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: '错误', desc: 'PIMPERSONGZRYHMCGridView视图表格updateAction参数未配置' });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
                        if(item.pimperson){
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'PIMPERSONGZRYHMCGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
