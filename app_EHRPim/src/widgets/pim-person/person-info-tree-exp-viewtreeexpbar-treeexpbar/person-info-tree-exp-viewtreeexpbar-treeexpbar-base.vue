<template>
    <split id="personinfotreeexpviewtreeexpbar" class='app-tree-exp-bar' v-model="split" mode="horizontal" @on-move-end="onSplitChange">
        <div slot='left'>
...
import PersonInfoTreeExpViewtreeexpbarService from './person-info-tree-exp-viewtreeexpbar-treeexpbar-service';
...
export default class PersonInfoTreeExpViewtreeexpbarBase extends Vue implements ControlInterface {
    ...
    public getExpItemView(arg: any = {}): any {
        ...
        if (Object.is(expmode, 'SUBORMORGINFO')) {
            return {  
                viewname: 'pimpersongrid-view', 
                parentdata: {"srfparentdefname":"n_ormorgid_eq","srfparentdename":"ORMORG","n_zgcode_eq":"10","srfparentmode":"DER1N_PIMPERSON_ORMORG_ORMORGID","nodeid":"%NODEID%"},
                deKeyField:'pimperson'
			};
        }
        if (Object.is(expmode, 'SUBSUBORMORGINFO')) {
            return {  
                viewname: 'pimpersongrid-view', 
                parentdata: {"n_zgcode_eq":"10","nodeid":"%NODEID%"},
                deKeyField:'pimperson'
			};
        }
        if (Object.is(expmode, 'ORMORGSECTOR')) {
            return {  
                viewname: 'pimpersongrid-view', 
                parentdata: {"n_zgcode_eq":"10","nodeid":"%NODEID%"},
                deKeyField:'pimperson'
        ...
        if (Object.is(expmode, 'ORMORGINFO')) {
            return {  
                viewname: 'pimpersongrid-view', 
                parentdata: {},
                deKeyField:'pimperson'
    ...
    */
    public afterMounted(){ 
        if(this.$store.getters.getViewSplit(this.viewUID)){
            ...
            let containerWidth:number = (document.getElementById("personinfotreeexpviewtreeexpbar") as any).offsetWidth;
            if(this.ctrlWidth){
...
<style lang='less'>
@import './person-info-tree-exp-viewtreeexpbar-treeexpbar.less';
</style>
