    ...
    <el-table v-if="isDisplay === true"
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('jlsj')">
                <el-table-column show-overflow-tooltip :prop="'jlsj'" :label="$t('entities.pimrewardpunishment.main_grid.columns.jlsj')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimrewardpunishment.main_grid.columns.jlsj')}}
                      </span>
                    </template>
            ...
            <template v-if="getColumnState('pimrewardpunishmentname')">
                <el-table-column show-overflow-tooltip :prop="'pimrewardpunishmentname'" :label="$t('entities.pimrewardpunishment.main_grid.columns.pimrewardpunishmentname')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimrewardpunishment.main_grid.columns.pimrewardpunishmentname')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('lx')">
                <el-table-column show-overflow-tooltip :prop="'lx'" :label="$t('entities.pimrewardpunishment.main_grid.columns.lx')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimrewardpunishment.main_grid.columns.lx')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
            ...
            <template v-if="getColumnState('department')">
                <el-table-column show-overflow-tooltip :prop="'department'" :label="$t('entities.pimrewardpunishment.main_grid.columns.department')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimrewardpunishment.main_grid.columns.department')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
...
export default class MainBase extends Vue implements ControlInterface {
    ...
    */
    ...
    public allColumns: any[] = [
        {
            name: 'jlsj',
            label: '获取时间',
            langtag: 'entities.pimrewardpunishment.main_grid.columns.jlsj',
            show: true,
            util: 'PX'
        },
        {
            ...
            label: '名称',
            langtag: 'entities.pimrewardpunishment.main_grid.columns.pimrewardpunishmentname',
            show: true,
            util: 'PX'
        },
        {
            name: 'lx',
            label: '奖惩分类',
            langtag: 'entities.pimrewardpunishment.main_grid.columns.lx',
            show: true,
            util: 'px'
        },
        {
            name: 'department',
            label: '授予单位',
            langtag: 'entities.pimrewardpunishment.main_grid.columns.department',
            show: true,
            util: 'px'
        },
    ]
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'PIMREWARDPUNISHMENTPimJanglGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'PIMREWARDPUNISHMENTPimJanglGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('pimrewardpunishment_main_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('pimrewardpunishment_main_grid', JSON.stringify(this.allColumns));
    }
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: '错误', desc: 'PIMREWARDPUNISHMENTPimJanglGridView视图表格createAction参数未配置' });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: '错误', desc: 'PIMREWARDPUNISHMENTPimJanglGridView视图表格updateAction参数未配置' });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'PIMREWARDPUNISHMENTPimJanglGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
