        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
                ...
                <el-table-column show-overflow-tooltip :prop="'zgzsbh'" :label="$t('entities.pimvocational.main_grid.columns.zgzsbh')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimvocational.main_grid.columns.zgzsbh')}}
                      </span>
                    </template>
            ...
            <template v-if="getColumnState('zsmc')">
                <el-table-column show-overflow-tooltip :prop="'zsmc'" :label="$t('entities.pimvocational.main_grid.columns.zsmc')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimvocational.main_grid.columns.zsmc')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.zsmc}}</span>
            ...
            <template v-if="getColumnState('zslx')">
                <el-table-column show-overflow-tooltip :prop="'zslx'" :label="$t('entities.pimvocational.main_grid.columns.zslx')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimvocational.main_grid.columns.zslx')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
            ...
            <template v-if="getColumnState('zghqrq')">
                <el-table-column show-overflow-tooltip :prop="'zghqrq'" :label="$t('entities.pimvocational.main_grid.columns.zghqrq')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimvocational.main_grid.columns.zghqrq')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('zgsydw')">
                <el-table-column show-overflow-tooltip :prop="'zgsydw'" :label="$t('entities.pimvocational.main_grid.columns.zgsydw')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimvocational.main_grid.columns.zgsydw')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('fzyxq')">
                <el-table-column show-overflow-tooltip :prop="'fzyxq'" :label="$t('entities.pimvocational.main_grid.columns.fzyxq')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimvocational.main_grid.columns.fzyxq')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.fzyxq}}</span>
...
export default class MainBase extends Vue implements ControlInterface {
    ...
    */
    ...
    public allColumns: any[] = [
        {
            name: 'zgzsbh',
            label: '证书编号',
            langtag: 'entities.pimvocational.main_grid.columns.zgzsbh',
            show: true,
            util: 'px'
        },
        {
            name: 'zsmc',
            label: '证书名称',
            langtag: 'entities.pimvocational.main_grid.columns.zsmc',
            show: true,
            util: 'px'
        },
        {
            name: 'zslx',
            label: '证书类型',
            langtag: 'entities.pimvocational.main_grid.columns.zslx',
            show: true,
            util: 'px'
        },
        {
            name: 'zghqrq',
            label: '签发日期',
            langtag: 'entities.pimvocational.main_grid.columns.zghqrq',
            show: true,
            util: 'px'
        },
        {
            name: 'zgsydw',
            label: '签发机构',
            langtag: 'entities.pimvocational.main_grid.columns.zgsydw',
            show: true,
            util: 'px'
        },
        {
            name: 'fzyxq',
            label: '发证有效期',
            langtag: 'entities.pimvocational.main_grid.columns.fzyxq',
            show: true,
            util: 'px'
        },
    ]
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'PIMVOCATIONALPimZigGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'PIMVOCATIONALPimZigGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('pimvocational_main_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('pimvocational_main_grid', JSON.stringify(this.allColumns));
    }
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: '错误', desc: 'PIMVOCATIONALPimZigGridView视图表格createAction参数未配置' });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: '错误', desc: 'PIMVOCATIONALPimZigGridView视图表格updateAction参数未配置' });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'PIMVOCATIONALPimZigGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
