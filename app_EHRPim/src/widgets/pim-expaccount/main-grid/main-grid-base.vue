        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
                ...
                <el-table-column show-overflow-tooltip :prop="'fylb'" :label="$t('entities.pimexpaccount.main_grid.columns.fylb')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimexpaccount.main_grid.columns.fylb')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
            <codelist :value="row.fylb" tag='EhrCodeList0162' codelistType='STATIC' renderMode="STR" valueSeparator=";" textSeparator="、" ></codelist>
                        </template>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('fyje')">
                <el-table-column show-overflow-tooltip :prop="'fyje'" :label="$t('entities.pimexpaccount.main_grid.columns.fyje')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimexpaccount.main_grid.columns.fyje')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('ffrs')">
                <el-table-column show-overflow-tooltip :prop="'ffrs'" :label="$t('entities.pimexpaccount.main_grid.columns.ffrs')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimexpaccount.main_grid.columns.ffrs')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.ffrs}}</span>
            ...
            <template v-if="getColumnState('ffsj')">
                <el-table-column show-overflow-tooltip :prop="'ffsj'" :label="$t('entities.pimexpaccount.main_grid.columns.ffsj')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimexpaccount.main_grid.columns.ffsj')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('ffybz')">
                <el-table-column show-overflow-tooltip :prop="'ffybz'" :label="$t('entities.pimexpaccount.main_grid.columns.ffybz')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimexpaccount.main_grid.columns.ffybz')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.ffybz}}</span>
...
export default class MainBase extends Vue implements ControlInterface {
     ...
     * @memberof MainBase
     */
    public appEntityService: PimExpaccountService = new PimExpaccountService({ $store: this.$store });
    ...
    */
    ...
    public allColumns: any[] = [
        {
            name: 'fylb',
            label: '费用类别',
            langtag: 'entities.pimexpaccount.main_grid.columns.fylb',
            show: true,
            util: 'px'
        },
        {
            name: 'fyje',
            label: '费用金额',
            langtag: 'entities.pimexpaccount.main_grid.columns.fyje',
            show: true,
            util: 'px'
        },
        {
            name: 'ffrs',
            label: '发放人数',
            langtag: 'entities.pimexpaccount.main_grid.columns.ffrs',
            show: true,
            util: 'px'
        },
        {
            name: 'ffsj',
            label: '发放时间',
            langtag: 'entities.pimexpaccount.main_grid.columns.ffsj',
            show: true,
            util: 'px'
        },
        {
            name: 'ffybz',
            label: '费用标准',
            langtag: 'entities.pimexpaccount.main_grid.columns.ffybz',
            show: true,
            util: 'px'
        },
    ]
    ...
    public rules: any = {
        srfkey: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '费用台账标识 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '费用台账标识 值不能为空', trigger: 'blur' },
        ],
    }
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'PIMEXPACCOUNTGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'PIMEXPACCOUNTGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        _datas.forEach((record: any, index: number) => {
            let srfmajortext = record.pimexpaccountname;
            if (index < 5) {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ pimexpaccount: _keys }),Object.assign({ pimexpaccount: _keys },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    public rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.pimexpaccount,$event.pimexpaccount);
        });
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('pimexpaccount_main_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('pimexpaccount_main_grid', JSON.stringify(this.allColumns));
    }
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: '错误', desc: 'PIMEXPACCOUNTGridView视图表格createAction参数未配置' });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: '错误', desc: 'PIMEXPACCOUNTGridView视图表格updateAction参数未配置' });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
                        if(item.pimexpaccount){
                            Object.assign(this.context,{pimexpaccount:item.pimexpaccount});
                        }
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'PIMEXPACCOUNTGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
    ...
    public getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.pimexpaccount,args.row.pimexpaccount);
        });
