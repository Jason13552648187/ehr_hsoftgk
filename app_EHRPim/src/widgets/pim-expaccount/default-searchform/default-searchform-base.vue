...
export default class DefaultBase extends Vue implements ControlInterface {
    ...
    public rules: any = {
        n_fylb_eq: [
            { type: 'string', message: '费用类别(=) 值必须为字符串类型', trigger: 'change' },
            { type: 'string', message: '费用类别(=) 值必须为字符串类型', trigger: 'blur' },
            { required: false, type: 'string', message: '费用类别(=) 值不能为空', trigger: 'change' },
            { required: false, type: 'string', message: '费用类别(=) 值不能为空', trigger: 'blur' },
        ],
    }
...
, 
        n_fylb_eq: new FormItemModel({ caption: '费用类别(=)', detailType: 'FORMITEM', name: 'n_fylb_eq', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 3 })
, 
    };
     ...
     * @memberof DefaultBase
     */
    @Watch('data.n_fylb_eq')
    onN_fylb_eqChange(newVal: any, oldVal: any) {
        this.formDataChange({ name: 'n_fylb_eq', newVal: newVal, oldVal: oldVal });
    }
    ...
    public load(opt: any = {}): void {
        if(!this.loadAction){
            this.$Notice.error({ title: '错误', desc: 'PIMEXPACCOUNTGridView视图搜索表单loadAction参数未配置' });
            return;
        }
        const arg: any = { ...opt };
    ...
    public loadDraft(opt: any = {},mode?:string): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'PIMEXPACCOUNTGridView视图搜索表单loaddraftAction参数未配置' });
            return;
        }
        const arg: any = { ...opt } ;
        Object.assign(arg,{viewparams:this.viewparams});
