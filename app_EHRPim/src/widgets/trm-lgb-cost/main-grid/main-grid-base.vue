        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
                ...
                <el-table-column show-overflow-tooltip :prop="'trmlgbcostname'" :label="$t('entities.trmlgbcost.main_grid.columns.trmlgbcostname')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.trmlgbcost.main_grid.columns.trmlgbcostname')}}
                      </span>
                    </template>
            ...
            <template v-if="getColumnState('kssj')">
                <el-table-column show-overflow-tooltip :prop="'kssj'" :label="$t('entities.trmlgbcost.main_grid.columns.kssj')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.trmlgbcost.main_grid.columns.kssj')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('jssj')">
                <el-table-column show-overflow-tooltip :prop="'jssj'" :label="$t('entities.trmlgbcost.main_grid.columns.jssj')" :width="170"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.trmlgbcost.main_grid.columns.jssj')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('je')">
                <el-table-column show-overflow-tooltip :prop="'je'" :label="$t('entities.trmlgbcost.main_grid.columns.je')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.trmlgbcost.main_grid.columns.je')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('hdsm')">
                <el-table-column show-overflow-tooltip :prop="'hdsm'" :label="$t('entities.trmlgbcost.main_grid.columns.hdsm')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.trmlgbcost.main_grid.columns.hdsm')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.hdsm}}</span>
...
export default class MainBase extends Vue implements ControlInterface {
     ...
     * @memberof MainBase
     */
    public appEntityService: TrmLgbCostService = new TrmLgbCostService({ $store: this.$store });
    ...
    */
    ...
    public allColumns: any[] = [
        {
            name: 'trmlgbcostname',
            label: '活动',
            langtag: 'entities.trmlgbcost.main_grid.columns.trmlgbcostname',
            show: true,
            util: 'PX'
        },
        {
            name: 'kssj',
            label: '开始时间',
            langtag: 'entities.trmlgbcost.main_grid.columns.kssj',
            show: true,
            util: 'PX'
        },
        {
            name: 'jssj',
            label: '结束时间',
            langtag: 'entities.trmlgbcost.main_grid.columns.jssj',
            show: true,
            util: 'PX'
        },
        {
            name: 'je',
            label: '金额',
            langtag: 'entities.trmlgbcost.main_grid.columns.je',
            show: true,
            util: 'px'
        },
        {
            name: 'hdsm',
            label: '活动说明',
            langtag: 'entities.trmlgbcost.main_grid.columns.hdsm',
            show: true,
            util: 'px'
        },
    ]
    ...
    public rules: any = {
        srfkey: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '老干部费用标识 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '老干部费用标识 值不能为空', trigger: 'blur' },
        ],
    }
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'TRMLGBCOSTGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'TRMLGBCOSTGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        _datas.forEach((record: any, index: number) => {
            let srfmajortext = record.trmlgbcostname;
            if (index < 5) {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ trmlgbcost: _keys }),Object.assign({ trmlgbcost: _keys },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    public rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.trmlgbcost,$event.trmlgbcost);
        });
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('trmlgbcost_main_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('trmlgbcost_main_grid', JSON.stringify(this.allColumns));
    }
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: '错误', desc: 'TRMLGBCOSTGridView视图表格createAction参数未配置' });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: '错误', desc: 'TRMLGBCOSTGridView视图表格updateAction参数未配置' });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
                        if(item.trmlgbcost){
                            Object.assign(this.context,{trmlgbcost:item.trmlgbcost});
                        }
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'TRMLGBCOSTGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
    ...
    public getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.trmlgbcost,args.row.trmlgbcost);
        });
