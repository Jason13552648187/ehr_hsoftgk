<template>
  ...
  <row>
    <i-col span="20" class="form-content">
      <row>
                    <i-col v-show="detailsModel.n_ormpostname_like.visible" :style="{}"  :sm="{ span: 8, offset: 0 }" :md="{ span: 8, offset: 0 }" :lg="{ span: 8, offset: 0 }" :xl="{ span: 8, offset: 0 }">
              <app-form-item name='n_ormpostname_like' :itemRules="this.rules.n_ormpostname_like" class='' :caption="$t('entities.ormpost.default_searchform.details.n_ormpostname_like')" uiStyle="DEFAULT" :labelWidth="100" :isShowCaption="true" :error="detailsModel.n_ormpostname_like.error" :isEmptyCaption="false" labelPos="RIGHT"> 
          ...
          <i-col v-show="detailsModel.n_ormorgname_like.visible" :style="{}"  :sm="{ span: 8, offset: 0 }" :md="{ span: 8, offset: 0 }" :lg="{ span: 8, offset: 0 }" :xl="{ span: 8, offset: 0 }">
              <app-form-item name='n_ormorgname_like' :itemRules="this.rules.n_ormorgname_like" class='' :caption="$t('entities.ormpost.default_searchform.details.n_ormorgname_like')" uiStyle="DEFAULT" :labelWidth="100" :isShowCaption="true" :error="detailsModel.n_ormorgname_like.error" :isEmptyCaption="false" labelPos="RIGHT"> 
...
export default class DefaultBase extends Vue implements ControlInterface {
    ...
    public rules: any = {
        ...
        n_startstopsign_eq: [
            { type: 'string', message: '启用标识 值必须为字符串类型', trigger: 'change' },
            { type: 'string', message: '启用标识 值必须为字符串类型', trigger: 'blur' },
            { required: false, type: 'string', message: '启用标识 值不能为空', trigger: 'change' },
            { required: false, type: 'string', message: '启用标识 值不能为空', trigger: 'blur' },
        ],
        n_gwtype_eq: [
            { type: 'string', message: '岗位类型 值必须为字符串类型', trigger: 'change' },
            { type: 'string', message: '岗位类型 值必须为字符串类型', trigger: 'blur' },
            { required: false, type: 'string', message: '岗位类型 值不能为空', trigger: 'change' },
            { required: false, type: 'string', message: '岗位类型 值不能为空', trigger: 'blur' },
        ],
    }
...
, 
        n_ormpostname_like: new FormItemModel({ caption: '岗位集名称', detailType: 'FORMITEM', name: 'n_ormpostname_like', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 3 })
, 
...
, 
        n_startstopsign_eq: new FormItemModel({ caption: '启用标识', detailType: 'FORMITEM', name: 'n_startstopsign_eq', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 3 })
, 
        n_gwtype_eq: new FormItemModel({ caption: '岗位类型', detailType: 'FORMITEM', name: 'n_gwtype_eq', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 3 })
, 
    };
    ...
    /**
     * 监控表单属性 n_startstopsign_eq 值
     *
     * @param {*} newVal
     * @param {*} oldVal
     * @memberof DefaultBase
     */
    @Watch('data.n_startstopsign_eq')
    onN_startstopsign_eqChange(newVal: any, oldVal: any) {
        this.formDataChange({ name: 'n_startstopsign_eq', newVal: newVal, oldVal: oldVal });
    }

    /**
     * 监控表单属性 n_gwtype_eq 值
     *
     * @param {*} newVal
     * @param {*} oldVal
     * @memberof DefaultBase
     */
    @Watch('data.n_gwtype_eq')
    onN_gwtype_eqChange(newVal: any, oldVal: any) {
        this.formDataChange({ name: 'n_gwtype_eq', newVal: newVal, oldVal: oldVal });
    }
    ...
    public load(opt: any = {}): void {
        if(!this.loadAction){
            this.$Notice.error({ title: '错误', desc: 'ORMPOSTEJORMORGIDPickupGridView视图搜索表单loadAction参数未配置' });
            return;
        }
        const arg: any = { ...opt };
    ...
    public loadDraft(opt: any = {},mode?:string): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'ORMPOSTEJORMORGIDPickupGridView视图搜索表单loaddraftAction参数未配置' });
            return;
        }
        const arg: any = { ...opt } ;
        Object.assign(arg,{viewparams:this.viewparams});
