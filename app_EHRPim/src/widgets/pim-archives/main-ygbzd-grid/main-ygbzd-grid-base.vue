        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
                ...
                <el-table-column show-overflow-tooltip :prop="'dabh'" :label="$t('entities.pimarchives.main_ygbzd_grid.columns.dabh')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimarchives.main_ygbzd_grid.columns.dabh')}}
                      </span>
                    </template>
            ...
            <template v-if="getColumnState('ormorgname3')">
                <el-table-column show-overflow-tooltip :prop="'ormorgname3'" :label="$t('entities.pimarchives.main_ygbzd_grid.columns.ormorgname3')" :width="250"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimarchives.main_ygbzd_grid.columns.ormorgname3')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('dabgd')">
                <el-table-column show-overflow-tooltip :prop="'dabgd'" :label="$t('entities.pimarchives.main_ygbzd_grid.columns.dabgd')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimarchives.main_ygbzd_grid.columns.dabgd')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.dabgd}}</span>
            ...
            <template v-if="getColumnState('archivescentername')">
                <el-table-column show-overflow-tooltip :prop="'archivescentername'" :label="$t('entities.pimarchives.main_ygbzd_grid.columns.archivescentername')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimarchives.main_ygbzd_grid.columns.archivescentername')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('dastate')">
                <el-table-column show-overflow-tooltip :prop="'dastate'" :label="$t('entities.pimarchives.main_ygbzd_grid.columns.dastate')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimarchives.main_ygbzd_grid.columns.dastate')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
            ...
            <template v-if="getColumnState('ygbh')">
                <el-table-column show-overflow-tooltip :prop="'ygbh'" :label="$t('entities.pimarchives.main_ygbzd_grid.columns.ygbh')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimarchives.main_ygbzd_grid.columns.ygbh')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.ygbh}}</span>
            ...
            <template v-if="getColumnState('pimpersonname')">
                <el-table-column show-overflow-tooltip :prop="'pimpersonname'" :label="$t('entities.pimarchives.main_ygbzd_grid.columns.pimpersonname')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimarchives.main_ygbzd_grid.columns.pimpersonname')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('sszzdzs')">
                <el-table-column show-overflow-tooltip :prop="'sszzdzs'" :label="$t('entities.pimarchives.main_ygbzd_grid.columns.sszzdzs')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimarchives.main_ygbzd_grid.columns.sszzdzs')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('ygzt')">
                <el-table-column show-overflow-tooltip :prop="'ygzt'" :label="$t('entities.pimarchives.main_ygbzd_grid.columns.ygzt')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimarchives.main_ygbzd_grid.columns.ygzt')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
            ...
            <template v-if="getColumnState('uagridcolumn1')">
                <el-table-column :column-key="'uagridcolumn1'" :label="$t('entities.pimarchives.main_ygbzd_grid.columns.uagridcolumn1')" :width="100"  :align="'right'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimarchives.main_ygbzd_grid.columns.uagridcolumn1')}}
                      </span>
                    </template>
                    <template slot-scope="scope">
                        <div style="text-align: center;">
                            <a @click="uiAction(scope.row, 'FILEIN', $event)">
                              <i class=''></i>
                              {{$t('entities.pimarchives.main_ygbzd_grid.uiactions.filein')}}
                            </a>
                        </div>
                    </template>
                </el-table-column>
...
import Main_YGBZDService from './main-ygbzd-grid-service';
...
export default class Main_YGBZDBase extends Vue implements ControlInterface {

    /**
     * 名称
     *
     * @type {string}
     * @memberof Main_YGBZDBase
     */
     ...
     * 建构部件服务对象
     *
     * @type {Main_YGBZDService}
     * @memberof Main_YGBZDBase
     */
    public service: Main_YGBZDService = new Main_YGBZDService({ $store: this.$store });
    ...
    public grid_uagridcolumn1_u5a04a4b_click(params: any = {}, tag?: any, $event?: any) {
        ...
        // 界面行为
        const curUIService:PimArchivesUIService  = new PimArchivesUIService();
        curUIService.PimArchives_FILEIN(datas,contextJO, paramJO,  $event, xData,this,"PimArchives");
    }
    ...
    */
    ...
    public allColumns: any[] = [
        {
            name: 'dabh',
            label: '档案编号',
            langtag: 'entities.pimarchives.main_ygbzd_grid.columns.dabh',
            show: true,
            util: 'px'
        },
        {
            name: 'ormorgname3',
            label: '管理单位',
            langtag: 'entities.pimarchives.main_ygbzd_grid.columns.ormorgname3',
            show: true,
            util: 'PX'
        },
        {
            name: 'dabgd',
            label: '档案保管地',
            langtag: 'entities.pimarchives.main_ygbzd_grid.columns.dabgd',
            show: true,
            util: 'px'
        },
        {
            ...
            label: '档案室',
            langtag: 'entities.pimarchives.main_ygbzd_grid.columns.archivescentername',
            show: true,
            util: 'px'
        },
        {
            name: 'dastate',
            label: '档案状态',
            langtag: 'entities.pimarchives.main_ygbzd_grid.columns.dastate',
            show: true,
            util: 'px'
        },
        {
            name: 'ygbh',
            label: '员工编号',
            langtag: 'entities.pimarchives.main_ygbzd_grid.columns.ygbh',
            show: true,
            util: 'px'
        },
        {
            name: 'pimpersonname',
            label: '员工姓名',
            langtag: 'entities.pimarchives.main_ygbzd_grid.columns.pimpersonname',
            show: true,
            util: 'PX'
        },
        {
            name: 'sszzdzs',
            label: '所属单位',
            langtag: 'entities.pimarchives.main_ygbzd_grid.columns.sszzdzs',
            show: true,
            util: 'px'
        },
        {
            name: 'ygzt',
            label: '员工状态',
            langtag: 'entities.pimarchives.main_ygbzd_grid.columns.ygzt',
            show: true,
            util: 'px'
        },
        {
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'PIMARCHIVESLOYEENOFILEGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'PIMARCHIVESLOYEENOFILEGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('pimarchives_main_ygbzd_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('pimarchives_main_ygbzd_grid', JSON.stringify(this.allColumns));
    }
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: '错误', desc: 'PIMARCHIVESLOYEENOFILEGridView视图表格createAction参数未配置' });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: '错误', desc: 'PIMARCHIVESLOYEENOFILEGridView视图表格updateAction参数未配置' });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
                        if(item.pimarchives){
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'PIMARCHIVESLOYEENOFILEGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
