...
<div class='grid' style="height:100%;">
    ...
    <el-table v-if="isDisplay === true"
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('serialnumber')">
                <el-table-column show-overflow-tooltip :prop="'serialnumber'" :label="$t('entities.pimarchivalcatalogue.main_grid.columns.serialnumber')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimarchivalcatalogue.main_grid.columns.serialnumber')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
                                <app-span name='serialnumber' editorType="TEXTBOX" :value="row.serialnumber"></app-span>
                        </template>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('archivalcataloguename')">
                <el-table-column show-overflow-tooltip :prop="'archivalcataloguename'" :label="$t('entities.pimarchivalcatalogue.main_grid.columns.archivalcataloguename')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimarchivalcatalogue.main_grid.columns.archivalcataloguename')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
                                <app-span name='archivalcataloguename' editorType="TEXTBOX" :value="row.archivalcataloguename"></app-span>
                        </template>
                    </template>
                </el-table-column>
            </template>
...
export default class MainBase extends Vue implements ControlInterface {
     ...
     * @memberof MainBase
     */
    public appEntityService: PimArchivalCatalogueService = new PimArchivalCatalogueService({ $store: this.$store });
    ...
    */
    ...
    public allColumns: any[] = [
        {
            name: 'serialnumber',
            label: '排序号',
            langtag: 'entities.pimarchivalcatalogue.main_grid.columns.serialnumber',
            show: true,
            util: 'PX'
        },
        {
            ...
            label: '档案目录名称',
            langtag: 'entities.pimarchivalcatalogue.main_grid.columns.archivalcataloguename',
            show: true,
            util: 'PX'
        },
    ]
    ...
    public getGridRowModel(){
        return {
          archivalcataloguename: new FormItemModel(),
          serialnumber: new FormItemModel(),
     ...
     */
    public rules: any = {
        archivalcataloguename: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '档案目录名称 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '档案目录名称 值不能为空', trigger: 'blur' },
        ],
        serialnumber: [
        ...
        srfkey: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '档案目录标识 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '档案目录标识 值不能为空', trigger: 'blur' },
        ],
    }
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'ARCHIVALCATALOGUEPickupGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'ARCHIVALCATALOGUEPickupGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        _datas.forEach((record: any, index: number) => {
            let srfmajortext = record.archivalcataloguename;
            if (index < 5) {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ pimarchivalcatalogue: _keys }),Object.assign({ pimarchivalcatalogue: _keys },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    public rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.archivalcatalogue,$event.archivalcatalogue);
        });
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('archivalcatalogue_main_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('archivalcatalogue_main_grid', JSON.stringify(this.allColumns));
    }
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: '错误', desc: 'ARCHIVALCATALOGUEPickupGridView视图表格createAction参数未配置' });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: '错误', desc: 'ARCHIVALCATALOGUEPickupGridView视图表格updateAction参数未配置' });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
                        if(item.pimarchivalcatalogue){
                            Object.assign(this.context,{pimarchivalcatalogue:item.pimarchivalcatalogue});
                        }
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'ARCHIVALCATALOGUEPickupGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
    ...
    public getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.pimarchivalcatalogue,args.row.pimarchivalcatalogue);
        });
