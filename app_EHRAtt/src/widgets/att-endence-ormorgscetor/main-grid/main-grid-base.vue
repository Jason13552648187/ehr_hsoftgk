    ...
    <el-table v-if="isDisplay === true"
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('ormorgsectorname')">
                <el-table-column show-overflow-tooltip :prop="'ormorgsectorname'" :label="$t('entities.attendenceormorgscetor.main_grid.columns.ormorgsectorname')" :width="200"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.attendenceormorgscetor.main_grid.columns.ormorgsectorname')}}
                      </span>
                    </template>
            ...
            <template v-if="getColumnState('dz')">
                <el-table-column show-overflow-tooltip :prop="'dz'" :label="$t('entities.attendenceormorgscetor.main_grid.columns.dz')" :width="400"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.attendenceormorgscetor.main_grid.columns.dz')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.dz}}</span>
            ...
            <template v-if="getColumnState('uagridcolumn1')">
                <el-table-column :column-key="'uagridcolumn1'" :label="$t('entities.attendenceormorgscetor.main_grid.columns.uagridcolumn1')" :width="200"  :align="'center'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.attendenceormorgscetor.main_grid.columns.uagridcolumn1')}}
                      </span>
                    </template>
                    <template slot-scope="scope">
                        <div style="text-align: center;">
                            <a @click="uiAction(scope.row, 'XGKQDZ', $event)">
                              <i class=''></i>
                              {{$t('entities.attendenceormorgscetor.main_grid.uiactions.xgkqdz')}}
                            </a>
                        </div>
                    </template>
                </el-table-column>
...
import AttEndenceOrmorgscetorService from '@/service/att-endence-ormorgscetor/att-endence-ormorgscetor-service';
import MainService from './main-grid-service';

import AttEndenceOrmorgscetorUIService from '@/uiservice/att-endence-ormorgscetor/att-endence-ormorgscetor-ui-service';
...
export default class MainBase extends Vue implements ControlInterface {
     ...
     * 实体服务对象
     *
     * @type {AttEndenceOrmorgscetorService}
     * @memberof MainBase
     */
    public appEntityService: AttEndenceOrmorgscetorService = new AttEndenceOrmorgscetorService({ $store: this.$store });
     ...
     * @memberof 
     */
    public grid_uagridcolumn1_u5343277_click(params: any = {}, tag?: any, $event?: any) {
        // 取数
        let datas: any[] = [];
        let xData: any = null;
        // _this 指向容器对象
        ...
        // 界面行为
        const curUIService:AttEndenceOrmorgscetorUIService  = new AttEndenceOrmorgscetorUIService();
        curUIService.AttEndenceOrmorgscetor_XGKQDZ(datas,contextJO, paramJO,  $event, xData,this,"AttEndenceOrmorgscetor");
    }
    ...
    */
    ...
    public allColumns: any[] = [
        {
            ...
            label: '部门名称',
            langtag: 'entities.attendenceormorgscetor.main_grid.columns.ormorgsectorname',
            show: true,
            util: 'PX'
        },
        {
            name: 'dz',
            label: '考勤地址',
            langtag: 'entities.attendenceormorgscetor.main_grid.columns.dz',
            show: true,
            util: 'PX'
        },
        {
            name: 'uagridcolumn1',
            label: '修改部门考勤地址',
            langtag: 'entities.attendenceormorgscetor.main_grid.columns.uagridcolumn1',
            show: true,
            util: 'PX'
        },
    ]
    ...
    public rules: any = {
        srfkey: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '考勤部门标识 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '考勤部门标识 值不能为空', trigger: 'blur' },
        ],
    }
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'AttEndenceOrmorgscetorGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'AttEndenceOrmorgscetorGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        _datas.forEach((record: any, index: number) => {
            let srfmajortext = record.attendenceormorgsectorname;
            if (index < 5) {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ attendenceormorgscetor: _keys }),Object.assign({ attendenceormorgscetor: _keys },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    public rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.attendenceormorgsector,$event.attendenceormorgsector);
        });
	...
	public uiAction(row: any, tag: any, $event: any) {
        ...
        if(Object.is('XGKQDZ', tag)) {
            this.grid_uagridcolumn1_u5343277_click(row, tag, $event);
        }
    }
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('attendenceormorgsector_main_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('attendenceormorgsector_main_grid', JSON.stringify(this.allColumns));
    }
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: '错误', desc: 'AttEndenceOrmorgscetorGridView视图表格createAction参数未配置' });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: '错误', desc: 'AttEndenceOrmorgscetorGridView视图表格updateAction参数未配置' });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
                        if(item.attendenceormorgscetor){
                            Object.assign(this.context,{attendenceormorgscetor:item.attendenceormorgscetor});
                        }
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'AttEndenceOrmorgscetorGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
    ...
    public getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.attendenceormorgscetor,args.row.attendenceormorgscetor);
        });
