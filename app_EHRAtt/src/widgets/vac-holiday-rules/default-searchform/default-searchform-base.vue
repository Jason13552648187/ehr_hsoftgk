  ...
  <row>
    <i-col span="20" class="form-content">
      <row>
              ...
              <app-form-item name='n_ormorgname_like' :itemRules="this.rules.n_ormorgname_like" class='' :caption="$t('entities.vacholidayrules.default_searchform.details.n_ormorgname_like')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.n_ormorgname_like.error" :isEmptyCaption="false" labelPos="LEFT"> 
          ...
          <i-col v-show="detailsModel.n_nd_eq.visible" :style="{}"  :lg="{ span: 8, offset: 0 }">
              <app-form-item name='n_nd_eq' :itemRules="this.rules.n_nd_eq" class='' :caption="$t('entities.vacholidayrules.default_searchform.details.n_nd_eq')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.n_nd_eq.error" :isEmptyCaption="false" labelPos="LEFT"> 
          ...
          <i-col v-show="detailsModel.n_vacholidayrulesname_like.visible" :style="{}"  :lg="{ span: 8, offset: 0 }">
              <app-form-item name='n_vacholidayrulesname_like' :itemRules="this.rules.n_vacholidayrulesname_like" class='' :caption="$t('entities.vacholidayrules.default_searchform.details.n_vacholidayrulesname_like')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.n_vacholidayrulesname_like.error" :isEmptyCaption="false" labelPos="LEFT"> 
              <input-box v-model="data.n_vacholidayrulesname_like"  @enter="onEnter($event)"    :disabled="detailsModel.n_vacholidayrulesname_like.disabled" type='text'  style=""></input-box>
          </app-form-item>
...
export default class DefaultBase extends Vue implements ControlInterface {
    ...
    public data: any = {
        ...
        n_nd_eq: null,
        n_vacholidayrulesname_like: null,
    };
    ...
    public rules: any = {
        ...
        n_vacholidayrulesname_like: [
            { type: 'string', message: '考勤规则名称(%) 值必须为字符串类型', trigger: 'change' },
            { type: 'string', message: '考勤规则名称(%) 值必须为字符串类型', trigger: 'blur' },
            { required: false, type: 'string', message: '考勤规则名称(%) 值不能为空', trigger: 'change' },
            { required: false, type: 'string', message: '考勤规则名称(%) 值不能为空', trigger: 'blur' },
        ],
    }
...
, 
        n_vacholidayrulesname_like: new FormItemModel({ caption: '考勤规则名称(%)', detailType: 'FORMITEM', name: 'n_vacholidayrulesname_like', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 3 })
, 
    };
    ...
    /**
     * 监控表单属性 n_vacholidayrulesname_like 值
     *
     * @param {*} newVal
     * @param {*} oldVal
     * @memberof DefaultBase
     */
    @Watch('data.n_vacholidayrulesname_like')
    onN_vacholidayrulesname_likeChange(newVal: any, oldVal: any) {
        this.formDataChange({ name: 'n_vacholidayrulesname_like', newVal: newVal, oldVal: oldVal });
    }
    ...
    public load(opt: any = {}): void {
        if(!this.loadAction){
            this.$Notice.error({ title: '错误', desc: 'VacHolidayRulesGridView视图搜索表单loadAction参数未配置' });
            return;
        }
        const arg: any = { ...opt };
    ...
    public loadDraft(opt: any = {},mode?:string): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'VacHolidayRulesGridView视图搜索表单loaddraftAction参数未配置' });
            return;
        }
        const arg: any = { ...opt } ;
        Object.assign(arg,{viewparams:this.viewparams});
