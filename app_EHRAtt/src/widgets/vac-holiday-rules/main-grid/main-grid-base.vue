    ...
    <el-table v-if="isDisplay === true"
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('ormorgname')">
                <el-table-column show-overflow-tooltip :prop="'ormorgname'" :label="$t('entities.vacholidayrules.main_grid.columns.ormorgname')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.vacholidayrules.main_grid.columns.ormorgname')}}
                      </span>
                    </template>
            ...
            <template v-if="getColumnState('nd')">
                <el-table-column show-overflow-tooltip :prop="'nd'" :label="$t('entities.vacholidayrules.main_grid.columns.nd')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.vacholidayrules.main_grid.columns.nd')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
            ...
            <template v-if="getColumnState('vacholidayrulesname')">
                <el-table-column show-overflow-tooltip :prop="'vacholidayrulesname'" :label="$t('entities.vacholidayrules.main_grid.columns.vacholidayrulesname')" :width="350"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.vacholidayrules.main_grid.columns.vacholidayrulesname')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('sfqy')">
                <el-table-column show-overflow-tooltip :prop="'sfqy'" :label="$t('entities.vacholidayrules.main_grid.columns.sfqy')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.vacholidayrules.main_grid.columns.sfqy')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
            ...
            <template v-if="getColumnState('updateman')">
                <el-table-column show-overflow-tooltip :prop="'updateman'" :label="$t('entities.vacholidayrules.main_grid.columns.updateman')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.vacholidayrules.main_grid.columns.updateman')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
            ...
            <template v-if="getColumnState('updatedate')">
                <el-table-column show-overflow-tooltip :prop="'updatedate'" :label="$t('entities.vacholidayrules.main_grid.columns.updatedate')" :width="180"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.vacholidayrules.main_grid.columns.updatedate')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
...
export default class MainBase extends Vue implements ControlInterface {
    ...
    */
    ...
    public allColumns: any[] = [
        {
            name: 'ormorgname',
            label: '组织名称',
            langtag: 'entities.vacholidayrules.main_grid.columns.ormorgname',
            show: true,
            util: 'px'
        },
        {
            name: 'nd',
            label: '年度',
            langtag: 'entities.vacholidayrules.main_grid.columns.nd',
            show: true,
            util: 'px'
        },
        {
            ...
            label: '考勤规则名称',
            langtag: 'entities.vacholidayrules.main_grid.columns.vacholidayrulesname',
            show: true,
            util: 'PX'
        },
        {
            name: 'sfqy',
            label: '是否启用',
            langtag: 'entities.vacholidayrules.main_grid.columns.sfqy',
            show: true,
            util: 'PX'
        },
        {
            name: 'updateman',
            label: '更新人',
            langtag: 'entities.vacholidayrules.main_grid.columns.updateman',
            show: true,
            util: 'PX'
        },
        {
            name: 'updatedate',
            label: '更新时间',
            langtag: 'entities.vacholidayrules.main_grid.columns.updatedate',
            show: true,
            util: 'PX'
        },
    ]
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'VacHolidayRulesGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'VacHolidayRulesGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        _datas.forEach((record: any, index: number) => {
            let srfmajortext = record.vacholidayrulesname;
            if (index < 5) {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ vacholidayrules: _keys }),Object.assign({ vacholidayrules: _keys },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    public rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.vacholidayrules,$event.vacholidayrules);
        });
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('vacholidayrules_main_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('vacholidayrules_main_grid', JSON.stringify(this.allColumns));
    }
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: '错误', desc: 'VacHolidayRulesGridView视图表格createAction参数未配置' });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: '错误', desc: 'VacHolidayRulesGridView视图表格updateAction参数未配置' });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
                        if(item.vacholidayrules){
                            Object.assign(this.context,{vacholidayrules:item.vacholidayrules});
                        }
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'VacHolidayRulesGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
    ...
    public getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.vacholidayrules,args.row.vacholidayrules);
        });
