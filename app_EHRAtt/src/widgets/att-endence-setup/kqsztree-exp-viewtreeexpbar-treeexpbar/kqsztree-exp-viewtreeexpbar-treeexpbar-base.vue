<template>
    <split id="kqsztreeexpviewtreeexpbar" class='app-tree-exp-bar' v-model="split" mode="horizontal" @on-move-end="onSplitChange">
        <div slot='left'>
...
import KQSZTreeExpViewtreeexpbarService from './kqsztree-exp-viewtreeexpbar-treeexpbar-service';
...
export default class KQSZTreeExpViewtreeexpbarBase extends Vue implements ControlInterface {
     ...
     * 建构部件服务对象
     *
     * @type {KQSZTreeExpViewtreeexpbarService}
     * @memberof KQSZTreeExpViewtreeexpbarBase
     */
    public service: KQSZTreeExpViewtreeexpbarService = new KQSZTreeExpViewtreeexpbarService({ $store: this.$store });
    ...
    public getExpItemView(arg: any = {}): any {
        ...
        if (Object.is(expmode, 'ORMORG')) {
            return {  
                viewname: 'att-endence-setup-grid-view', 
        ...
        if (Object.is(expmode, 'SUBORG')) {
            return {  
                viewname: 'att-endence-setup-grid-view', 
                parentdata: {"srfparentdefname":"n_ormorgid_eq","srfparentdename":"ORMORG","srfparentmode":"DER1N_ATTENDENCESETUP_ORMORG_ORMORGID","nodeid":"%NODEID%"},
                deKeyField:'attendencesetup'
			};
        }
        return null;
     ...
     * vue 声明周期
     *
     * @memberof @memberof KQSZTreeExpViewtreeexpbarBase
     */
    public created() {
        this.afterCreated();
    }
    ...
    */
    public afterMounted(){ 
        if(this.$store.getters.getViewSplit(this.viewUID)){
            ...
            let containerWidth:number = (document.getElementById("kqsztreeexpviewtreeexpbar") as any).offsetWidth;
            if(this.ctrlWidth){
...
<style lang='less'>
@import './kqsztree-exp-viewtreeexpbar-treeexpbar.less';
</style>
