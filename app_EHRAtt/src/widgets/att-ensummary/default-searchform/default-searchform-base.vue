  ...
  <row>
    <i-col span="20" class="form-content">
      <row>
              ...
              <app-form-item name='n_nd_eq' :itemRules="this.rules.n_nd_eq" class='' :caption="$t('entities.attensummary.default_searchform.details.n_nd_eq')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.n_nd_eq.error" :isEmptyCaption="false" labelPos="LEFT"> 
          ...
          <i-col v-show="detailsModel.n_yf_eq.visible" :style="{}"  :lg="{ span: 6, offset: 0 }">
              <app-form-item name='n_yf_eq' :itemRules="this.rules.n_yf_eq" class='' :caption="$t('entities.attensummary.default_searchform.details.n_yf_eq')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.n_yf_eq.error" :isEmptyCaption="false" labelPos="LEFT"> 
          ...
          <i-col v-show="detailsModel.n_pimpersonname_like.visible" :style="{}"  :lg="{ span: 6, offset: 0 }">
              <app-form-item name='n_pimpersonname_like' :itemRules="this.rules.n_pimpersonname_like" class='' :caption="$t('entities.attensummary.default_searchform.details.n_pimpersonname_like')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.n_pimpersonname_like.error" :isEmptyCaption="false" labelPos="LEFT"> 
...
export default class DefaultBase extends Vue implements ControlInterface {
    ...
    public rules: any = {
        ...
        n_yf_eq: [
            { type: 'string', message: '月份(=) 值必须为字符串类型', trigger: 'change' },
            { type: 'string', message: '月份(=) 值必须为字符串类型', trigger: 'blur' },
            { required: false, type: 'string', message: '月份(=) 值不能为空', trigger: 'change' },
            { required: false, type: 'string', message: '月份(=) 值不能为空', trigger: 'blur' },
        ],
...
, 
        n_yf_eq: new FormItemModel({ caption: '月份(=)', detailType: 'FORMITEM', name: 'n_yf_eq', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 3 })
, 
...
, 
     ...
     */
    @Watch('data.n_yf_eq')
    onN_yf_eqChange(newVal: any, oldVal: any) {
        this.formDataChange({ name: 'n_yf_eq', newVal: newVal, oldVal: oldVal });
    }
    ...
    public load(opt: any = {}): void {
        if(!this.loadAction){
            this.$Notice.error({ title: '错误', desc: 'AttEnsummaryGridView视图搜索表单loadAction参数未配置' });
            return;
        }
        const arg: any = { ...opt };
    ...
    public loadDraft(opt: any = {},mode?:string): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'AttEnsummaryGridView视图搜索表单loaddraftAction参数未配置' });
            return;
        }
        const arg: any = { ...opt } ;
        Object.assign(arg,{viewparams:this.viewparams});
