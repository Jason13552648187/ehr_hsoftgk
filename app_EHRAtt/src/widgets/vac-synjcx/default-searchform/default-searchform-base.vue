  ...
  <row>
    <i-col span="20" class="form-content">
      <row>
              ...
              <app-form-item name='n_pimpersonname_like' :itemRules="this.rules.n_pimpersonname_like" class='' :caption="$t('entities.vacsynjcx.default_searchform.details.n_pimpersonname_like')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.n_pimpersonname_like.error" :isEmptyCaption="false" labelPos="LEFT"> 
          ...
          <i-col v-show="detailsModel.n_zz_like.visible" :style="{}"  :lg="{ span: 6, offset: 0 }">
              <app-form-item name='n_zz_like' :itemRules="this.rules.n_zz_like" class='' :caption="$t('entities.vacsynjcx.default_searchform.details.n_zz_like')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.n_zz_like.error" :isEmptyCaption="false" labelPos="LEFT"> 
              <input-box v-model="data.n_zz_like"  @enter="onEnter($event)"    :disabled="detailsModel.n_zz_like.disabled" type='text'  style=""></input-box>
          </app-form-item>
          
          </i-col>
          <i-col v-show="detailsModel.n_bm_like.visible" :style="{}"  :lg="{ span: 6, offset: 0 }">
              <app-form-item name='n_bm_like' :itemRules="this.rules.n_bm_like" class='' :caption="$t('entities.vacsynjcx.default_searchform.details.n_bm_like')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.n_bm_like.error" :isEmptyCaption="false" labelPos="LEFT"> 
...
, 
        n_zz_like: new FormItemModel({ caption: '组织名称(%)', detailType: 'FORMITEM', name: 'n_zz_like', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 3 })
, 
        n_bm_like: new FormItemModel({ caption: '部门名称(%)', detailType: 'FORMITEM', name: 'n_bm_like', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 3 })
, 
    };
    ...
    public load(opt: any = {}): void {
        if(!this.loadAction){
            this.$Notice.error({ title: '错误', desc: 'VacSynjcxGridView视图搜索表单loadAction参数未配置' });
            return;
        }
        const arg: any = { ...opt };
    ...
    public loadDraft(opt: any = {},mode?:string): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'VacSynjcxGridView视图搜索表单loaddraftAction参数未配置' });
            return;
        }
        const arg: any = { ...opt } ;
        Object.assign(arg,{viewparams:this.viewparams});
