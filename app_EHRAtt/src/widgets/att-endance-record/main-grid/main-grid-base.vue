    ...
    <el-table v-if="isDisplay === true"
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('pimpersonname')">
                <el-table-column show-overflow-tooltip :prop="'pimpersonname'" :label="$t('entities.attendancerecord.main_grid.columns.pimpersonname')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.attendancerecord.main_grid.columns.pimpersonname')}}
                      </span>
                    </template>
            ...
            <template v-if="getColumnState('ygbh')">
                <el-table-column show-overflow-tooltip :prop="'ygbh'" :label="$t('entities.attendancerecord.main_grid.columns.ygbh')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.attendancerecord.main_grid.columns.ygbh')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.ygbh}}</span>
            ...
            <template v-if="getColumnState('zjhm')">
                <el-table-column show-overflow-tooltip :prop="'zjhm'" :label="$t('entities.attendancerecord.main_grid.columns.zjhm')" :width="200"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.attendancerecord.main_grid.columns.zjhm')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.zjhm}}</span>
            ...
            <template v-if="getColumnState('zzdzs')">
                <el-table-column show-overflow-tooltip :prop="'zzdzs'" :label="$t('entities.attendancerecord.main_grid.columns.zzdzs')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.attendancerecord.main_grid.columns.zzdzs')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.zzdzs}}</span>
            ...
            <template v-if="getColumnState('ormorgsectorname')">
                <el-table-column show-overflow-tooltip :prop="'ormorgsectorname'" :label="$t('entities.attendancerecord.main_grid.columns.ormorgsectorname')" :width="200"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.attendancerecord.main_grid.columns.ormorgsectorname')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('attenddate')">
                <el-table-column show-overflow-tooltip :prop="'attenddate'" :label="$t('entities.attendancerecord.main_grid.columns.attenddate')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.attendancerecord.main_grid.columns.attenddate')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <app-format-data format="YYYY-MM-DD" :data="row.attenddate"></app-format-data>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('dksjqd')">
                <el-table-column show-overflow-tooltip :prop="'dksjqd'" :label="$t('entities.attendancerecord.main_grid.columns.dksjqd')" :width="200"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.attendancerecord.main_grid.columns.dksjqd')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <app-format-data format="YYYY-MM-DD hh:mm:ss" :data="row.dksjqd"></app-format-data>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('dksjqt')">
                <el-table-column show-overflow-tooltip :prop="'dksjqt'" :label="$t('entities.attendancerecord.main_grid.columns.dksjqt')" :width="200"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.attendancerecord.main_grid.columns.dksjqt')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <app-format-data format="YYYY-MM-DD hh:mm:ss" :data="row.dksjqt"></app-format-data>
                    </template>
                </el-table-column>
            </template>
...
import AttEndanceRecordService from '@/service/att-endance-record/att-endance-record-service';
...
export default class MainBase extends Vue implements ControlInterface {
     ...
     * 实体服务对象
     *
     * @type {AttEndanceRecordService}
     * @memberof MainBase
     */
    public appEntityService: AttEndanceRecordService = new AttEndanceRecordService({ $store: this.$store });
    ...
    */
    ...
    public allColumns: any[] = [
        {
            name: 'pimpersonname',
            label: '员工姓名',
            langtag: 'entities.attendancerecord.main_grid.columns.pimpersonname',
            show: true,
            util: 'PX'
        },
        {
            name: 'ygbh',
            label: '员工编号',
            langtag: 'entities.attendancerecord.main_grid.columns.ygbh',
            show: true,
            util: 'px'
        },
        {
            name: 'zjhm',
            label: '证件号码',
            langtag: 'entities.attendancerecord.main_grid.columns.zjhm',
            show: true,
            util: 'PX'
        },
        {
            name: 'zzdzs',
            label: '组织名称',
            langtag: 'entities.attendancerecord.main_grid.columns.zzdzs',
            show: true,
            util: 'px'
        },
        {
            ...
            label: '部门/项目部',
            langtag: 'entities.attendancerecord.main_grid.columns.ormorgsectorname',
            show: true,
            util: 'PX'
        },
        {
            name: 'attenddate',
            label: '考勤日期',
            langtag: 'entities.attendancerecord.main_grid.columns.attenddate',
            show: true,
            util: 'px'
        },
        {
            name: 'dksjqd',
            label: '上班打卡时间',
            langtag: 'entities.attendancerecord.main_grid.columns.dksjqd',
            show: true,
            util: 'PX'
        },
        {
            name: 'dksjqt',
            label: '下班打卡时间',
            langtag: 'entities.attendancerecord.main_grid.columns.dksjqt',
            show: true,
            util: 'PX'
        },
    ]
    ...
    public rules: any = {
        srfkey: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '考勤记录标识 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '考勤记录标识 值不能为空', trigger: 'blur' },
        ],
    }
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'AttEndanceRecordGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'AttEndanceRecordGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        _datas.forEach((record: any, index: number) => {
            let srfmajortext = record.attendancerecordname;
            if (index < 5) {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ attendancerecord: _keys }),Object.assign({ attendancerecord: _keys },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    public rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.attendancerecord,$event.attendancerecord);
        });
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('attendancerecord_main_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('attendancerecord_main_grid', JSON.stringify(this.allColumns));
    }
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: '错误', desc: 'AttEndanceRecordGridView视图表格createAction参数未配置' });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: '错误', desc: 'AttEndanceRecordGridView视图表格updateAction参数未配置' });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
                        if(item.attendancerecord){
                            Object.assign(this.context,{attendancerecord:item.attendancerecord});
                        }
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'AttEndanceRecordGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
    ...
    public getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.attendancerecord,args.row.attendancerecord);
        });
