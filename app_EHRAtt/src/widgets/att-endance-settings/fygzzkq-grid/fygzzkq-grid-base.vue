    ...
    <el-table v-if="isDisplay === true"
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('ygbh')">
                <el-table-column show-overflow-tooltip :prop="'ygbh'" :label="$t('entities.attendancesettings.fygzzkq_grid.columns.ygbh')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.attendancesettings.fygzzkq_grid.columns.ygbh')}}
                      </span>
                    </template>
            ...
            <template v-if="getColumnState('pimpersonname')">
                <el-table-column show-overflow-tooltip :prop="'pimpersonname'" :label="$t('entities.attendancesettings.fygzzkq_grid.columns.pimpersonname')" :width="200"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.attendancesettings.fygzzkq_grid.columns.pimpersonname')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('zzdzs')">
                <el-table-column show-overflow-tooltip :prop="'zzdzs'" :label="$t('entities.attendancesettings.fygzzkq_grid.columns.zzdzs')" :width="200"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.attendancesettings.fygzzkq_grid.columns.zzdzs')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.zzdzs}}</span>
            ...
            <template v-if="getColumnState('ormorgsectorname')">
                <el-table-column show-overflow-tooltip :prop="'ormorgsectorname'" :label="$t('entities.attendancesettings.fygzzkq_grid.columns.ormorgsectorname')" :width="200"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.attendancesettings.fygzzkq_grid.columns.ormorgsectorname')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('ygzt')">
                <el-table-column show-overflow-tooltip :prop="'ygzt'" :label="$t('entities.attendancesettings.fygzzkq_grid.columns.ygzt')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.attendancesettings.fygzzkq_grid.columns.ygzt')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
            ...
            <template v-if="getColumnState('attendencesetupname')">
                <el-table-column show-overflow-tooltip :prop="'attendencesetupname'" :label="$t('entities.attendancesettings.fygzzkq_grid.columns.attendencesetupname')" :width="200"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.attendancesettings.fygzzkq_grid.columns.attendencesetupname')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('ormorgname2')">
                <el-table-column show-overflow-tooltip :prop="'ormorgname2'" :label="$t('entities.attendancesettings.fygzzkq_grid.columns.ormorgname2')" :width="200"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.attendancesettings.fygzzkq_grid.columns.ormorgname2')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('dkfs')">
                <el-table-column show-overflow-tooltip :prop="'dkfs'" :label="$t('entities.attendancesettings.fygzzkq_grid.columns.dkfs')" :width="200"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.attendancesettings.fygzzkq_grid.columns.dkfs')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
...
import FYGZZKQService from './fygzzkq-grid-service';
...
export default class FYGZZKQBase extends Vue implements ControlInterface {

    /**
     * 名称
     *
     * @type {string}
     * @memberof FYGZZKQBase
     */
     ...
     * @memberof FYGZZKQBase
     */
    public service: FYGZZKQService = new FYGZZKQService({ $store: this.$store });
     ...
     * @memberof FYGZZKQBase
     */
    public appEntityService: AttEndanceSettingsService = new AttEndanceSettingsService({ $store: this.$store });
    ...
    */
    ...
    public allColumns: any[] = [
        {
            name: 'ygbh',
            label: '员工编号',
            langtag: 'entities.attendancesettings.fygzzkq_grid.columns.ygbh',
            show: true,
            util: 'PX'
        },
        {
            name: 'pimpersonname',
            label: '员工姓名',
            langtag: 'entities.attendancesettings.fygzzkq_grid.columns.pimpersonname',
            show: true,
            util: 'PX'
        },
        {
            name: 'zzdzs',
            label: '人员组织简称',
            langtag: 'entities.attendancesettings.fygzzkq_grid.columns.zzdzs',
            show: true,
            util: 'PX'
        },
        {
            ...
            label: '部门',
            langtag: 'entities.attendancesettings.fygzzkq_grid.columns.ormorgsectorname',
            show: true,
            util: 'PX'
        },
        {
            name: 'ygzt',
            label: '员工状态',
            langtag: 'entities.attendancesettings.fygzzkq_grid.columns.ygzt',
            show: true,
            util: 'px'
        },
        {
            ...
            label: '所属考勤表',
            langtag: 'entities.attendancesettings.fygzzkq_grid.columns.attendencesetupname',
            show: true,
            util: 'PX'
        },
        {
            name: 'ormorgname2',
            label: '考勤表组织简称',
            langtag: 'entities.attendancesettings.fygzzkq_grid.columns.ormorgname2',
            show: true,
            util: 'PX'
        },
        {
            name: 'dkfs',
            label: '打卡方式',
            langtag: 'entities.attendancesettings.fygzzkq_grid.columns.dkfs',
            show: true,
            util: 'PX'
        },
    ]
    ...
    public rules: any = {
        dkfs: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '打卡方式 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '打卡方式 值不能为空', trigger: 'blur' },
        ],
        pimpersonid: [
        ...
        srfkey: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '考勤人员标识 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '考勤人员标识 值不能为空', trigger: 'blur' },
        ],
    }
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'AttEndanceSettingsFYGZZKQGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'AttEndanceSettingsFYGZZKQGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        _datas.forEach((record: any, index: number) => {
            let srfmajortext = record.attendancesettingsname;
            if (index < 5) {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ attendancesettings: _keys }),Object.assign({ attendancesettings: _keys },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    public rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.attendancesettings,$event.attendancesettings);
        });
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('attendancesettings_fygzzkq_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('attendancesettings_fygzzkq_grid', JSON.stringify(this.allColumns));
    }
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: '错误', desc: 'AttEndanceSettingsFYGZZKQGridView视图表格createAction参数未配置' });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: '错误', desc: 'AttEndanceSettingsFYGZZKQGridView视图表格updateAction参数未配置' });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
                        if(item.attendancesettings){
                            Object.assign(this.context,{attendancesettings:item.attendancesettings});
                        }
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'AttEndanceSettingsFYGZZKQGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
    ...
    public getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.attendancesettings,args.row.attendancesettings);
        });
