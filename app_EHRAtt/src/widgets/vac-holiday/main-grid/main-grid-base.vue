...
<div class='grid' style="height:100%;">
    ...
    <el-table v-if="isDisplay === true"
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('vacholidayname')">
                <el-table-column show-overflow-tooltip :prop="'vacholidayname'" :label="$t('entities.vacholiday.main_grid.columns.vacholidayname')" :width="200"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.vacholiday.main_grid.columns.vacholidayname')}}
                      </span>
                    </template>
              ...
              @change="($event)=>{gridEditItemChange(row, column.property, $event, $index)}">
                        ...
                        <template v-if="!actualIsOpenEdit">
                                <app-span name='vacholidayname' editorType="TEXTBOX" :value="row.vacholidayname"></app-span>
                        </template>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('jjrlx')">
                <el-table-column show-overflow-tooltip :prop="'jjrlx'" :label="$t('entities.vacholiday.main_grid.columns.jjrlx')" :width="200"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.vacholiday.main_grid.columns.jjrlx')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template v-if="actualIsOpenEdit">
             ...
             <dropdown-list 
              ...
              @change="($event)=>{gridEditItemChange(row, column.property, $event, $index)}">
                        ...
                        <template v-if="!actualIsOpenEdit">
            <codelist :value="row.jjrlx" tag='EhrCodeList0052' codelistType='STATIC' ></codelist>
                        </template>
                    </template>
                </el-table-column>
            </template>
...
export default class MainBase extends Vue implements ControlInterface {
     ...
     * @memberof MainBase
     */
    public appEntityService: VacHolidayService = new VacHolidayService({ $store: this.$store });
     ...
     * @memberof MainBase
     */
    public minorSortPSDEF: string = 'kssj';
    ...
    */
    ...
    public allColumns: any[] = [
        {
            name: 'vacholidayname',
            label: '名称',
            langtag: 'entities.vacholiday.main_grid.columns.vacholidayname',
            show: true,
            util: 'PX'
        },
        {
    ...
    public rules: any = {
        ...
        jssj: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '结束时间 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '结束时间 值不能为空', trigger: 'blur' },
        ],
        kssj: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '开始时间 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '开始时间 值不能为空', trigger: 'blur' },
        ],
        vacholidayrulesid: [
            ...
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '考勤规则标识 值不能为空', trigger: 'blur' },
        ],
        srfkey: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '节假日管理标识 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '节假日管理标识 值不能为空', trigger: 'blur' },
        ],
        jjrlx: [
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'VacHolidayJJRTXSBGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'VacHolidayJJRTXSBGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        _datas.forEach((record: any, index: number) => {
            let srfmajortext = record.vacholidayname;
            if (index < 5) {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ vacholiday: _keys }),Object.assign({ vacholiday: _keys },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    public rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.vacholiday,$event.vacholiday);
        });
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('vacholiday_main_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('vacholiday_main_grid', JSON.stringify(this.allColumns));
    }
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: '错误', desc: 'VacHolidayJJRTXSBGridView视图表格createAction参数未配置' });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: '错误', desc: 'VacHolidayJJRTXSBGridView视图表格updateAction参数未配置' });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
                        if(item.vacholiday){
                            Object.assign(this.context,{vacholiday:item.vacholiday});
                        }
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'VacHolidayJJRTXSBGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
    ...
    public getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.vacholiday,args.row.vacholiday);
        });
    ...
    public createDefault(row: any){                    
        if (row.hasOwnProperty('vacholidayrulesid')) {
            row['vacholidayrulesid'] = this.viewparams['srfparentkey'];
        }
    }
}
</script>
