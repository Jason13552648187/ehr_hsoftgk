    ...
    <el-table v-if="isDisplay === true"
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('qjzl')">
                <el-table-column show-overflow-tooltip :prop="'qjzl'" :label="$t('entities.vacleavetips.main_grid.columns.qjzl')" :width="200"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.vacleavetips.main_grid.columns.qjzl')}}
                      </span>
                    </template>
             ...
             <dropdown-list 
              ...
              @change="($event)=>{gridEditItemChange(row, column.property, $event, $index)}">
                        ...
                        <template v-if="!actualIsOpenEdit">
            <codelist :value="row.qjzl" tag='EhrCodeList0013' codelistType='DYNAMIC' renderMode="STR" valueSeparator=";" textSeparator="、" ></codelist>
                        </template>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('tips')">
                <el-table-column show-overflow-tooltip :prop="'tips'" :label="$t('entities.vacleavetips.main_grid.columns.tips')" :min-width="300"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.vacleavetips.main_grid.columns.tips')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template v-if="actualIsOpenEdit">
              ...
              @change="($event)=>{gridEditItemChange(row, column.property, $event, $index)}">
                        ...
                        <template v-if="!actualIsOpenEdit">
                                <app-span name='tips' editorType="TEXTBOX" :value="row.tips"></app-span>
                        </template>
                    </template>
                </el-table-column>
            </template>
...
import VacLeaveTipsService from '@/service/vac-leave-tips/vac-leave-tips-service';
...
export default class MainBase extends Vue implements ControlInterface {
     ...
     * 实体服务对象
     *
     * @type {VacLeaveTipsService}
     * @memberof MainBase
     */
    public appEntityService: VacLeaveTipsService = new VacLeaveTipsService({ $store: this.$store });
    ...
    */
    ...
    public allColumns: any[] = [
        {
            name: 'qjzl',
            label: '请假种类',
            langtag: 'entities.vacleavetips.main_grid.columns.qjzl',
            show: true,
            util: 'PX'
        },
        {
            name: 'tips',
            label: '温馨提示',
            langtag: 'entities.vacleavetips.main_grid.columns.tips',
            show: true,
            util: 'STAR'
        },
    ]
    ...
    public getGridRowModel(){
        return {
          qjzl: new FormItemModel(),
          vacholidayrulesid: new FormItemModel(),
          tips: new FormItemModel(),
    ...
    public rules: any = {
        qjzl: [
             { required: true, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '请假种类 值不能为空', trigger: 'change' },
            { required: true, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '请假种类 值不能为空', trigger: 'blur' },
        ],
        vacholidayrulesid: [
        ...
        tips: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '温馨提示 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '温馨提示 值不能为空', trigger: 'blur' },
        ],
        srfkey: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '请假提示标识 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '请假提示标识 值不能为空', trigger: 'blur' },
        ],
    }
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'VacLeaveTipsQJTSGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'VacLeaveTipsQJTSGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        _datas.forEach((record: any, index: number) => {
            let srfmajortext = record.vacleavetipsname;
            if (index < 5) {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ vacleavetips: _keys }),Object.assign({ vacleavetips: _keys },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    public rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.vacleavetips,$event.vacleavetips);
        });
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('vacleavetips_main_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('vacleavetips_main_grid', JSON.stringify(this.allColumns));
    }
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: '错误', desc: 'VacLeaveTipsQJTSGridView视图表格createAction参数未配置' });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: '错误', desc: 'VacLeaveTipsQJTSGridView视图表格updateAction参数未配置' });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
                        if(item.vacleavetips){
                            Object.assign(this.context,{vacleavetips:item.vacleavetips});
                        }
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'VacLeaveTipsQJTSGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
    ...
    public gridEditItemChange(row: any, property: string, value: any, rowIndex: number){
        ...
        if(Object.is(property, 'qjzl')){
            const details: string[] = ['qjzl'];
            this.updateGridEditItem('CheckRepeat', row, details, true);
        }
    }
    ...
    public getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.vacleavetips,args.row.vacleavetips);
        });
