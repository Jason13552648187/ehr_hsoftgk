...
<div class='grid' style="height:100%;">
    ...
    <el-table v-if="isDisplay === true"
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('dm')">
                <el-table-column show-overflow-tooltip :prop="'dm'" :label="$t('entities.trmcoursesystem.main_grid.columns.dm')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.trmcoursesystem.main_grid.columns.dm')}}
                      </span>
                    </template>
            ...
            <template v-if="getColumnState('trmcoursesystemname')">
                <el-table-column show-overflow-tooltip :prop="'trmcoursesystemname'" :label="$t('entities.trmcoursesystem.main_grid.columns.trmcoursesystemname')" :width="200"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.trmcoursesystem.main_grid.columns.trmcoursesystemname')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('dygx')">
                <el-table-column show-overflow-tooltip :prop="'dygx'" :label="$t('entities.trmcoursesystem.main_grid.columns.dygx')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.trmcoursesystem.main_grid.columns.dygx')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.dygx}}</span>
            ...
            <template v-if="getColumnState('jb')">
                <el-table-column show-overflow-tooltip :prop="'jb'" :label="$t('entities.trmcoursesystem.main_grid.columns.jb')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.trmcoursesystem.main_grid.columns.jb')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
            <codelist :value="row.jb" tag='EhrCodeList0082' codelistType='STATIC' renderMode="STR" valueSeparator=";" textSeparator="、" ></codelist>
                        </template>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('nr')">
                <el-table-column show-overflow-tooltip :prop="'nr'" :label="$t('entities.trmcoursesystem.main_grid.columns.nr')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.trmcoursesystem.main_grid.columns.nr')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.nr}}</span>
...
export default class MainBase extends Vue implements ControlInterface {
     ...
     * @memberof MainBase
     */
    public appEntityService: TrmCourseSystemService = new TrmCourseSystemService({ $store: this.$store });
    ...
    */
    ...
    public allColumns: any[] = [
        ...
        },
        {
            name: 'dygx',
            label: '对应关系',
            langtag: 'entities.trmcoursesystem.main_grid.columns.dygx',
            show: true,
            util: 'px'
        },
        {
            name: 'jb',
            label: '级别',
            langtag: 'entities.trmcoursesystem.main_grid.columns.jb',
            show: true,
            util: 'px'
        },
        {
    ...
    public rules: any = {
        srfkey: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '课程体系标识 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '课程体系标识 值不能为空', trigger: 'blur' },
        ],
    }
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'TrmCourseSystemPickupGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'TrmCourseSystemPickupGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        _datas.forEach((record: any, index: number) => {
            let srfmajortext = record.trmcoursesystemname;
            if (index < 5) {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ trmcoursesystem: _keys }),Object.assign({ trmcoursesystem: _keys },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    public rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.trmcoursesystem,$event.trmcoursesystem);
        });
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('trmcoursesystem_main_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('trmcoursesystem_main_grid', JSON.stringify(this.allColumns));
    }
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: '错误', desc: 'TrmCourseSystemPickupGridView视图表格createAction参数未配置' });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: '错误', desc: 'TrmCourseSystemPickupGridView视图表格updateAction参数未配置' });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
                        if(item.trmcoursesystem){
                            Object.assign(this.context,{trmcoursesystem:item.trmcoursesystem});
                        }
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'TrmCourseSystemPickupGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
    ...
    public getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.trmcoursesystem,args.row.trmcoursesystem);
        });
