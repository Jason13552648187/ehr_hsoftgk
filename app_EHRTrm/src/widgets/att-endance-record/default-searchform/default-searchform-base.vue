    ...
    <i-col span="20" class="form-content">
      <row>
          ...
          <i-col v-show="detailsModel.n_ormorgname_like.visible" :style="{}"  :lg="{ span: 6, offset: 0 }">
              <app-form-item name='n_ormorgname_like' :itemRules="this.rules.n_ormorgname_like" class='' :caption="$t('entities.attendancerecord.default_searchform.details.n_ormorgname_like')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.n_ormorgname_like.error" :isEmptyCaption="false" labelPos="LEFT"> 
          ...
          <i-col v-show="detailsModel.n_ormorgsectorname_like.visible" :style="{}"  :lg="{ span: 6, offset: 0 }">
              <app-form-item name='n_ormorgsectorname_like' :itemRules="this.rules.n_ormorgsectorname_like" class='' :caption="$t('entities.attendancerecord.default_searchform.details.n_ormorgsectorname_like')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.n_ormorgsectorname_like.error" :isEmptyCaption="false" labelPos="LEFT"> 
...
export default class DefaultBase extends Vue implements ControlInterface {
    ...
    public rules: any = {
        ...
        n_dkfs_eq: [
            { type: 'string', message: '考勤方式(=) 值必须为字符串类型', trigger: 'change' },
            { type: 'string', message: '考勤方式(=) 值必须为字符串类型', trigger: 'blur' },
            { required: false, type: 'string', message: '考勤方式(=) 值不能为空', trigger: 'change' },
            { required: false, type: 'string', message: '考勤方式(=) 值不能为空', trigger: 'blur' },
        ],
        n_ormorgname_like: [
...
, 
        n_dkfs_eq: new FormItemModel({ caption: '考勤方式(=)', detailType: 'FORMITEM', name: 'n_dkfs_eq', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 3 })
, 
...
, 
        n_ormorgsectorname_like: new FormItemModel({ caption: '部门/项目部(%)', detailType: 'FORMITEM', name: 'n_ormorgsectorname_like', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 3 })
, 
    };
     ...
     * @memberof DefaultBase
     */
    @Watch('data.n_dkfs_eq')
    onN_dkfs_eqChange(newVal: any, oldVal: any) {
        this.formDataChange({ name: 'n_dkfs_eq', newVal: newVal, oldVal: oldVal });
    }
    ...
    public load(opt: any = {}): void {
        if(!this.loadAction){
            this.$Notice.error({ title: '错误', desc: 'AttEndanceRecordGridView视图搜索表单loadAction参数未配置' });
            return;
        }
        const arg: any = { ...opt };
    ...
    public loadDraft(opt: any = {},mode?:string): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'AttEndanceRecordGridView视图搜索表单loaddraftAction参数未配置' });
            return;
        }
        const arg: any = { ...opt } ;
        Object.assign(arg,{viewparams:this.viewparams});
