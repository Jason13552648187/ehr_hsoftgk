<template>
  ...
  <row>
    <i-col span="20" class="form-content">
      <row>
                    <i-col v-show="detailsModel.n_trmtrainoutapplyname_like.visible" :style="{}"  :lg="{ span: 8, offset: 0 }">
              <app-form-item name='n_trmtrainoutapplyname_like' :itemRules="this.rules.n_trmtrainoutapplyname_like" class='' :caption="$t('entities.trmtrainoutapply.default_searchform.details.n_trmtrainoutapplyname_like')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.n_trmtrainoutapplyname_like.error" :isEmptyCaption="false" labelPos="LEFT"> 
              <input-box v-model="data.n_trmtrainoutapplyname_like"  @enter="onEnter($event)"    :disabled="detailsModel.n_trmtrainoutapplyname_like.disabled" type='text'  style=""></input-box>
          </app-form-item>
          ...
          <i-col v-show="detailsModel.n_pimpersonname_like.visible" :style="{}"  :lg="{ span: 8, offset: 0 }">
              <app-form-item name='n_pimpersonname_like' :itemRules="this.rules.n_pimpersonname_like" class='' :caption="$t('entities.trmtrainoutapply.default_searchform.details.n_pimpersonname_like')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.n_pimpersonname_like.error" :isEmptyCaption="false" labelPos="LEFT"> 
...
import TrmTrainOutApplyService from '@/service/trm-train-out-apply/trm-train-out-apply-service';
...
export default class DefaultBase extends Vue implements ControlInterface {
     ...
     * 实体服务对象
     *
     * @type {TrmTrainOutApplyService}
     * @memberof DefaultBase
     */
    public appEntityService: TrmTrainOutApplyService = new TrmTrainOutApplyService({ $store: this.$store });
     ...
     */
    public data: any = {
        n_trmtrainoutapplyname_like: null,
        n_pxfs_eq: null,
        n_pxlb_eq: null,
        n_ywlx_eq: null,
        n_wbpxjg_eq: null,
     ...
     */
    public rules: any = {
        n_trmtrainoutapplyname_like: [
            { type: 'string', message: '培训名称 值必须为字符串类型', trigger: 'change' },
            { type: 'string', message: '培训名称 值必须为字符串类型', trigger: 'blur' },
            { required: false, type: 'string', message: '培训名称 值不能为空', trigger: 'change' },
            { required: false, type: 'string', message: '培训名称 值不能为空', trigger: 'blur' },
        ],
        n_pxfs_eq: [
        ...
        n_ywlx_eq: [
            { type: 'string', message: '业务类型(=) 值必须为字符串类型', trigger: 'change' },
            { type: 'string', message: '业务类型(=) 值必须为字符串类型', trigger: 'blur' },
            { required: false, type: 'string', message: '业务类型(=) 值不能为空', trigger: 'change' },
            { required: false, type: 'string', message: '业务类型(=) 值不能为空', trigger: 'blur' },
        ],
        n_wbpxjg_eq: [
            { type: 'string', message: '外部培训机构(=) 值必须为字符串类型', trigger: 'change' },
            { type: 'string', message: '外部培训机构(=) 值必须为字符串类型', trigger: 'blur' },
...
, 
        n_trmtrainoutapplyname_like: new FormItemModel({ caption: '培训名称', detailType: 'FORMITEM', name: 'n_trmtrainoutapplyname_like', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 3 })
, 
...
, 
        n_ywlx_eq: new FormItemModel({ caption: '业务类型(=)', detailType: 'FORMITEM', name: 'n_ywlx_eq', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 3 })
, 
        n_wbpxjg_eq: new FormItemModel({ caption: '外部培训机构(=)', detailType: 'FORMITEM', name: 'n_wbpxjg_eq', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 3 })
, 
...
, 
    ...
    /**
     * 监控表单属性 n_trmtrainoutapplyname_like 值
     *
     * @param {*} newVal
     * @param {*} oldVal
     * @memberof DefaultBase
     */
    @Watch('data.n_trmtrainoutapplyname_like')
    onN_trmtrainoutapplyname_likeChange(newVal: any, oldVal: any) {
        this.formDataChange({ name: 'n_trmtrainoutapplyname_like', newVal: newVal, oldVal: oldVal });
    }
     ...
     * @memberof DefaultBase
     */
    @Watch('data.n_ywlx_eq')
    onN_ywlx_eqChange(newVal: any, oldVal: any) {
        this.formDataChange({ name: 'n_ywlx_eq', newVal: newVal, oldVal: oldVal });
    }

    /**
     * 监控表单属性 n_wbpxjg_eq 值
     *
     * @param {*} newVal
     * @param {*} oldVal
     * @memberof DefaultBase
     */
    @Watch('data.n_wbpxjg_eq')
    onN_wbpxjg_eqChange(newVal: any, oldVal: any) {
        this.formDataChange({ name: 'n_wbpxjg_eq', newVal: newVal, oldVal: oldVal });
    }
    ...
    public load(opt: any = {}): void {
        if(!this.loadAction){
            this.$Notice.error({ title: '错误', desc: 'TrmTrainOutApplyGridView视图搜索表单loadAction参数未配置' });
            return;
        }
        const arg: any = { ...opt };
    ...
    public loadDraft(opt: any = {},mode?:string): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'TrmTrainOutApplyGridView视图搜索表单loaddraftAction参数未配置' });
            return;
        }
        const arg: any = { ...opt } ;
        Object.assign(arg,{viewparams:this.viewparams});
