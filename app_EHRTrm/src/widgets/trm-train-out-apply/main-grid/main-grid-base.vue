...
<div class='grid' style="height:100%;">
    ...
    <el-table v-if="isDisplay === true"
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('trmtrainoutapplyname')">
                <el-table-column show-overflow-tooltip :prop="'trmtrainoutapplyname'" :label="$t('entities.trmtrainoutapply.main_grid.columns.trmtrainoutapplyname')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.trmtrainoutapply.main_grid.columns.trmtrainoutapplyname')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.trmtrainoutapplyname}}</span>
                    </template>
                </el-table-column>
            </template>
            ...
            <template v-if="getColumnState('pxnr')">
                <el-table-column show-overflow-tooltip :prop="'pxnr'" :label="$t('entities.trmtrainoutapply.main_grid.columns.pxnr')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.trmtrainoutapply.main_grid.columns.pxnr')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.pxnr}}</span>
            ...
            <template v-if="getColumnState('pxmb')">
                <el-table-column show-overflow-tooltip :prop="'pxmb'" :label="$t('entities.trmtrainoutapply.main_grid.columns.pxmb')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.trmtrainoutapply.main_grid.columns.pxmb')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.pxmb}}</span>
            ...
            <template v-if="getColumnState('pxdd')">
                <el-table-column show-overflow-tooltip :prop="'pxdd'" :label="$t('entities.trmtrainoutapply.main_grid.columns.pxdd')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.trmtrainoutapply.main_grid.columns.pxdd')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.pxdd}}</span>
            ...
            <template v-if="getColumnState('pxksrq')">
                <el-table-column show-overflow-tooltip :prop="'pxksrq'" :label="$t('entities.trmtrainoutapply.main_grid.columns.pxksrq')" :width="140"  :align="'left'" :sortable="'custom'">
                    ...
                    <template v-slot="{row,column,$index}">
                        <app-format-data format="YYYY-MM-DD" :data="row.pxksrq"></app-format-data>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('pxjsrq')">
                <el-table-column show-overflow-tooltip :prop="'pxjsrq'" :label="$t('entities.trmtrainoutapply.main_grid.columns.pxjsrq')" :width="140"  :align="'left'" :sortable="'custom'">
                    ...
                    <template v-slot="{row,column,$index}">
                        <app-format-data format="YYYY-MM-DD" :data="row.pxjsrq"></app-format-data>
                    </template>
                </el-table-column>
            </template>
    ...
    */
    ...
    public allColumns: any[] = [
        ...
        },
        {
            name: 'pxfs',
            label: '培训方式',
            langtag: 'entities.trmtrainoutapply.main_grid.columns.pxfs',
            show: true,
            util: 'px'
        },
        {
            name: 'pxnr',
            label: '培训内容',
            langtag: 'entities.trmtrainoutapply.main_grid.columns.pxnr',
            show: true,
            util: 'px'
        },
        {
            name: 'pxmb',
            label: '培训目标',
            langtag: 'entities.trmtrainoutapply.main_grid.columns.pxmb',
            show: true,
            util: 'px'
        },
        {
            name: 'pxdd',
            label: '培训地点',
            langtag: 'entities.trmtrainoutapply.main_grid.columns.pxdd',
            show: true,
            util: 'px'
        },
        {
            name: 'pxlb',
            label: '培训类别',
            langtag: 'entities.trmtrainoutapply.main_grid.columns.pxlb',
            show: true,
            util: 'px'
        },
        {
            name: 'pxksrq',
            label: '培训开始日期',
            langtag: 'entities.trmtrainoutapply.main_grid.columns.pxksrq',
            show: true,
            util: 'px'
        },
        {
            name: 'pxjsrq',
            label: '培训结束日期',
            langtag: 'entities.trmtrainoutapply.main_grid.columns.pxjsrq',
            show: true,
            util: 'px'
        },
    ]
    ...
    public rules: any = {
        srfkey: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '外部培训申请标识 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '外部培训申请标识 值不能为空', trigger: 'blur' },
        ],
    }
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'TrmTrainOutApplyGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'TrmTrainOutApplyGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        _datas.forEach((record: any, index: number) => {
            let srfmajortext = record.trmtrainoutapplyname;
            if (index < 5) {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ trmtrainoutapply: _keys }),Object.assign({ trmtrainoutapply: _keys },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    public rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.trmtrainoutapply,$event.trmtrainoutapply);
        });
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('trmtrainoutapply_main_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('trmtrainoutapply_main_grid', JSON.stringify(this.allColumns));
    }
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: '错误', desc: 'TrmTrainOutApplyGridView视图表格createAction参数未配置' });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: '错误', desc: 'TrmTrainOutApplyGridView视图表格updateAction参数未配置' });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
                        if(item.trmtrainoutapply){
                            Object.assign(this.context,{trmtrainoutapply:item.trmtrainoutapply});
                        }
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'TrmTrainOutApplyGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
    ...
    public getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.trmtrainoutapply,args.row.trmtrainoutapply);
        });
