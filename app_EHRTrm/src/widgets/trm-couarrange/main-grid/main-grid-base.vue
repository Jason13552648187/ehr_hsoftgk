    ...
    <el-table v-if="isDisplay === true"
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('trmtrainplantermname')">
                <el-table-column show-overflow-tooltip :prop="'trmtrainplantermname'" :label="$t('entities.trmcouarrange.main_grid.columns.trmtrainplantermname')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.trmcouarrange.main_grid.columns.trmtrainplantermname')}}
                      </span>
                    </template>
            ...
            <template v-if="getColumnState('trmtraincoursename')">
                <el-table-column show-overflow-tooltip :prop="'trmtraincoursename'" :label="$t('entities.trmcouarrange.main_grid.columns.trmtraincoursename')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.trmcouarrange.main_grid.columns.trmtraincoursename')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.trmtraincoursename}}</span>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('trmtrainagencyname')">
                <el-table-column show-overflow-tooltip :prop="'trmtrainagencyname'" :label="$t('entities.trmcouarrange.main_grid.columns.trmtrainagencyname')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.trmcouarrange.main_grid.columns.trmtrainagencyname')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('trmtrainaddressname')">
                <el-table-column show-overflow-tooltip :prop="'trmtrainaddressname'" :label="$t('entities.trmcouarrange.main_grid.columns.trmtrainaddressname')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.trmcouarrange.main_grid.columns.trmtrainaddressname')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('trmtrainteachername')">
                <el-table-column show-overflow-tooltip :prop="'trmtrainteachername'" :label="$t('entities.trmcouarrange.main_grid.columns.trmtrainteachername')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.trmcouarrange.main_grid.columns.trmtrainteachername')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.trmtrainteachername}}</span>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('skkssj')">
                <el-table-column show-overflow-tooltip :prop="'skkssj'" :label="$t('entities.trmcouarrange.main_grid.columns.skkssj')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.trmcouarrange.main_grid.columns.skkssj')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <app-format-data format="YYYY-MM-DD" :data="row.skkssj"></app-format-data>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('skjssj')">
                <el-table-column show-overflow-tooltip :prop="'skjssj'" :label="$t('entities.trmcouarrange.main_grid.columns.skjssj')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.trmcouarrange.main_grid.columns.skjssj')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <app-format-data format="YYYY-MM-DD" :data="row.skjssj"></app-format-data>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('pj')">
                <el-table-column show-overflow-tooltip :prop="'pj'" :label="$t('entities.trmcouarrange.main_grid.columns.pj')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.trmcouarrange.main_grid.columns.pj')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.pj}}</span>
...
import TrmCouarrangeService from '@/service/trm-couarrange/trm-couarrange-service';
...
export default class MainBase extends Vue implements ControlInterface {
     ...
     * 实体服务对象
     *
     * @type {TrmCouarrangeService}
     * @memberof MainBase
     */
    public appEntityService: TrmCouarrangeService = new TrmCouarrangeService({ $store: this.$store });
    ...
    */
    ...
    public allColumns: any[] = [
        {
            ...
            label: '班级',
            langtag: 'entities.trmcouarrange.main_grid.columns.trmtrainplantermname',
            show: true,
            util: 'px'
        },
        {
            ...
            label: '课程',
            langtag: 'entities.trmcouarrange.main_grid.columns.trmtraincoursename',
            show: true,
            util: 'px'
        },
        {
            ...
            label: '培训机构',
            langtag: 'entities.trmcouarrange.main_grid.columns.trmtrainagencyname',
            show: true,
            util: 'px'
        },
        {
            ...
            label: '培训场地',
            langtag: 'entities.trmcouarrange.main_grid.columns.trmtrainaddressname',
            show: true,
            util: 'px'
        },
        {
            ...
            label: '讲师',
            langtag: 'entities.trmcouarrange.main_grid.columns.trmtrainteachername',
            show: true,
            util: 'px'
        },
        {
            name: 'skkssj',
            label: '授课开始时间',
            langtag: 'entities.trmcouarrange.main_grid.columns.skkssj',
            show: true,
            util: 'px'
        },
        {
            name: 'skjssj',
            label: '授课结束时间',
            langtag: 'entities.trmcouarrange.main_grid.columns.skjssj',
            show: true,
            util: 'px'
        },
        {
            name: 'pj',
            label: '评价',
            langtag: 'entities.trmcouarrange.main_grid.columns.pj',
            show: true,
            util: 'px'
        },
    ]
    ...
    public rules: any = {
        srfkey: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '课程安排标识 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '课程安排标识 值不能为空', trigger: 'blur' },
        ],
    }
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'TrmCouarrangeGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'TrmCouarrangeGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        _datas.forEach((record: any, index: number) => {
            let srfmajortext = record.trmcouarrangename;
            if (index < 5) {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ trmcouarrange: _keys }),Object.assign({ trmcouarrange: _keys },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    public rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.trmcouarrange,$event.trmcouarrange);
        });
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('trmcouarrange_main_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('trmcouarrange_main_grid', JSON.stringify(this.allColumns));
    }
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: '错误', desc: 'TrmCouarrangeGridView视图表格createAction参数未配置' });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: '错误', desc: 'TrmCouarrangeGridView视图表格updateAction参数未配置' });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
                        if(item.trmcouarrange){
                            Object.assign(this.context,{trmcouarrange:item.trmcouarrange});
                        }
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'TrmCouarrangeGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
    ...
    public getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.trmcouarrange,args.row.trmcouarrange);
        });
    ...
    public getCellClassName(args:{row: any, column: any, rowIndex: number, columnIndex:number}){
        let hasRowEdit:any = {
          'trmtrainplantermname':false,
          'trmtraincoursename':false,
          'trmtrainagencyname':false,
          'trmtrainaddressname':false,
          'trmtrainteachername':false,
          'skkssj':false,
          'skjssj':false,
          'pj':false,
        }
