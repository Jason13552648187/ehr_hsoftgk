    ...
    <el-table v-if="isDisplay === true"
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('trmtraincoursename')">
                <el-table-column show-overflow-tooltip :prop="'trmtraincoursename'" :label="$t('entities.trmtrainplanterm.main_grid.columns.trmtraincoursename')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.trmtrainplanterm.main_grid.columns.trmtraincoursename')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('trmtrainteachername')">
                <el-table-column show-overflow-tooltip :prop="'trmtrainteachername'" :label="$t('entities.trmtrainplanterm.main_grid.columns.trmtrainteachername')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.trmtrainplanterm.main_grid.columns.trmtrainteachername')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('bzr')">
                <el-table-column show-overflow-tooltip :prop="'bzr'" :label="$t('entities.trmtrainplanterm.main_grid.columns.bzr')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.trmtrainplanterm.main_grid.columns.bzr')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.bzr}}</span>
            ...
            <template v-if="getColumnState('bjzt')">
                <el-table-column show-overflow-tooltip :prop="'bjzt'" :label="$t('entities.trmtrainplanterm.main_grid.columns.bjzt')" :width="120"  :align="'left'" :sortable="'custom'">
                    ...
                    <template v-slot="{row,column,$index}">
                        <template >
            <codelist :value="row.bjzt" tag='EhrCodeList0017' codelistType='STATIC' ></codelist>
                        </template>
                    </template>
                </el-table-column>
            </template>
    ...
    */
    ...
    public allColumns: any[] = [
        {
            name: 'nd',
            label: '年度',
            langtag: 'entities.trmtrainplanterm.main_grid.columns.nd',
            show: true,
            util: 'px'
        },
        {
        ...
        },
        {
            ...
            label: '培训机构',
            langtag: 'entities.trmtrainplanterm.main_grid.columns.trmtrainagencyname',
            show: true,
            util: 'PX'
        },
        {
        ...
        },
        {
            ...
            label: '讲师',
            langtag: 'entities.trmtrainplanterm.main_grid.columns.trmtrainteachername',
            show: true,
            util: 'px'
        },
        {
            name: 'bzr',
            label: '班主任',
            langtag: 'entities.trmtrainplanterm.main_grid.columns.bzr',
            show: true,
            util: 'px'
        },
        {
            name: 'bjzt',
            label: '班级状态',
            langtag: 'entities.trmtrainplanterm.main_grid.columns.bjzt',
            show: true,
            util: 'px'
        },
    ]
    ...
    public async remove(datas: any[]): Promise<any> {
        ...
        _datas.forEach((record: any, index: number) => {
            let srfmajortext = record.trmtrainplanname;
            if (index < 5) {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ trmtrainplanterm: _keys }),Object.assign({ trmtrainplanterm: _keys },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    public rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.trmtrainplanterm,$event.trmtrainplanterm);
        });
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('trmtrainplanterm_main_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('trmtrainplanterm_main_grid', JSON.stringify(this.allColumns));
    }
                ...
                if(Object.is(item.rowDataState, 'create')){
                    ...
                    if(!this.updateAction){
                        ...
                        if(item.trmtrainplanterm){
                            Object.assign(this.context,{trmtrainplanterm:item.trmtrainplanterm});
                        }
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'TrmTrainPlantermBJGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
    ...
    public getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.trmtrainplanterm,args.row.trmtrainplanterm);
        });
