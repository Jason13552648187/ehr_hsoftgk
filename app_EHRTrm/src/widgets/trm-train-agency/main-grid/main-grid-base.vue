        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('jglx')">
                <el-table-column show-overflow-tooltip :prop="'jglx'" :label="$t('entities.trmtrainagency.main_grid.columns.jglx')" :width="120"  :align="'left'" :sortable="'custom'">
                    ...
                    <template v-slot="{row,column,$index}">
                        <template >
            <codelist :value="row.jglx" tag='EhrCodeList0111' codelistType='STATIC' ></codelist>
                        </template>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('jglb')">
                <el-table-column show-overflow-tooltip :prop="'jglb'" :label="$t('entities.trmtrainagency.main_grid.columns.jglb')" :width="120"  :align="'left'" :sortable="'custom'">
                    ...
                    <template v-slot="{row,column,$index}">
                        <template >
            <codelist :value="row.jglb" tag='EhrCodeList0130' codelistType='DYNAMIC' renderMode="STR" valueSeparator=";" textSeparator="、" ></codelist>
                        </template>
                    </template>
                </el-table-column>
            </template>
    ...
    */
    ...
    public allColumns: any[] = [
        ...
        },
        {
            name: 'jglx',
            label: '机构类型',
            langtag: 'entities.trmtrainagency.main_grid.columns.jglx',
            show: true,
            util: 'px'
        },
        {
            name: 'jglb',
            label: '机构性质',
            langtag: 'entities.trmtrainagency.main_grid.columns.jglb',
            show: true,
            util: 'px'
        },
        {
            ...
            label: '上级机构',
            langtag: 'entities.trmtrainagency.main_grid.columns.trmtrainagencyname2',
            show: true,
            util: 'PX'
        },
        {
            name: 'zcxx',
            label: '注册信息',
            langtag: 'entities.trmtrainagency.main_grid.columns.zcxx',
            show: true,
            util: 'px'
        },
        {
            name: 'pimpersonname',
            label: '联系人',
            langtag: 'entities.trmtrainagency.main_grid.columns.pimpersonname',
            show: true,
            util: 'PX'
        },
        {
            name: 'lxdh',
            label: '联系电话',
            langtag: 'entities.trmtrainagency.main_grid.columns.lxdh',
            show: true,
            util: 'px'
        },
        {
    ...
    public rules: any = {
        sfgx: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '是否共享 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '是否共享 值不能为空', trigger: 'blur' },
        ],
        srfkey: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '培训机构标识 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '培训机构标识 值不能为空', trigger: 'blur' },
        ],
    }
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'TrmTrainAgencyGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'TrmTrainAgencyGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        _datas.forEach((record: any, index: number) => {
            let srfmajortext = record.trmtrainagencyname;
            if (index < 5) {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ trmtrainagency: _keys }),Object.assign({ trmtrainagency: _keys },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    public rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.trmtrainagency,$event.trmtrainagency);
        });
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: '错误', desc: 'TrmTrainAgencyGridView视图表格createAction参数未配置' });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: '错误', desc: 'TrmTrainAgencyGridView视图表格updateAction参数未配置' });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
                        if(item.trmtrainagency){
                            Object.assign(this.context,{trmtrainagency:item.trmtrainagency});
                        }
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'TrmTrainAgencyGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
    ...
    public getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.trmtrainagency,args.row.trmtrainagency);
        });
