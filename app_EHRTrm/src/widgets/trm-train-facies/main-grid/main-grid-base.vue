...
<div class='grid' style="height:100%;">
    ...
    <el-table v-if="isDisplay === true"
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('sslb')">
                <el-table-column show-overflow-tooltip :prop="'sslb'" :label="$t('entities.trmtrainfacies.main_grid.columns.sslb')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.trmtrainfacies.main_grid.columns.sslb')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
            <codelist :value="row.sslb" tag='EhrCodeList0185' codelistType='DYNAMIC' renderMode="STR" valueSeparator=";" textSeparator="、" ></codelist>
                        </template>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('sssl')">
                <el-table-column show-overflow-tooltip :prop="'sssl'" :label="$t('entities.trmtrainfacies.main_grid.columns.sssl')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.trmtrainfacies.main_grid.columns.sssl')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.sssl}}</span>
            ...
            <template v-if="getColumnState('dd')">
                <el-table-column show-overflow-tooltip :prop="'dd'" :label="$t('entities.trmtrainfacies.main_grid.columns.dd')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.trmtrainfacies.main_grid.columns.dd')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.dd}}</span>
            ...
            <template v-if="getColumnState('yysb')">
                <el-table-column show-overflow-tooltip :prop="'yysb'" :label="$t('entities.trmtrainfacies.main_grid.columns.yysb')" :width="300"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.trmtrainfacies.main_grid.columns.yysb')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
            <codelist :value="row.yysb" tag='EhrCodeList0248' codelistType='STATIC' renderMode="STR" valueSeparator=";" textSeparator="、" ></codelist>
                        </template>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('bgsb')">
                <el-table-column show-overflow-tooltip :prop="'bgsb'" :label="$t('entities.trmtrainfacies.main_grid.columns.bgsb')" :width="300"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.trmtrainfacies.main_grid.columns.bgsb')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
            <codelist :value="row.bgsb" tag='EhrCodeList0091' codelistType='STATIC' renderMode="STR" valueSeparator=";" textSeparator="、" ></codelist>
                        </template>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('dgsb')">
                <el-table-column show-overflow-tooltip :prop="'dgsb'" :label="$t('entities.trmtrainfacies.main_grid.columns.dgsb')" :width="300"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.trmtrainfacies.main_grid.columns.dgsb')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
            <codelist :value="row.dgsb" tag='EhrCodeList0010' codelistType='STATIC' renderMode="STR" valueSeparator=";" textSeparator="、" ></codelist>
                        </template>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('sb')">
                <el-table-column show-overflow-tooltip :prop="'sb'" :label="$t('entities.trmtrainfacies.main_grid.columns.sb')" :width="300"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.trmtrainfacies.main_grid.columns.sb')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
            <codelist :value="row.sb" tag='EhrCodeList0189' codelistType='STATIC' renderMode="STR" valueSeparator=";" textSeparator="、" ></codelist>
                        </template>
                    </template>
                </el-table-column>
            </template>
...
import TrmTrainFaciesService from '@/service/trm-train-facies/trm-train-facies-service';
...
export default class MainBase extends Vue implements ControlInterface {
     ...
     * 实体服务对象
     *
     * @type {TrmTrainFaciesService}
     * @memberof MainBase
     */
    public appEntityService: TrmTrainFaciesService = new TrmTrainFaciesService({ $store: this.$store });
    ...
    */
    ...
    public allColumns: any[] = [
        {
            name: 'sslb',
            label: '类型',
            langtag: 'entities.trmtrainfacies.main_grid.columns.sslb',
            show: true,
            util: 'px'
        },
        {
            name: 'sssl',
            label: '可容纳人数',
            langtag: 'entities.trmtrainfacies.main_grid.columns.sssl',
            show: true,
            util: 'px'
        },
        {
            name: 'dd',
            label: '地址',
            langtag: 'entities.trmtrainfacies.main_grid.columns.dd',
            show: true,
            util: 'px'
        },
        {
            name: 'yysb',
            label: '影音设备',
            langtag: 'entities.trmtrainfacies.main_grid.columns.yysb',
            show: true,
            util: 'PX'
        },
        {
            name: 'bgsb',
            label: '办公设备',
            langtag: 'entities.trmtrainfacies.main_grid.columns.bgsb',
            show: true,
            util: 'PX'
        },
        {
            name: 'dgsb',
            label: '灯光设备',
            langtag: 'entities.trmtrainfacies.main_grid.columns.dgsb',
            show: true,
            util: 'PX'
        },
        {
            name: 'sb',
            label: '特殊设备',
            langtag: 'entities.trmtrainfacies.main_grid.columns.sb',
            show: true,
            util: 'PX'
        },
    ]
    ...
    public rules: any = {
        srfkey: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '培训设施标识 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '培训设施标识 值不能为空', trigger: 'blur' },
        ],
    }
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'TrmTrainFaciesGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'TrmTrainFaciesGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        _datas.forEach((record: any, index: number) => {
            let srfmajortext = record.trmtrainfaciesname;
            if (index < 5) {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ trmtrainfacies: _keys }),Object.assign({ trmtrainfacies: _keys },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    public rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.trmtrainfacies,$event.trmtrainfacies);
        });
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('trmtrainfacies_main_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('trmtrainfacies_main_grid', JSON.stringify(this.allColumns));
    }
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: '错误', desc: 'TrmTrainFaciesGridView视图表格createAction参数未配置' });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: '错误', desc: 'TrmTrainFaciesGridView视图表格updateAction参数未配置' });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
                        if(item.trmtrainfacies){
                            Object.assign(this.context,{trmtrainfacies:item.trmtrainfacies});
                        }
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'TrmTrainFaciesGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
    ...
    public getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.trmtrainfacies,args.row.trmtrainfacies);
        });
