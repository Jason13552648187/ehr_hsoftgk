    ...
    <el-table v-if="isDisplay === true"
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('pimpersonname')">
                <el-table-column show-overflow-tooltip :prop="'pimpersonname'" :label="$t('entities.trmhmatser.main_grid.columns.pimpersonname')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.trmhmatser.main_grid.columns.pimpersonname')}}
                      </span>
                    </template>
            ...
            <template v-if="getColumnState('zjhm')">
                <el-table-column show-overflow-tooltip :prop="'zjhm'" :label="$t('entities.trmhmatser.main_grid.columns.zjhm')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.trmhmatser.main_grid.columns.zjhm')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.zjhm}}</span>
            ...
            <template v-if="getColumnState('xb')">
                <el-table-column show-overflow-tooltip :prop="'xb'" :label="$t('entities.trmhmatser.main_grid.columns.xb')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.trmhmatser.main_grid.columns.xb')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
            ...
            <template v-if="getColumnState('nj')">
                <el-table-column show-overflow-tooltip :prop="'nj'" :label="$t('entities.trmhmatser.main_grid.columns.nj')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.trmhmatser.main_grid.columns.nj')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.nj}}</span>
            ...
            <template v-if="getColumnState('lxdh')">
                <el-table-column show-overflow-tooltip :prop="'lxdh'" :label="$t('entities.trmhmatser.main_grid.columns.lxdh')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.trmhmatser.main_grid.columns.lxdh')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.lxdh}}</span>
            ...
            <template v-if="getColumnState('ormorgname')">
                <el-table-column show-overflow-tooltip :prop="'ormorgname'" :label="$t('entities.trmhmatser.main_grid.columns.ormorgname')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.trmhmatser.main_grid.columns.ormorgname')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
...
import TrmHmatserService from '@/service/trm-hmatser/trm-hmatser-service';
...
export default class MainBase extends Vue implements ControlInterface {
     ...
     * 实体服务对象
     *
     * @type {TrmHmatserService}
     * @memberof MainBase
     */
    public appEntityService: TrmHmatserService = new TrmHmatserService({ $store: this.$store });
    ...
    */
    ...
    public allColumns: any[] = [
        {
            name: 'pimpersonname',
            label: '班主任',
            langtag: 'entities.trmhmatser.main_grid.columns.pimpersonname',
            show: true,
            util: 'px'
        },
        {
            name: 'zjhm',
            label: '身份证号码',
            langtag: 'entities.trmhmatser.main_grid.columns.zjhm',
            show: true,
            util: 'px'
        },
        {
            name: 'xb',
            label: '性别',
            langtag: 'entities.trmhmatser.main_grid.columns.xb',
            show: true,
            util: 'px'
        },
        {
            name: 'nj',
            label: '年龄',
            langtag: 'entities.trmhmatser.main_grid.columns.nj',
            show: true,
            util: 'px'
        },
        {
            name: 'lxdh',
            label: '联系方式',
            langtag: 'entities.trmhmatser.main_grid.columns.lxdh',
            show: true,
            util: 'px'
        },
        {
            name: 'ormorgname',
            label: '所属单位',
            langtag: 'entities.trmhmatser.main_grid.columns.ormorgname',
            show: true,
            util: 'px'
        },
    ]
    ...
    public rules: any = {
        srfkey: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '班主任标识 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '班主任标识 值不能为空', trigger: 'blur' },
        ],
    }
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'TrmHmatserGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'TrmHmatserGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        _datas.forEach((record: any, index: number) => {
            let srfmajortext = record.trmhmatsername;
            if (index < 5) {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ trmhmatser: _keys }),Object.assign({ trmhmatser: _keys },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    public rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.trmhmatser,$event.trmhmatser);
        });
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('trmhmatser_main_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('trmhmatser_main_grid', JSON.stringify(this.allColumns));
    }
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: '错误', desc: 'TrmHmatserGridView视图表格createAction参数未配置' });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: '错误', desc: 'TrmHmatserGridView视图表格updateAction参数未配置' });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
                        if(item.trmhmatser){
                            Object.assign(this.context,{trmhmatser:item.trmhmatser});
                        }
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'TrmHmatserGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
    ...
    public getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.trmhmatser,args.row.trmhmatser);
        });
