            ...
            <template v-if="getColumnState('trmtrafficname')">
                <el-table-column show-overflow-tooltip :prop="'trmtrafficname'" :label="$t('entities.trmtraffic.main_grid.columns.trmtrafficname')" :width="200"  :align="'left'" :sortable="'custom'">
                    ...
                    <template v-slot="{row,column,$index}">
                        <span>{{row.trmtrafficname}}</span>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('jtfs')">
                <el-table-column show-overflow-tooltip :prop="'jtfs'" :label="$t('entities.trmtraffic.main_grid.columns.jtfs')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.trmtraffic.main_grid.columns.jtfs')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
            <codelist :value="row.jtfs" tag='EhrCodeList0175' codelistType='STATIC' ></codelist>
                        </template>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('xlmc')">
                <el-table-column show-overflow-tooltip :prop="'xlmc'" :label="$t('entities.trmtraffic.main_grid.columns.xlmc')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.trmtraffic.main_grid.columns.xlmc')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.xlmc}}</span>
...
import TrmTrafficService from '@/service/trm-traffic/trm-traffic-service';
...
export default class MainBase extends Vue implements ControlInterface {
     ...
     * 实体服务对象
     *
     * @type {TrmTrafficService}
     * @memberof MainBase
     */
    public appEntityService: TrmTrafficService = new TrmTrafficService({ $store: this.$store });
    ...
    */
    ...
    public allColumns: any[] = [
        {
            name: 'trmtrafficname',
            label: '交通名称',
            langtag: 'entities.trmtraffic.main_grid.columns.trmtrafficname',
            show: true,
            util: 'PX'
        },
        {
            name: 'jtfs',
            label: '交通方式',
            langtag: 'entities.trmtraffic.main_grid.columns.jtfs',
            show: true,
            util: 'px'
        },
        {
            name: 'xlmc',
            label: '交通线路',
            langtag: 'entities.trmtraffic.main_grid.columns.xlmc',
            show: true,
            util: 'px'
        },
    ]
    ...
    public rules: any = {
        srfkey: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '交通标识 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '交通标识 值不能为空', trigger: 'blur' },
        ],
    }
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'TrmTrafficGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'TrmTrafficGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        _datas.forEach((record: any, index: number) => {
            let srfmajortext = record.trmtrafficname;
            if (index < 5) {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ trmtraffic: _keys }),Object.assign({ trmtraffic: _keys },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    public rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.trmtraffic,$event.trmtraffic);
        });
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('trmtraffic_main_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('trmtraffic_main_grid', JSON.stringify(this.allColumns));
    }
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: '错误', desc: 'TrmTrafficGridView视图表格createAction参数未配置' });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: '错误', desc: 'TrmTrafficGridView视图表格updateAction参数未配置' });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
                        if(item.trmtraffic){
                            Object.assign(this.context,{trmtraffic:item.trmtraffic});
                        }
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'TrmTrafficGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
    ...
    public getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.trmtraffic,args.row.trmtraffic);
        });
