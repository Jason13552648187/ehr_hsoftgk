        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('trmdemdeftionname')">
                <el-table-column show-overflow-tooltip :prop="'trmdemdeftionname'" :label="$t('entities.trmtrainplan.main_grid.columns.trmdemdeftionname')" :width="200"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.trmtrainplan.main_grid.columns.trmdemdeftionname')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('npxks')">
                <el-table-column show-overflow-tooltip :prop="'npxks'" :label="$t('entities.trmtrainplan.main_grid.columns.npxks')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.trmtrainplan.main_grid.columns.npxks')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                            <app-format-data dataType="FLOAT" precision="0" :data="row.npxks"></app-format-data>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('npxqs')">
                <el-table-column show-overflow-tooltip :prop="'npxqs'" :label="$t('entities.trmtrainplan.main_grid.columns.npxqs')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.trmtrainplan.main_grid.columns.npxqs')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.npxqs}}</span>
...
export default class MainBase extends Vue implements ControlInterface {
     ...
     * @memberof MainBase
     */
    public appEntityService: TrmTrainPlanService = new TrmTrainPlanService({ $store: this.$store });
    ...
    */
    ...
    public allColumns: any[] = [
        ...
        },
        {
            name: 'npxks',
            label: '拟培训课时',
            langtag: 'entities.trmtrainplan.main_grid.columns.npxks',
            show: true,
            util: 'px'
        },
        {
            name: 'npxqs',
            label: '拟培训期数',
            langtag: 'entities.trmtrainplan.main_grid.columns.npxqs',
            show: true,
            util: 'px'
        },
        {
    ...
    public rules: any = {
        srfkey: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '培训计划标识 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '培训计划标识 值不能为空', trigger: 'blur' },
        ],
    }
    ...
    public async remove(datas: any[]): Promise<any> {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ trmtrainplan: _keys }),Object.assign({ trmtrainplan: _keys },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    public rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.trmtrainplan,$event.trmtrainplan);
        });
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('trmtrainplan_main_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('trmtrainplan_main_grid', JSON.stringify(this.allColumns));
    }
                ...
                if(Object.is(item.rowDataState, 'create')){
                    ...
                    if(!this.updateAction){
                        ...
                        if(item.trmtrainplan){
                            Object.assign(this.context,{trmtrainplan:item.trmtrainplan});
                        }
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'TrmTrainPlanJHFBGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
    ...
    public getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.trmtrainplan,args.row.trmtrainplan);
        });
