    ...
    <el-table v-if="isDisplay === true"
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('pxbj')">
                <el-table-column show-overflow-tooltip :prop="'pxbj'" :label="$t('entities.trmtrainperson.main_grid.columns.pxbj')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.trmtrainperson.main_grid.columns.pxbj')}}
                      </span>
                    </template>
            ...
            <template v-if="getColumnState('trainbegin')">
                <el-table-column show-overflow-tooltip :prop="'trainbegin'" :label="$t('entities.trmtrainperson.main_grid.columns.trainbegin')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.trmtrainperson.main_grid.columns.trainbegin')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('trainend')">
                <el-table-column show-overflow-tooltip :prop="'trainend'" :label="$t('entities.trmtrainperson.main_grid.columns.trainend')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.trmtrainperson.main_grid.columns.trainend')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('pxkc')">
                <el-table-column show-overflow-tooltip :prop="'pxkc'" :label="$t('entities.trmtrainperson.main_grid.columns.pxkc')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.trmtrainperson.main_grid.columns.pxkc')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.pxkc}}</span>
            ...
            <template v-if="getColumnState('pxjg')">
                <el-table-column show-overflow-tooltip :prop="'pxjg'" :label="$t('entities.trmtrainperson.main_grid.columns.pxjg')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.trmtrainperson.main_grid.columns.pxjg')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.pxjg}}</span>
            ...
            <template v-if="getColumnState('trainres')">
                <el-table-column show-overflow-tooltip :prop="'trainres'" :label="$t('entities.trmtrainperson.main_grid.columns.trainres')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.trmtrainperson.main_grid.columns.trainres')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('fs')">
                <el-table-column show-overflow-tooltip :prop="'fs'" :label="$t('entities.trmtrainperson.main_grid.columns.fs')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.trmtrainperson.main_grid.columns.fs')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
...
export default class MainBase extends Vue implements ControlInterface {
     ...
     * @memberof MainBase
     */
    public appEntityService: TrmTrainPersonService = new TrmTrainPersonService({ $store: this.$store });
    ...
    */
    ...
    public allColumns: any[] = [
        {
            name: 'pxbj',
            label: '培训班级',
            langtag: 'entities.trmtrainperson.main_grid.columns.pxbj',
            show: true,
            util: 'px'
        },
        {
            name: 'trainbegin',
            label: '培训开始时间',
            langtag: 'entities.trmtrainperson.main_grid.columns.trainbegin',
            show: true,
            util: 'px'
        },
        {
            name: 'trainend',
            label: '培训结束时间',
            langtag: 'entities.trmtrainperson.main_grid.columns.trainend',
            show: true,
            util: 'px'
        },
        {
            name: 'pxkc',
            label: '培训课程',
            langtag: 'entities.trmtrainperson.main_grid.columns.pxkc',
            show: true,
            util: 'px'
        },
        {
            name: 'pxjg',
            label: '培训机构',
            langtag: 'entities.trmtrainperson.main_grid.columns.pxjg',
            show: true,
            util: 'px'
        },
        {
            name: 'trainres',
            label: '培训结果',
            langtag: 'entities.trmtrainperson.main_grid.columns.trainres',
            show: true,
            util: 'px'
        },
        {
            name: 'fs',
            label: '分数',
            langtag: 'entities.trmtrainperson.main_grid.columns.fs',
            show: true,
            util: 'px'
        },
    ]
    ...
    public rules: any = {
        srfkey: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '培训人员标识 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '培训人员标识 值不能为空', trigger: 'blur' },
        ],
    }
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'TrmTrainPersonGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'TrmTrainPersonGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        _datas.forEach((record: any, index: number) => {
            let srfmajortext = record.trmtrianpersonname;
            if (index < 5) {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ trmtrainperson: _keys }),Object.assign({ trmtrainperson: _keys },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    public rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.trmtrianperson,$event.trmtrianperson);
        });
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('trmtrianperson_main_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('trmtrianperson_main_grid', JSON.stringify(this.allColumns));
    }
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: '错误', desc: 'TrmTrainPersonGridView视图表格createAction参数未配置' });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: '错误', desc: 'TrmTrainPersonGridView视图表格updateAction参数未配置' });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
                        if(item.trmtrainperson){
                            Object.assign(this.context,{trmtrainperson:item.trmtrainperson});
                        }
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'TrmTrainPersonGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
    ...
    public getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.trmtrainperson,args.row.trmtrainperson);
        });
