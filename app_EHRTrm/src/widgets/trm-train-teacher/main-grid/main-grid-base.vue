    ...
    <el-table v-if="isDisplay === true"
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('zjkssj')">
                <el-table-column show-overflow-tooltip :prop="'zjkssj'" :label="$t('entities.trmtrainteacher.main_grid.columns.zjkssj')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.trmtrainteacher.main_grid.columns.zjkssj')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('zzdzs')">
                <el-table-column show-overflow-tooltip :prop="'zzdzs'" :label="$t('entities.trmtrainteacher.main_grid.columns.zzdzs')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.trmtrainteacher.main_grid.columns.zzdzs')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.zzdzs}}</span>
...
export default class MainBase extends Vue implements ControlInterface {
    ...
    */
    ...
    public allColumns: any[] = [
        {
            name: 'pimpersonname',
            label: '姓名',
            langtag: 'entities.trmtrainteacher.main_grid.columns.pimpersonname',
            show: true,
            util: 'PX'
        },
        {
            name: 'dj',
            label: '等级',
            langtag: 'entities.trmtrainteacher.main_grid.columns.dj',
            show: true,
            util: 'px'
        },
        {
            name: 'jslx',
            label: '讲师类型',
            langtag: 'entities.trmtrainteacher.main_grid.columns.jslx',
            show: true,
            util: 'px'
        },
        {
            name: 'zjhm',
            label: '证件号码',
            langtag: 'entities.trmtrainteacher.main_grid.columns.zjhm',
            show: true,
            util: 'PX'
        },
        {
            name: 'xb',
            label: '性别',
            langtag: 'entities.trmtrainteacher.main_grid.columns.xb',
            show: true,
            util: 'px'
        },
        {
            name: 'nl',
            label: '年龄',
            langtag: 'entities.trmtrainteacher.main_grid.columns.nl',
            show: true,
            util: 'px'
        },
        {
        ...
        },
        {
            name: 'zjkssj',
            label: '执教开始时间',
            langtag: 'entities.trmtrainteacher.main_grid.columns.zjkssj',
            show: true,
            util: 'px'
        },
        {
            name: 'zzdzs',
            label: '所属单位',
            langtag: 'entities.trmtrainteacher.main_grid.columns.zzdzs',
            show: true,
            util: 'px'
        },
        {
            name: 'sfgx',
            label: '是否共享',
            langtag: 'entities.trmtrainteacher.main_grid.columns.sfgx',
            show: true,
            util: 'px'
        },
    ]
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'TrmTrainTeacherGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'TrmTrainTeacherGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('trmtrainteacher_main_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('trmtrainteacher_main_grid', JSON.stringify(this.allColumns));
    }
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: '错误', desc: 'TrmTrainTeacherGridView视图表格createAction参数未配置' });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: '错误', desc: 'TrmTrainTeacherGridView视图表格updateAction参数未配置' });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'TrmTrainTeacherGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
