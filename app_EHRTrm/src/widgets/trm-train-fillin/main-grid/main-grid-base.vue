    ...
    <el-table v-if="isDisplay === true"
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('trmdepartname')">
                <el-table-column show-overflow-tooltip :prop="'trmdepartname'" :label="$t('entities.trmtrainfillin.main_grid.columns.trmdepartname')" :width="300"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.trmtrainfillin.main_grid.columns.trmdepartname')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.trmdepartname}}</span>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('zzdzs')">
                <el-table-column show-overflow-tooltip :prop="'zzdzs'" :label="$t('entities.trmtrainfillin.main_grid.columns.zzdzs')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.trmtrainfillin.main_grid.columns.zzdzs')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.zzdzs}}</span>
            ...
            <template v-if="getColumnState('shortname')">
                <el-table-column show-overflow-tooltip :prop="'shortname'" :label="$t('entities.trmtrainfillin.main_grid.columns.shortname')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.trmtrainfillin.main_grid.columns.shortname')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('nd')">
                <el-table-column show-overflow-tooltip :prop="'nd'" :label="$t('entities.trmtrainfillin.main_grid.columns.nd')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.trmtrainfillin.main_grid.columns.nd')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
            ...
            <template v-if="getColumnState('jd')">
                <el-table-column show-overflow-tooltip :prop="'jd'" :label="$t('entities.trmtrainfillin.main_grid.columns.jd')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.trmtrainfillin.main_grid.columns.jd')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
            ...
            <template v-if="getColumnState('jzrq')">
                <el-table-column show-overflow-tooltip :prop="'jzrq'" :label="$t('entities.trmtrainfillin.main_grid.columns.jzrq')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.trmtrainfillin.main_grid.columns.jzrq')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.jzrq}}</span>
...
export default class MainBase extends Vue implements ControlInterface {
     ...
     * 实体服务对象
     *
     * @type {TrmTrainFillinService}
     * @memberof MainBase
     */
    public appEntityService: TrmTrainFillinService = new TrmTrainFillinService({ $store: this.$store });
    ...
    */
    ...
    public allColumns: any[] = [
        {
            name: 'trmdepartname',
            label: '培训需求通知',
            langtag: 'entities.trmtrainfillin.main_grid.columns.trmdepartname',
            show: true,
            util: 'PX'
        },
        {
            name: 'zzdzs',
            label: '组织',
            langtag: 'entities.trmtrainfillin.main_grid.columns.zzdzs',
            show: true,
            util: 'px'
        },
        {
            name: 'shortname',
            label: '部门',
            langtag: 'entities.trmtrainfillin.main_grid.columns.shortname',
            show: true,
            util: 'px'
        },
        {
            name: 'nd',
            label: '年度',
            langtag: 'entities.trmtrainfillin.main_grid.columns.nd',
            show: true,
            util: 'px'
        },
        {
            name: 'jd',
            label: '季度',
            langtag: 'entities.trmtrainfillin.main_grid.columns.jd',
            show: true,
            util: 'px'
        },
        {
            name: 'jzrq',
            label: '截止时间',
            langtag: 'entities.trmtrainfillin.main_grid.columns.jzrq',
            show: true,
            util: 'px'
        },
    ]
    ...
    public rules: any = {
        srfkey: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '培训项目部门填报标识 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '培训项目部门填报标识 值不能为空', trigger: 'blur' },
        ],
    }
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'TrmTrainFillinGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'TrmTrainFillinGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        _datas.forEach((record: any, index: number) => {
            let srfmajortext = record.trmtrainfillinname;
            if (index < 5) {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ trmtrainfillin: _keys }),Object.assign({ trmtrainfillin: _keys },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    public rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.trmtrainfillin,$event.trmtrainfillin);
        });
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('trmtrainfillin_main_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('trmtrainfillin_main_grid', JSON.stringify(this.allColumns));
    }
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: '错误', desc: 'TrmTrainFillinGridView视图表格createAction参数未配置' });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: '错误', desc: 'TrmTrainFillinGridView视图表格updateAction参数未配置' });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
                        if(item.trmtrainfillin){
                            Object.assign(this.context,{trmtrainfillin:item.trmtrainfillin});
                        }
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'TrmTrainFillinGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
    ...
    public getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.trmtrainfillin,args.row.trmtrainfillin);
        });
