    ...
    <el-table v-if="isDisplay === true"
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('trmtrainteachername')">
                <el-table-column show-overflow-tooltip :prop="'trmtrainteachername'" :label="$t('entities.trmteachercharge.main_grid.columns.trmtrainteachername')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.trmteachercharge.main_grid.columns.trmtrainteachername')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('jsdj')">
                <el-table-column show-overflow-tooltip :prop="'jsdj'" :label="$t('entities.trmteachercharge.main_grid.columns.jsdj')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.trmteachercharge.main_grid.columns.jsdj')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
            ...
            <template v-if="getColumnState('dj')">
                <el-table-column show-overflow-tooltip :prop="'dj'" :label="$t('entities.trmteachercharge.main_grid.columns.dj')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.trmteachercharge.main_grid.columns.dj')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                            <app-format-data dataType="FLOAT" precision="0" :data="row.dj"></app-format-data>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('xs')">
                <el-table-column show-overflow-tooltip :prop="'xs'" :label="$t('entities.trmteachercharge.main_grid.columns.xs')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.trmteachercharge.main_grid.columns.xs')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                            <app-format-data dataType="FLOAT" precision="0" :data="row.xs"></app-format-data>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('zfy')">
                <el-table-column show-overflow-tooltip :prop="'zfy'" :label="$t('entities.trmteachercharge.main_grid.columns.zfy')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.trmteachercharge.main_grid.columns.zfy')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                            <app-format-data dataType="FLOAT" precision="0" :data="row.zfy"></app-format-data>
                    </template>
                </el-table-column>
            </template>
...
export default class MainBase extends Vue implements ControlInterface {
    ...
    */
    ...
    public allColumns: any[] = [
        {
            ...
            label: '培训班',
            langtag: 'entities.trmteachercharge.main_grid.columns.trmtrainplantermname',
            show: true,
            util: 'px'
        },
        {
            ...
            label: '讲师',
            langtag: 'entities.trmteachercharge.main_grid.columns.trmtrainteachername',
            show: true,
            util: 'px'
        },
        {
            name: 'jsdj',
            label: '讲师等级',
            langtag: 'entities.trmteachercharge.main_grid.columns.jsdj',
            show: true,
            util: 'px'
        },
        {
            name: 'dj',
            label: '单价',
            langtag: 'entities.trmteachercharge.main_grid.columns.dj',
            show: true,
            util: 'px'
        },
        {
            name: 'xs',
            label: '学时',
            langtag: 'entities.trmteachercharge.main_grid.columns.xs',
            show: true,
            util: 'px'
        },
        {
            name: 'zfy',
            label: '总费用',
            langtag: 'entities.trmteachercharge.main_grid.columns.zfy',
            show: true,
            util: 'px'
        },
    ]
    ...
    public rules: any = {
        srfkey: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '讲师收费记录标识 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '讲师收费记录标识 值不能为空', trigger: 'blur' },
        ],
    }
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'TrmTeacherChargeGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'TrmTeacherChargeGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        _datas.forEach((record: any, index: number) => {
            let srfmajortext = record.trmteacherchargename;
            if (index < 5) {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ trmteachercharge: _keys }),Object.assign({ trmteachercharge: _keys },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    public rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.trmteachercharge,$event.trmteachercharge);
        });
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('trmteachercharge_main_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('trmteachercharge_main_grid', JSON.stringify(this.allColumns));
    }
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: '错误', desc: 'TrmTeacherChargeGridView视图表格createAction参数未配置' });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: '错误', desc: 'TrmTeacherChargeGridView视图表格updateAction参数未配置' });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
                        if(item.trmteachercharge){
                            Object.assign(this.context,{trmteachercharge:item.trmteachercharge});
                        }
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'TrmTeacherChargeGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
    ...
    public getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.trmteachercharge,args.row.trmteachercharge);
        });
