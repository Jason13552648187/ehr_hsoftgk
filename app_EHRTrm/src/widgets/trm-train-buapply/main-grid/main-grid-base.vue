...
<div class='grid' style="height:100%;">
    ...
    <el-table v-if="isDisplay === true"
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('jflx')">
                <el-table-column show-overflow-tooltip :prop="'jflx'" :label="$t('entities.trmtrainbuapply.main_grid.columns.jflx')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.trmtrainbuapply.main_grid.columns.jflx')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
            <codelist :value="row.jflx" tag='EhrCodeList0067' codelistType='STATIC' renderMode="STR" valueSeparator=";" textSeparator="、" ></codelist>
                        </template>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('tzysxm')">
                <el-table-column show-overflow-tooltip :prop="'tzysxm'" :label="$t('entities.trmtrainbuapply.main_grid.columns.tzysxm')" :width="120"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
            ...
            <template v-if="getColumnState('tzje')">
                <el-table-column show-overflow-tooltip :prop="'tzje'" :label="$t('entities.trmtrainbuapply.main_grid.columns.tzje')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.trmtrainbuapply.main_grid.columns.tzje')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                            <app-format-data dataType="FLOAT" precision="0" :data="row.tzje"></app-format-data>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('clf')">
                <el-table-column show-overflow-tooltip :prop="'clf'" :label="$t('entities.trmtrainbuapply.main_grid.columns.clf')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.trmtrainbuapply.main_grid.columns.clf')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                            <app-format-data dataType="FLOAT" precision="0" :data="row.clf"></app-format-data>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('szf')">
                <el-table-column show-overflow-tooltip :prop="'szf'" :label="$t('entities.trmtrainbuapply.main_grid.columns.szf')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.trmtrainbuapply.main_grid.columns.szf')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                            <app-format-data dataType="FLOAT" precision="0" :data="row.szf"></app-format-data>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('cdzj')">
                <el-table-column show-overflow-tooltip :prop="'cdzj'" :label="$t('entities.trmtrainbuapply.main_grid.columns.cdzj')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.trmtrainbuapply.main_grid.columns.cdzj')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                            <app-format-data dataType="FLOAT" precision="0" :data="row.cdzj"></app-format-data>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('jgfy')">
                <el-table-column show-overflow-tooltip :prop="'jgfy'" :label="$t('entities.trmtrainbuapply.main_grid.columns.jgfy')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.trmtrainbuapply.main_grid.columns.jgfy')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                            <app-format-data dataType="FLOAT" precision="0" :data="row.jgfy"></app-format-data>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('qtfy')">
                <el-table-column show-overflow-tooltip :prop="'qtfy'" :label="$t('entities.trmtrainbuapply.main_grid.columns.qtfy')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.trmtrainbuapply.main_grid.columns.qtfy')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                            <app-format-data dataType="FLOAT" precision="0" :data="row.qtfy"></app-format-data>
                    </template>
                </el-table-column>
            </template>
...
import TrmTrainBuapplyService from '@/service/trm-train-buapply/trm-train-buapply-service';
...
export default class MainBase extends Vue implements ControlInterface {
     ...
     * 实体服务对象
     *
     * @type {TrmTrainBuapplyService}
     * @memberof MainBase
     */
    public appEntityService: TrmTrainBuapplyService = new TrmTrainBuapplyService({ $store: this.$store });
    ...
    */
    ...
    public allColumns: any[] = [
        ...
        },
        {
            name: 'jflx',
            label: '经费类型',
            langtag: 'entities.trmtrainbuapply.main_grid.columns.jflx',
            show: true,
            util: 'px'
        },
        {
        ...
        },
        {
            name: 'tzje',
            label: '实际费用',
            langtag: 'entities.trmtrainbuapply.main_grid.columns.tzje',
            show: true,
            util: 'PX'
        },
        {
            name: 'clf',
            label: '差旅费',
            langtag: 'entities.trmtrainbuapply.main_grid.columns.clf',
            show: true,
            util: 'px'
        },
        {
            name: 'szf',
            label: '师资费',
            langtag: 'entities.trmtrainbuapply.main_grid.columns.szf',
            show: true,
            util: 'px'
        },
        {
            name: 'cdzj',
            label: '场地租金',
            langtag: 'entities.trmtrainbuapply.main_grid.columns.cdzj',
            show: true,
            util: 'px'
        },
        {
            name: 'jgfy',
            label: '机构费用',
            langtag: 'entities.trmtrainbuapply.main_grid.columns.jgfy',
            show: true,
            util: 'px'
        },
        {
            name: 'qtfy',
            label: '其他费用',
            langtag: 'entities.trmtrainbuapply.main_grid.columns.qtfy',
            show: true,
            util: 'px'
        },
    ]
    ...
    public rules: any = {
        srfkey: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '培训预算申请标识 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '培训预算申请标识 值不能为空', trigger: 'blur' },
        ],
    }
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'TrmTrainBuapplyGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'TrmTrainBuapplyGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        _datas.forEach((record: any, index: number) => {
            let srfmajortext = record.trmtrainbuapplyname;
            if (index < 5) {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ trmtrainbuapply: _keys }),Object.assign({ trmtrainbuapply: _keys },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    public rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.trmtrainbuapply,$event.trmtrainbuapply);
        });
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('trmtrainbuapply_main_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('trmtrainbuapply_main_grid', JSON.stringify(this.allColumns));
    }
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: '错误', desc: 'TrmTrainBuapplyGridView视图表格createAction参数未配置' });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: '错误', desc: 'TrmTrainBuapplyGridView视图表格updateAction参数未配置' });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
                        if(item.trmtrainbuapply){
                            Object.assign(this.context,{trmtrainbuapply:item.trmtrainbuapply});
                        }
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'TrmTrainBuapplyGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
    ...
    public getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.trmtrainbuapply,args.row.trmtrainbuapply);
        });
