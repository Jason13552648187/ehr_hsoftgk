    ...
    <el-table v-if="isDisplay === true"
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('orgname')">
                <el-table-column show-overflow-tooltip :prop="'orgname'" :label="$t('entities.ormorgsector.bzcx_grid.columns.orgname')" :width="400"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.ormorgsector.bzcx_grid.columns.orgname')}}
                      </span>
                    </template>
            ...
            <template v-if="getColumnState('orgsectorname')">
                <el-table-column show-overflow-tooltip :prop="'orgsectorname'" :label="$t('entities.ormorgsector.bzcx_grid.columns.orgsectorname')" :width="300"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.ormorgsector.bzcx_grid.columns.orgsectorname')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('orgcode')">
                <el-table-column show-overflow-tooltip :prop="'orgcode'" :label="$t('entities.ormorgsector.bzcx_grid.columns.orgcode')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.ormorgsector.bzcx_grid.columns.orgcode')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('bmbzrs')">
                <el-table-column show-overflow-tooltip :prop="'bmbzrs'" :label="$t('entities.ormorgsector.bzcx_grid.columns.bmbzrs')" :width="130"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.ormorgsector.bzcx_grid.columns.bmbzrs')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.bmbzrs}}</span>
                    </template>
                </el-table-column>
            </template>
...
export default class BZCXBase extends Vue implements ControlInterface {
    ...
    */
    ...
    public allColumns: any[] = [
        {
            name: 'orgname',
            label: '组织机构',
            langtag: 'entities.ormorgsector.bzcx_grid.columns.orgname',
            show: true,
            util: 'PX'
        },
        {
            name: 'orgsectorname',
            label: '部门/项目部名称',
            langtag: 'entities.ormorgsector.bzcx_grid.columns.orgsectorname',
            show: true,
            util: 'PX'
        },
        {
            name: 'orgcode',
            label: '部门编号',
            langtag: 'entities.ormorgsector.bzcx_grid.columns.orgcode',
            show: true,
            util: 'px'
        },
        {
            name: 'bmbzrs',
            label: '人数编制',
            langtag: 'entities.ormorgsector.bzcx_grid.columns.bmbzrs',
            show: true,
            util: 'PX'
        },
        {
            name: 'sjrs',
            label: '实际编制人数',
            langtag: 'entities.ormorgsector.bzcx_grid.columns.sjrs',
            show: true,
            util: 'PX'
        },
    ]
    ...
    public getGridRowModel(){
        return {
          sjbzrs_color: new FormItemModel(),
    ...
    public rules: any = {
        sjbzrs_color: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '超编人数的颜色 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '超编人数的颜色 值不能为空', trigger: 'blur' },
        ],
        srfkey: [
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'ORMORGSECTORBZCXGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'ORMORGSECTORBZCXGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: '错误', desc: 'ORMORGSECTORBZCXGridView视图表格createAction参数未配置' });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: '错误', desc: 'ORMORGSECTORBZCXGridView视图表格updateAction参数未配置' });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
