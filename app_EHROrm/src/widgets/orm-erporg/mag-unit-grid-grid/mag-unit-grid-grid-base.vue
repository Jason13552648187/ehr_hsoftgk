...
<div class='grid' style="height:100%;">
    ...
    <el-table v-if="isDisplay === true"
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('ormerporgname')">
                <el-table-column show-overflow-tooltip :prop="'ormerporgname'" :label="$t('entities.ormerporg.magunitgrid_grid.columns.ormerporgname')" :width="380"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.ormerporg.magunitgrid_grid.columns.ormerporgname')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.ormerporgname}}</span>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('orgcode')">
                <el-table-column show-overflow-tooltip :prop="'orgcode'" :label="$t('entities.ormerporg.magunitgrid_grid.columns.orgcode')" :width="130"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.ormerporg.magunitgrid_grid.columns.orgcode')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('orgtype')">
                <el-table-column show-overflow-tooltip :prop="'orgtype'" :label="$t('entities.ormerporg.magunitgrid_grid.columns.orgtype')" :width="130"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.ormerporg.magunitgrid_grid.columns.orgtype')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('orgtypename')">
                <el-table-column show-overflow-tooltip :prop="'orgtypename'" :label="$t('entities.ormerporg.magunitgrid_grid.columns.orgtypename')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.ormerporg.magunitgrid_grid.columns.orgtypename')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
...
import MagUnitGridService from './mag-unit-grid-grid-service';
...
export default class MagUnitGridBase extends Vue implements ControlInterface {

    /**
     * 名称
     *
     * @type {string}
     * @memberof MagUnitGridBase
     */
     ...
     * 建构部件服务对象
     *
     * @type {MagUnitGridService}
     * @memberof MagUnitGridBase
     */
    public service: MagUnitGridService = new MagUnitGridService({ $store: this.$store });
    ...
    */
    ...
    public allColumns: any[] = [
        {
            name: 'ormerporgname',
            label: '组织名称',
            langtag: 'entities.ormerporg.magunitgrid_grid.columns.ormerporgname',
            show: true,
            util: 'PX'
        },
        {
            name: 'orgcode',
            label: '组织编码',
            langtag: 'entities.ormerporg.magunitgrid_grid.columns.orgcode',
            show: true,
            util: 'PX'
        },
        {
            name: 'orgtype',
            label: '组织类型',
            langtag: 'entities.ormerporg.magunitgrid_grid.columns.orgtype',
            show: true,
            util: 'PX'
        },
        {
            name: 'orgtypename',
            label: '组织类型释义',
            langtag: 'entities.ormerporg.magunitgrid_grid.columns.orgtypename',
            show: true,
            util: 'PX'
        },
    ]
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'ORMERPORGLegalPickupGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'ORMERPORGLegalPickupGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        _datas.forEach((record: any, index: number) => {
            let srfmajortext = record.ormerporgname;
            if (index < 5) {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ ormerporg: _keys }),Object.assign({ ormerporg: _keys },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    public rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.ormerporg,$event.ormerporg);
        });
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('ormerporg_magunitgrid_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('ormerporg_magunitgrid_grid', JSON.stringify(this.allColumns));
    }
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: '错误', desc: 'ORMERPORGLegalPickupGridView视图表格createAction参数未配置' });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: '错误', desc: 'ORMERPORGLegalPickupGridView视图表格updateAction参数未配置' });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
                        if(item.ormerporg){
                            Object.assign(this.context,{ormerporg:item.ormerporg});
                        }
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'ORMERPORGLegalPickupGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
    ...
    public getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.ormerporg,args.row.ormerporg);
        });
...
<style lang='less'>
@import './mag-unit-grid-grid.less';
</style>