...
export default class OrgTreeBase extends Vue implements ControlInterface {
    ...
    */
    ...
    public allColumns: any[] = [
        {
            name: 'px',
            label: '排序号',
            langtag: 'entities.ormorg.orgtree_grid.columns.px',
            show: true,
            util: 'PX'
        },
        {
            name: 'orgname',
            label: '组织名称',
            langtag: 'entities.ormorg.orgtree_grid.columns.orgname',
            show: true,
            util: 'PX'
        },
        {
            name: 'shortname',
            label: '组织简称',
            langtag: 'entities.ormorg.orgtree_grid.columns.shortname',
            show: true,
            util: 'PX'
        },
        {
            name: 'orgcode',
            label: '组织编号',
            langtag: 'entities.ormorg.orgtree_grid.columns.orgcode',
            show: true,
            util: 'PX'
        },
        {
            name: 'gsss',
            label: '所属区域',
            langtag: 'entities.ormorg.orgtree_grid.columns.gsss',
            show: true,
            util: 'PX'
        },
        {
            name: 'btqy',
            label: '补贴标准',
            langtag: 'entities.ormorg.orgtree_grid.columns.btqy',
            show: true,
            util: 'PX'
        },
        {
            name: 'zzcjsj',
            label: '组织启用时间',
            langtag: 'entities.ormorg.orgtree_grid.columns.zzcjsj',
            show: true,
            util: 'PX'
        },
    ]
    ...
    public rules: any = {
        zzlx: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '组织类型 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '组织类型 值不能为空', trigger: 'blur' },
        ],
        orgcode: [
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'ORMORGOrgTreeGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'ORMORGOrgTreeGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'ORMORGOrgTreeGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
