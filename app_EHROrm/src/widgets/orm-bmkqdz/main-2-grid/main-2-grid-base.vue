        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('attendancerange')">
              ...
              @change="($event)=>{gridEditItemChange(row, column.property, $event, $index)}">
                        ...
                        <template v-if="!actualIsOpenEdit">
            <codelist :value="row.attendancerange" tag='EhrCodeList0148' codelistType='STATIC' ></codelist>
                        </template>
                    </template>
                </el-table-column>
            </template>
...
export default class Main_2Base extends Vue implements ControlInterface {
    ...
    */
    ...
    public allColumns: any[] = [
        {
            name: 'ormbmkqdzname',
            label: '部门地址',
            langtag: 'entities.ormbmkqdz.main_2_grid.columns.ormbmkqdzname',
            show: true,
            util: 'PX'
        },
        {
            ...
            label: '考勤范围',
            langtag: 'entities.ormbmkqdz.main_2_grid.columns.attendancerange',
            show: true,
            util: 'PX'
        },
        {
            name: 'updateman',
            label: '更新人',
            langtag: 'entities.ormbmkqdz.main_2_grid.columns.updateman',
            show: true,
            util: 'PX'
        },
        {
            name: 'updatedate',
            label: '更新时间',
            langtag: 'entities.ormbmkqdz.main_2_grid.columns.updatedate',
            show: true,
            util: 'PX'
        },
    ]
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'ORMBMKQDZBMKQDZGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'ORMBMKQDZBMKQDZGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('ormbmkqdz_main_2_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('ormbmkqdz_main_2_grid', JSON.stringify(this.allColumns));
    }
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'ORMBMKQDZBMKQDZGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
