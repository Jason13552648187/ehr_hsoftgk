...
<div class='grid' style="height:100%;">
    ...
    <el-table v-if="isDisplay === true"
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('ormorgdzname')">
                <el-table-column show-overflow-tooltip :prop="'ormorgdzname'" :label="$t('entities.ormorgdz.main_grid.columns.ormorgdzname')" :width="300"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.ormorgdz.main_grid.columns.ormorgdzname')}}
                      </span>
                    </template>
              ...
              @change="($event)=>{gridEditItemChange(row, column.property, $event, $index)}">
                        ...
                        <template v-if="!actualIsOpenEdit">
                                <app-span name='ormorgdzname' editorType="TEXTBOX" :value="row.ormorgdzname"></app-span>
                        </template>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('deforgaddress')">
                <el-table-column show-overflow-tooltip :prop="'deforgaddress'" :label="$t('entities.ormorgdz.main_grid.columns.deforgaddress')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.ormorgdz.main_grid.columns.deforgaddress')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template v-if="actualIsOpenEdit">
             ...
             <dropdown-list 
              ...
              @change="($event)=>{gridEditItemChange(row, column.property, $event, $index)}">
                        ...
                        <template v-if="!actualIsOpenEdit">
            <codelist :value="row.deforgaddress" tag='EhrCodeList0097' codelistType='STATIC' ></codelist>
                        </template>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('createman')">
                <el-table-column show-overflow-tooltip :prop="'createman'" :label="$t('entities.ormorgdz.main_grid.columns.createman')" :width="130"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.ormorgdz.main_grid.columns.createman')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
            ...
            <template v-if="getColumnState('createdate')">
                <el-table-column show-overflow-tooltip :prop="'createdate'" :label="$t('entities.ormorgdz.main_grid.columns.createdate')" :width="170"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.ormorgdz.main_grid.columns.createdate')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('updateman')">
                <el-table-column show-overflow-tooltip :prop="'updateman'" :label="$t('entities.ormorgdz.main_grid.columns.updateman')" :width="130"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.ormorgdz.main_grid.columns.updateman')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
            ...
            <template v-if="getColumnState('updatedate')">
                <el-table-column show-overflow-tooltip :prop="'updatedate'" :label="$t('entities.ormorgdz.main_grid.columns.updatedate')" :width="170"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.ormorgdz.main_grid.columns.updatedate')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
...
export default class MainBase extends Vue implements ControlInterface {
     ...
     * @memberof MainBase
     */
    public appEntityService: OrmOrgdzService = new OrmOrgdzService({ $store: this.$store });
    ...
    */
    ...
    public allColumns: any[] = [
        {
            name: 'ormorgdzname',
            label: '组织地址',
            langtag: 'entities.ormorgdz.main_grid.columns.ormorgdzname',
            show: true,
            util: 'PX'
        },
        {
            name: 'deforgaddress',
            label: '默认组织地址',
            langtag: 'entities.ormorgdz.main_grid.columns.deforgaddress',
            show: true,
            util: 'PX'
        },
        {
            name: 'createman',
            label: '建立人',
            langtag: 'entities.ormorgdz.main_grid.columns.createman',
            show: true,
            util: 'PX'
        },
        {
            name: 'createdate',
            label: '建立时间',
            langtag: 'entities.ormorgdz.main_grid.columns.createdate',
            show: true,
            util: 'PX'
        },
        {
            name: 'updateman',
            label: '更新人',
            langtag: 'entities.ormorgdz.main_grid.columns.updateman',
            show: true,
            util: 'PX'
        },
        {
            name: 'updatedate',
            label: '更新时间',
            langtag: 'entities.ormorgdz.main_grid.columns.updatedate',
            show: true,
            util: 'PX'
        },
    ]
    ...
    public getGridRowModel(){
        return {
          ...
          ormorgdzname: new FormItemModel(),
          deforgaddress: new FormItemModel(),
    ...
    public rules: any = {
        ...
        ormorgid: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '组织ID 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '组织ID 值不能为空', trigger: 'blur' },
        ],
        ormorgdzname: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '组织地址 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '组织地址 值不能为空', trigger: 'blur' },
        ],
        deforgaddress: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '默认组织地址 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '默认组织地址 值不能为空', trigger: 'blur' },
        ],
        srfkey: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '组织地址维护标识 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '组织地址维护标识 值不能为空', trigger: 'blur' },
        ],
        ormorgname: [
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'ORMORGDZGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'ORMORGDZGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        _datas.forEach((record: any, index: number) => {
            let srfmajortext = record.ormorgdzname;
            if (index < 5) {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ ormorgdz: _keys }),Object.assign({ ormorgdz: _keys },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    public rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.ormorgdz,$event.ormorgdz);
        });
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('ormorgdz_main_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('ormorgdz_main_grid', JSON.stringify(this.allColumns));
    }
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: '错误', desc: 'ORMORGDZGridView视图表格createAction参数未配置' });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: '错误', desc: 'ORMORGDZGridView视图表格updateAction参数未配置' });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
                        if(item.ormorgdz){
                            Object.assign(this.context,{ormorgdz:item.ormorgdz});
                        }
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'ORMORGDZGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
    ...
    public getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.ormorgdz,args.row.ormorgdz);
        });
    ...
    public createDefault(row: any){                    
        if (row.hasOwnProperty('deforgaddress')) {
            row['deforgaddress'] = '1';
        }
    }
}
