...
<div class='grid' style="height:100%;">
    ...
    <el-table v-if="isDisplay === true"
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('sallogname')">
                <el-table-column show-overflow-tooltip :prop="'sallogname'" :label="$t('entities.sallog.main_grid.columns.sallogname')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.sallog.main_grid.columns.sallogname')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.sallogname}}</span>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('salsalaryname')">
                <el-table-column show-overflow-tooltip :prop="'salsalaryname'" :label="$t('entities.sallog.main_grid.columns.salsalaryname')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.sallog.main_grid.columns.salsalaryname')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.salsalaryname}}</span>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('salsalarydetailname')">
                <el-table-column show-overflow-tooltip :prop="'salsalarydetailname'" :label="$t('entities.sallog.main_grid.columns.salsalarydetailname')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.sallog.main_grid.columns.salsalarydetailname')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.salsalarydetailname}}</span>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('val1')">
                <el-table-column show-overflow-tooltip :prop="'val1'" :label="$t('entities.sallog.main_grid.columns.val1')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.sallog.main_grid.columns.val1')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.val1}}</span>
            ...
            <template v-if="getColumnState('val2')">
                <el-table-column show-overflow-tooltip :prop="'val2'" :label="$t('entities.sallog.main_grid.columns.val2')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.sallog.main_grid.columns.val2')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.val2}}</span>
            ...
            <template v-if="getColumnState('createman')">
                <el-table-column show-overflow-tooltip :prop="'createman'" :label="$t('entities.sallog.main_grid.columns.createman')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.sallog.main_grid.columns.createman')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
            ...
            <template v-if="getColumnState('createdate')">
                <el-table-column show-overflow-tooltip :prop="'createdate'" :label="$t('entities.sallog.main_grid.columns.createdate')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.sallog.main_grid.columns.createdate')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
...
import SalLogService from '@/service/sal-log/sal-log-service';
...
export default class MainBase extends Vue implements ControlInterface {
     ...
     * @memberof MainBase
     */
    public appEntityService: SalLogService = new SalLogService({ $store: this.$store });
    ...
    */
    ...
    public allColumns: any[] = [
        {
            name: 'sallogname',
            label: '操作类型',
            langtag: 'entities.sallog.main_grid.columns.sallogname',
            show: true,
            util: 'PX'
        },
        {
            name: 'salsalaryname',
            label: '员工薪酬',
            langtag: 'entities.sallog.main_grid.columns.salsalaryname',
            show: true,
            util: 'px'
        },
        {
            ...
            label: '要素项',
            langtag: 'entities.sallog.main_grid.columns.salsalarydetailname',
            show: true,
            util: 'px'
        },
        {
            name: 'val1',
            label: '原值',
            langtag: 'entities.sallog.main_grid.columns.val1',
            show: true,
            util: 'px'
        },
        {
            name: 'val2',
            label: '新值',
            langtag: 'entities.sallog.main_grid.columns.val2',
            show: true,
            util: 'px'
        },
        {
            name: 'createman',
            label: '操作人',
            langtag: 'entities.sallog.main_grid.columns.createman',
            show: true,
            util: 'px'
        },
        {
            name: 'createdate',
            label: '操作时间',
            langtag: 'entities.sallog.main_grid.columns.createdate',
            show: true,
            util: 'px'
        },
    ]
    ...
    public rules: any = {
        srfkey: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '薪酬调整日志标识 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '薪酬调整日志标识 值不能为空', trigger: 'blur' },
        ],
    }
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'SalLogGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'SalLogGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        _datas.forEach((record: any, index: number) => {
            let srfmajortext = record.sallogname;
            if (index < 5) {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ sallog: _keys }),Object.assign({ sallog: _keys },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    public rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.sallog,$event.sallog);
        });
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('sallog_main_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('sallog_main_grid', JSON.stringify(this.allColumns));
    }
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: '错误', desc: 'SalLogGridView视图表格createAction参数未配置' });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: '错误', desc: 'SalLogGridView视图表格updateAction参数未配置' });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
                        if(item.sallog){
                            Object.assign(this.context,{sallog:item.sallog});
                        }
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'SalLogGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
    ...
    public getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.sallog,args.row.sallog);
        });
