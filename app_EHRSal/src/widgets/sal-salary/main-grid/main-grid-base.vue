    ...
    <el-table v-if="isDisplay === true"
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('ormorgname')">
                <el-table-column show-overflow-tooltip :prop="'ormorgname'" :label="$t('entities.salsalary.main_grid.columns.ormorgname')" :width="250"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.salsalary.main_grid.columns.ormorgname')}}
                      </span>
                    </template>
            ...
            <template v-if="getColumnState('ormorgsectorname')">
                <el-table-column show-overflow-tooltip :prop="'ormorgsectorname'" :label="$t('entities.salsalary.main_grid.columns.ormorgsectorname')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.salsalary.main_grid.columns.ormorgsectorname')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('ormdutyname')">
                <el-table-column show-overflow-tooltip :prop="'ormdutyname'" :label="$t('entities.salsalary.main_grid.columns.ormdutyname')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.salsalary.main_grid.columns.ormdutyname')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('ormpostname')">
                <el-table-column show-overflow-tooltip :prop="'ormpostname'" :label="$t('entities.salsalary.main_grid.columns.ormpostname')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.salsalary.main_grid.columns.ormpostname')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('xc')">
                <el-table-column show-overflow-tooltip :prop="'xc'" :label="$t('entities.salsalary.main_grid.columns.xc')" :width="150"  :align="'right'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.salsalary.main_grid.columns.xc')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('state')">
                <el-table-column show-overflow-tooltip :prop="'state'" :label="$t('entities.salsalary.main_grid.columns.state')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.salsalary.main_grid.columns.state')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
...
import SalSalaryService from '@/service/sal-salary/sal-salary-service';
...
export default class MainBase extends Vue implements ControlInterface {
     ...
     * 实体服务对象
     *
     * @type {SalSalaryService}
     * @memberof MainBase
     */
    public appEntityService: SalSalaryService = new SalSalaryService({ $store: this.$store });
    ...
    */
    ...
    public allColumns: any[] = [
        {
            name: 'ormorgname',
            label: '组织',
            langtag: 'entities.salsalary.main_grid.columns.ormorgname',
            show: true,
            util: 'PX'
        },
        {
            ...
            label: '部门',
            langtag: 'entities.salsalary.main_grid.columns.ormorgsectorname',
            show: true,
            util: 'px'
        },
        {
            name: 'ormdutyname',
            label: '职务',
            langtag: 'entities.salsalary.main_grid.columns.ormdutyname',
            show: true,
            util: 'px'
        },
        {
            name: 'ormpostname',
            label: '岗位',
            langtag: 'entities.salsalary.main_grid.columns.ormpostname',
            show: true,
            util: 'px'
        },
        {
    ...
    public rules: any = {
        srfkey: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '员工薪酬标识 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '员工薪酬标识 值不能为空', trigger: 'blur' },
        ],
    }
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'SalSalaryGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'SalSalaryGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        _datas.forEach((record: any, index: number) => {
            let srfmajortext = record.salsalaryname;
            if (index < 5) {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ salsalary: _keys }),Object.assign({ salsalary: _keys },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    public rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.salsalary,$event.salsalary);
        });
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('salsalary_main_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('salsalary_main_grid', JSON.stringify(this.allColumns));
    }
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: '错误', desc: 'SalSalaryGridView视图表格createAction参数未配置' });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: '错误', desc: 'SalSalaryGridView视图表格updateAction参数未配置' });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
                        if(item.salsalary){
                            Object.assign(this.context,{salsalary:item.salsalary});
                        }
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'SalSalaryGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
    ...
    public getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.salsalary,args.row.salsalary);
        });
