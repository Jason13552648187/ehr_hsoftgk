    ...
    <el-table v-if="isDisplay === true"
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('salrulename')">
                <el-table-column show-overflow-tooltip :prop="'salrulename'" :label="$t('entities.salrule.main_grid.columns.salrulename')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.salrule.main_grid.columns.salrulename')}}
                      </span>
                    </template>
            ...
            <template v-if="getColumnState('salparamname')">
                <el-table-column show-overflow-tooltip :prop="'salparamname'" :label="$t('entities.salrule.main_grid.columns.salparamname')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.salrule.main_grid.columns.salparamname')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('memo')">
                <el-table-column show-overflow-tooltip :prop="'memo'" :label="$t('entities.salrule.main_grid.columns.memo')" :width="200"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.salrule.main_grid.columns.memo')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.memo}}</span>
            ...
            <template v-if="getColumnState('updateman')">
                <el-table-column show-overflow-tooltip :prop="'updateman'" :label="$t('entities.salrule.main_grid.columns.updateman')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.salrule.main_grid.columns.updateman')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
            ...
            <template v-if="getColumnState('updatedate')">
                <el-table-column show-overflow-tooltip :prop="'updatedate'" :label="$t('entities.salrule.main_grid.columns.updatedate')" :width="200"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.salrule.main_grid.columns.updatedate')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
...
import SalRuleService from '@/service/sal-rule/sal-rule-service';
...
export default class MainBase extends Vue implements ControlInterface {
     ...
     * @memberof MainBase
     */
    public appEntityService: SalRuleService = new SalRuleService({ $store: this.$store });
    ...
    */
    ...
    public allColumns: any[] = [
        {
            name: 'salrulename',
            label: '薪酬规则名称',
            langtag: 'entities.salrule.main_grid.columns.salrulename',
            show: true,
            util: 'PX'
        },
        {
            name: 'salparamname',
            label: '薪酬计算指标名称',
            langtag: 'entities.salrule.main_grid.columns.salparamname',
            show: true,
            util: 'px'
        },
        {
            name: 'memo',
            label: '备注',
            langtag: 'entities.salrule.main_grid.columns.memo',
            show: true,
            util: 'PX'
        },
        {
            name: 'updateman',
            label: '更新人',
            langtag: 'entities.salrule.main_grid.columns.updateman',
            show: true,
            util: 'PX'
        },
        {
            name: 'updatedate',
            label: '更新时间',
            langtag: 'entities.salrule.main_grid.columns.updatedate',
            show: true,
            util: 'PX'
        },
    ]
    ...
    public rules: any = {
        srfkey: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '薪酬规则标识 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '薪酬规则标识 值不能为空', trigger: 'blur' },
        ],
    }
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'SalRulePickupGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'SalRulePickupGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        _datas.forEach((record: any, index: number) => {
            let srfmajortext = record.salrulename;
            if (index < 5) {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ salrule: _keys }),Object.assign({ salrule: _keys },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    public rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.salrule,$event.salrule);
        });
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('salrule_main_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('salrule_main_grid', JSON.stringify(this.allColumns));
    }
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: '错误', desc: 'SalRulePickupGridView视图表格createAction参数未配置' });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: '错误', desc: 'SalRulePickupGridView视图表格updateAction参数未配置' });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
                        if(item.salrule){
                            Object.assign(this.context,{salrule:item.salrule});
                        }
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'SalRulePickupGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
    ...
    public getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.salrule,args.row.salrule);
        });
