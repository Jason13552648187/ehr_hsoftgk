    ...
    <el-table v-if="isDisplay === true"
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('pimpersonname')">
                <el-table-column show-overflow-tooltip :prop="'pimpersonname'" :label="$t('entities.salpersonstd.main_grid.columns.pimpersonname')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.salpersonstd.main_grid.columns.pimpersonname')}}
                      </span>
                    </template>
            ...
            <template v-if="getColumnState('ormorgname')">
                <el-table-column show-overflow-tooltip :prop="'ormorgname'" :label="$t('entities.salpersonstd.main_grid.columns.ormorgname')" :width="250"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.salpersonstd.main_grid.columns.ormorgname')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('ormorgsectorname')">
                <el-table-column show-overflow-tooltip :prop="'ormorgsectorname'" :label="$t('entities.salpersonstd.main_grid.columns.ormorgsectorname')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.salpersonstd.main_grid.columns.ormorgsectorname')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('ormdutyname')">
                <el-table-column show-overflow-tooltip :prop="'ormdutyname'" :label="$t('entities.salpersonstd.main_grid.columns.ormdutyname')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.salpersonstd.main_grid.columns.ormdutyname')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('ormpostname')">
                <el-table-column show-overflow-tooltip :prop="'ormpostname'" :label="$t('entities.salpersonstd.main_grid.columns.ormpostname')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.salpersonstd.main_grid.columns.ormpostname')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('saltype')">
                <el-table-column show-overflow-tooltip :prop="'saltype'" :label="$t('entities.salpersonstd.main_grid.columns.saltype')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.salpersonstd.main_grid.columns.saltype')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
            <codelist :value="row.saltype" tag='EhrCodeList0036' codelistType='DYNAMIC' renderMode="STR" valueSeparator=";" textSeparator="、" ></codelist>
                        </template>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('begintime')">
                <el-table-column show-overflow-tooltip :prop="'begintime'" :label="$t('entities.salpersonstd.main_grid.columns.begintime')" :width="120"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.salpersonstd.main_grid.columns.begintime')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('endtime')">
                <el-table-column show-overflow-tooltip :prop="'endtime'" :label="$t('entities.salpersonstd.main_grid.columns.endtime')" :width="120"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.salpersonstd.main_grid.columns.endtime')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('changereason')">
                <el-table-column show-overflow-tooltip :prop="'changereason'" :label="$t('entities.salpersonstd.main_grid.columns.changereason')" :width="200"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.salpersonstd.main_grid.columns.changereason')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
...
import SalPersonStdService from '@/service/sal-person-std/sal-person-std-service';
...
export default class MainBase extends Vue implements ControlInterface {
     ...
     * 实体服务对象
     *
     * @type {SalPersonStdService}
     * @memberof MainBase
     */
    public appEntityService: SalPersonStdService = new SalPersonStdService({ $store: this.$store });
    ...
    */
    ...
    public allColumns: any[] = [
        {
            name: 'pimpersonname',
            label: '员工姓名',
            langtag: 'entities.salpersonstd.main_grid.columns.pimpersonname',
            show: true,
            util: 'px'
        },
        {
            name: 'ormorgname',
            label: '组织',
            langtag: 'entities.salpersonstd.main_grid.columns.ormorgname',
            show: true,
            util: 'PX'
        },
        {
            ...
            label: '部门',
            langtag: 'entities.salpersonstd.main_grid.columns.ormorgsectorname',
            show: true,
            util: 'px'
        },
        {
            name: 'ormdutyname',
            label: '职务',
            langtag: 'entities.salpersonstd.main_grid.columns.ormdutyname',
            show: true,
            util: 'px'
        },
        {
            name: 'ormpostname',
            label: '岗位',
            langtag: 'entities.salpersonstd.main_grid.columns.ormpostname',
            show: true,
            util: 'px'
        },
        {
            name: 'saltype',
            label: '薪酬类型',
            langtag: 'entities.salpersonstd.main_grid.columns.saltype',
            show: true,
            util: 'px'
        },
        {
            name: 'begintime',
            label: '生效日期',
            langtag: 'entities.salpersonstd.main_grid.columns.begintime',
            show: true,
            util: 'PX'
        },
        {
            name: 'endtime',
            label: '失效日期',
            langtag: 'entities.salpersonstd.main_grid.columns.endtime',
            show: true,
            util: 'PX'
        },
        {
            name: 'changereason',
            label: '变更原因',
            langtag: 'entities.salpersonstd.main_grid.columns.changereason',
            show: true,
            util: 'PX'
        },
    ]
    ...
    public rules: any = {
        srfkey: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '员工薪酬标准标识 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '员工薪酬标准标识 值不能为空', trigger: 'blur' },
        ],
    }
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'SalPersonStdGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'SalPersonStdGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        _datas.forEach((record: any, index: number) => {
            let srfmajortext = record.salpersonstdname;
            if (index < 5) {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ salpersonstd: _keys }),Object.assign({ salpersonstd: _keys },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    public rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.salpersonstd,$event.salpersonstd);
        });
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('salpersonstd_main_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('salpersonstd_main_grid', JSON.stringify(this.allColumns));
    }
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: '错误', desc: 'SalPersonStdGridView视图表格createAction参数未配置' });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: '错误', desc: 'SalPersonStdGridView视图表格updateAction参数未配置' });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
                        if(item.salpersonstd){
                            Object.assign(this.context,{salpersonstd:item.salpersonstd});
                        }
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'SalPersonStdGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
    ...
    public getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.salpersonstd,args.row.salpersonstd);
        });
