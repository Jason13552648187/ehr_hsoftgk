  ...
  <row>
    <i-col span="20" class="form-content">
      <row>
              ...
              <input-box v-model="data.n_salitemname_like"  @enter="onEnter($event)"    :disabled="detailsModel.n_salitemname_like.disabled" type='text'  style=""></input-box>
          </app-form-item>
...
export default class DefaultBase extends Vue implements ControlInterface {
    ...
    public rules: any = {
        n_salitemname_like: [
            { type: 'string', message: '要素项(%) 值必须为字符串类型', trigger: 'change' },
            { type: 'string', message: '要素项(%) 值必须为字符串类型', trigger: 'blur' },
            { required: false, type: 'string', message: '要素项(%) 值不能为空', trigger: 'change' },
            { required: false, type: 'string', message: '要素项(%) 值不能为空', trigger: 'blur' },
        ],
    }
...
, 
        n_salitemname_like: new FormItemModel({ caption: '要素项(%)', detailType: 'FORMITEM', name: 'n_salitemname_like', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 3 })
, 
    };

    /**
     * 监控表单属性 n_salitemname_like 值
     *
     * @param {*} newVal
     * @param {*} oldVal
     * @memberof DefaultBase
     */
    @Watch('data.n_salitemname_like')
    onN_salitemname_likeChange(newVal: any, oldVal: any) {
        this.formDataChange({ name: 'n_salitemname_like', newVal: newVal, oldVal: oldVal });
    }
    ...
    public load(opt: any = {}): void {
        if(!this.loadAction){
            this.$Notice.error({ title: '错误', desc: 'SalItemGridView视图搜索表单loadAction参数未配置' });
            return;
        }
        const arg: any = { ...opt };
    ...
    public loadDraft(opt: any = {},mode?:string): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'SalItemGridView视图搜索表单loaddraftAction参数未配置' });
            return;
        }
        const arg: any = { ...opt } ;
        Object.assign(arg,{viewparams:this.viewparams});
