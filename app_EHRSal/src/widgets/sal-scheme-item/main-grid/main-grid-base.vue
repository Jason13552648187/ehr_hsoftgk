...
<div class='grid' style="height:100%;">
    ...
    <el-table v-if="isDisplay === true"
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('salitemname')">
                <el-table-column show-overflow-tooltip :prop="'salitemname'" :label="$t('entities.salschemeitem.main_grid.columns.salitemname')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.salschemeitem.main_grid.columns.salitemname')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.salitemname}}</span>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('sitemtype')">
                <el-table-column show-overflow-tooltip :prop="'sitemtype'" :label="$t('entities.salschemeitem.main_grid.columns.sitemtype')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.salschemeitem.main_grid.columns.sitemtype')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
            <codelist :value="row.sitemtype" tag='EhrCodeList0212' codelistType='STATIC' renderMode="STR" valueSeparator=";" textSeparator="、" ></codelist>
                        </template>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('salrulename')">
                <el-table-column show-overflow-tooltip :prop="'salrulename'" :label="$t('entities.salschemeitem.main_grid.columns.salrulename')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.salschemeitem.main_grid.columns.salrulename')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.salrulename}}</span>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('gs')">
                <el-table-column show-overflow-tooltip :prop="'gs'" :label="$t('entities.salschemeitem.main_grid.columns.gs')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.salschemeitem.main_grid.columns.gs')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.gs}}</span>
            ...
            <template v-if="getColumnState('sitemadd')">
                <el-table-column show-overflow-tooltip :prop="'sitemadd'" :label="$t('entities.salschemeitem.main_grid.columns.sitemadd')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.salschemeitem.main_grid.columns.sitemadd')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
            <codelist :value="row.sitemadd" tag='EhrCodeList0128' codelistType='STATIC' renderMode="STR" valueSeparator=";" textSeparator="、" ></codelist>
                        </template>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('memo')">
                <el-table-column show-overflow-tooltip :prop="'memo'" :label="$t('entities.salschemeitem.main_grid.columns.memo')" :width="200"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.salschemeitem.main_grid.columns.memo')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.memo}}</span>
...
import SalSchemeItemService from '@/service/sal-scheme-item/sal-scheme-item-service';
...
export default class MainBase extends Vue implements ControlInterface {
     ...
     * 实体服务对象
     *
     * @type {SalSchemeItemService}
     * @memberof MainBase
     */
    public appEntityService: SalSchemeItemService = new SalSchemeItemService({ $store: this.$store });
    ...
    */
    ...
    public allColumns: any[] = [
        ...
        },
        {
            name: 'salitemname',
            label: '要素项',
            langtag: 'entities.salschemeitem.main_grid.columns.salitemname',
            show: true,
            util: 'px'
        },
        {
            name: 'sitemtype',
            label: '要素项类型',
            langtag: 'entities.salschemeitem.main_grid.columns.sitemtype',
            show: true,
            util: 'px'
        },
        {
            name: 'salrulename',
            label: '薪酬规则',
            langtag: 'entities.salschemeitem.main_grid.columns.salrulename',
            show: true,
            util: 'px'
        },
        {
            name: 'gs',
            label: '公式',
            langtag: 'entities.salschemeitem.main_grid.columns.gs',
            show: true,
            util: 'px'
        },
        {
            name: 'sitemadd',
            label: '增/减',
            langtag: 'entities.salschemeitem.main_grid.columns.sitemadd',
            show: true,
            util: 'px'
        },
        {
            name: 'memo',
            label: '备注',
            langtag: 'entities.salschemeitem.main_grid.columns.memo',
            show: true,
            util: 'PX'
        },
    ]
    ...
    public rules: any = {
        srfkey: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '工资单要素项标识 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '工资单要素项标识 值不能为空', trigger: 'blur' },
        ],
    }
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'SalSchemeItemGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'SalSchemeItemGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        _datas.forEach((record: any, index: number) => {
            let srfmajortext = record.salitemname;
            if (index < 5) {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ salschemeitem: _keys }),Object.assign({ salschemeitem: _keys },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    public rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.salschemeitem,$event.salschemeitem);
        });
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('salschemeitem_main_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('salschemeitem_main_grid', JSON.stringify(this.allColumns));
    }
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: '错误', desc: 'SalSchemeItemGridView视图表格createAction参数未配置' });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: '错误', desc: 'SalSchemeItemGridView视图表格updateAction参数未配置' });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
                        if(item.salschemeitem){
                            Object.assign(this.context,{salschemeitem:item.salschemeitem});
                        }
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'SalSchemeItemGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
    ...
    public getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.salschemeitem,args.row.salschemeitem);
        });
