...
<div class='grid' style="height:100%;">
    ...
    <el-table v-if="isDisplay === true"
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('salschemename')">
                <el-table-column show-overflow-tooltip :prop="'salschemename'" :label="$t('entities.salscheme.main_grid.columns.salschemename')" :width="200"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.salscheme.main_grid.columns.salschemename')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.salschemename}}</span>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('ormorgname')">
                <el-table-column show-overflow-tooltip :prop="'ormorgname'" :label="$t('entities.salscheme.main_grid.columns.ormorgname')" :width="250"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.salscheme.main_grid.columns.ormorgname')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('saltype')">
                <el-table-column show-overflow-tooltip :prop="'saltype'" :label="$t('entities.salscheme.main_grid.columns.saltype')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.salscheme.main_grid.columns.saltype')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
            ...
            <template v-if="getColumnState('validflag')">
                <el-table-column show-overflow-tooltip :prop="'validflag'" :label="$t('entities.salscheme.main_grid.columns.validflag')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.salscheme.main_grid.columns.validflag')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
            <codelist :value="row.validflag" tag='EhrCodeList0401' codelistType='STATIC' renderMode="STR" valueSeparator=";" textSeparator="、" ></codelist>
                        </template>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('memo')">
                <el-table-column show-overflow-tooltip :prop="'memo'" :label="$t('entities.salscheme.main_grid.columns.memo')" :width="200"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.salscheme.main_grid.columns.memo')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.memo}}</span>
            ...
            <template v-if="getColumnState('updateman')">
                <el-table-column show-overflow-tooltip :prop="'updateman'" :label="$t('entities.salscheme.main_grid.columns.updateman')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.salscheme.main_grid.columns.updateman')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
            ...
            <template v-if="getColumnState('updatedate')">
                <el-table-column show-overflow-tooltip :prop="'updatedate'" :label="$t('entities.salscheme.main_grid.columns.updatedate')" :width="200"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.salscheme.main_grid.columns.updatedate')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
...
import SalSchemeService from '@/service/sal-scheme/sal-scheme-service';
...
export default class MainBase extends Vue implements ControlInterface {
     ...
     * 实体服务对象
     *
     * @type {SalSchemeService}
     * @memberof MainBase
     */
    public appEntityService: SalSchemeService = new SalSchemeService({ $store: this.$store });
    ...
    */
    ...
    public allColumns: any[] = [
        {
            name: 'salschemename',
            label: '工资单名称',
            langtag: 'entities.salscheme.main_grid.columns.salschemename',
            show: true,
            util: 'PX'
        },
        {
            name: 'ormorgname',
            label: '组织',
            langtag: 'entities.salscheme.main_grid.columns.ormorgname',
            show: true,
            util: 'PX'
        },
        {
            name: 'saltype',
            label: '薪酬类型',
            langtag: 'entities.salscheme.main_grid.columns.saltype',
            show: true,
            util: 'px'
        },
        {
            name: 'validflag',
            label: '是否启用',
            langtag: 'entities.salscheme.main_grid.columns.validflag',
            show: true,
            util: 'px'
        },
        {
            name: 'memo',
            label: '备注',
            langtag: 'entities.salscheme.main_grid.columns.memo',
            show: true,
            util: 'PX'
        },
        {
            name: 'updateman',
            label: '更新人',
            langtag: 'entities.salscheme.main_grid.columns.updateman',
            show: true,
            util: 'PX'
        },
        {
            name: 'updatedate',
            label: '更新时间',
            langtag: 'entities.salscheme.main_grid.columns.updatedate',
            show: true,
            util: 'PX'
        },
    ]
    ...
    public rules: any = {
        srfkey: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '工资单标识 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '工资单标识 值不能为空', trigger: 'blur' },
        ],
    }
    ...
    public async remove(datas: any[]): Promise<any> {
        ...
        _datas.forEach((record: any, index: number) => {
            let srfmajortext = record.salschemename;
            if (index < 5) {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ salscheme: _keys }),Object.assign({ salscheme: _keys },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    public rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.salscheme,$event.salscheme);
        });
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('salscheme_main_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('salscheme_main_grid', JSON.stringify(this.allColumns));
    }
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    ...
                    if(!this.updateAction){
                        ...
                        Object.assign(item,{viewparams:this.viewparams});
                        if(item.salscheme){
                            Object.assign(this.context,{salscheme:item.salscheme});
                        }
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'SalSchemePickupGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
    ...
    public getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.salscheme,args.row.salscheme);
        });
