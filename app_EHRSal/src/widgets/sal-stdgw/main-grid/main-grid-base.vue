    ...
    <el-table v-if="isDisplay === true"
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('ormorgname')">
                <el-table-column show-overflow-tooltip :prop="'ormorgname'" :label="$t('entities.salstdgw.main_grid.columns.ormorgname')" :width="250"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.salstdgw.main_grid.columns.ormorgname')}}
                      </span>
                    </template>
            ...
            <template v-if="getColumnState('qssj')">
                <el-table-column show-overflow-tooltip :prop="'qssj'" :label="$t('entities.salstdgw.main_grid.columns.qssj')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.salstdgw.main_grid.columns.qssj')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <app-format-data format="YYYY-MM-DD" :data="row.qssj"></app-format-data>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('jssj')">
                <el-table-column show-overflow-tooltip :prop="'jssj'" :label="$t('entities.salstdgw.main_grid.columns.jssj')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.salstdgw.main_grid.columns.jssj')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('bz')">
                <el-table-column show-overflow-tooltip :prop="'bz'" :label="$t('entities.salstdgw.main_grid.columns.bz')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.salstdgw.main_grid.columns.bz')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('updateman')">
                <el-table-column show-overflow-tooltip :prop="'updateman'" :label="$t('entities.salstdgw.main_grid.columns.updateman')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.salstdgw.main_grid.columns.updateman')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
            ...
            <template v-if="getColumnState('updatedate')">
                <el-table-column show-overflow-tooltip :prop="'updatedate'" :label="$t('entities.salstdgw.main_grid.columns.updatedate')" :width="200"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.salstdgw.main_grid.columns.updatedate')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
...
import SalStdgwService from '@/service/sal-stdgw/sal-stdgw-service';
...
export default class MainBase extends Vue implements ControlInterface {
     ...
     * 实体服务对象
     *
     * @type {SalStdgwService}
     * @memberof MainBase
     */
    public appEntityService: SalStdgwService = new SalStdgwService({ $store: this.$store });
    ...
    */
    ...
    public allColumns: any[] = [
        {
            name: 'ormorgname',
            label: '组织',
            langtag: 'entities.salstdgw.main_grid.columns.ormorgname',
            show: true,
            util: 'PX'
        },
        {
            name: 'qssj',
            label: '起始时间',
            langtag: 'entities.salstdgw.main_grid.columns.qssj',
            show: true,
            util: 'px'
        },
        {
            name: 'jssj',
            label: '结束时间',
            langtag: 'entities.salstdgw.main_grid.columns.jssj',
            show: true,
            util: 'px'
        },
        {
            name: 'bz',
            label: '补贴标准',
            langtag: 'entities.salstdgw.main_grid.columns.bz',
            show: true,
            util: 'px'
        },
        {
            name: 'updateman',
            label: '更新人',
            langtag: 'entities.salstdgw.main_grid.columns.updateman',
            show: true,
            util: 'PX'
        },
        {
            name: 'updatedate',
            label: '更新时间',
            langtag: 'entities.salstdgw.main_grid.columns.updatedate',
            show: true,
            util: 'PX'
        },
    ]
    ...
    public rules: any = {
        srfkey: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '高温津贴标准标识 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '高温津贴标准标识 值不能为空', trigger: 'blur' },
        ],
    }
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'SalStdgwGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'SalStdgwGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        _datas.forEach((record: any, index: number) => {
            let srfmajortext = record.salstdgwname;
            if (index < 5) {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ salstdgw: _keys }),Object.assign({ salstdgw: _keys },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    public rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.salstdgw,$event.salstdgw);
        });
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('salstdgw_main_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('salstdgw_main_grid', JSON.stringify(this.allColumns));
    }
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: '错误', desc: 'SalStdgwGridView视图表格createAction参数未配置' });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: '错误', desc: 'SalStdgwGridView视图表格updateAction参数未配置' });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
                        if(item.salstdgw){
                            Object.assign(this.context,{salstdgw:item.salstdgw});
                        }
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'SalStdgwGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
    ...
    public getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.salstdgw,args.row.salstdgw);
        });
