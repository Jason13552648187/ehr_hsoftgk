    ...
    <el-table v-if="isDisplay === true"
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('salparamname')">
                <el-table-column show-overflow-tooltip :prop="'salparamname'" :label="$t('entities.salruledetail.main_grid.columns.salparamname')" :width="200"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.salruledetail.main_grid.columns.salparamname')}}
                      </span>
                    </template>
            ...
            <template v-if="getColumnState('checkrule')">
                <el-table-column show-overflow-tooltip :prop="'checkrule'" :label="$t('entities.salruledetail.main_grid.columns.checkrule')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.salruledetail.main_grid.columns.checkrule')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
            <codelist :value="row.checkrule" tag='EhrCodeList0068' codelistType='STATIC' ></codelist>
                        </template>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('salparamname2')">
                <el-table-column show-overflow-tooltip :prop="'salparamname2'" :label="$t('entities.salruledetail.main_grid.columns.salparamname2')" :width="200"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.salruledetail.main_grid.columns.salparamname2')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.salparamname2}}</span>
                    </template>
                </el-table-column>
            </template>
...
import SalRuleDetailService from '@/service/sal-rule-detail/sal-rule-detail-service';
...
export default class MainBase extends Vue implements ControlInterface {
     ...
     * 实体服务对象
     *
     * @type {SalRuleDetailService}
     * @memberof MainBase
     */
    public appEntityService: SalRuleDetailService = new SalRuleDetailService({ $store: this.$store });
    ...
    */
    ...
    public allColumns: any[] = [
        {
            name: 'salparamname',
            label: '薪酬计算指标(原值)',
            langtag: 'entities.salruledetail.main_grid.columns.salparamname',
            show: true,
            util: 'PX'
        },
        {
            name: 'checkrule',
            label: '比较规则',
            langtag: 'entities.salruledetail.main_grid.columns.checkrule',
            show: true,
            util: 'px'
        },
        {
            name: 'salparamname2',
            label: '薪酬计算指标(对应值)',
            langtag: 'entities.salruledetail.main_grid.columns.salparamname2',
            show: true,
            util: 'PX'
        },
    ]
    ...
    public rules: any = {
        srfkey: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '薪酬规则明细标识 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '薪酬规则明细标识 值不能为空', trigger: 'blur' },
        ],
    }
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'SalRuleDetailGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'SalRuleDetailGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        _datas.forEach((record: any, index: number) => {
            let srfmajortext = record.salruledetailname;
            if (index < 5) {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ salruledetail: _keys }),Object.assign({ salruledetail: _keys },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    public rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.salruledetail,$event.salruledetail);
        });
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('salruledetail_main_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('salruledetail_main_grid', JSON.stringify(this.allColumns));
    }
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: '错误', desc: 'SalRuleDetailGridView视图表格createAction参数未配置' });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: '错误', desc: 'SalRuleDetailGridView视图表格updateAction参数未配置' });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
                        if(item.salruledetail){
                            Object.assign(this.context,{salruledetail:item.salruledetail});
                        }
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'SalRuleDetailGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
    ...
    public getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.salruledetail,args.row.salruledetail);
        });
