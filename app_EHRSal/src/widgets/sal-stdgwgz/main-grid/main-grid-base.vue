    ...
    <el-table v-if="isDisplay === true"
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('ormorgname')">
                <el-table-column show-overflow-tooltip :prop="'ormorgname'" :label="$t('entities.salstdgwgz.main_grid.columns.ormorgname')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.salstdgwgz.main_grid.columns.ormorgname')}}
                      </span>
                    </template>
            ...
            <template v-if="getColumnState('ormdutyname')">
                <el-table-column show-overflow-tooltip :prop="'ormdutyname'" :label="$t('entities.salstdgwgz.main_grid.columns.ormdutyname')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.salstdgwgz.main_grid.columns.ormdutyname')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('dj')">
                <el-table-column show-overflow-tooltip :prop="'dj'" :label="$t('entities.salstdgwgz.main_grid.columns.dj')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.salstdgwgz.main_grid.columns.dj')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
            ...
            <template v-if="getColumnState('bz')">
                <el-table-column show-overflow-tooltip :prop="'bz'" :label="$t('entities.salstdgwgz.main_grid.columns.bz')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.salstdgwgz.main_grid.columns.bz')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('jbgzbl')">
                <el-table-column show-overflow-tooltip :prop="'jbgzbl'" :label="$t('entities.salstdgwgz.main_grid.columns.jbgzbl')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.salstdgwgz.main_grid.columns.jbgzbl')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('jxgzbl')">
                <el-table-column show-overflow-tooltip :prop="'jxgzbl'" :label="$t('entities.salstdgwgz.main_grid.columns.jxgzbl')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.salstdgwgz.main_grid.columns.jxgzbl')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
...
import SalStdgwgzService from '@/service/sal-stdgwgz/sal-stdgwgz-service';
...
export default class MainBase extends Vue implements ControlInterface {
     ...
     * 实体服务对象
     *
     * @type {SalStdgwgzService}
     * @memberof MainBase
     */
    public appEntityService: SalStdgwgzService = new SalStdgwgzService({ $store: this.$store });
    ...
    */
    ...
    public allColumns: any[] = [
        {
            name: 'ormorgname',
            label: '组织',
            langtag: 'entities.salstdgwgz.main_grid.columns.ormorgname',
            show: true,
            util: 'px'
        },
        {
            name: 'ormdutyname',
            label: '职务',
            langtag: 'entities.salstdgwgz.main_grid.columns.ormdutyname',
            show: true,
            util: 'px'
        },
        {
            name: 'dj',
            label: '工资档级',
            langtag: 'entities.salstdgwgz.main_grid.columns.dj',
            show: true,
            util: 'px'
        },
        {
            name: 'bz',
            label: '工资总额',
            langtag: 'entities.salstdgwgz.main_grid.columns.bz',
            show: true,
            util: 'px'
        },
        {
            name: 'jbgzbl',
            label: '基本工资比例(%)',
            langtag: 'entities.salstdgwgz.main_grid.columns.jbgzbl',
            show: true,
            util: 'px'
        },
        {
            name: 'jxgzbl',
            label: '绩效工资比例(%)',
            langtag: 'entities.salstdgwgz.main_grid.columns.jxgzbl',
            show: true,
            util: 'px'
        },
    ]
    ...
    public rules: any = {
        srfkey: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '岗位工资标准标识 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '岗位工资标准标识 值不能为空', trigger: 'blur' },
        ],
    }
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'SalStdgwgzGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'SalStdgwgzGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        _datas.forEach((record: any, index: number) => {
            let srfmajortext = record.salstdgwgzname;
            if (index < 5) {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ salstdgwgz: _keys }),Object.assign({ salstdgwgz: _keys },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    public rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.salstdgwgz,$event.salstdgwgz);
        });
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('salstdgwgz_main_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('salstdgwgz_main_grid', JSON.stringify(this.allColumns));
    }
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: '错误', desc: 'SalStdgwgzGridView视图表格createAction参数未配置' });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: '错误', desc: 'SalStdgwgzGridView视图表格updateAction参数未配置' });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
                        if(item.salstdgwgz){
                            Object.assign(this.context,{salstdgwgz:item.salstdgwgz});
                        }
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'SalStdgwgzGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
    ...
    public getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.salstdgwgz,args.row.salstdgwgz);
        });
