...
<div class='grid' style="height:100%;">
    ...
    <el-table v-if="isDisplay === true"
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('salschemeitemname')">
                <el-table-column show-overflow-tooltip :prop="'salschemeitemname'" :label="$t('entities.salpersonstddetail.main_grid.columns.salschemeitemname')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.salpersonstddetail.main_grid.columns.salschemeitemname')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.salschemeitemname}}</span>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('ywms')">
                <el-table-column show-overflow-tooltip :prop="'ywms'" :label="$t('entities.salpersonstddetail.main_grid.columns.ywms')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.salpersonstddetail.main_grid.columns.ywms')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.ywms}}</span>
            ...
            <template v-if="getColumnState('xc')">
                <el-table-column show-overflow-tooltip :prop="'xc'" :label="$t('entities.salpersonstddetail.main_grid.columns.xc')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.salpersonstddetail.main_grid.columns.xc')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                            <app-format-data dataType="FLOAT" precision="2" :data="row.xc"></app-format-data>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('updateman')">
                <el-table-column show-overflow-tooltip :prop="'updateman'" :label="$t('entities.salpersonstddetail.main_grid.columns.updateman')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.salpersonstddetail.main_grid.columns.updateman')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
            ...
            <template v-if="getColumnState('updatedate')">
                <el-table-column show-overflow-tooltip :prop="'updatedate'" :label="$t('entities.salpersonstddetail.main_grid.columns.updatedate')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.salpersonstddetail.main_grid.columns.updatedate')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
...
import SALPERSONSTDDETAILService from '@/service/salpersonstddetail/salpersonstddetail-service';
...
export default class MainBase extends Vue implements ControlInterface {
     ...
     * 实体服务对象
     *
     * @type {SALPERSONSTDDETAILService}
     * @memberof MainBase
     */
    public appEntityService: SALPERSONSTDDETAILService = new SALPERSONSTDDETAILService({ $store: this.$store });
    ...
    */
    ...
    public allColumns: any[] = [
        {
            ...
            label: '要素项',
            langtag: 'entities.salpersonstddetail.main_grid.columns.salschemeitemname',
            show: true,
            util: 'px'
        },
        {
            name: 'ywms',
            label: '实际情况描述',
            langtag: 'entities.salpersonstddetail.main_grid.columns.ywms',
            show: true,
            util: 'px'
        },
        {
            name: 'xc',
            label: '薪酬',
            langtag: 'entities.salpersonstddetail.main_grid.columns.xc',
            show: true,
            util: 'px'
        },
        {
            name: 'updateman',
            label: '更新人',
            langtag: 'entities.salpersonstddetail.main_grid.columns.updateman',
            show: true,
            util: 'PX'
        },
        {
            name: 'updatedate',
            label: '更新时间',
            langtag: 'entities.salpersonstddetail.main_grid.columns.updatedate',
            show: true,
            util: 'PX'
        },
    ]
    ...
    public rules: any = {
        srfkey: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '员工薪酬标准明细标识 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '员工薪酬标准明细标识 值不能为空', trigger: 'blur' },
        ],
    }
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'SALPERSONSTDDETAILGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'SALPERSONSTDDETAILGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        _datas.forEach((record: any, index: number) => {
            let srfmajortext = record.salpersonstddetailname;
            if (index < 5) {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ salpersonstddetail: _keys }),Object.assign({ salpersonstddetail: _keys },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    public rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.salpersonstddetail,$event.salpersonstddetail);
        });
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('salpersonstddetail_main_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('salpersonstddetail_main_grid', JSON.stringify(this.allColumns));
    }
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: '错误', desc: 'SALPERSONSTDDETAILGridView视图表格createAction参数未配置' });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: '错误', desc: 'SALPERSONSTDDETAILGridView视图表格updateAction参数未配置' });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
                        if(item.salpersonstddetail){
                            Object.assign(this.context,{salpersonstddetail:item.salpersonstddetail});
                        }
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'SALPERSONSTDDETAILGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
    ...
    public getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.salpersonstddetail,args.row.salpersonstddetail);
        });
