    ...
    <el-table v-if="isDisplay === true"
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('ygbh')">
                <el-table-column show-overflow-tooltip :prop="'ygbh'" :label="$t('entities.pcmydjdmx.jdgl2_grid.columns.ygbh')" :width="130"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pcmydjdmx.jdgl2_grid.columns.ygbh')}}
                      </span>
                    </template>
            ...
            <template v-if="getColumnState('pimpersonname')">
                <el-table-column show-overflow-tooltip :prop="'pimpersonname'" :label="$t('entities.pcmydjdmx.jdgl2_grid.columns.pimpersonname')" :width="130"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pcmydjdmx.jdgl2_grid.columns.pimpersonname')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('shortname')">
                <el-table-column show-overflow-tooltip :prop="'shortname'" :label="$t('entities.pcmydjdmx.jdgl2_grid.columns.shortname')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pcmydjdmx.jdgl2_grid.columns.shortname')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('jdzzdzs')">
                <el-table-column show-overflow-tooltip :prop="'jdzzdzs'" :label="$t('entities.pcmydjdmx.jdgl2_grid.columns.jdzzdzs')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pcmydjdmx.jdgl2_grid.columns.jdzzdzs')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.jdzzdzs}}</span>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('jdshortname')">
                <el-table-column show-overflow-tooltip :prop="'jdshortname'" :label="$t('entities.pcmydjdmx.jdgl2_grid.columns.jdshortname')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pcmydjdmx.jdgl2_grid.columns.jdshortname')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.jdshortname}}</span>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('jdksrq')">
                <el-table-column show-overflow-tooltip :prop="'jdksrq'" :label="$t('entities.pcmydjdmx.jdgl2_grid.columns.jdksrq')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pcmydjdmx.jdgl2_grid.columns.jdksrq')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
                                <app-span name='jdksrq' editorType="DATEPICKER" :value="row.jdksrq"></app-span>
                        </template>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('jdjsrq')">
                <el-table-column show-overflow-tooltip :prop="'jdjsrq'" :label="$t('entities.pcmydjdmx.jdgl2_grid.columns.jdjsrq')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pcmydjdmx.jdgl2_grid.columns.jdjsrq')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
                                <app-span name='jdjsrq' editorType="DATEPICKER" :value="row.jdjsrq"></app-span>
                        </template>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('checkstatus')">
                <el-table-column show-overflow-tooltip :prop="'checkstatus'" :label="$t('entities.pcmydjdmx.jdgl2_grid.columns.checkstatus')" :width="120"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pcmydjdmx.jdgl2_grid.columns.checkstatus')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
...
export default class JDGL2Base extends Vue implements ControlInterface {
     ...
     * @memberof JDGL2Base
     */
    public service: JDGL2Service = new JDGL2Service({ $store: this.$store });
    ...
    */
    ...
    public allColumns: any[] = [
        ...
        },
        {
            name: 'ygbh',
            label: '员工编号',
            langtag: 'entities.pcmydjdmx.jdgl2_grid.columns.ygbh',
            show: true,
            util: 'PX'
        },
        {
            name: 'pimpersonname',
            label: '员工姓名',
            langtag: 'entities.pcmydjdmx.jdgl2_grid.columns.pimpersonname',
            show: true,
            util: 'PX'
        },
        {
            name: 'zzdzs',
            label: '所属组织',
            langtag: 'entities.pcmydjdmx.jdgl2_grid.columns.zzdzs',
            show: true,
            util: 'px'
        },
        {
            name: 'shortname',
            label: '所属部门',
            langtag: 'entities.pcmydjdmx.jdgl2_grid.columns.shortname',
            show: true,
            util: 'px'
        },
        {
            name: 'jdzzdzs',
            label: '借调组织',
            langtag: 'entities.pcmydjdmx.jdgl2_grid.columns.jdzzdzs',
            show: true,
            util: 'px'
        },
        {
            name: 'jdshortname',
            label: '借调部门',
            langtag: 'entities.pcmydjdmx.jdgl2_grid.columns.jdshortname',
            show: true,
            util: 'px'
        },
        {
            name: 'jdksrq',
            label: '借调开始日期',
            langtag: 'entities.pcmydjdmx.jdgl2_grid.columns.jdksrq',
            show: true,
            util: 'PX'
        },
        {
            name: 'jdjsrq',
            label: '借调结束日期',
            langtag: 'entities.pcmydjdmx.jdgl2_grid.columns.jdjsrq',
            show: true,
            util: 'PX'
        },
        {
            name: 'checkstatus',
            label: '审核状态',
            langtag: 'entities.pcmydjdmx.jdgl2_grid.columns.checkstatus',
            show: true,
            util: 'PX'
        },
    ]
    ...
    public getGridRowModel(){
        return {
          ...
          jdksrq: new FormItemModel(),
          srfkey: new FormItemModel(),
          ormdutyname: new FormItemModel(),
          jdjsrq: new FormItemModel(),
        }
    }
    ...
    public rules: any = {
        ...
        ormpostname: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '借调岗位 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '借调岗位 值不能为空', trigger: 'blur' },
        ],
        jdksrq: [
             { required: true, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '借调开始日期 值不能为空', trigger: 'change' },
            { required: true, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '借调开始日期 值不能为空', trigger: 'blur' },
        ],
        srfkey: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '异动借调明细标识 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '异动借调明细标识 值不能为空', trigger: 'blur' },
        ],
        ormdutyname: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '借调职务 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '借调职务 值不能为空', trigger: 'blur' },
        ],
        jdjsrq: [
             { required: true, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '借调结束日期 值不能为空', trigger: 'change' },
            { required: true, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '借调结束日期 值不能为空', trigger: 'blur' },
        ],
    }
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'PcmYdjdmxJDGLGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'PcmYdjdmxJDGLGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        _datas.forEach((record: any, index: number) => {
            let srfmajortext = record.pcmydjdmxname;
            if (index < 5) {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ pcmydjdmx: _keys }),Object.assign({ pcmydjdmx: _keys },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    public rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.pcmydjdmx,$event.pcmydjdmx);
        });
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('pcmydjdmx_jdgl2_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('pcmydjdmx_jdgl2_grid', JSON.stringify(this.allColumns));
    }
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: '错误', desc: 'PcmYdjdmxJDGLGridView视图表格createAction参数未配置' });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: '错误', desc: 'PcmYdjdmxJDGLGridView视图表格updateAction参数未配置' });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
                        if(item.pcmydjdmx){
                            Object.assign(this.context,{pcmydjdmx:item.pcmydjdmx});
                        }
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'PcmYdjdmxJDGLGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
