    ...
    <el-table v-if="isDisplay === true"
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('pimpersonname')">
                <el-table-column show-overflow-tooltip :prop="'pimpersonname'" :label="$t('entities.pcmxygzzsqmx.main_grid.columns.pimpersonname')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pcmxygzzsqmx.main_grid.columns.pimpersonname')}}
                      </span>
                    </template>
            ...
            <template v-if="getColumnState('zz')">
                <el-table-column show-overflow-tooltip :prop="'zz'" :label="$t('entities.pcmxygzzsqmx.main_grid.columns.zz')" :width="250"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pcmxygzzsqmx.main_grid.columns.zz')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.zz}}</span>
            ...
            <template v-if="getColumnState('bm')">
                <el-table-column show-overflow-tooltip :prop="'bm'" :label="$t('entities.pcmxygzzsqmx.main_grid.columns.bm')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pcmxygzzsqmx.main_grid.columns.bm')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.bm}}</span>
            ...
            <template v-if="getColumnState('zj')">
                <el-table-column show-overflow-tooltip :prop="'zj'" :label="$t('entities.pcmxygzzsqmx.main_grid.columns.zj')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pcmxygzzsqmx.main_grid.columns.zj')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
            <codelist :value="row.zj" tag='EhrCodeList0025' codelistType='DYNAMIC' renderMode="STR" valueSeparator=";" textSeparator="、" ></codelist>
                        </template>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('zw')">
                <el-table-column show-overflow-tooltip :prop="'zw'" :label="$t('entities.pcmxygzzsqmx.main_grid.columns.zw')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pcmxygzzsqmx.main_grid.columns.zw')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
            ...
            <template v-if="getColumnState('gw')">
                <el-table-column show-overflow-tooltip :prop="'gw'" :label="$t('entities.pcmxygzzsqmx.main_grid.columns.gw')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pcmxygzzsqmx.main_grid.columns.gw')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
...
import PcmXygzzsqmxService from '@/service/pcm-xygzzsqmx/pcm-xygzzsqmx-service';
...
export default class MainBase extends Vue implements ControlInterface {
     ...
     * 实体服务对象
     *
     * @type {PcmXygzzsqmxService}
     * @memberof MainBase
     */
    public appEntityService: PcmXygzzsqmxService = new PcmXygzzsqmxService({ $store: this.$store });
    ...
    */
    ...
    public allColumns: any[] = [
        {
            name: 'pimpersonname',
            label: '员工',
            langtag: 'entities.pcmxygzzsqmx.main_grid.columns.pimpersonname',
            show: true,
            util: 'px'
        },
        {
            name: 'zz',
            label: '组织',
            langtag: 'entities.pcmxygzzsqmx.main_grid.columns.zz',
            show: true,
            util: 'PX'
        },
        {
            name: 'bm',
            label: '部门',
            langtag: 'entities.pcmxygzzsqmx.main_grid.columns.bm',
            show: true,
            util: 'px'
        },
        {
            name: 'zj',
            label: '职级',
            langtag: 'entities.pcmxygzzsqmx.main_grid.columns.zj',
            show: true,
            util: 'PX'
        },
        {
            name: 'zw',
            label: '职务',
            langtag: 'entities.pcmxygzzsqmx.main_grid.columns.zw',
            show: true,
            util: 'px'
        },
        {
            name: 'gw',
            label: '岗位',
            langtag: 'entities.pcmxygzzsqmx.main_grid.columns.gw',
            show: true,
            util: 'PX'
        },
    ]
    ...
    public rules: any = {
        srfkey: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '试用期员工转正申请明细标识 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '试用期员工转正申请明细标识 值不能为空', trigger: 'blur' },
        ],
    }
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'PcmXygzzsqmxGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'PcmXygzzsqmxGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        _datas.forEach((record: any, index: number) => {
            let srfmajortext = record.pcmxygzzsqmxname;
            if (index < 5) {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ pcmxygzzsqmx: _keys }),Object.assign({ pcmxygzzsqmx: _keys },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    public rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.pcmxygzzsqmx,$event.pcmxygzzsqmx);
        });
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('pcmxygzzsqmx_main_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('pcmxygzzsqmx_main_grid', JSON.stringify(this.allColumns));
    }
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: '错误', desc: 'PcmXygzzsqmxGridView视图表格createAction参数未配置' });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: '错误', desc: 'PcmXygzzsqmxGridView视图表格updateAction参数未配置' });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
                        if(item.pcmxygzzsqmx){
                            Object.assign(this.context,{pcmxygzzsqmx:item.pcmxygzzsqmx});
                        }
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'PcmXygzzsqmxGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
    ...
    public getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.pcmxygzzsqmx,args.row.pcmxygzzsqmx);
        });
