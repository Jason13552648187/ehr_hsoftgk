  ...
  <row>
    <i-col span="20" class="form-content">
      <row>
              ...
              <app-form-item name='n_pimpersonname_like' :itemRules="this.rules.n_pimpersonname_like" class='' :caption="$t('entities.pcmxygzzjlmx.default_searchform.details.n_pimpersonname_like')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.n_pimpersonname_like.error" :isEmptyCaption="false" labelPos="RIGHT"> 
          ...
          <i-col v-show="detailsModel.n_ygbh_like.visible" :style="{}"  :sm="{ span: 6, offset: 0 }" :md="{ span: 6, offset: 0 }" :lg="{ span: 6, offset: 0 }" :xl="{ span: 6, offset: 0 }">
              <app-form-item name='n_ygbh_like' :itemRules="this.rules.n_ygbh_like" class='' :caption="$t('entities.pcmxygzzjlmx.default_searchform.details.n_ygbh_like')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.n_ygbh_like.error" :isEmptyCaption="false" labelPos="RIGHT"> 
          ...
          <i-col v-show="detailsModel.n_duration_eq.visible" :style="{}"  :sm="{ span: 6, offset: 0 }" :md="{ span: 6, offset: 0 }" :lg="{ span: 6, offset: 0 }" :xl="{ span: 6, offset: 0 }">
              <app-form-item name='n_duration_eq' :itemRules="this.rules.n_duration_eq" class='' :caption="$t('entities.pcmxygzzjlmx.default_searchform.details.n_duration_eq')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.n_duration_eq.error" :isEmptyCaption="false" labelPos="RIGHT"> 
          ...
          <i-col v-show="detailsModel.n_checkstatus_eq.visible" :style="{}"  :sm="{ span: 6, offset: 0 }" :md="{ span: 6, offset: 0 }" :lg="{ span: 6, offset: 0 }" :xl="{ span: 6, offset: 0 }">
              <app-form-item name='n_checkstatus_eq' :itemRules="this.rules.n_checkstatus_eq" class='' :caption="$t('entities.pcmxygzzjlmx.default_searchform.details.n_checkstatus_eq')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.n_checkstatus_eq.error" :isEmptyCaption="false" labelPos="RIGHT"> 
...
export default class DefaultBase extends Vue implements ControlInterface {
    ...
    public rules: any = {
        ...
        n_checkstatus_eq: [
            { type: 'string', message: '审核状态 值必须为字符串类型', trigger: 'change' },
            { type: 'string', message: '审核状态 值必须为字符串类型', trigger: 'blur' },
            { required: false, type: 'string', message: '审核状态 值不能为空', trigger: 'change' },
            { required: false, type: 'string', message: '审核状态 值不能为空', trigger: 'blur' },
        ],
    }
...
, 
        n_checkstatus_eq: new FormItemModel({ caption: '审核状态', detailType: 'FORMITEM', name: 'n_checkstatus_eq', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 3 })
, 
    };
    ...
    /**
     * 监控表单属性 n_checkstatus_eq 值
     *
     * @param {*} newVal
     * @param {*} oldVal
     * @memberof DefaultBase
     */
    @Watch('data.n_checkstatus_eq')
    onN_checkstatus_eqChange(newVal: any, oldVal: any) {
        this.formDataChange({ name: 'n_checkstatus_eq', newVal: newVal, oldVal: oldVal });
    }
    ...
    public load(opt: any = {}): void {
        if(!this.loadAction){
            this.$Notice.error({ title: '错误', desc: 'PcmXygzzjlmxZZSQLSJMGridView视图搜索表单loadAction参数未配置' });
            return;
        }
        const arg: any = { ...opt };
    ...
    public loadDraft(opt: any = {},mode?:string): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'PcmXygzzjlmxZZSQLSJMGridView视图搜索表单loaddraftAction参数未配置' });
            return;
        }
        const arg: any = { ...opt } ;
        Object.assign(arg,{viewparams:this.viewparams});
