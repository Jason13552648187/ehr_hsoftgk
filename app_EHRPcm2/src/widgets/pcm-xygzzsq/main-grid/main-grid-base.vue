...
<div class='grid' style="height:100%;">
    ...
    <el-table v-if="isDisplay === true"
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('pcmxygzzsqname')">
                <el-table-column show-overflow-tooltip :prop="'pcmxygzzsqname'" :label="$t('entities.pcmxygzzsq.main_grid.columns.pcmxygzzsqname')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pcmxygzzsq.main_grid.columns.pcmxygzzsqname')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.pcmxygzzsqname}}</span>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('czr')">
                <el-table-column show-overflow-tooltip :prop="'czr'" :label="$t('entities.pcmxygzzsq.main_grid.columns.czr')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pcmxygzzsq.main_grid.columns.czr')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.czr}}</span>
            ...
            <template v-if="getColumnState('czrq')">
                <el-table-column show-overflow-tooltip :prop="'czrq'" :label="$t('entities.pcmxygzzsq.main_grid.columns.czrq')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pcmxygzzsq.main_grid.columns.czrq')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <app-format-data format="YYYY-MM-DD" :data="row.czrq"></app-format-data>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('sxrq')">
                <el-table-column show-overflow-tooltip :prop="'sxrq'" :label="$t('entities.pcmxygzzsq.main_grid.columns.sxrq')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pcmxygzzsq.main_grid.columns.sxrq')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
...
export default class MainBase extends Vue implements ControlInterface {
    ...
    */
    ...
    public allColumns: any[] = [
        {
            name: 'pcmxygzzsqname',
            label: '标题',
            langtag: 'entities.pcmxygzzsq.main_grid.columns.pcmxygzzsqname',
            show: true,
            util: 'PX'
        },
        {
            name: 'czr',
            label: '操作人',
            langtag: 'entities.pcmxygzzsq.main_grid.columns.czr',
            show: true,
            util: 'px'
        },
        {
            name: 'czrq',
            label: '操作日期',
            langtag: 'entities.pcmxygzzsq.main_grid.columns.czrq',
            show: true,
            util: 'px'
        },
        {
            name: 'sxrq',
            label: '生效日期',
            langtag: 'entities.pcmxygzzsq.main_grid.columns.sxrq',
            show: true,
            util: 'px'
        },
    ]
    ...
    public rules: any = {
        srfkey: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '试用期员工转正申请标识 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '试用期员工转正申请标识 值不能为空', trigger: 'blur' },
        ],
    }
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'PcmXygzzsqPickupGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'PcmXygzzsqPickupGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        _datas.forEach((record: any, index: number) => {
            let srfmajortext = record.pcmxygzzsqname;
            if (index < 5) {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ pcmxygzzsq: _keys }),Object.assign({ pcmxygzzsq: _keys },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    public rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.pcmxygzzsq,$event.pcmxygzzsq);
        });
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('pcmxygzzsq_main_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('pcmxygzzsq_main_grid', JSON.stringify(this.allColumns));
    }
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: '错误', desc: 'PcmXygzzsqPickupGridView视图表格createAction参数未配置' });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: '错误', desc: 'PcmXygzzsqPickupGridView视图表格updateAction参数未配置' });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
                        if(item.pcmxygzzsq){
                            Object.assign(this.context,{pcmxygzzsq:item.pcmxygzzsq});
                        }
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'PcmXygzzsqPickupGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
    ...
    public getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.pcmxygzzsq,args.row.pcmxygzzsq);
        });
