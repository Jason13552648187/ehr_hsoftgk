...
<div class='grid' style="height:100%;">
    ...
    <el-table v-if="isDisplay === true"
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('pcmxygzzkhjgjlname')">
                <el-table-column show-overflow-tooltip :prop="'pcmxygzzkhjgjlname'" :label="$t('entities.pcmxygzzkhjgjl.main_grid.columns.pcmxygzzkhjgjlname')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pcmxygzzkhjgjl.main_grid.columns.pcmxygzzkhjgjlname')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.pcmxygzzkhjgjlname}}</span>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('ygbh')">
                <el-table-column show-overflow-tooltip :prop="'ygbh'" :label="$t('entities.pcmxygzzkhjgjl.main_grid.columns.ygbh')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pcmxygzzkhjgjl.main_grid.columns.ygbh')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.ygbh}}</span>
            ...
            <template v-if="getColumnState('qssj')">
                <el-table-column show-overflow-tooltip :prop="'qssj'" :label="$t('entities.pcmxygzzkhjgjl.main_grid.columns.qssj')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pcmxygzzkhjgjl.main_grid.columns.qssj')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <app-format-data format="YYYY-MM-DD hh:mm:ss" :data="row.qssj"></app-format-data>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('jssj')">
                <el-table-column show-overflow-tooltip :prop="'jssj'" :label="$t('entities.pcmxygzzkhjgjl.main_grid.columns.jssj')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pcmxygzzkhjgjl.main_grid.columns.jssj')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <app-format-data format="YYYY-MM-DD hh:mm:ss" :data="row.jssj"></app-format-data>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('fs')">
                <el-table-column show-overflow-tooltip :prop="'fs'" :label="$t('entities.pcmxygzzkhjgjl.main_grid.columns.fs')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pcmxygzzkhjgjl.main_grid.columns.fs')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('pj')">
                <el-table-column show-overflow-tooltip :prop="'pj'" :label="$t('entities.pcmxygzzkhjgjl.main_grid.columns.pj')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pcmxygzzkhjgjl.main_grid.columns.pj')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
...
import PcmXygzzkhjgjlService from '@/service/pcm-xygzzkhjgjl/pcm-xygzzkhjgjl-service';
...
export default class MainBase extends Vue implements ControlInterface {
     ...
     * 实体服务对象
     *
     * @type {PcmXygzzkhjgjlService}
     * @memberof MainBase
     */
    public appEntityService: PcmXygzzkhjgjlService = new PcmXygzzkhjgjlService({ $store: this.$store });
    ...
    */
    ...
    public allColumns: any[] = [
        {
            ...
            label: '员工',
            langtag: 'entities.pcmxygzzkhjgjl.main_grid.columns.pcmxygzzkhjgjlname',
            show: true,
            util: 'PX'
        },
        {
            name: 'ygbh',
            label: '员工编号',
            langtag: 'entities.pcmxygzzkhjgjl.main_grid.columns.ygbh',
            show: true,
            util: 'px'
        },
        {
            name: 'qssj',
            label: '起始时间',
            langtag: 'entities.pcmxygzzkhjgjl.main_grid.columns.qssj',
            show: true,
            util: 'px'
        },
        {
            name: 'jssj',
            label: '结束时间',
            langtag: 'entities.pcmxygzzkhjgjl.main_grid.columns.jssj',
            show: true,
            util: 'px'
        },
        {
            name: 'fs',
            label: '分数',
            langtag: 'entities.pcmxygzzkhjgjl.main_grid.columns.fs',
            show: true,
            util: 'px'
        },
        {
            name: 'pj',
            label: '评价',
            langtag: 'entities.pcmxygzzkhjgjl.main_grid.columns.pj',
            show: true,
            util: 'PX'
        },
    ]
    ...
    public rules: any = {
        srfkey: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '新员工转正考核结果记录标识 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '新员工转正考核结果记录标识 值不能为空', trigger: 'blur' },
        ],
    }
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'PcmXygzzkhjgjlPickupGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'PcmXygzzkhjgjlPickupGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        _datas.forEach((record: any, index: number) => {
            let srfmajortext = record.pcmxygzzkhjgjlname;
            if (index < 5) {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ pcmxygzzkhjgjl: _keys }),Object.assign({ pcmxygzzkhjgjl: _keys },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    public rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.pcmxygzzkhjgjl,$event.pcmxygzzkhjgjl);
        });
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('pcmxygzzkhjgjl_main_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('pcmxygzzkhjgjl_main_grid', JSON.stringify(this.allColumns));
    }
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: '错误', desc: 'PcmXygzzkhjgjlPickupGridView视图表格createAction参数未配置' });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: '错误', desc: 'PcmXygzzkhjgjlPickupGridView视图表格updateAction参数未配置' });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
                        if(item.pcmxygzzkhjgjl){
                            Object.assign(this.context,{pcmxygzzkhjgjl:item.pcmxygzzkhjgjl});
                        }
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'PcmXygzzkhjgjlPickupGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
    ...
    public getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.pcmxygzzkhjgjl,args.row.pcmxygzzkhjgjl);
        });
