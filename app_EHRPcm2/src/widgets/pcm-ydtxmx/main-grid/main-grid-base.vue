    ...
    <el-table v-if="isDisplay === true"
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('ygbh')">
                <el-table-column show-overflow-tooltip :prop="'ygbh'" :label="$t('entities.pcmydtxmx.main_grid.columns.ygbh')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pcmydtxmx.main_grid.columns.ygbh')}}
                      </span>
                    </template>
            ...
            <template v-if="getColumnState('pimpersonname')">
                <el-table-column show-overflow-tooltip :prop="'pimpersonname'" :label="$t('entities.pcmydtxmx.main_grid.columns.pimpersonname')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pcmydtxmx.main_grid.columns.pimpersonname')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('zz')">
                <el-table-column show-overflow-tooltip :prop="'zz'" :label="$t('entities.pcmydtxmx.main_grid.columns.zz')" :width="280"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pcmydtxmx.main_grid.columns.zz')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.zz}}</span>
            ...
            <template v-if="getColumnState('bm')">
                <el-table-column show-overflow-tooltip :prop="'bm'" :label="$t('entities.pcmydtxmx.main_grid.columns.bm')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pcmydtxmx.main_grid.columns.bm')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.bm}}</span>
            ...
            <template v-if="getColumnState('yzw')">
                <el-table-column show-overflow-tooltip :prop="'yzw'" :label="$t('entities.pcmydtxmx.main_grid.columns.yzw')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pcmydtxmx.main_grid.columns.yzw')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
            ...
            <template v-if="getColumnState('ygw')">
                <el-table-column show-overflow-tooltip :prop="'ygw'" :label="$t('entities.pcmydtxmx.main_grid.columns.ygw')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pcmydtxmx.main_grid.columns.ygw')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
            ...
            <template v-if="getColumnState('lxdh')">
                <el-table-column show-overflow-tooltip :prop="'lxdh'" :label="$t('entities.pcmydtxmx.main_grid.columns.lxdh')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pcmydtxmx.main_grid.columns.lxdh')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.lxdh}}</span>
            ...
            <template v-if="getColumnState('csrq')">
                <el-table-column show-overflow-tooltip :prop="'csrq'" :label="$t('entities.pcmydtxmx.main_grid.columns.csrq')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pcmydtxmx.main_grid.columns.csrq')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.csrq}}</span>
            ...
            <template v-if="getColumnState('sxrq')">
                <el-table-column show-overflow-tooltip :prop="'sxrq'" :label="$t('entities.pcmydtxmx.main_grid.columns.sxrq')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pcmydtxmx.main_grid.columns.sxrq')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.sxrq}}</span>
...
export default class MainBase extends Vue implements ControlInterface {
    ...
    */
    ...
    public allColumns: any[] = [
        {
            name: 'ygbh',
            label: '员工编号',
            langtag: 'entities.pcmydtxmx.main_grid.columns.ygbh',
            show: true,
            util: 'px'
        },
        {
            name: 'pimpersonname',
            label: '员工姓名',
            langtag: 'entities.pcmydtxmx.main_grid.columns.pimpersonname',
            show: true,
            util: 'px'
        },
        {
            name: 'zz',
            label: '所属组织',
            langtag: 'entities.pcmydtxmx.main_grid.columns.zz',
            show: true,
            util: 'PX'
        },
        {
            name: 'bm',
            label: '所属部门',
            langtag: 'entities.pcmydtxmx.main_grid.columns.bm',
            show: true,
            util: 'PX'
        },
        {
            name: 'yzw',
            label: '所属职务',
            langtag: 'entities.pcmydtxmx.main_grid.columns.yzw',
            show: true,
            util: 'px'
        },
        {
            name: 'ygw',
            label: '所属岗位',
            langtag: 'entities.pcmydtxmx.main_grid.columns.ygw',
            show: true,
            util: 'px'
        },
        {
            name: 'lxdh',
            label: '联系电话',
            langtag: 'entities.pcmydtxmx.main_grid.columns.lxdh',
            show: true,
            util: 'px'
        },
        {
            name: 'csrq',
            label: '出生日期',
            langtag: 'entities.pcmydtxmx.main_grid.columns.csrq',
            show: true,
            util: 'px'
        },
        {
            name: 'sxrq',
            label: '退休日期',
            langtag: 'entities.pcmydtxmx.main_grid.columns.sxrq',
            show: true,
            util: 'PX'
        },
    ]
    ...
    public rules: any = {
        srfkey: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '异动退休明细标识 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '异动退休明细标识 值不能为空', trigger: 'blur' },
        ],
    }
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'PcmYdtxmxGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'PcmYdtxmxGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        _datas.forEach((record: any, index: number) => {
            let srfmajortext = record.pcmydtxmxname;
            if (index < 5) {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ pcmydtxmx: _keys }),Object.assign({ pcmydtxmx: _keys },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    public rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.pcmydtxmx,$event.pcmydtxmx);
        });
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('pcmydtxmx_main_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('pcmydtxmx_main_grid', JSON.stringify(this.allColumns));
    }
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: '错误', desc: 'PcmYdtxmxGridView视图表格createAction参数未配置' });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: '错误', desc: 'PcmYdtxmxGridView视图表格updateAction参数未配置' });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
                        if(item.pcmydtxmx){
                            Object.assign(this.context,{pcmydtxmx:item.pcmydtxmx});
                        }
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'PcmYdtxmxGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
    ...
    public getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.pcmydtxmx,args.row.pcmydtxmx);
        });
