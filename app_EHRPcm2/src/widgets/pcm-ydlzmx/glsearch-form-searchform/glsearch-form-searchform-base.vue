  ...
  <row>
    <i-col span="20" class="form-content">
      <row>
              ...
              <app-form-item name='n_ygbh_like' :itemRules="this.rules.n_ygbh_like" class='' :caption="$t('entities.pcmydlzmx.glsearchform_searchform.details.n_ygbh_like')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.n_ygbh_like.error" :isEmptyCaption="false" labelPos="RIGHT"> 
          ...
          <i-col v-show="detailsModel.n_pimpersonname_like.visible" :style="{}"  :md="{ span: 6, offset: 0 }" :lg="{ span: 6, offset: 0 }" :xl="{ span: 6, offset: 0 }">
              <app-form-item name='n_pimpersonname_like' :itemRules="this.rules.n_pimpersonname_like" class='' :caption="$t('entities.pcmydlzmx.glsearchform_searchform.details.n_pimpersonname_like')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.n_pimpersonname_like.error" :isEmptyCaption="false" labelPos="RIGHT"> 
          ...
          <i-col v-show="detailsModel.n_zz_like.visible" :style="{}"  :md="{ span: 6, offset: 0 }" :lg="{ span: 6, offset: 0 }" :xl="{ span: 6, offset: 0 }">
              <app-form-item name='n_zz_like' :itemRules="this.rules.n_zz_like" class='' :caption="$t('entities.pcmydlzmx.glsearchform_searchform.details.n_zz_like')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.n_zz_like.error" :isEmptyCaption="false" labelPos="RIGHT"> 
              <input-box v-model="data.n_zz_like"  @enter="onEnter($event)"    :disabled="detailsModel.n_zz_like.disabled" type='text'  style=""></input-box>
          </app-form-item>
          
          </i-col>
          <i-col v-show="detailsModel.n_bm_like.visible" :style="{}"  :md="{ span: 6, offset: 0 }" :lg="{ span: 6, offset: 0 }" :xl="{ span: 6, offset: 0 }">
              <app-form-item name='n_bm_like' :itemRules="this.rules.n_bm_like" class='' :caption="$t('entities.pcmydlzmx.glsearchform_searchform.details.n_bm_like')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.n_bm_like.error" :isEmptyCaption="false" labelPos="RIGHT"> 
              <input-box v-model="data.n_bm_like"  @enter="onEnter($event)"    :disabled="detailsModel.n_bm_like.disabled" type='text'  style=""></input-box>
          </app-form-item>
...
import GLSearchFormService from './glsearch-form-searchform-service';
...
export default class GLSearchFormBase extends Vue implements ControlInterface {

    /**
     * 名称
     *
     * @type {string}
     * @memberof GLSearchFormBase
     */
     ...
     * 建构部件服务对象
     *
     * @type {GLSearchFormService}
     * @memberof GLSearchFormBase
     */
    public service: GLSearchFormService = new GLSearchFormService({ $store: this.$store });
...
, 
        n_bm_like: new FormItemModel({ caption: '所属部门', detailType: 'FORMITEM', name: 'n_bm_like', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 3 })
, 
    };
     ...
     * @memberof GLSearchFormBase
     */
    @Watch('data.n_bm_like')
    onN_bm_likeChange(newVal: any, oldVal: any) {
        this.formDataChange({ name: 'n_bm_like', newVal: newVal, oldVal: oldVal });
    }
    ...
    public load(opt: any = {}): void {
        if(!this.loadAction){
            this.$Notice.error({ title: '错误', desc: 'PcmYdlzmxGLGridView视图搜索表单loadAction参数未配置' });
            return;
        }
        const arg: any = { ...opt };
    ...
    public loadDraft(opt: any = {},mode?:string): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'PcmYdlzmxGLGridView视图搜索表单loaddraftAction参数未配置' });
            return;
        }
        const arg: any = { ...opt } ;
        Object.assign(arg,{viewparams:this.viewparams});
...
<style lang='less'>
@import './glsearch-form-searchform.less';
</style>