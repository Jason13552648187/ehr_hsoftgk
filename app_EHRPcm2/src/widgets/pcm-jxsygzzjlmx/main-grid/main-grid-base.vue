    ...
    <el-table v-if="isDisplay === true"
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('zz')">
                <el-table-column show-overflow-tooltip :prop="'zz'" :label="$t('entities.pcmjxsygzzjlmx.main_grid.columns.zz')" :width="170"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
            ...
            <template v-if="getColumnState('bm')">
                <el-table-column show-overflow-tooltip :prop="'bm'" :label="$t('entities.pcmjxsygzzjlmx.main_grid.columns.bm')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pcmjxsygzzjlmx.main_grid.columns.bm')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.bm}}</span>
            ...
            <template v-if="getColumnState('duration')">
                <el-table-column show-overflow-tooltip :prop="'duration'" :label="$t('entities.pcmjxsygzzjlmx.main_grid.columns.duration')" :width="180"  :align="'left'" :sortable="'custom'">
                    ...
                    <template v-slot="{row,column,$index}">
                        <template >
            <codelist :value="row.duration" tag='EhrCodeList0187' codelistType='STATIC' ></codelist>
                        </template>
                    </template>
                </el-table-column>
            </template>
    ...
    */
    ...
    public allColumns: any[] = [
        ...
        },
        {
            name: 'pimpersonname',
            label: '员工姓名',
            langtag: 'entities.pcmjxsygzzjlmx.main_grid.columns.pimpersonname',
            show: true,
            util: 'px'
        },
        {
            name: 'zz',
            label: '组织',
            langtag: 'entities.pcmjxsygzzjlmx.main_grid.columns.zz',
            show: true,
            util: 'PX'
        },
        {
            name: 'bm',
            label: '部门',
            langtag: 'entities.pcmjxsygzzjlmx.main_grid.columns.bm',
            show: true,
            util: 'PX'
        },
        {
            name: 'zzgw',
            label: '转正岗位',
            langtag: 'entities.pcmjxsygzzjlmx.main_grid.columns.zzgw',
            show: true,
            util: 'px'
        },
        {
            name: 'duration',
            label: '见习期限（月）',
            langtag: 'entities.pcmjxsygzzjlmx.main_grid.columns.duration',
            show: true,
            util: 'PX'
        },
        {
            name: 'rzqd',
            label: '入职渠道',
            langtag: 'entities.pcmjxsygzzjlmx.main_grid.columns.rzqd',
            show: true,
            util: 'px'
        },
        {
        ...
        },
        {
            name: 'fs',
            label: '分数',
            langtag: 'entities.pcmjxsygzzjlmx.main_grid.columns.fs',
            show: true,
            util: 'px'
        },
        {
    ...
    public rules: any = {
        srfkey: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '见习生员工转正记录引用明细标识 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '见习生员工转正记录引用明细标识 值不能为空', trigger: 'blur' },
        ],
    }
    ...
    public async remove(datas: any[]): Promise<any> {
        ...
        _datas.forEach((record: any, index: number) => {
            let srfmajortext = record.pcmjxsygzzjlmxname;
            if (index < 5) {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ pcmjxsygzzjlmx: _keys }),Object.assign({ pcmjxsygzzjlmx: _keys },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    public rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.pcmjxsygzzjlmx,$event.pcmjxsygzzjlmx);
        });
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('pcmjxsygzzjlmx_main_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('pcmjxsygzzjlmx_main_grid', JSON.stringify(this.allColumns));
    }
                ...
                if(Object.is(item.rowDataState, 'create')){
                    ...
                    if(!this.updateAction){
                        ...
                        if(item.pcmjxsygzzjlmx){
                            Object.assign(this.context,{pcmjxsygzzjlmx:item.pcmjxsygzzjlmx});
                        }
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'PcmJxsygzzjlmxApproavlGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
    ...
    public getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.pcmjxsygzzjlmx,args.row.pcmjxsygzzjlmx);
        });
