    ...
    <el-table v-if="isDisplay === true"
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('ygbh')">
                <el-table-column show-overflow-tooltip :prop="'ygbh'" :label="$t('entities.pcmddsqdmx.ddeditform_grid.columns.ygbh')" :width="130"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pcmddsqdmx.ddeditform_grid.columns.ygbh')}}
                      </span>
                    </template>
            ...
            <template v-if="getColumnState('pimdistirbutionname')">
                <el-table-column show-overflow-tooltip :prop="'pimdistirbutionname'" :label="$t('entities.pcmddsqdmx.ddeditform_grid.columns.pimdistirbutionname')" :width="120"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pcmddsqdmx.ddeditform_grid.columns.pimdistirbutionname')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <app-column-link deKeyField='pimdistirbution' :context="JSON.parse(JSON.stringify(context))" :viewparams="JSON.parse(JSON.stringify(viewparams))" :data="row" :linkview="{viewname: 'pim-distirbution-redirect-view', height: 0,width: 0,title: $t('entities.pimdistirbution.views.redirectview.title'),placement: '', isRedirectView: true,deResParameters: [
            { pathName: 'pimpeople', parameterName: 'pimperson' },
            ]
            ,parameters: [
            { pathName: 'pimdistirbutions', parameterName: 'pimdistirbution' },
            { pathName: 'redirectview', parameterName: 'redirectview' }
            ]}" valueitem="pimdistirbutionid">
                            <span>{{row.pimdistirbutionname}}</span>
                        </app-column-link >
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('sxrq')">
                <el-table-column show-overflow-tooltip :prop="'sxrq'" :label="$t('entities.pcmddsqdmx.ddeditform_grid.columns.sxrq')" :width="130"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pcmddsqdmx.ddeditform_grid.columns.sxrq')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('ormorgname')">
                <el-table-column show-overflow-tooltip :prop="'ormorgname'" :label="$t('entities.pcmddsqdmx.ddeditform_grid.columns.ormorgname')" :width="200"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pcmddsqdmx.ddeditform_grid.columns.ormorgname')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('orgname')">
                <el-table-column show-overflow-tooltip :prop="'orgname'" :label="$t('entities.pcmddsqdmx.ddeditform_grid.columns.orgname')" :width="200"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pcmddsqdmx.ddeditform_grid.columns.orgname')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template v-if="actualIsOpenEdit">
            ...
            <template v-if="getColumnState('ormorgsectorname')">
                <el-table-column show-overflow-tooltip :prop="'ormorgsectorname'" :label="$t('entities.pcmddsqdmx.ddeditform_grid.columns.ormorgsectorname')" :width="120"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pcmddsqdmx.ddeditform_grid.columns.ormorgsectorname')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('orgsectorname')">
                <el-table-column show-overflow-tooltip :prop="'orgsectorname'" :label="$t('entities.pcmddsqdmx.ddeditform_grid.columns.orgsectorname')" :width="120"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pcmddsqdmx.ddeditform_grid.columns.orgsectorname')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template v-if="actualIsOpenEdit">
            ...
            <app-picker 
              ...
              @formitemvaluechange="($event)=>{onGridItemValueChange(row,$event,$index)}">
                        ...
                        <template v-if="!actualIsOpenEdit">
                                <app-span name='orgsectorname' editorType="PICKER" :value="row.orgsectorname"></app-span>
                        </template>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('ormyzw')">
                <el-table-column show-overflow-tooltip :prop="'ormyzw'" :label="$t('entities.pcmddsqdmx.ddeditform_grid.columns.ormyzw')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pcmddsqdmx.ddeditform_grid.columns.ormyzw')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.ormyzw}}</span>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('ormdutyname')">
                <el-table-column show-overflow-tooltip :prop="'ormdutyname'" :label="$t('entities.pcmddsqdmx.ddeditform_grid.columns.ormdutyname')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pcmddsqdmx.ddeditform_grid.columns.ormdutyname')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template v-if="actualIsOpenEdit">
            ...
            <template v-if="getColumnState('ormygw')">
                <el-table-column show-overflow-tooltip :prop="'ormygw'" :label="$t('entities.pcmddsqdmx.ddeditform_grid.columns.ormygw')" :width="120"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pcmddsqdmx.ddeditform_grid.columns.ormygw')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.ormygw}}</span>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('ormpostname')">
                <el-table-column show-overflow-tooltip :prop="'ormpostname'" :label="$t('entities.pcmddsqdmx.ddeditform_grid.columns.ormpostname')" :width="120"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pcmddsqdmx.ddeditform_grid.columns.ormpostname')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template v-if="actualIsOpenEdit">
...
import DDEditFormService from './ddedit-form-grid-service';
...
export default class DDEditFormBase extends Vue implements ControlInterface {

    /**
     * 名称
     *
     * @type {string}
     * @memberof DDEditFormBase
     */
     ...
     * 建构部件服务对象
     *
     * @type {DDEditFormService}
     * @memberof DDEditFormBase
     */
    public service: DDEditFormService = new DDEditFormService({ $store: this.$store });
    ...
    */
    ...
    public allColumns: any[] = [
        {
            name: 'ygbh',
            label: '员工编号',
            langtag: 'entities.pcmddsqdmx.ddeditform_grid.columns.ygbh',
            show: true,
            util: 'PX'
        },
        {
            ...
            label: '员工姓名',
            langtag: 'entities.pcmddsqdmx.ddeditform_grid.columns.pimdistirbutionname',
            show: true,
            util: 'PX'
        },
        {
            name: 'sxrq',
            label: '生效日期',
            langtag: 'entities.pcmddsqdmx.ddeditform_grid.columns.sxrq',
            show: true,
            util: 'PX'
        },
        {
            name: 'ormorgname',
            label: '原组织',
            langtag: 'entities.pcmddsqdmx.ddeditform_grid.columns.ormorgname',
            show: true,
            util: 'PX'
        },
        {
            name: 'orgname',
            label: '新组织',
            langtag: 'entities.pcmddsqdmx.ddeditform_grid.columns.orgname',
            show: true,
            util: 'PX'
        },
        {
            ...
            label: '原部门/项目',
            langtag: 'entities.pcmddsqdmx.ddeditform_grid.columns.ormorgsectorname',
            show: true,
            util: 'PX'
        },
        {
            name: 'orgsectorname',
            label: '新部门/项目',
            langtag: 'entities.pcmddsqdmx.ddeditform_grid.columns.orgsectorname',
            show: true,
            util: 'PX'
        },
        {
            name: 'ormyzw',
            label: '原职务',
            langtag: 'entities.pcmddsqdmx.ddeditform_grid.columns.ormyzw',
            show: true,
            util: 'px'
        },
        {
            name: 'ormdutyname',
            label: '新职务',
            langtag: 'entities.pcmddsqdmx.ddeditform_grid.columns.ormdutyname',
            show: true,
            util: 'PX'
        },
        {
            name: 'ormygw',
            label: '原岗位',
            langtag: 'entities.pcmddsqdmx.ddeditform_grid.columns.ormygw',
            show: true,
            util: 'PX'
        },
        {
            name: 'ormpostname',
            label: '新岗位',
            langtag: 'entities.pcmddsqdmx.ddeditform_grid.columns.ormpostname',
            show: true,
            util: 'PX'
        },
    ]
    ...
    public getGridRowModel(){
        return {
          orgsectorid: new FormItemModel(),
          ormdutyid: new FormItemModel(),
          orgid: new FormItemModel(),
          ormpostid: new FormItemModel(),
          ormpostname: new FormItemModel(),
          orgname: new FormItemModel(),
          orgsectorname: new FormItemModel(),
    ...
    public rules: any = {
        ...
        ormpostname: [
             { required: true, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '新岗位 值不能为空', trigger: 'change' },
            { required: true, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '新岗位 值不能为空', trigger: 'blur' },
        ],
        orgname: [
             { required: true, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '新组织 值不能为空', trigger: 'change' },
            { required: true, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '新组织 值不能为空', trigger: 'blur' },
        ],
        orgsectorname: [
             { required: true, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '新部门/项目 值不能为空', trigger: 'change' },
            { required: true, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '新部门/项目 值不能为空', trigger: 'blur' },
        ],
        srfkey: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '调动申请单明细标识 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '调动申请单明细标识 值不能为空', trigger: 'blur' },
        ],
        ormdutyname: [
             { required: true, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '新职务 值不能为空', trigger: 'change' },
            { required: true, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '新职务 值不能为空', trigger: 'blur' },
        ],
        sxrq: [
             { required: true, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '生效日期 值不能为空', trigger: 'change' },
            { required: true, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '生效日期 值不能为空', trigger: 'blur' },
        ],
    }
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'PcmDdsqdmxPDDGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'PcmDdsqdmxPDDGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        _datas.forEach((record: any, index: number) => {
            let srfmajortext = record.pcmddsqdmxname;
            if (index < 5) {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ pcmddsqdmx: _keys }),Object.assign({ pcmddsqdmx: _keys },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    public rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.pcmddsqdmx,$event.pcmddsqdmx);
        });
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('pcmddsqdmx_ddeditform_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('pcmddsqdmx_ddeditform_grid', JSON.stringify(this.allColumns));
    }
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: '错误', desc: 'PcmDdsqdmxPDDGridView视图表格createAction参数未配置' });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: '错误', desc: 'PcmDdsqdmxPDDGridView视图表格updateAction参数未配置' });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
                        if(item.pcmddsqdmx){
                            Object.assign(this.context,{pcmddsqdmx:item.pcmddsqdmx});
                        }
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'PcmDdsqdmxPDDGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
