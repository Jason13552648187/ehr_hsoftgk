  ...
  <row>
    <i-col span="20" class="form-content">
      <row>
              ...
              <app-form-item name='n_ygbh_like' :itemRules="this.rules.n_ygbh_like" class='' :caption="$t('entities.pcmddsqdmx.default_searchform.details.n_ygbh_like')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.n_ygbh_like.error" :isEmptyCaption="false" labelPos="RIGHT"> 
          ...
          <i-col v-show="detailsModel.n_pimdistirbutionname_like.visible" :style="{}"  :sm="{ span: 6, offset: 0 }" :md="{ span: 6, offset: 0 }" :lg="{ span: 6, offset: 0 }" :xl="{ span: 6, offset: 0 }">
              <app-form-item name='n_pimdistirbutionname_like' :itemRules="this.rules.n_pimdistirbutionname_like" class='' :caption="$t('entities.pcmddsqdmx.default_searchform.details.n_pimdistirbutionname_like')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.n_pimdistirbutionname_like.error" :isEmptyCaption="false" labelPos="RIGHT"> 
              <input-box v-model="data.n_pimdistirbutionname_like"  @enter="onEnter($event)"    :disabled="detailsModel.n_pimdistirbutionname_like.disabled" type='text'  style=""></input-box>
          </app-form-item>
...
export default class DefaultBase extends Vue implements ControlInterface {
    ...
    public data: any = {
        n_ygbh_like: null,
        n_pimdistirbutionname_like: null,
...
, 
        n_pimdistirbutionname_like: new FormItemModel({ caption: '员工姓名(%)', detailType: 'FORMITEM', name: 'n_pimdistirbutionname_like', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 3 })
, 
...
, 
    ...
    /**
     * 监控表单属性 n_pimdistirbutionname_like 值
     *
     * @param {*} newVal
     * @param {*} oldVal
     * @memberof DefaultBase
     */
    @Watch('data.n_pimdistirbutionname_like')
    onN_pimdistirbutionname_likeChange(newVal: any, oldVal: any) {
        this.formDataChange({ name: 'n_pimdistirbutionname_like', newVal: newVal, oldVal: oldVal });
    }
    ...
    public load(opt: any = {}): void {
        if(!this.loadAction){
            this.$Notice.error({ title: '错误', desc: 'PcmDdsqdmxPickupGridView视图搜索表单loadAction参数未配置' });
            return;
        }
        const arg: any = { ...opt };
    ...
    public loadDraft(opt: any = {},mode?:string): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'PcmDdsqdmxPickupGridView视图搜索表单loaddraftAction参数未配置' });
            return;
        }
        const arg: any = { ...opt } ;
        Object.assign(arg,{viewparams:this.viewparams});
