    ...
    <el-table v-if="isDisplay === true"
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('zyzgzy')">
                <el-table-column show-overflow-tooltip :prop="'zyzgzy'" :label="$t('entities.pcmcertofreg.main_grid.columns.zyzgzy')" :width="250"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pcmcertofreg.main_grid.columns.zyzgzy')}}
                      </span>
                    </template>
            ...
            <app-picker 
              ...
              name='zyzgzy'
              deMajorField='zyzgzy'
              deKeyField='pimqualmajor'
              :service="service"
              :acParams="{ serviceName: 'PimQualMajorService', interfaceName: 'FetchDefault'}"
              valueitem='pimqualmajorid' 
              :value="row[column.property]" 
              editortype="" 
              :pickupView="{ viewname: 'pimqualmajorpickup-view', title: $t('entities.pimqualmajor.views.pickupview.title'), deResParameters: [], parameters: [{ pathName: 'pimqualmajors', parameterName: 'pimqualmajor' }, { pathName: 'pickupview', parameterName: 'pickupview' } ], placement:'' }"
              style=""  
              @formitemvaluechange="($event)=>{onGridItemValueChange(row,$event,$index)}">
                        ...
                        <template v-if="!actualIsOpenEdit">
                                <app-span name='zyzgzy' editorType="PICKER" :value="row.zyzgzy"></app-span>
                        </template>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('zsfj')">
                <el-table-column show-overflow-tooltip :prop="'zsfj'" :label="$t('entities.pcmcertofreg.main_grid.columns.zsfj')" :width="200"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pcmcertofreg.main_grid.columns.zsfj')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template v-if="actualIsOpenEdit">
              ...
              @change="($event)=>{gridEditItemChange(row, column.property, $event, $index)}">
                        ...
                        <template v-if="!actualIsOpenEdit">
                                <app-span name='zsfj' editorType="TEXTBOX" :value="row.zsfj"></app-span>
                        </template>
                    </template>
                </el-table-column>
            </template>
...
export default class MainBase extends Vue implements ControlInterface {
     ...
     * @memberof MainBase
     */
    public appEntityService: PcmCertofregService = new PcmCertofregService({ $store: this.$store });
    ...
    */
    ...
    public allColumns: any[] = [
        {
            name: 'zyzgzy',
            label: '证书名称（*）',
            langtag: 'entities.pcmcertofreg.main_grid.columns.zyzgzy',
            show: true,
            util: 'PX'
        },
        {
            name: 'zsfj',
            label: '证书附件',
            langtag: 'entities.pcmcertofreg.main_grid.columns.zsfj',
            show: true,
            util: 'PX'
        },
    ]
    ...
    public getGridRowModel(){
        return {
          zyzgzy: new FormItemModel(),
          pimqualmajorid: new FormItemModel(),
          zsfj: new FormItemModel(),
    ...
    public rules: any = {
        zyzgzy: [
             { required: true, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '证书名称（*） 值不能为空', trigger: 'change' },
            { required: true, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '证书名称（*） 值不能为空', trigger: 'blur' },
        ],
        pimqualmajorid: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '执业资格专业编码 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '执业资格专业编码 值不能为空', trigger: 'blur' },
        ],
        zsfj: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '证书附件 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '证书附件 值不能为空', trigger: 'blur' },
        ],
        pcmprofileid: [
        ...
        srfkey: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '注册证书标识 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '注册证书标识 值不能为空', trigger: 'blur' },
        ],
    }
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'PCMCERTOFREGZCZSGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'PCMCERTOFREGZCZSGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        _datas.forEach((record: any, index: number) => {
            let srfmajortext = record.pcmcertofregname;
            if (index < 5) {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ pcmcertofreg: _keys }),Object.assign({ pcmcertofreg: _keys },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    public rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.pcmcertofreg,$event.pcmcertofreg);
        });
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('pcmcertofreg_main_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('pcmcertofreg_main_grid', JSON.stringify(this.allColumns));
    }
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: '错误', desc: 'PCMCERTOFREGZCZSGridView视图表格createAction参数未配置' });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: '错误', desc: 'PCMCERTOFREGZCZSGridView视图表格updateAction参数未配置' });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
                        if(item.pcmcertofreg){
                            Object.assign(this.context,{pcmcertofreg:item.pcmcertofreg});
                        }
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'PCMCERTOFREGZCZSGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
