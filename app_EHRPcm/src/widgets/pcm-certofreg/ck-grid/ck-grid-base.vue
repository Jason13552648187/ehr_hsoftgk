    ...
    <el-table v-if="isDisplay === true"
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('zyzgzy')">
                <el-table-column show-overflow-tooltip :prop="'zyzgzy'" :label="$t('entities.pcmcertofreg.ck_grid.columns.zyzgzy')" :width="250"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pcmcertofreg.ck_grid.columns.zyzgzy')}}
                      </span>
                    </template>
            ...
            <template v-if="getColumnState('zsfj')">
                <el-table-column show-overflow-tooltip :prop="'zsfj'" :label="$t('entities.pcmcertofreg.ck_grid.columns.zsfj')" :width="200"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pcmcertofreg.ck_grid.columns.zsfj')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
...
export default class CKBase extends Vue implements ControlInterface {
    ...
    */
    ...
    public allColumns: any[] = [
        {
            name: 'zyzgzy',
            label: '证书名称（*）',
            langtag: 'entities.pcmcertofreg.ck_grid.columns.zyzgzy',
            show: true,
            util: 'PX'
        },
        {
            name: 'zsfj',
            label: '证书附件',
            langtag: 'entities.pcmcertofreg.ck_grid.columns.zsfj',
            show: true,
            util: 'PX'
        },
    ]
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'PCMCERTOFREGZCZS_WSSWBJGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'PCMCERTOFREGZCZS_WSSWBJGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('pcmcertofreg_ck_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('pcmcertofreg_ck_grid', JSON.stringify(this.allColumns));
    }
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: '错误', desc: 'PCMCERTOFREGZCZS_WSSWBJGridView视图表格createAction参数未配置' });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: '错误', desc: 'PCMCERTOFREGZCZS_WSSWBJGridView视图表格updateAction参数未配置' });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'PCMCERTOFREGZCZS_WSSWBJGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
