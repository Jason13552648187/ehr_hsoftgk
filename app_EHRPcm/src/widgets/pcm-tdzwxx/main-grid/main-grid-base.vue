    ...
    <el-table v-if="isDisplay === true"
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('tdzwxxname')">
                <el-table-column show-overflow-tooltip :prop="'tdzwxxname'" :label="$t('entities.pcmtdzwxx.main_grid.columns.tdzwxxname')" :width="180"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pcmtdzwxx.main_grid.columns.tdzwxxname')}}
                      </span>
                    </template>
              ...
              @change="($event)=>{gridEditItemChange(row, column.property, $event, $index)}">
                        ...
                        <template v-if="!actualIsOpenEdit">
                                <app-span name='tdzwxxname' editorType="TEXTBOX" :value="row.tdzwxxname"></app-span>
                        </template>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('initapplydate')">
                <el-table-column show-overflow-tooltip :prop="'initapplydate'" :label="$t('entities.pcmtdzwxx.main_grid.columns.initapplydate')" :width="200"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pcmtdzwxx.main_grid.columns.initapplydate')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template v-if="actualIsOpenEdit">
                        ...
                        <template v-if="!actualIsOpenEdit">
                                <app-span name='initapplydate' editorType="DATEPICKER" :value="row.initapplydate"></app-span>
                        </template>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('department')">
                <el-table-column show-overflow-tooltip :prop="'department'" :label="$t('entities.pcmtdzwxx.main_grid.columns.department')" :width="200"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pcmtdzwxx.main_grid.columns.department')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template v-if="actualIsOpenEdit">
              ...
              @change="($event)=>{gridEditItemChange(row, column.property, $event, $index)}">
                        ...
                        <template v-if="!actualIsOpenEdit">
                                <app-span name='department' editorType="TEXTBOX" :value="row.department"></app-span>
                        </template>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('phaseinfo')">
                <el-table-column show-overflow-tooltip :prop="'phaseinfo'" :label="$t('entities.pcmtdzwxx.main_grid.columns.phaseinfo')" :width="200"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pcmtdzwxx.main_grid.columns.phaseinfo')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template v-if="actualIsOpenEdit">
              ...
              @change="($event)=>{gridEditItemChange(row, column.property, $event, $index)}">
                        ...
                        <template v-if="!actualIsOpenEdit">
                                <app-span name='phaseinfo' editorType="TEXTBOX" :value="row.phaseinfo"></app-span>
                        </template>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('statusinfo')">
                <el-table-column show-overflow-tooltip :prop="'statusinfo'" :label="$t('entities.pcmtdzwxx.main_grid.columns.statusinfo')" :width="200"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pcmtdzwxx.main_grid.columns.statusinfo')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template v-if="actualIsOpenEdit">
              ...
              @change="($event)=>{gridEditItemChange(row, column.property, $event, $index)}">
                        ...
                        <template v-if="!actualIsOpenEdit">
                                <app-span name='statusinfo' editorType="TEXTBOX" :value="row.statusinfo"></app-span>
                        </template>
                    </template>
                </el-table-column>
            </template>
...
export default class MainBase extends Vue implements ControlInterface {
     ...
     * @memberof MainBase
     */
    public appEntityService: PcmTdzwxxService = new PcmTdzwxxService({ $store: this.$store });
     ...
     * @memberof MainBase
     */
    public minorSortPSDEF: string = 'initapplydate';
    ...
    */
    ...
    public allColumns: any[] = [
        {
            name: 'tdzwxxname',
            label: '职位名称（*）',
            langtag: 'entities.pcmtdzwxx.main_grid.columns.tdzwxxname',
            show: true,
            util: 'PX'
        },
        {
            name: 'initapplydate',
            label: '最初投递时间（*）',
            langtag: 'entities.pcmtdzwxx.main_grid.columns.initapplydate',
            show: true,
            util: 'PX'
        },
        {
            name: 'department',
            label: '投递组织（*）',
            langtag: 'entities.pcmtdzwxx.main_grid.columns.department',
            show: true,
            util: 'PX'
        },
        {
            name: 'phaseinfo',
            label: '所在阶段信息（*）',
            langtag: 'entities.pcmtdzwxx.main_grid.columns.phaseinfo',
            show: true,
            util: 'PX'
        },
        {
            name: 'statusinfo',
            label: '所在状态信息（*）',
            langtag: 'entities.pcmtdzwxx.main_grid.columns.statusinfo',
            show: true,
            util: 'PX'
        },
    ]
    ...
    public getGridRowModel(){
        return {
          phaseinfo: new FormItemModel(),
          initapplydate: new FormItemModel(),
          tdzwxxname: new FormItemModel(),
          department: new FormItemModel(),
          statusinfo: new FormItemModel(),
          pcmprofileid: new FormItemModel(),
          srfkey: new FormItemModel(),
          jobid: new FormItemModel(),
        }
    }
    ...
    public rules: any = {
        phaseinfo: [
             { required: true, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '所在阶段信息（*） 值不能为空', trigger: 'change' },
            { required: true, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '所在阶段信息（*） 值不能为空', trigger: 'blur' },
        ],
        initapplydate: [
             { required: true, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '最初投递时间（*） 值不能为空', trigger: 'change' },
            { required: true, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '最初投递时间（*） 值不能为空', trigger: 'blur' },
        ],
        tdzwxxname: [
             { required: true, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '职位名称（*） 值不能为空', trigger: 'change' },
            { required: true, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '职位名称（*） 值不能为空', trigger: 'blur' },
        ],
        department: [
             { required: true, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '投递组织（*） 值不能为空', trigger: 'change' },
            { required: true, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '投递组织（*） 值不能为空', trigger: 'blur' },
        ],
        statusinfo: [
             { required: true, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '所在状态信息（*） 值不能为空', trigger: 'change' },
            { required: true, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '所在状态信息（*） 值不能为空', trigger: 'blur' },
        ],
        pcmprofileid: [
        ...
        srfkey: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '投递职位摘要信息标识 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '投递职位摘要信息标识 值不能为空', trigger: 'blur' },
        ],
        jobid: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '职位ID 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '职位ID 值不能为空', trigger: 'blur' },
        ],
    }
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'TDZWXXTDZWGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'TDZWXXTDZWGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        _datas.forEach((record: any, index: number) => {
            let srfmajortext = record.tdzwxxname;
            if (index < 5) {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ pcmtdzwxx: _keys }),Object.assign({ pcmtdzwxx: _keys },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    public rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.tdzwxx,$event.tdzwxx);
        });
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('tdzwxx_main_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('tdzwxx_main_grid', JSON.stringify(this.allColumns));
    }
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: '错误', desc: 'TDZWXXTDZWGridView视图表格createAction参数未配置' });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: '错误', desc: 'TDZWXXTDZWGridView视图表格updateAction参数未配置' });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
                        if(item.pcmtdzwxx){
                            Object.assign(this.context,{pcmtdzwxx:item.pcmtdzwxx});
                        }
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'TDZWXXTDZWGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
    ...
    public getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.pcmtdzwxx,args.row.pcmtdzwxx);
        });
