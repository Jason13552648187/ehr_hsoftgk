...
<div class='grid' style="height:100%;">
    ...
    <el-table v-if="isDisplay === true"
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('cadrename')">
                <el-table-column show-overflow-tooltip :prop="'cadrename'" :label="$t('entities.pcmschooloffice.main_grid.columns.cadrename')" :width="250"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pcmschooloffice.main_grid.columns.cadrename')}}
                      </span>
                    </template>
              ...
              @change="($event)=>{gridEditItemChange(row, column.property, $event, $index)}">
                        ...
                        <template v-if="!actualIsOpenEdit">
                                <app-span name='cadrename' editorType="TEXTBOX" :value="row.cadrename"></app-span>
                        </template>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('cadrestartdatetime')">
                <el-table-column show-overflow-tooltip :prop="'cadrestartdatetime'" :label="$t('entities.pcmschooloffice.main_grid.columns.cadrestartdatetime')" :width="180"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pcmschooloffice.main_grid.columns.cadrestartdatetime')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template v-if="actualIsOpenEdit">
                        ...
                        <template v-if="!actualIsOpenEdit">
                                <app-span name='cadrestartdatetime' editorType="DATEPICKER" :value="row.cadrestartdatetime"></app-span>
                        </template>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('cadreenddatetime')">
                <el-table-column show-overflow-tooltip :prop="'cadreenddatetime'" :label="$t('entities.pcmschooloffice.main_grid.columns.cadreenddatetime')" :width="180"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pcmschooloffice.main_grid.columns.cadreenddatetime')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template v-if="actualIsOpenEdit">
                        ...
                        <template v-if="!actualIsOpenEdit">
                                <app-span name='cadreenddatetime' editorType="DATEPICKER" :value="row.cadreenddatetime"></app-span>
                        </template>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('cadredescription')">
                <el-table-column show-overflow-tooltip :prop="'cadredescription'" :label="$t('entities.pcmschooloffice.main_grid.columns.cadredescription')" :min-width="1"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pcmschooloffice.main_grid.columns.cadredescription')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template v-if="actualIsOpenEdit">
              ...
              @change="($event)=>{gridEditItemChange(row, column.property, $event, $index)}">
                        ...
                        <template v-if="!actualIsOpenEdit">
                                <app-span name='cadredescription' editorType="TEXTBOX" :value="row.cadredescription"></app-span>
                        </template>
                    </template>
                </el-table-column>
            </template>
...
export default class MainBase extends Vue implements ControlInterface {
     ...
     * @memberof MainBase
     */
    public appEntityService: PcmSchoolOfficeService = new PcmSchoolOfficeService({ $store: this.$store });
    ...
    */
    ...
    public allColumns: any[] = [
        {
            name: 'cadrename',
            label: '职务名称（*）',
            langtag: 'entities.pcmschooloffice.main_grid.columns.cadrename',
            show: true,
            util: 'PX'
        },
        {
            ...
            label: '开始时间（*）',
            langtag: 'entities.pcmschooloffice.main_grid.columns.cadrestartdatetime',
            show: true,
            util: 'PX'
        },
        {
            ...
            label: '结束时间（*）',
            langtag: 'entities.pcmschooloffice.main_grid.columns.cadreenddatetime',
            show: true,
            util: 'PX'
        },
        {
            ...
            label: '在校职务描述',
            langtag: 'entities.pcmschooloffice.main_grid.columns.cadredescription',
            show: true,
            util: 'STAR'
        },
    ]
    ...
    public getGridRowModel(){
        return {
          cadrestartdatetime: new FormItemModel(),
          cadreenddatetime: new FormItemModel(),
          cadredescription: new FormItemModel(),
          pcmprofileid: new FormItemModel(),
          srfkey: new FormItemModel(),
          cadrename: new FormItemModel(),
        }
    }
    ...
    public rules: any = {
        ...
        cadredescription: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '在校职务描述 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '在校职务描述 值不能为空', trigger: 'blur' },
        ],
        pcmprofileid: [
        ...
        srfkey: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '在校职务标识 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '在校职务标识 值不能为空', trigger: 'blur' },
        ],
        cadrename: [
             { required: true, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '职务名称（*） 值不能为空', trigger: 'change' },
            { required: true, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '职务名称（*） 值不能为空', trigger: 'blur' },
        ],
    }
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'PCMSCHOOLOFFICEZXZW_WSSWBJGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'PCMSCHOOLOFFICEZXZW_WSSWBJGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        _datas.forEach((record: any, index: number) => {
            let srfmajortext = record.pcmschoolofficename;
            if (index < 5) {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ pcmschooloffice: _keys }),Object.assign({ pcmschooloffice: _keys },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    public rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.pcmschooloffice,$event.pcmschooloffice);
        });
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('pcmschooloffice_main_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('pcmschooloffice_main_grid', JSON.stringify(this.allColumns));
    }
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: '错误', desc: 'PCMSCHOOLOFFICEZXZW_WSSWBJGridView视图表格createAction参数未配置' });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: '错误', desc: 'PCMSCHOOLOFFICEZXZW_WSSWBJGridView视图表格updateAction参数未配置' });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
                        if(item.pcmschooloffice){
                            Object.assign(this.context,{pcmschooloffice:item.pcmschooloffice});
                        }
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'PCMSCHOOLOFFICEZXZW_WSSWBJGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
    ...
    public getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.pcmschooloffice,args.row.pcmschooloffice);
        });
    ...
    public getCellClassName(args:{row: any, column: any, rowIndex: number, columnIndex:number}){
        let hasRowEdit:any = {
          'cadrename':true,
          'cadrestartdatetime':true,
          'cadreenddatetime':true,
          'cadredescription':true,
        }
