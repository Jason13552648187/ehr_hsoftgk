    ...
    <el-table v-if="isDisplay === true"
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('xh')">
                <el-table-column show-overflow-tooltip :prop="'xh'" :label="$t('entities.pcmxkml.main_grid.columns.xh')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pcmxkml.main_grid.columns.xh')}}
                      </span>
                    </template>
            ...
            <template v-if="getColumnState('nd')">
                <el-table-column show-overflow-tooltip :prop="'nd'" :label="$t('entities.pcmxkml.main_grid.columns.nd')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pcmxkml.main_grid.columns.nd')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template v-if="actualIsOpenEdit">
            ...
            <template v-if="getColumnState('xkml')">
                <el-table-column show-overflow-tooltip :prop="'xkml'" :label="$t('entities.pcmxkml.main_grid.columns.xkml')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pcmxkml.main_grid.columns.xkml')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template v-if="actualIsOpenEdit">
             ...
             <dropdown-list 
              ...
              @change="($event)=>{gridEditItemChange(row, column.property, $event, $index)}">
                        ...
                        <template v-if="!actualIsOpenEdit">
            <codelist :value="row.xkml" tag='EhrCodeList0116' codelistType='DYNAMIC' ></codelist>
                        </template>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('pcmxkmlname')">
                <el-table-column show-overflow-tooltip :prop="'pcmxkmlname'" :label="$t('entities.pcmxkml.main_grid.columns.pcmxkmlname')" :width="200"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pcmxkml.main_grid.columns.pcmxkmlname')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template v-if="actualIsOpenEdit">
              ...
              @change="($event)=>{gridEditItemChange(row, column.property, $event, $index)}">
                        ...
                        <template v-if="!actualIsOpenEdit">
                                <app-span name='pcmxkmlname' editorType="TEXTBOX" :value="row.pcmxkmlname"></app-span>
                        </template>
                    </template>
                </el-table-column>
            </template>
...
export default class MainBase extends Vue implements ControlInterface {
     ...
     * @memberof MainBase
     */
    public appEntityService: PcmXkmlService = new PcmXkmlService({ $store: this.$store });
    ...
    */
    ...
    public allColumns: any[] = [
        {
            name: 'xh',
            label: '排序号',
            langtag: 'entities.pcmxkml.main_grid.columns.xh',
            show: true,
            util: 'PX'
        },
        {
            name: 'nd',
            label: '年度',
            langtag: 'entities.pcmxkml.main_grid.columns.nd',
            show: true,
            util: 'PX'
        },
        {
            name: 'xkml',
            label: '学科门类',
            langtag: 'entities.pcmxkml.main_grid.columns.xkml',
            show: true,
            util: 'PX'
        },
        {
            name: 'pcmxkmlname',
            label: '学科名称',
            langtag: 'entities.pcmxkml.main_grid.columns.pcmxkmlname',
            show: true,
            util: 'PX'
        },
    ]
    ...
    public rules: any = {
        ...
        xkml: [
             { required: true, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '学科门类 值不能为空', trigger: 'change' },
            { required: true, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '学科门类 值不能为空', trigger: 'blur' },
        ],
        nd: [
        ...
        pcmxkmlname: [
             { required: true, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '学科名称 值不能为空', trigger: 'change' },
            { required: true, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '学科名称 值不能为空', trigger: 'blur' },
        ],
        srfkey: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '学科目录标识 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '学科目录标识 值不能为空', trigger: 'blur' },
        ],
    }
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'PCMXKMLCurNDGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'PCMXKMLCurNDGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        _datas.forEach((record: any, index: number) => {
            let srfmajortext = record.pcmxkmlname;
            if (index < 5) {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ pcmxkml: _keys }),Object.assign({ pcmxkml: _keys },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    public rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.pcmxkml,$event.pcmxkml);
        });
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('pcmxkml_main_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('pcmxkml_main_grid', JSON.stringify(this.allColumns));
    }
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: '错误', desc: 'PCMXKMLCurNDGridView视图表格createAction参数未配置' });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: '错误', desc: 'PCMXKMLCurNDGridView视图表格updateAction参数未配置' });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
                        if(item.pcmxkml){
                            Object.assign(this.context,{pcmxkml:item.pcmxkml});
                        }
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'PCMXKMLCurNDGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
    ...
    public getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.pcmxkml,args.row.pcmxkml);
        });
