<template>
    <split id="xkmltreeexpviewtreeexpbar" class='app-tree-exp-bar' v-model="split" mode="horizontal" @on-move-end="onSplitChange">
        <div slot='left'>
...
import XKMLTreeExpViewtreeexpbarService from './xkmltree-exp-viewtreeexpbar-treeexpbar-service';
...
export default class XKMLTreeExpViewtreeexpbarBase extends Vue implements ControlInterface {

    /**
     * 名称
     *
     * @type {string}
     * @memberof XKMLTreeExpViewtreeexpbarBase
     */
     ...
     * 建构部件服务对象
     *
     * @type {XKMLTreeExpViewtreeexpbarService}
     * @memberof XKMLTreeExpViewtreeexpbarBase
     */
    public service: XKMLTreeExpViewtreeexpbarService = new XKMLTreeExpViewtreeexpbarService({ $store: this.$store });
    ...
    public getExpItemView(arg: any = {}): any {
        ...
        if (Object.is(expmode, 'PCMXKML')) {
            return {  
                viewname: 'pcmxkmlcur-ndgrid-view', 
     ...
     * vue 声明周期
     *
     * @memberof @memberof XKMLTreeExpViewtreeexpbarBase
     */
    public created() {
        this.afterCreated();
    }
    ...
    */
    public afterMounted(){ 
        if(this.$store.getters.getViewSplit(this.viewUID)){
            ...
            let containerWidth:number = (document.getElementById("xkmltreeexpviewtreeexpbar") as any).offsetWidth;
            if(this.ctrlWidth){
...
<style lang='less'>
@import './xkmltree-exp-viewtreeexpbar-treeexpbar.less';
</style>
