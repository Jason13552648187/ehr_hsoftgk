...
<div class='grid' style="height:100%;">
    ...
    <el-table v-if="isDisplay === true"
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('awardname')">
                <el-table-column show-overflow-tooltip :prop="'awardname'" :label="$t('entities.pcmawardswons.main_grid.columns.awardname')" :width="300"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pcmawardswons.main_grid.columns.awardname')}}
                      </span>
                    </template>
              ...
              @change="($event)=>{gridEditItemChange(row, column.property, $event, $index)}">
                        ...
                        <template v-if="!actualIsOpenEdit">
                                <app-span name='awardname' editorType="TEXTBOX" :value="row.awardname"></app-span>
                        </template>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('awardtime')">
                <el-table-column show-overflow-tooltip :prop="'awardtime'" :label="$t('entities.pcmawardswons.main_grid.columns.awardtime')" :width="180"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pcmawardswons.main_grid.columns.awardtime')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template v-if="actualIsOpenEdit">
                        ...
                        <template v-if="!actualIsOpenEdit">
                                <app-span name='awardtime' editorType="DATEPICKER" :value="row.awardtime"></app-span>
                        </template>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('awardlevel')">
                <el-table-column show-overflow-tooltip :prop="'awardlevel'" :label="$t('entities.pcmawardswons.main_grid.columns.awardlevel')" :width="250"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pcmawardswons.main_grid.columns.awardlevel')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template v-if="actualIsOpenEdit">
             ...
             <dropdown-list 
              ...
              @change="($event)=>{gridEditItemChange(row, column.property, $event, $index)}">
                        ...
                        <template v-if="!actualIsOpenEdit">
            <codelist :value="row.awardlevel" tag='EhrCodeList0152' codelistType='STATIC' ></codelist>
                        </template>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('judgeorg')">
                <el-table-column show-overflow-tooltip :prop="'judgeorg'" :label="$t('entities.pcmawardswons.main_grid.columns.judgeorg')" :width="400"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pcmawardswons.main_grid.columns.judgeorg')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template v-if="actualIsOpenEdit">
              ...
              @change="($event)=>{gridEditItemChange(row, column.property, $event, $index)}">
                        ...
                        <template v-if="!actualIsOpenEdit">
                                <app-span name='judgeorg' editorType="TEXTBOX" :value="row.judgeorg"></app-span>
                        </template>
                    </template>
                </el-table-column>
            </template>
...
export default class MainBase extends Vue implements ControlInterface {
     ...
     * @memberof MainBase
     */
    public appEntityService: PcmAwardsWonsService = new PcmAwardsWonsService({ $store: this.$store });
    ...
    */
    ...
    public allColumns: any[] = [
        {
            name: 'awardname',
            label: '获奖名称（*）',
            langtag: 'entities.pcmawardswons.main_grid.columns.awardname',
            show: true,
            util: 'PX'
        },
        {
            name: 'awardtime',
            label: '获奖时间（*）',
            langtag: 'entities.pcmawardswons.main_grid.columns.awardtime',
            show: true,
            util: 'PX'
        },
        {
            name: 'awardlevel',
            label: '获奖等级（*）',
            langtag: 'entities.pcmawardswons.main_grid.columns.awardlevel',
            show: true,
            util: 'PX'
        },
        {
            name: 'judgeorg',
            label: '评定机构',
            langtag: 'entities.pcmawardswons.main_grid.columns.judgeorg',
            show: true,
            util: 'PX'
        },
    ]
    ...
    public getGridRowModel(){
        return {
          awardname: new FormItemModel(),
          pcmprofileid: new FormItemModel(),
          srfkey: new FormItemModel(),
          judgeorg: new FormItemModel(),
          awardlevel: new FormItemModel(),
          awardtime: new FormItemModel(),
        }
    }
    ...
    public rules: any = {
        awardname: [
             { required: true, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '获奖名称（*） 值不能为空', trigger: 'change' },
            { required: true, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '获奖名称（*） 值不能为空', trigger: 'blur' },
        ],
        pcmprofileid: [
        ...
        srfkey: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '获奖情况标识 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '获奖情况标识 值不能为空', trigger: 'blur' },
        ],
        judgeorg: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '评定机构 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '评定机构 值不能为空', trigger: 'blur' },
        ],
        awardlevel: [
             { required: true, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '获奖等级（*） 值不能为空', trigger: 'change' },
            { required: true, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '获奖等级（*） 值不能为空', trigger: 'blur' },
        ],
        awardtime: [
             { required: true, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '获奖时间（*） 值不能为空', trigger: 'change' },
            { required: true, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '获奖时间（*） 值不能为空', trigger: 'blur' },
        ],
    }
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'PCMAWARDSWONSHJQKGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'PCMAWARDSWONSHJQKGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        _datas.forEach((record: any, index: number) => {
            let srfmajortext = record.pcmawardswonsname;
            if (index < 5) {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ pcmawardswons: _keys }),Object.assign({ pcmawardswons: _keys },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    public rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.pcmawardswons,$event.pcmawardswons);
        });
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('pcmawardswons_main_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('pcmawardswons_main_grid', JSON.stringify(this.allColumns));
    }
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: '错误', desc: 'PCMAWARDSWONSHJQKGridView视图表格createAction参数未配置' });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: '错误', desc: 'PCMAWARDSWONSHJQKGridView视图表格updateAction参数未配置' });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
                        if(item.pcmawardswons){
                            Object.assign(this.context,{pcmawardswons:item.pcmawardswons});
                        }
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'PCMAWARDSWONSHJQKGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
    ...
    public getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.pcmawardswons,args.row.pcmawardswons);
        });
