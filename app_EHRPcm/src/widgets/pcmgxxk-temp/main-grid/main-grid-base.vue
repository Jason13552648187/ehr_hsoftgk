    ...
    <el-table v-if="isDisplay === true"
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('nd')">
                <el-table-column show-overflow-tooltip :prop="'nd'" :label="$t('entities.pcmgxxktemp.main_grid.columns.nd')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pcmgxxktemp.main_grid.columns.nd')}}
                      </span>
                    </template>
            ...
            <template v-if="getColumnState('pcmgxmlname')">
                <el-table-column show-overflow-tooltip :prop="'pcmgxmlname'" :label="$t('entities.pcmgxxktemp.main_grid.columns.pcmgxmlname')" :width="200"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pcmgxxktemp.main_grid.columns.pcmgxmlname')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.pcmgxmlname}}</span>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('nd2')">
                <el-table-column show-overflow-tooltip :prop="'nd2'" :label="$t('entities.pcmgxxktemp.main_grid.columns.nd2')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pcmgxxktemp.main_grid.columns.nd2')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
            <codelist :value="row.nd2" tag='EhrCodeList0115' codelistType='STATIC' renderMode="STR" valueSeparator=";" textSeparator="、" ></codelist>
                        </template>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('pcmxkmlname')">
                <el-table-column show-overflow-tooltip :prop="'pcmxkmlname'" :label="$t('entities.pcmgxxktemp.main_grid.columns.pcmxkmlname')" :width="200"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pcmgxxktemp.main_grid.columns.pcmxkmlname')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.pcmxkmlname}}</span>
                    </template>
                </el-table-column>
            </template>
...
export default class MainBase extends Vue implements ControlInterface {
     ...
     * @memberof MainBase
     */
    public appEntityService: PCMGxxkTempService = new PCMGxxkTempService({ $store: this.$store });
    ...
    */
    ...
    public allColumns: any[] = [
        {
            name: 'nd',
            label: '高校名录年度',
            langtag: 'entities.pcmgxxktemp.main_grid.columns.nd',
            show: true,
            util: 'px'
        },
        {
            name: 'pcmgxmlname',
            label: '高校名称',
            langtag: 'entities.pcmgxxktemp.main_grid.columns.pcmgxmlname',
            show: true,
            util: 'PX'
        },
        {
            name: 'nd2',
            label: '学科目录年度',
            langtag: 'entities.pcmgxxktemp.main_grid.columns.nd2',
            show: true,
            util: 'px'
        },
        {
            name: 'pcmxkmlname',
            label: '学科名称',
            langtag: 'entities.pcmgxxktemp.main_grid.columns.pcmxkmlname',
            show: true,
            util: 'PX'
        },
    ]
    ...
    public rules: any = {
        srfkey: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '高校学科中间表标识 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '高校学科中间表标识 值不能为空', trigger: 'blur' },
        ],
    }
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'PCMGXXKTEMPGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'PCMGXXKTEMPGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        _datas.forEach((record: any, index: number) => {
            let srfmajortext = record.pcmgxxktempname;
            if (index < 5) {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ pcmgxxktemp: _keys }),Object.assign({ pcmgxxktemp: _keys },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    public rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.pcmgxxktemp,$event.pcmgxxktemp);
        });
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('pcmgxxktemp_main_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('pcmgxxktemp_main_grid', JSON.stringify(this.allColumns));
    }
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: '错误', desc: 'PCMGXXKTEMPGridView视图表格createAction参数未配置' });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: '错误', desc: 'PCMGXXKTEMPGridView视图表格updateAction参数未配置' });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
                        if(item.pcmgxxktemp){
                            Object.assign(this.context,{pcmgxxktemp:item.pcmgxxktemp});
                        }
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'PCMGXXKTEMPGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
    ...
    public getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.pcmgxxktemp,args.row.pcmgxxktemp);
        });
