...
<div class='grid' style="height:100%;">
    ...
    <el-table v-if="isDisplay === true"
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('psychologystatus')">
                <el-table-column show-overflow-tooltip :prop="'psychologystatus'" :label="$t('entities.pcmtestresult.main_grid.columns.psychologystatus')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pcmtestresult.main_grid.columns.psychologystatus')}}
                      </span>
                    </template>
              ...
              @change="($event)=>{gridEditItemChange(row, column.property, $event, $index)}">
                        ...
                        <template v-if="!actualIsOpenEdit">
                                <app-span name='psychologystatus' editorType="TEXTBOX" :value="row.psychologystatus"></app-span>
                        </template>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('abilitygrade')">
                <el-table-column show-overflow-tooltip :prop="'abilitygrade'" :label="$t('entities.pcmtestresult.main_grid.columns.abilitygrade')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pcmtestresult.main_grid.columns.abilitygrade')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template v-if="actualIsOpenEdit">
              ...
              @change="($event)=>{gridEditItemChange(row, column.property, $event, $index)}">
                        ...
                        <template v-if="!actualIsOpenEdit">
                                <app-span name='abilitygrade' editorType="TEXTBOX" :value="row.abilitygrade"></app-span>
                        </template>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('jobfitdegree')">
                <el-table-column show-overflow-tooltip :prop="'jobfitdegree'" :label="$t('entities.pcmtestresult.main_grid.columns.jobfitdegree')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pcmtestresult.main_grid.columns.jobfitdegree')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template v-if="actualIsOpenEdit">
              ...
              @change="($event)=>{gridEditItemChange(row, column.property, $event, $index)}">
                        ...
                        <template v-if="!actualIsOpenEdit">
                                <app-span name='jobfitdegree' editorType="TEXTBOX" :value="row.jobfitdegree"></app-span>
                        </template>
                    </template>
                </el-table-column>
            </template>
...
export default class MainBase extends Vue implements ControlInterface {
     ...
     * @memberof MainBase
     */
    public appEntityService: PcmTestResultService = new PcmTestResultService({ $store: this.$store });
    ...
    */
    ...
    public allColumns: any[] = [
        {
            ...
            label: '心理健康（*）',
            langtag: 'entities.pcmtestresult.main_grid.columns.psychologystatus',
            show: true,
            util: 'PX'
        },
        {
            name: 'abilitygrade',
            label: '能力（*）',
            langtag: 'entities.pcmtestresult.main_grid.columns.abilitygrade',
            show: true,
            util: 'PX'
        },
        {
            name: 'jobfitdegree',
            label: '胜任力（*）',
            langtag: 'entities.pcmtestresult.main_grid.columns.jobfitdegree',
            show: true,
            util: 'PX'
        },
    ]
    ...
    public getGridRowModel(){
        return {
          abilitygrade: new FormItemModel(),
          psychologystatus: new FormItemModel(),
          jobfitdegree: new FormItemModel(),
    ...
    public rules: any = {
        abilitygrade: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '能力（*） 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '能力（*） 值不能为空', trigger: 'blur' },
        ],
        psychologystatus: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '心理健康（*） 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '心理健康（*） 值不能为空', trigger: 'blur' },
        ],
        jobfitdegree: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '胜任力（*） 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '胜任力（*） 值不能为空', trigger: 'blur' },
        ],
        pcmprofileid: [
        ...
        srfkey: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '测评结果信息标识 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '测评结果信息标识 值不能为空', trigger: 'blur' },
        ],
    }
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'TestResultCPJG_WSSWBJGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'TestResultCPJG_WSSWBJGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        _datas.forEach((record: any, index: number) => {
            let srfmajortext = record.testresultname;
            if (index < 5) {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ pcmtestresult: _keys }),Object.assign({ pcmtestresult: _keys },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    public rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.testresult,$event.testresult);
        });
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('testresult_main_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('testresult_main_grid', JSON.stringify(this.allColumns));
    }
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: '错误', desc: 'TestResultCPJG_WSSWBJGridView视图表格createAction参数未配置' });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: '错误', desc: 'TestResultCPJG_WSSWBJGridView视图表格updateAction参数未配置' });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
                        if(item.pcmtestresult){
                            Object.assign(this.context,{pcmtestresult:item.pcmtestresult});
                        }
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'TestResultCPJG_WSSWBJGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
    ...
    public getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.pcmtestresult,args.row.pcmtestresult);
        });
