    ...
    <el-table v-if="isDisplay === true"
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('xh')">
                <el-table-column show-overflow-tooltip :prop="'xh'" :label="$t('entities.pcmgxml.main_grid.columns.xh')" :width="100"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pcmgxml.main_grid.columns.xh')}}
                      </span>
                    </template>
            ...
            <template v-if="getColumnState('nd')">
                <el-table-column show-overflow-tooltip :prop="'nd'" :label="$t('entities.pcmgxml.main_grid.columns.nd')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pcmgxml.main_grid.columns.nd')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
            ...
            <template v-if="getColumnState('nationarea')">
                <el-table-column show-overflow-tooltip :prop="'nationarea'" :label="$t('entities.pcmgxml.main_grid.columns.nationarea')" :width="200"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pcmgxml.main_grid.columns.nationarea')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
                                <app-span name='nationarea' editorType="TEXTBOX" :value="row.nationarea"></app-span>
                        </template>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('xxxz')">
                <el-table-column show-overflow-tooltip :prop="'xxxz'" :label="$t('entities.pcmgxml.main_grid.columns.xxxz')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pcmgxml.main_grid.columns.xxxz')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
            ...
            <template v-if="getColumnState('pcmgxmlname')">
                <el-table-column show-overflow-tooltip :prop="'pcmgxmlname'" :label="$t('entities.pcmgxml.main_grid.columns.pcmgxmlname')" :width="200"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pcmgxml.main_grid.columns.pcmgxmlname')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
                                <app-span name='pcmgxmlname' editorType="TEXTBOX" :value="row.pcmgxmlname"></app-span>
                        </template>
                    </template>
                </el-table-column>
            </template>
...
export default class MainBase extends Vue implements ControlInterface {
    ...
    */
    ...
    public allColumns: any[] = [
        {
            name: 'xh',
            label: '排序号',
            langtag: 'entities.pcmgxml.main_grid.columns.xh',
            show: true,
            util: 'PX'
        },
        {
            name: 'nd',
            label: '年度',
            langtag: 'entities.pcmgxml.main_grid.columns.nd',
            show: true,
            util: 'PX'
        },
        {
            name: 'nationarea',
            label: '国家或地区',
            langtag: 'entities.pcmgxml.main_grid.columns.nationarea',
            show: true,
            util: 'PX'
        },
        {
            name: 'xxxz',
            label: '学校性质',
            langtag: 'entities.pcmgxml.main_grid.columns.xxxz',
            show: true,
            util: 'PX'
        },
        {
            name: 'pcmgxmlname',
            label: '高校名称',
            langtag: 'entities.pcmgxml.main_grid.columns.pcmgxmlname',
            show: true,
            util: 'PX'
        },
    ]
    ...
    public getGridRowModel(){
        return {
          nationarea: new FormItemModel(),
          xh: new FormItemModel(),
          nd: new FormItemModel(),
          xxxz: new FormItemModel(),
          pcmgxmlname: new FormItemModel(),
    ...
    public rules: any = {
        nationarea: [
             { required: true, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '国家或地区 值不能为空', trigger: 'change' },
            { required: true, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '国家或地区 值不能为空', trigger: 'blur' },
        ],
        xh: [
             { required: true, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '排序号 值不能为空', trigger: 'change' },
            { required: true, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '排序号 值不能为空', trigger: 'blur' },
        ],
        nd: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '年度 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '年度 值不能为空', trigger: 'blur' },
        ],
        xxxz: [
             { required: true, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '学校性质 值不能为空', trigger: 'change' },
            { required: true, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '学校性质 值不能为空', trigger: 'blur' },
        ],
        pcmgxmlname: [
             { required: true, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '高校名称 值不能为空', trigger: 'change' },
            { required: true, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '高校名称 值不能为空', trigger: 'blur' },
        ],
        srfkey: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '高校名录标识 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '高校名录标识 值不能为空', trigger: 'blur' },
        ],
    }
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'PCMGXMLPickupGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'PCMGXMLPickupGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        _datas.forEach((record: any, index: number) => {
            let srfmajortext = record.pcmgxmlname;
            if (index < 5) {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ pcmgxml: _keys }),Object.assign({ pcmgxml: _keys },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    public rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.pcmgxml,$event.pcmgxml);
        });
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('pcmgxml_main_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('pcmgxml_main_grid', JSON.stringify(this.allColumns));
    }
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: '错误', desc: 'PCMGXMLPickupGridView视图表格createAction参数未配置' });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: '错误', desc: 'PCMGXMLPickupGridView视图表格updateAction参数未配置' });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
                        if(item.pcmgxml){
                            Object.assign(this.context,{pcmgxml:item.pcmgxml});
                        }
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'PCMGXMLPickupGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
    ...
    public getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.pcmgxml,args.row.pcmgxml);
        });
