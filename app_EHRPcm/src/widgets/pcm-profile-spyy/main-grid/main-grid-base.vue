...
<div class='grid' style="height:100%;">
    ...
    <el-table v-if="isDisplay === true"
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('yybm')">
                <el-table-column show-overflow-tooltip :prop="'yybm'" :label="$t('entities.pcmprofilespyy.main_grid.columns.yybm')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pcmprofilespyy.main_grid.columns.yybm')}}
                      </span>
                    </template>
              ...
              @change="($event)=>{gridEditItemChange(row, column.property, $event, $index)}">
                        ...
                        <template v-if="!actualIsOpenEdit">
                                <app-span name='yybm' editorType="TEXTBOX" :value="row.yybm"></app-span>
                        </template>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('yymc')">
                <el-table-column show-overflow-tooltip :prop="'yymc'" :label="$t('entities.pcmprofilespyy.main_grid.columns.yymc')" :width="300"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pcmprofilespyy.main_grid.columns.yymc')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template v-if="actualIsOpenEdit">
              ...
              @change="($event)=>{gridEditItemChange(row, column.property, $event, $index)}">
                        ...
                        <template v-if="!actualIsOpenEdit">
                                <app-span name='yymc' editorType="TEXTBOX" :value="row.yymc"></app-span>
                        </template>
                    </template>
                </el-table-column>
            </template>
...
export default class MainBase extends Vue implements ControlInterface {
     ...
     * @memberof MainBase
     */
    public appEntityService: PcmProfileSpyyService = new PcmProfileSpyyService({ $store: this.$store });
     ...
     * @memberof MainBase
     */
    public minorSortPSDEF: string = 'yybm';
    ...
    */
    ...
    public allColumns: any[] = [
        {
            name: 'yybm',
            label: '原因编码',
            langtag: 'entities.pcmprofilespyy.main_grid.columns.yybm',
            show: true,
            util: 'PX'
        },
        {
    ...
    public getGridRowModel(){
        return {
          yymc: new FormItemModel(),
          yybm: new FormItemModel(),
    ...
    public rules: any = {
        yymc: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '原因名称 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '原因名称 值不能为空', trigger: 'blur' },
        ],
        yybm: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '原因编码 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '原因编码 值不能为空', trigger: 'blur' },
        ],
        srfkey: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '应聘者审批原因标识 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '应聘者审批原因标识 值不能为空', trigger: 'blur' },
        ],
    }
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'PCMPROFILESPYYGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'PCMPROFILESPYYGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        _datas.forEach((record: any, index: number) => {
            let srfmajortext = record.pcmprofilespyyname;
            if (index < 5) {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ pcmprofilespyy: _keys }),Object.assign({ pcmprofilespyy: _keys },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    public rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.pcmprofilespyy,$event.pcmprofilespyy);
        });
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('pcmprofilespyy_main_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('pcmprofilespyy_main_grid', JSON.stringify(this.allColumns));
    }
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: '错误', desc: 'PCMPROFILESPYYGridView视图表格createAction参数未配置' });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: '错误', desc: 'PCMPROFILESPYYGridView视图表格updateAction参数未配置' });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
                        if(item.pcmprofilespyy){
                            Object.assign(this.context,{pcmprofilespyy:item.pcmprofilespyy});
                        }
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'PCMPROFILESPYYGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
    ...
    public getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.pcmprofilespyy,args.row.pcmprofilespyy);
        });
