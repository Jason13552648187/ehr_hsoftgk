<template>
  ...
  <row>
    <i-col span="20" class="form-content">
      <row>
                    <i-col v-show="detailsModel.n_nd_eq.visible" :style="{}"  :lg="{ span: 24, offset: 0 }">
              <app-form-item name='n_nd_eq' :itemRules="this.rules.n_nd_eq" class='' :caption="$t('entities.pcmzpmegl.default_searchform.details.n_nd_eq')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.n_nd_eq.error" :isEmptyCaption="false" labelPos="LEFT"> 
              <input-box v-model="data.n_nd_eq"  @enter="onEnter($event)"    :disabled="detailsModel.n_nd_eq.disabled" type='text'  style=""></input-box>
          </app-form-item>
...
, 
    ...
    public load(opt: any = {}): void {
        if(!this.loadAction){
            this.$Notice.error({ title: '错误', desc: 'PCMZPMEGLCurNDGridView视图搜索表单loadAction参数未配置' });
            return;
        }
        const arg: any = { ...opt };
    ...
    public loadDraft(opt: any = {},mode?:string): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'PCMZPMEGLCurNDGridView视图搜索表单loaddraftAction参数未配置' });
            return;
        }
        const arg: any = { ...opt } ;
        Object.assign(arg,{viewparams:this.viewparams});
