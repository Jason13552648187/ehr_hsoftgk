    ...
    <el-table v-if="isDisplay === true"
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('contentcode')">
                <el-table-column show-overflow-tooltip :prop="'contentcode'" :label="$t('entities.parexamcontent.main_grid.columns.contentcode')" :width="130"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.parexamcontent.main_grid.columns.contentcode')}}
                      </span>
                    </template>
            ...
            <template v-if="getColumnState('parexamcontentname')">
                <el-table-column show-overflow-tooltip :prop="'parexamcontentname'" :label="$t('entities.parexamcontent.main_grid.columns.parexamcontentname')" :width="200"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.parexamcontent.main_grid.columns.parexamcontentname')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.parexamcontentname}}</span>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('updateman')">
                <el-table-column show-overflow-tooltip :prop="'updateman'" :label="$t('entities.parexamcontent.main_grid.columns.updateman')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.parexamcontent.main_grid.columns.updateman')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
            ...
            <template v-if="getColumnState('updatedate')">
                <el-table-column show-overflow-tooltip :prop="'updatedate'" :label="$t('entities.parexamcontent.main_grid.columns.updatedate')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.parexamcontent.main_grid.columns.updatedate')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
...
import ParExamContentService from '@/service/par-exam-content/par-exam-content-service';
...
export default class MainBase extends Vue implements ControlInterface {
     ...
     * 实体服务对象
     *
     * @type {ParExamContentService}
     * @memberof MainBase
     */
    public appEntityService: ParExamContentService = new ParExamContentService({ $store: this.$store });
    ...
    */
    ...
    public allColumns: any[] = [
        {
            name: 'contentcode',
            label: '内容编码',
            langtag: 'entities.parexamcontent.main_grid.columns.contentcode',
            show: true,
            util: 'PX'
        },
        {
            ...
            label: '考核内容',
            langtag: 'entities.parexamcontent.main_grid.columns.parexamcontentname',
            show: true,
            util: 'PX'
        },
        {
            name: 'updateman',
            label: '更新人',
            langtag: 'entities.parexamcontent.main_grid.columns.updateman',
            show: true,
            util: 'PX'
        },
        {
            name: 'updatedate',
            label: '更新时间',
            langtag: 'entities.parexamcontent.main_grid.columns.updatedate',
            show: true,
            util: 'PX'
        },
    ]
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'ParExamContentGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'ParExamContentGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        _datas.forEach((record: any, index: number) => {
            let srfmajortext = record.parexamcontentname;
            if (index < 5) {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ parexamcontent: _keys }),Object.assign({ parexamcontent: _keys },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    public rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.parexamcontent,$event.parexamcontent);
        });
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('parexamcontent_main_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('parexamcontent_main_grid', JSON.stringify(this.allColumns));
    }
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: '错误', desc: 'ParExamContentGridView视图表格createAction参数未配置' });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: '错误', desc: 'ParExamContentGridView视图表格updateAction参数未配置' });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
                        if(item.parexamcontent){
                            Object.assign(this.context,{parexamcontent:item.parexamcontent});
                        }
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'ParExamContentGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
    ...
    public getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.parexamcontent,args.row.parexamcontent);
        });
