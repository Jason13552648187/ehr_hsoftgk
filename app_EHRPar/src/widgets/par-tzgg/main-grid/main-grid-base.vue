...
<div class='grid' style="height:100%;">
    ...
    <el-table v-if="isDisplay === true"
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('partzggname')">
                <el-table-column show-overflow-tooltip :prop="'partzggname'" :label="$t('entities.partzgg.main_grid.columns.partzggname')" :width="260"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.partzgg.main_grid.columns.partzggname')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.partzggname}}</span>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('zqlx')">
                <el-table-column show-overflow-tooltip :prop="'zqlx'" :label="$t('entities.partzgg.main_grid.columns.zqlx')" :width="120"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.partzgg.main_grid.columns.zqlx')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
            <codelist :value="row.zqlx" tag='EhrCodeList0077' codelistType='STATIC' ></codelist>
                        </template>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('nd')">
                <el-table-column show-overflow-tooltip :prop="'nd'" :label="$t('entities.partzgg.main_grid.columns.nd')" :width="100"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.partzgg.main_grid.columns.nd')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
            ...
            <template v-if="getColumnState('yd')">
                <el-table-column show-overflow-tooltip :prop="'yd'" :label="$t('entities.partzgg.main_grid.columns.yd')" :width="110"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.partzgg.main_grid.columns.yd')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
            ...
            <template v-if="getColumnState('tzlx')">
                <el-table-column show-overflow-tooltip :prop="'tzlx'" :label="$t('entities.partzgg.main_grid.columns.tzlx')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.partzgg.main_grid.columns.tzlx')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
            <codelist :value="row.tzlx" tag='EhrCodeList0064' codelistType='STATIC' ></codelist>
                        </template>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('jzsj')">
                <el-table-column show-overflow-tooltip :prop="'jzsj'" :label="$t('entities.partzgg.main_grid.columns.jzsj')" :width="120"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.partzgg.main_grid.columns.jzsj')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <app-format-data format="YYYY-MM-DD" :data="row.jzsj"></app-format-data>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('uagridcolumn1')">
                <el-table-column :column-key="'uagridcolumn1'" :label="$t('entities.partzgg.main_grid.columns.uagridcolumn1')" :width="150"  :align="'center'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.partzgg.main_grid.columns.uagridcolumn1')}}
                      </span>
                    </template>
                    <template slot-scope="scope">
                        <div style="text-align: center;">
                            <a @click="uiAction(scope.row, 'Edit', $event)">
                              ...
                              {{$t('entities.partzgg.main_grid.uiactions.edit')}}
                            </a>
                            <divider type='vertical'></divider>
                            <a @click="uiAction(scope.row, 'Remove', $event)">
                              <i class='fa fa-remove'></i>
                              {{$t('entities.partzgg.main_grid.uiactions.remove')}}
                            </a>
                        </div>
                    </template>
                </el-table-column>
...
export default class MainBase extends Vue implements ControlInterface {
     ...
     * @memberof MainBase
     */
    public appEntityService: ParTzggService = new ParTzggService({ $store: this.$store });
     ...
     * @memberof 
     */
    public grid_uagridcolumn1_u4ae5f14_click(params: any = {}, tag?: any, $event?: any) {
        // 取数
        let datas: any[] = [];
        let xData: any = null;
        // _this 指向容器对象
        ...
        // 界面行为
        this.Edit(datas, contextJO,paramJO,  $event, xData,this,"ParTzgg");
    }
     ...
     * @memberof 
     */
    public grid_uagridcolumn1_uc1ca01e_click(params: any = {}, tag?: any, $event?: any) {
        // 取数
        let datas: any[] = [];
        let xData: any = null;
        // _this 指向容器对象
        ...
        // 界面行为
        this.Remove(datas, contextJO,paramJO,  $event, xData,this,"ParTzgg");
    }
    ...
    public Edit(args: any[],contextJO?:any, params?: any, $event?: any, xData?: any,actionContext?:any,srfParentDeName?:string) {
        ...
        if (_this.opendata && _this.opendata instanceof Function) {
            const data: any = { };
            if (args.length > 0) {
                Object.assign(data, { partzgg: args[0].partzgg })
            }
    ...
    */
    ...
    public allColumns: any[] = [
        {
            name: 'partzggname',
            label: '标题',
            langtag: 'entities.partzgg.main_grid.columns.partzggname',
            show: true,
            util: 'PX'
        },
        {
            name: 'zqlx',
            label: '周期类型',
            langtag: 'entities.partzgg.main_grid.columns.zqlx',
            show: true,
            util: 'PX'
        },
        {
            name: 'nd',
            label: '年度',
            langtag: 'entities.partzgg.main_grid.columns.nd',
            show: true,
            util: 'PX'
        },
        {
            name: 'yd',
            label: '月度',
            langtag: 'entities.partzgg.main_grid.columns.yd',
            show: true,
            util: 'PX'
        },
        {
            name: 'tzlx',
            label: '通知类型',
            langtag: 'entities.partzgg.main_grid.columns.tzlx',
            show: true,
            util: 'PX'
        },
        {
            name: 'jzsj',
            label: '截止时间',
            langtag: 'entities.partzgg.main_grid.columns.jzsj',
            show: true,
            util: 'PX'
        },
        {
            name: 'uagridcolumn1',
            label: '操作',
            langtag: 'entities.partzgg.main_grid.columns.uagridcolumn1',
            show: true,
            util: 'PX'
        },
    ]
    ...
    public rules: any = {
        srfkey: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '通知公告标识 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '通知公告标识 值不能为空', trigger: 'blur' },
        ],
    }
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'ParTzggGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'ParTzggGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        _datas.forEach((record: any, index: number) => {
            let srfmajortext = record.partzggname;
            if (index < 5) {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ partzgg: _keys }),Object.assign({ partzgg: _keys },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    public rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.partzgg,$event.partzgg);
        });
	...
	public uiAction(row: any, tag: any, $event: any) {
        ...
        if(Object.is('Edit', tag)) {
            this.grid_uagridcolumn1_u4ae5f14_click(row, tag, $event);
        }
        if(Object.is('Remove', tag)) {
            this.grid_uagridcolumn1_uc1ca01e_click(row, tag, $event);
        }
    }
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('partzgg_main_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('partzgg_main_grid', JSON.stringify(this.allColumns));
    }
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: '错误', desc: 'ParTzggGridView视图表格createAction参数未配置' });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: '错误', desc: 'ParTzggGridView视图表格updateAction参数未配置' });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
                        if(item.partzgg){
                            Object.assign(this.context,{partzgg:item.partzgg});
                        }
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'ParTzggGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
    ...
    public getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.partzgg,args.row.partzgg);
        });
