    ...
    <el-table v-if="isDisplay === true"
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('uagridcolumn1')">
                <el-table-column :column-key="'uagridcolumn1'" :label="$t('entities.parassesstemplate.main_grid.columns.uagridcolumn1')" :width="160"  :align="'center'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                          <i class='create'></i>
                        {{$t('entities.parassesstemplate.main_grid.columns.uagridcolumn1')}}
                      </span>
                    </template>
                    <template slot-scope="scope">
                        <div style="text-align: center;">
                            <a @click="uiAction(scope.row, 'OpenCkView', $event)">
                              <i class=''></i>
                              {{$t('entities.parassesstemplate.main_grid.uiactions.openckview')}}
                            </a>
                            <divider type='vertical'></divider>
                            <a @click="uiAction(scope.row, 'OpenEditView', $event)">
                              <i class=''></i>
                              {{$t('entities.parassesstemplate.main_grid.uiactions.openeditview')}}
                            </a>
                        </div>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('parassesstemplatename')">
                <el-table-column show-overflow-tooltip :prop="'parassesstemplatename'" :label="$t('entities.parassesstemplate.main_grid.columns.parassesstemplatename')" :width="240"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.parassesstemplate.main_grid.columns.parassesstemplatename')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.parassesstemplatename}}</span>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('khdx')">
                <el-table-column show-overflow-tooltip :prop="'khdx'" :label="$t('entities.parassesstemplate.main_grid.columns.khdx')" :width="160"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.parassesstemplate.main_grid.columns.khdx')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
            <codelist :value="row.khdx" tag='EhrCodeList0015' codelistType='DYNAMIC' renderMode="STR" valueSeparator=";" textSeparator="、" ></codelist>
                        </template>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('nd')">
                <el-table-column show-overflow-tooltip :prop="'nd'" :label="$t('entities.parassesstemplate.main_grid.columns.nd')" :width="130"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.parassesstemplate.main_grid.columns.nd')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
            ...
            <template v-if="getColumnState('sfyx')">
                <el-table-column show-overflow-tooltip :prop="'sfyx'" :label="$t('entities.parassesstemplate.main_grid.columns.sfyx')" :width="120"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.parassesstemplate.main_grid.columns.sfyx')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
            <codelist :value="row.sfyx" tag='EhrCodeList0054' codelistType='STATIC' renderMode="STR" valueSeparator=";" textSeparator="、" ></codelist>
                        </template>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('updateman')">
                <el-table-column show-overflow-tooltip :prop="'updateman'" :label="$t('entities.parassesstemplate.main_grid.columns.updateman')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.parassesstemplate.main_grid.columns.updateman')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
            ...
            <template v-if="getColumnState('updatedate')">
                <el-table-column show-overflow-tooltip :prop="'updatedate'" :label="$t('entities.parassesstemplate.main_grid.columns.updatedate')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.parassesstemplate.main_grid.columns.updatedate')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
...
import ParAssessTemplateUIService from '@/uiservice/par-assess-template/par-assess-template-ui-service';
...
export default class MainBase extends Vue implements ControlInterface {
     ...
     * @memberof MainBase
     */
    public appEntityService: ParAssessTemplateService = new ParAssessTemplateService({ $store: this.$store });
     ...
     * @memberof 
     */
    public grid_uagridcolumn1_u063d401_click(params: any = {}, tag?: any, $event?: any) {
        // 取数
        let datas: any[] = [];
        let xData: any = null;
        // _this 指向容器对象
        ...
        // 界面行为
        const curUIService:ParAssessTemplateUIService  = new ParAssessTemplateUIService();
        curUIService.ParAssessTemplate_OpenCkView(datas,contextJO, paramJO,  $event, xData,this,"ParAssessTemplate");
    }
     ...
     * @memberof 
     */
    public grid_uagridcolumn1_ua662ee8_click(params: any = {}, tag?: any, $event?: any) {
        // 取数
        let datas: any[] = [];
        let xData: any = null;
        // _this 指向容器对象
        ...
        // 界面行为
        const curUIService:ParAssessTemplateUIService  = new ParAssessTemplateUIService();
        curUIService.ParAssessTemplate_OpenEditView(datas,contextJO, paramJO,  $event, xData,this,"ParAssessTemplate");
    }
    ...
    */
    ...
    public allColumns: any[] = [
        {
            name: 'uagridcolumn1',
            label: '操作',
            langtag: 'entities.parassesstemplate.main_grid.columns.uagridcolumn1',
            show: true,
            util: 'PX'
        },
        {
            ...
            label: '考核模板名称',
            langtag: 'entities.parassesstemplate.main_grid.columns.parassesstemplatename',
            show: true,
            util: 'PX'
        },
        {
            name: 'khdx',
            label: '考核对象',
            langtag: 'entities.parassesstemplate.main_grid.columns.khdx',
            show: true,
            util: 'PX'
        },
        {
            name: 'nd',
            label: '年度',
            langtag: 'entities.parassesstemplate.main_grid.columns.nd',
            show: true,
            util: 'PX'
        },
        {
            name: 'sfyx',
            label: '是否有效',
            langtag: 'entities.parassesstemplate.main_grid.columns.sfyx',
            show: true,
            util: 'PX'
        },
        {
            name: 'updateman',
            label: '更新人',
            langtag: 'entities.parassesstemplate.main_grid.columns.updateman',
            show: true,
            util: 'PX'
        },
        {
            name: 'updatedate',
            label: '更新时间',
            langtag: 'entities.parassesstemplate.main_grid.columns.updatedate',
            show: true,
            util: 'PX'
        },
    ]
    ...
    public rules: any = {
        srfkey: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '考核模板标识 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '考核模板标识 值不能为空', trigger: 'blur' },
        ],
    }
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'ParAssessTemplateGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'ParAssessTemplateGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        _datas.forEach((record: any, index: number) => {
            let srfmajortext = record.parassesstemplatename;
            if (index < 5) {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ parassesstemplate: _keys }),Object.assign({ parassesstemplate: _keys },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    public rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.parassesstemplate,$event.parassesstemplate);
        });
	...
	public uiAction(row: any, tag: any, $event: any) {
        ...
        if(Object.is('OpenCkView', tag)) {
            this.grid_uagridcolumn1_u063d401_click(row, tag, $event);
        }
        if(Object.is('OpenEditView', tag)) {
            this.grid_uagridcolumn1_ua662ee8_click(row, tag, $event);
        }
    }
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('parassesstemplate_main_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('parassesstemplate_main_grid', JSON.stringify(this.allColumns));
    }
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: '错误', desc: 'ParAssessTemplateGridView视图表格createAction参数未配置' });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: '错误', desc: 'ParAssessTemplateGridView视图表格updateAction参数未配置' });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
                        if(item.parassesstemplate){
                            Object.assign(this.context,{parassesstemplate:item.parassesstemplate});
                        }
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'ParAssessTemplateGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
    ...
    public getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.parassesstemplate,args.row.parassesstemplate);
        });
