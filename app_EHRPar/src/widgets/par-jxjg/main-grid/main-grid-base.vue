    ...
    <el-table v-if="isDisplay === true"
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('yg')">
                <el-table-column show-overflow-tooltip :prop="'yg'" :label="$t('entities.parjxjg.main_grid.columns.yg')" :width="120"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.parjxjg.main_grid.columns.yg')}}
                      </span>
                    </template>
            ...
            <template v-if="getColumnState('nd')">
                <el-table-column show-overflow-tooltip :prop="'nd'" :label="$t('entities.parjxjg.main_grid.columns.nd')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.parjxjg.main_grid.columns.nd')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
            ...
            <template v-if="getColumnState('khlx')">
                <el-table-column show-overflow-tooltip :prop="'khlx'" :label="$t('entities.parjxjg.main_grid.columns.khlx')" :width="130"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.parjxjg.main_grid.columns.khlx')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
            ...
            <template v-if="getColumnState('zz')">
                <el-table-column show-overflow-tooltip :prop="'zz'" :label="$t('entities.parjxjg.main_grid.columns.zz')" :width="270"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.parjxjg.main_grid.columns.zz')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.zz}}</span>
            ...
            <template v-if="getColumnState('bm')">
                <el-table-column show-overflow-tooltip :prop="'bm'" :label="$t('entities.parjxjg.main_grid.columns.bm')" :width="130"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.parjxjg.main_grid.columns.bm')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.bm}}</span>
            ...
            <template v-if="getColumnState('zf')">
                <el-table-column show-overflow-tooltip :prop="'zf'" :label="$t('entities.parjxjg.main_grid.columns.zf')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.parjxjg.main_grid.columns.zf')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
...
export default class MainBase extends Vue implements ControlInterface {
    ...
    */
    ...
    public allColumns: any[] = [
        {
            name: 'yg',
            label: '员工',
            langtag: 'entities.parjxjg.main_grid.columns.yg',
            show: true,
            util: 'PX'
        },
        {
            name: 'nd',
            label: '考核年度',
            langtag: 'entities.parjxjg.main_grid.columns.nd',
            show: true,
            util: 'px'
        },
        {
            name: 'khlx',
            label: '考核对象',
            langtag: 'entities.parjxjg.main_grid.columns.khlx',
            show: true,
            util: 'PX'
        },
        {
            name: 'zz',
            label: '组织',
            langtag: 'entities.parjxjg.main_grid.columns.zz',
            show: true,
            util: 'PX'
        },
        {
            name: 'bm',
            label: '部门',
            langtag: 'entities.parjxjg.main_grid.columns.bm',
            show: true,
            util: 'PX'
        },
        {
            name: 'zf',
            label: '总分',
            langtag: 'entities.parjxjg.main_grid.columns.zf',
            show: true,
            util: 'px'
        },
    ]
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'ParJxjgGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'ParJxjgGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('parjxjg_main_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('parjxjg_main_grid', JSON.stringify(this.allColumns));
    }
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: '错误', desc: 'ParJxjgGridView视图表格createAction参数未配置' });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: '错误', desc: 'ParJxjgGridView视图表格updateAction参数未配置' });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
                        if(item.parjxjg){
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'ParJxjgGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
