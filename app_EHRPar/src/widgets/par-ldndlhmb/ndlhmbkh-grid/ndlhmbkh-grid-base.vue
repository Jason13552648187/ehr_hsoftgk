    ...
    <el-table v-if="isDisplay === true"
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('xh')">
                <el-table-column show-overflow-tooltip :prop="'xh'" :label="$t('entities.parldndlhmb.ndlhmbkh_grid.columns.xh')" :width="100"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.parldndlhmb.ndlhmbkh_grid.columns.xh')}}
                      </span>
                    </template>
            ...
            <template v-if="getColumnState('nd')">
                <el-table-column show-overflow-tooltip :prop="'nd'" :label="$t('entities.parldndlhmb.ndlhmbkh_grid.columns.nd')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.parldndlhmb.ndlhmbkh_grid.columns.nd')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
            ...
            <template v-if="getColumnState('pimpersonname')">
                <el-table-column show-overflow-tooltip :prop="'pimpersonname'" :label="$t('entities.parldndlhmb.ndlhmbkh_grid.columns.pimpersonname')" :width="120"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.parldndlhmb.ndlhmbkh_grid.columns.pimpersonname')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('sszz')">
                <el-table-column show-overflow-tooltip :prop="'sszz'" :label="$t('entities.parldndlhmb.ndlhmbkh_grid.columns.sszz')" :width="220"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.parldndlhmb.ndlhmbkh_grid.columns.sszz')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.sszz}}</span>
            ...
            <template v-if="getColumnState('ssbm')">
                <el-table-column show-overflow-tooltip :prop="'ssbm'" :label="$t('entities.parldndlhmb.ndlhmbkh_grid.columns.ssbm')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.parldndlhmb.ndlhmbkh_grid.columns.ssbm')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.ssbm}}</span>
...
import NDLHMBKHService from './ndlhmbkh-grid-service';
...
export default class NDLHMBKHBase extends Vue implements ControlInterface {

    /**
     * 名称
     *
     * @type {string}
     * @memberof NDLHMBKHBase
     */
     ...
     * 建构部件服务对象
     *
     * @type {NDLHMBKHService}
     * @memberof NDLHMBKHBase
     */
    public service: NDLHMBKHService = new NDLHMBKHService({ $store: this.$store });
    ...
    */
    ...
    public allColumns: any[] = [
        {
            name: 'xh',
            label: '排序号',
            langtag: 'entities.parldndlhmb.ndlhmbkh_grid.columns.xh',
            show: true,
            util: 'PX'
        },
        {
            name: 'nd',
            label: '年度',
            langtag: 'entities.parldndlhmb.ndlhmbkh_grid.columns.nd',
            show: true,
            util: 'PX'
        },
        {
            name: 'pimpersonname',
            label: '领导姓名',
            langtag: 'entities.parldndlhmb.ndlhmbkh_grid.columns.pimpersonname',
            show: true,
            util: 'PX'
        },
        {
            name: 'sszz',
            label: '所属组织',
            langtag: 'entities.parldndlhmb.ndlhmbkh_grid.columns.sszz',
            show: true,
            util: 'PX'
        },
        {
            name: 'ssbm',
            label: '所属部门',
            langtag: 'entities.parldndlhmb.ndlhmbkh_grid.columns.ssbm',
            show: true,
            util: 'PX'
        },
    ]
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'ParLdndlhmbKHGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'ParLdndlhmbKHGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('parldndlhmb_ndlhmbkh_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('parldndlhmb_ndlhmbkh_grid', JSON.stringify(this.allColumns));
    }
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: '错误', desc: 'ParLdndlhmbKHGridView视图表格createAction参数未配置' });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: '错误', desc: 'ParLdndlhmbKHGridView视图表格updateAction参数未配置' });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
                        if(item.parldndlhmb){
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'ParLdndlhmbKHGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
