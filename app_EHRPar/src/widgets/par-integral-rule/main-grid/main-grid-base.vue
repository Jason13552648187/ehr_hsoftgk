    ...
    <el-table v-if="isDisplay === true"
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
                ...
                <el-table-column :column-key="'uagridcolumn1'" :label="$t('entities.parintegralrule.main_grid.columns.uagridcolumn1')" :width="240"  :align="'center'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        ...
                        {{$t('entities.parintegralrule.main_grid.columns.uagridcolumn1')}}
                      </span>
                    </template>
                    <template slot-scope="scope">
                        <div style="text-align: center;">
                            <a @click="uiAction(scope.row, 'OpenEditView', $event)">
                              ...
                              {{$t('entities.parintegralrule.main_grid.uiactions.openeditview')}}
                            </a>
                            <divider type='vertical'></divider>
                            <a @click="uiAction(scope.row, 'Remove', $event)">
                              <i class='fa fa-remove'></i>
                              {{$t('entities.parintegralrule.main_grid.uiactions.remove')}}
                            </a>
                            <divider type='vertical'></divider>
                            <a @click="uiAction(scope.row, 'Edit', $event)">
                              <i class='fa fa-edit'></i>
                              {{$t('entities.parintegralrule.main_grid.uiactions.edit')}}
                            </a>
                        </div>
                    </template>
                </el-table-column>
            ...
            <template v-if="getColumnState('nd')">
                <el-table-column show-overflow-tooltip :prop="'nd'" :label="$t('entities.parintegralrule.main_grid.columns.nd')" :width="130"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.parintegralrule.main_grid.columns.nd')}}
                      </span>
                    </template>
            ...
            <template v-if="getColumnState('dj')">
                <el-table-column show-overflow-tooltip :prop="'dj'" :label="$t('entities.parintegralrule.main_grid.columns.dj')" :width="120"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.parintegralrule.main_grid.columns.dj')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
            <codelist :value="row.dj" tag='EhrCodeList0005' codelistType='STATIC' ></codelist>
                        </template>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('integrals')">
                <el-table-column show-overflow-tooltip :prop="'integrals'" :label="$t('entities.parintegralrule.main_grid.columns.integrals')" :width="120"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.parintegralrule.main_grid.columns.integrals')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.integrals}}</span>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('parintegralrulename')">
                <el-table-column show-overflow-tooltip :prop="'parintegralrulename'" :label="$t('entities.parintegralrule.main_grid.columns.parintegralrulename')" :width="200"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.parintegralrule.main_grid.columns.parintegralrulename')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.parintegralrulename}}</span>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('updateman')">
                <el-table-column show-overflow-tooltip :prop="'updateman'" :label="$t('entities.parintegralrule.main_grid.columns.updateman')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.parintegralrule.main_grid.columns.updateman')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
            ...
            <template v-if="getColumnState('updatedate')">
                <el-table-column show-overflow-tooltip :prop="'updatedate'" :label="$t('entities.parintegralrule.main_grid.columns.updatedate')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.parintegralrule.main_grid.columns.updatedate')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
...
import ParIntegralRuleService from '@/service/par-integral-rule/par-integral-rule-service';
import MainService from './main-grid-service';

import ParIntegralRuleUIService from '@/uiservice/par-integral-rule/par-integral-rule-ui-service';
...
export default class MainBase extends Vue implements ControlInterface {
     ...
     * 实体服务对象
     *
     * @type {ParIntegralRuleService}
     * @memberof MainBase
     */
    public appEntityService: ParIntegralRuleService = new ParIntegralRuleService({ $store: this.$store });
     ...
     * @memberof 
     */
    public grid_uagridcolumn1_u97797e4_click(params: any = {}, tag?: any, $event?: any) {
        // 取数
        let datas: any[] = [];
        let xData: any = null;
        // _this 指向容器对象
        ...
        // 界面行为
        const curUIService:ParIntegralRuleUIService  = new ParIntegralRuleUIService();
        curUIService.ParIntegralRule_OpenEditView(datas,contextJO, paramJO,  $event, xData,this,"ParIntegralRule");
    }
     ...
     * @memberof 
     */
    public grid_uagridcolumn1_uca237bd_click(params: any = {}, tag?: any, $event?: any) {
        // 取数
        let datas: any[] = [];
        let xData: any = null;
        // _this 指向容器对象
        ...
        // 界面行为
        this.Remove(datas, contextJO,paramJO,  $event, xData,this,"ParIntegralRule");
    }
     ...
     * @memberof 
     */
    public grid_uagridcolumn1_ub4c728f_click(params: any = {}, tag?: any, $event?: any) {
        // 取数
        let datas: any[] = [];
        let xData: any = null;
        // _this 指向容器对象
        ...
        // 界面行为
        this.Edit(datas, contextJO,paramJO,  $event, xData,this,"ParIntegralRule");
    }
     ...
     * @memberof ParIntegralRuleGridViewBase
     */
    ...
    public Edit(args: any[],contextJO?:any, params?: any, $event?: any, xData?: any,actionContext?:any,srfParentDeName?:string) {
        ...
        if (_this.opendata && _this.opendata instanceof Function) {
            const data: any = { };
            if (args.length > 0) {
                Object.assign(data, { parintegralrule: args[0].parintegralrule })
            }
    ...
    */
    ...
    public allColumns: any[] = [
        {
            name: 'uagridcolumn1',
            label: '操作',
            langtag: 'entities.parintegralrule.main_grid.columns.uagridcolumn1',
            show: true,
            util: 'PX'
        },
        {
            name: 'nd',
            label: '年度',
            langtag: 'entities.parintegralrule.main_grid.columns.nd',
            show: true,
            util: 'PX'
        },
        {
            name: 'dj',
            label: '定级',
            langtag: 'entities.parintegralrule.main_grid.columns.dj',
            show: true,
            util: 'PX'
        },
        {
            name: 'integrals',
            label: '积分',
            langtag: 'entities.parintegralrule.main_grid.columns.integrals',
            show: true,
            util: 'PX'
        },
        {
            ...
            label: '备注',
            langtag: 'entities.parintegralrule.main_grid.columns.parintegralrulename',
            show: true,
            util: 'PX'
        },
        {
            name: 'updateman',
            label: '更新人',
            langtag: 'entities.parintegralrule.main_grid.columns.updateman',
            show: true,
            util: 'PX'
        },
        {
            name: 'updatedate',
            label: '更新时间',
            langtag: 'entities.parintegralrule.main_grid.columns.updatedate',
            show: true,
            util: 'PX'
        },
    ]
    ...
    public rules: any = {
        srfkey: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '积分规则标识 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '积分规则标识 值不能为空', trigger: 'blur' },
        ],
    }
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'ParIntegralRuleGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'ParIntegralRuleGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        _datas.forEach((record: any, index: number) => {
            let srfmajortext = record.parintegralrulename;
            if (index < 5) {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ parintegralrule: _keys }),Object.assign({ parintegralrule: _keys },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    public rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.parintegralrule,$event.parintegralrule);
        });
	...
	public uiAction(row: any, tag: any, $event: any) {
        ...
        if(Object.is('OpenEditView', tag)) {
            this.grid_uagridcolumn1_u97797e4_click(row, tag, $event);
        }
        if(Object.is('Remove', tag)) {
            this.grid_uagridcolumn1_uca237bd_click(row, tag, $event);
        }
        if(Object.is('Edit', tag)) {
            this.grid_uagridcolumn1_ub4c728f_click(row, tag, $event);
        }
    }
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('parintegralrule_main_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('parintegralrule_main_grid', JSON.stringify(this.allColumns));
    }
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: '错误', desc: 'ParIntegralRuleGridView视图表格createAction参数未配置' });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: '错误', desc: 'ParIntegralRuleGridView视图表格updateAction参数未配置' });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
                        if(item.parintegralrule){
                            Object.assign(this.context,{parintegralrule:item.parintegralrule});
                        }
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'ParIntegralRuleGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
    ...
    public getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.parintegralrule,args.row.parintegralrule);
        });
