    ...
    */
    ...
    public async remove(datas: any[]): Promise<any> {
        ...
        _datas.forEach((record: any, index: number) => {
            let srfmajortext = record.parkhzcmxname;
            if (index < 5) {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ parkhzcmx: _keys }),Object.assign({ parkhzcmx: _keys },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    public rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.parkhzcmx,$event.parkhzcmx);
        });
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('parkhzcmx_main_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('parkhzcmx_main_grid', JSON.stringify(this.allColumns));
    }
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    ...
                    if(!this.updateAction){
                        ...
                        Object.assign(item,{viewparams:this.viewparams});
                        if(item.parkhzcmx){
                            Object.assign(this.context,{parkhzcmx:item.parkhzcmx});
                        }
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'ParKhzcmxGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
    ...
    public getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.parkhzcmx,args.row.parkhzcmx);
        });
