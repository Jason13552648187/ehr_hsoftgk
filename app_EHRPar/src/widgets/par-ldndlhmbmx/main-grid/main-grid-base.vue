    ...
    <el-table v-if="isDisplay === true"
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('mblx')">
                <el-table-column show-overflow-tooltip :prop="'mblx'" :label="$t('entities.parldndlhmbmx.main_grid.columns.mblx')" :width="120"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.parldndlhmbmx.main_grid.columns.mblx')}}
                      </span>
                    </template>
            ...
            <template v-if="getColumnState('parldndlhmbmxname')">
                <el-table-column show-overflow-tooltip :prop="'parldndlhmbmxname'" :label="$t('entities.parldndlhmbmx.main_grid.columns.parldndlhmbmxname')" :width="200"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.parldndlhmbmx.main_grid.columns.parldndlhmbmxname')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('mblhyq')">
                <el-table-column show-overflow-tooltip :prop="'mblhyq'" :label="$t('entities.parldndlhmbmx.main_grid.columns.mblhyq')" :width="300"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.parldndlhmbmx.main_grid.columns.mblhyq')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('fz')">
                <el-table-column show-overflow-tooltip :prop="'fz'" :label="$t('entities.parldndlhmbmx.main_grid.columns.fz')" :width="100"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.parldndlhmbmx.main_grid.columns.fz')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('khbf')">
                <el-table-column show-overflow-tooltip :prop="'khbf'" :label="$t('entities.parldndlhmbmx.main_grid.columns.khbf')" :width="300"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.parldndlhmbmx.main_grid.columns.khbf')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.khbf}}</span>
...
export default class MainBase extends Vue implements ControlInterface {
    ...
    */
    ...
    public allColumns: any[] = [
        {
            name: 'mblx',
            label: '目标类型',
            langtag: 'entities.parldndlhmbmx.main_grid.columns.mblx',
            show: true,
            util: 'PX'
        },
        {
            ...
            label: '量化目标',
            langtag: 'entities.parldndlhmbmx.main_grid.columns.parldndlhmbmxname',
            show: true,
            util: 'PX'
        },
        {
            name: 'mblhyq',
            label: '目标量化要求',
            langtag: 'entities.parldndlhmbmx.main_grid.columns.mblhyq',
            show: true,
            util: 'PX'
        },
        {
            name: 'fz',
            label: '分值',
            langtag: 'entities.parldndlhmbmx.main_grid.columns.fz',
            show: true,
            util: 'PX'
        },
        {
            name: 'khbf',
            label: '考核办法',
            langtag: 'entities.parldndlhmbmx.main_grid.columns.khbf',
            show: true,
            util: 'PX'
        },
    ]
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'ParLdndlhmbmxGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'ParLdndlhmbmxGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('parldndlhmbmx_main_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('parldndlhmbmx_main_grid', JSON.stringify(this.allColumns));
    }
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: '错误', desc: 'ParLdndlhmbmxGridView视图表格createAction参数未配置' });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: '错误', desc: 'ParLdndlhmbmxGridView视图表格updateAction参数未配置' });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'ParLdndlhmbmxGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
