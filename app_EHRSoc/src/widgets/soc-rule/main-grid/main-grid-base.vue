    ...
    <el-table v-if="isDisplay === true"
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('socrulename')">
                <el-table-column show-overflow-tooltip :prop="'socrulename'" :label="$t('entities.socrule.main_grid.columns.socrulename')" :width="200"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.socrule.main_grid.columns.socrulename')}}
                      </span>
                    </template>
            ...
            <template v-if="getColumnState('nyear')">
                <el-table-column show-overflow-tooltip :prop="'nyear'" :label="$t('entities.socrule.main_grid.columns.nyear')" :width="100"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.socrule.main_grid.columns.nyear')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
            ...
            <template v-if="getColumnState('memo')">
                <el-table-column show-overflow-tooltip :prop="'memo'" :label="$t('entities.socrule.main_grid.columns.memo')" :width="300"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.socrule.main_grid.columns.memo')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.memo}}</span>
...
export default class MainBase extends Vue implements ControlInterface {
     ...
     * @memberof MainBase
     */
    public appEntityService: SocRuleService = new SocRuleService({ $store: this.$store });
    ...
    */
    ...
    public allColumns: any[] = [
        {
            name: 'socrulename',
            label: '社保规则名称',
            langtag: 'entities.socrule.main_grid.columns.socrulename',
            show: true,
            util: 'PX'
        },
        {
        ...
        },
        {
            name: 'memo',
            label: '备注',
            langtag: 'entities.socrule.main_grid.columns.memo',
            show: true,
            util: 'PX'
        },
    ]
    ...
    public rules: any = {
        srfkey: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '社保规则标识 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '社保规则标识 值不能为空', trigger: 'blur' },
        ],
    }
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'SocRulePickupGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'SocRulePickupGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        _datas.forEach((record: any, index: number) => {
            let srfmajortext = record.socrulename;
            if (index < 5) {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ socrule: _keys }),Object.assign({ socrule: _keys },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    public rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.socrule,$event.socrule);
        });
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('socrule_main_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('socrule_main_grid', JSON.stringify(this.allColumns));
    }
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: '错误', desc: 'SocRulePickupGridView视图表格createAction参数未配置' });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: '错误', desc: 'SocRulePickupGridView视图表格updateAction参数未配置' });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
                        if(item.socrule){
                            Object.assign(this.context,{socrule:item.socrule});
                        }
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'SocRulePickupGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
    ...
    public getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.socrule,args.row.socrule);
        });
