    ...
    <el-table v-if="isDisplay === true"
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('ygbh')">
                <el-table-column show-overflow-tooltip :prop="'ygbh'" :label="$t('entities.socarchives.main_grid.columns.ygbh')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.socarchives.main_grid.columns.ygbh')}}
                      </span>
                    </template>
            ...
            <template v-if="getColumnState('pimpersonname')">
                <el-table-column show-overflow-tooltip :prop="'pimpersonname'" :label="$t('entities.socarchives.main_grid.columns.pimpersonname')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.socarchives.main_grid.columns.pimpersonname')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('ygzt')">
                <el-table-column show-overflow-tooltip :prop="'ygzt'" :label="$t('entities.socarchives.main_grid.columns.ygzt')" :width="120"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.socarchives.main_grid.columns.ygzt')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
            ...
            <template v-if="getColumnState('ormorgname')">
                <el-table-column show-overflow-tooltip :prop="'ormorgname'" :label="$t('entities.socarchives.main_grid.columns.ormorgname')" :min-width="100"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
            ...
            <template v-if="getColumnState('socrulename')">
                <el-table-column show-overflow-tooltip :prop="'socrulename'" :label="$t('entities.socarchives.main_grid.columns.socrulename')" :width="250"  :align="'left'" :sortable="'custom'">
                    ...
                    <template v-slot="{row,column,$index}">
                        <span>{{row.socrulename}}</span>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('state')">
                <el-table-column show-overflow-tooltip :prop="'state'" :label="$t('entities.socarchives.main_grid.columns.state')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.socarchives.main_grid.columns.state')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
...
export default class MainBase extends Vue implements ControlInterface {
     ...
     * @memberof MainBase
     */
    public appEntityService: SocArchivesService = new SocArchivesService({ $store: this.$store });
    ...
    */
    ...
    public allColumns: any[] = [
        {
            name: 'ygbh',
            label: '员工编号',
            langtag: 'entities.socarchives.main_grid.columns.ygbh',
            show: true,
            util: 'PX'
        },
        {
            name: 'pimpersonname',
            label: '员工姓名',
            langtag: 'entities.socarchives.main_grid.columns.pimpersonname',
            show: true,
            util: 'PX'
        },
        {
            name: 'ygzt',
            label: '员工状态',
            langtag: 'entities.socarchives.main_grid.columns.ygzt',
            show: true,
            util: 'PX'
        },
        {
            name: 'ormorgname',
            label: '组织',
            langtag: 'entities.socarchives.main_grid.columns.ormorgname',
            show: true,
            util: 'STAR'
        },
        {
            ...
            label: '部门',
            langtag: 'entities.socarchives.main_grid.columns.ormorgsectorname',
            show: true,
            util: 'PX'
        },
        {
            name: 'socaccountname',
            label: '参保账户',
            langtag: 'entities.socarchives.main_grid.columns.socaccountname',
            show: true,
            util: 'PX'
        },
        {
        ...
        },
        {
            name: 'state',
            label: '状态',
            langtag: 'entities.socarchives.main_grid.columns.state',
            show: true,
            util: 'PX'
        },
    ]
    ...
    public rules: any = {
        srfkey: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '社保档案标识 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '社保档案标识 值不能为空', trigger: 'blur' },
        ],
    }
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'SocArchivesStopGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'SocArchivesStopGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        _datas.forEach((record: any, index: number) => {
            let srfmajortext = record.socarchivesname;
            if (index < 5) {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ socarchives: _keys }),Object.assign({ socarchives: _keys },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    public rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.socarchives,$event.socarchives);
        });
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('socarchives_main_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('socarchives_main_grid', JSON.stringify(this.allColumns));
    }
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: '错误', desc: 'SocArchivesStopGridView视图表格createAction参数未配置' });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: '错误', desc: 'SocArchivesStopGridView视图表格updateAction参数未配置' });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
                        if(item.socarchives){
                            Object.assign(this.context,{socarchives:item.socarchives});
                        }
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'SocArchivesStopGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
    ...
    public getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.socarchives,args.row.socarchives);
        });
