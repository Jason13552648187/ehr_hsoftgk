        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
                ...
                <el-table-column show-overflow-tooltip :prop="'pimpersonname'" :label="$t('entities.vacleavemanage.main_grid.columns.pimpersonname')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.vacleavemanage.main_grid.columns.pimpersonname')}}
                      </span>
                    </template>
            ...
            <template v-if="getColumnState('ygbh')">
                <el-table-column show-overflow-tooltip :prop="'ygbh'" :label="$t('entities.vacleavemanage.main_grid.columns.ygbh')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.vacleavemanage.main_grid.columns.ygbh')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.ygbh}}</span>
            ...
            <template v-if="getColumnState('zzdzs')">
                <el-table-column show-overflow-tooltip :prop="'zzdzs'" :label="$t('entities.vacleavemanage.main_grid.columns.zzdzs')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.vacleavemanage.main_grid.columns.zzdzs')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.zzdzs}}</span>
            ...
            <template v-if="getColumnState('ormorgsectorname')">
                <el-table-column show-overflow-tooltip :prop="'ormorgsectorname'" :label="$t('entities.vacleavemanage.main_grid.columns.ormorgsectorname')" :width="200"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.vacleavemanage.main_grid.columns.ormorgsectorname')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('qjmx')">
                <el-table-column show-overflow-tooltip :prop="'qjmx'" :label="$t('entities.vacleavemanage.main_grid.columns.qjmx')" :min-width="1"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.vacleavemanage.main_grid.columns.qjmx')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.qjmx}}</span>
            ...
            <template v-if="getColumnState('qjsy')">
                <el-table-column show-overflow-tooltip :prop="'qjsy'" :label="$t('entities.vacleavemanage.main_grid.columns.qjsy')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.vacleavemanage.main_grid.columns.qjsy')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.qjsy}}</span>
            ...
            <template v-if="getColumnState('wfqqjstate')">
                <el-table-column show-overflow-tooltip :prop="'wfqqjstate'" :label="$t('entities.vacleavemanage.main_grid.columns.wfqqjstate')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.vacleavemanage.main_grid.columns.wfqqjstate')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
            <codelist :value="row.wfqqjstate" tag='EhrCodeList0134' codelistType='STATIC' ></codelist>
                        </template>
                    </template>
                </el-table-column>
            </template>
...
export default class MainBase extends Vue implements ControlInterface {
    ...
    */
    ...
    public allColumns: any[] = [
        {
            name: 'pimpersonname',
            label: '请假人员',
            langtag: 'entities.vacleavemanage.main_grid.columns.pimpersonname',
            show: true,
            util: 'PX'
        },
        {
            name: 'ygbh',
            label: '员工编号',
            langtag: 'entities.vacleavemanage.main_grid.columns.ygbh',
            show: true,
            util: 'px'
        },
        {
            name: 'zzdzs',
            label: '组织名称',
            langtag: 'entities.vacleavemanage.main_grid.columns.zzdzs',
            show: true,
            util: 'px'
        },
        {
            ...
            label: '部门/项目部',
            langtag: 'entities.vacleavemanage.main_grid.columns.ormorgsectorname',
            show: true,
            util: 'PX'
        },
        {
            name: 'qjmx',
            label: '请假明细',
            langtag: 'entities.vacleavemanage.main_grid.columns.qjmx',
            show: true,
            util: 'STAR'
        },
        {
            name: 'qjsy',
            label: '请假事由',
            langtag: 'entities.vacleavemanage.main_grid.columns.qjsy',
            show: true,
            util: 'px'
        },
        {
            name: 'wfqqjstate',
            label: '请销假状态',
            langtag: 'entities.vacleavemanage.main_grid.columns.wfqqjstate',
            show: true,
            util: 'PX'
        },
    ]
    ...
    public rules: any = {
        srfkey: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '请假管理标识 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '请假管理标识 值不能为空', trigger: 'blur' },
        ],
    }
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'VACLEAVEMANAGEYGZZGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'VACLEAVEMANAGEYGZZGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ vacleavemanage: _keys }),Object.assign({ vacleavemanage: _keys },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    public rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.vacleavemanage,$event.vacleavemanage);
        });
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('vacleavemanage_main_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('vacleavemanage_main_grid', JSON.stringify(this.allColumns));
    }
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: '错误', desc: 'VACLEAVEMANAGEYGZZGridView视图表格createAction参数未配置' });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: '错误', desc: 'VACLEAVEMANAGEYGZZGridView视图表格updateAction参数未配置' });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
                        if(item.vacleavemanage){
                            Object.assign(this.context,{vacleavemanage:item.vacleavemanage});
                        }
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'VACLEAVEMANAGEYGZZGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
    ...
    public getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.vacleavemanage,args.row.vacleavemanage);
        });
