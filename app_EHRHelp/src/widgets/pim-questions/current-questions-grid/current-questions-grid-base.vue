    ...
    <el-table v-if="isDisplay === true"
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('wtbh')">
                <el-table-column show-overflow-tooltip :prop="'wtbh'" :label="$t('entities.pimquestions.currentquestions_grid.columns.wtbh')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimquestions.currentquestions_grid.columns.wtbh')}}
                      </span>
                    </template>
            ...
            <template v-if="getColumnState('questionsname')">
                <el-table-column show-overflow-tooltip :prop="'questionsname'" :label="$t('entities.pimquestions.currentquestions_grid.columns.questionsname')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimquestions.currentquestions_grid.columns.questionsname')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('jjcd')">
                <el-table-column show-overflow-tooltip :prop="'jjcd'" :label="$t('entities.pimquestions.currentquestions_grid.columns.jjcd')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimquestions.currentquestions_grid.columns.jjcd')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
            <codelist :value="row.jjcd" tag='EhrCodeList0055' codelistType='STATIC' ></codelist>
                        </template>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('createman')">
                <el-table-column show-overflow-tooltip :prop="'createman'" :label="$t('entities.pimquestions.currentquestions_grid.columns.createman')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimquestions.currentquestions_grid.columns.createman')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
            ...
            <template v-if="getColumnState('createdate')">
                <el-table-column show-overflow-tooltip :prop="'createdate'" :label="$t('entities.pimquestions.currentquestions_grid.columns.createdate')" :width="180"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimquestions.currentquestions_grid.columns.createdate')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('wtms')">
                <el-table-column show-overflow-tooltip :prop="'wtms'" :label="$t('entities.pimquestions.currentquestions_grid.columns.wtms')" :min-width="1"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimquestions.currentquestions_grid.columns.wtms')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.wtms}}</span>
            ...
            <template v-if="getColumnState('wtzt')">
                <el-table-column show-overflow-tooltip :prop="'wtzt'" :label="$t('entities.pimquestions.currentquestions_grid.columns.wtzt')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimquestions.currentquestions_grid.columns.wtzt')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
            <codelist :value="row.wtzt" tag='EhrCodeList0238' codelistType='STATIC' ></codelist>
                        </template>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('wcsj')">
                <el-table-column show-overflow-tooltip :prop="'wcsj'" :label="$t('entities.pimquestions.currentquestions_grid.columns.wcsj')" :width="180"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimquestions.currentquestions_grid.columns.wcsj')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
...
import CurrentQuestionsService from './current-questions-grid-service';
...
export default class CurrentQuestionsBase extends Vue implements ControlInterface {

    /**
     * 名称
     *
     * @type {string}
     * @memberof CurrentQuestionsBase
     */
     ...
     * 建构部件服务对象
     *
     * @type {CurrentQuestionsService}
     * @memberof CurrentQuestionsBase
     */
    public service: CurrentQuestionsService = new CurrentQuestionsService({ $store: this.$store });
    ...
    */
    ...
    public allColumns: any[] = [
        {
            name: 'wtbh',
            label: '问题编号',
            langtag: 'entities.pimquestions.currentquestions_grid.columns.wtbh',
            show: true,
            util: 'px'
        },
        {
            name: 'questionsname',
            label: '问题',
            langtag: 'entities.pimquestions.currentquestions_grid.columns.questionsname',
            show: true,
            util: 'px'
        },
        {
            name: 'jjcd',
            label: '紧急程度',
            langtag: 'entities.pimquestions.currentquestions_grid.columns.jjcd',
            show: true,
            util: 'px'
        },
        {
            name: 'createman',
            label: '提出人',
            langtag: 'entities.pimquestions.currentquestions_grid.columns.createman',
            show: true,
            util: 'PX'
        },
        {
            name: 'createdate',
            label: '提出时间',
            langtag: 'entities.pimquestions.currentquestions_grid.columns.createdate',
            show: true,
            util: 'PX'
        },
        {
            name: 'wtms',
            label: '问题描述',
            langtag: 'entities.pimquestions.currentquestions_grid.columns.wtms',
            show: true,
            util: 'STAR'
        },
        {
            name: 'wtzt',
            label: '问题状态',
            langtag: 'entities.pimquestions.currentquestions_grid.columns.wtzt',
            show: true,
            util: 'px'
        },
        {
            name: 'wcsj',
            label: '完成时间',
            langtag: 'entities.pimquestions.currentquestions_grid.columns.wcsj',
            show: true,
            util: 'PX'
        },
    ]
    ...
    public rules: any = {
        srfkey: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '问题收集标识 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '问题收集标识 值不能为空', trigger: 'blur' },
        ],
    }
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'QUESTIONSCurrentQuestionsGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'QUESTIONSCurrentQuestionsGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        _datas.forEach((record: any, index: number) => {
            let srfmajortext = record.questionsname;
            if (index < 5) {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ pimquestions: _keys }),Object.assign({ pimquestions: _keys },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    public rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.questions,$event.questions);
        });
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('questions_currentquestions_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('questions_currentquestions_grid', JSON.stringify(this.allColumns));
    }
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: '错误', desc: 'QUESTIONSCurrentQuestionsGridView视图表格createAction参数未配置' });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: '错误', desc: 'QUESTIONSCurrentQuestionsGridView视图表格updateAction参数未配置' });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
                        if(item.pimquestions){
                            Object.assign(this.context,{pimquestions:item.pimquestions});
                        }
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'QUESTIONSCurrentQuestionsGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
...
<style lang='less'>
@import './current-questions-grid.less';
</style>