    ...
    <el-table v-if="isDisplay === true"
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('serialno')">
                <el-table-column show-overflow-tooltip :prop="'serialno'" :label="$t('entities.pimarchivescenter.main_grid.columns.serialno')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimarchivescenter.main_grid.columns.serialno')}}
                      </span>
                    </template>
            ...
            <template v-if="getColumnState('archivescentername')">
                <el-table-column show-overflow-tooltip :prop="'archivescentername'" :label="$t('entities.pimarchivescenter.main_grid.columns.archivescentername')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimarchivescenter.main_grid.columns.archivescentername')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
            ...
            <template v-if="getColumnState('cabinetno')">
                <el-table-column show-overflow-tooltip :prop="'cabinetno'" :label="$t('entities.pimarchivescenter.main_grid.columns.cabinetno')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimarchivescenter.main_grid.columns.cabinetno')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
            ...
            <template v-if="getColumnState('layerno')">
                <el-table-column show-overflow-tooltip :prop="'layerno'" :label="$t('entities.pimarchivescenter.main_grid.columns.layerno')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimarchivescenter.main_grid.columns.layerno')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
            <codelist :value="row.layerno" tag='EhrCodeList0020' codelistType='STATIC' ></codelist>
                        </template>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('bnumber')">
                <el-table-column show-overflow-tooltip :prop="'bnumber'" :label="$t('entities.pimarchivescenter.main_grid.columns.bnumber')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimarchivescenter.main_grid.columns.bnumber')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
...
export default class MainBase extends Vue implements ControlInterface {
     ...
     * @memberof MainBase
     */
    public appEntityService: PimArchivesCenterService = new PimArchivesCenterService({ $store: this.$store });
    ...
    */
    ...
    public allColumns: any[] = [
        {
            name: 'serialno',
            label: '排序号',
            langtag: 'entities.pimarchivescenter.main_grid.columns.serialno',
            show: true,
            util: 'PX'
        },
        {
            ...
            label: '档案室名称',
            langtag: 'entities.pimarchivescenter.main_grid.columns.archivescentername',
            show: true,
            util: 'PX'
        },
        {
            name: 'cabinetno',
            label: '柜号',
            langtag: 'entities.pimarchivescenter.main_grid.columns.cabinetno',
            show: true,
            util: 'PX'
        },
        {
            name: 'layerno',
            label: '层号',
            langtag: 'entities.pimarchivescenter.main_grid.columns.layerno',
            show: true,
            util: 'PX'
        },
        {
            name: 'bnumber',
            label: '编号',
            langtag: 'entities.pimarchivescenter.main_grid.columns.bnumber',
            show: true,
            util: 'PX'
        },
    ]
    ...
    public getGridRowModel(){
        return {
          layerno: new FormItemModel(),
          bnumber: new FormItemModel(),
          archivescentername: new FormItemModel(),
          srfkey: new FormItemModel(),
          cabinetno: new FormItemModel(),
          serialno: new FormItemModel(),
        }
    }
    ...
    public rules: any = {
        layerno: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '层号 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '层号 值不能为空', trigger: 'blur' },
        ],
        bnumber: [
        ...
        archivescentername: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '档案室名称 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '档案室名称 值不能为空', trigger: 'blur' },
        ],
        srfkey: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '档案室管理标识 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '档案室管理标识 值不能为空', trigger: 'blur' },
        ],
        cabinetno: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '柜号 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '柜号 值不能为空', trigger: 'blur' },
        ],
        serialno: [
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'ARCHIVESCENTERPickupGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'ARCHIVESCENTERPickupGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        _datas.forEach((record: any, index: number) => {
            let srfmajortext = record.position;
            if (index < 5) {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ pimarchivescenter: _keys }),Object.assign({ pimarchivescenter: _keys },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    public rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.archivescenter,$event.archivescenter);
        });
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('archivescenter_main_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('archivescenter_main_grid', JSON.stringify(this.allColumns));
    }
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: '错误', desc: 'ARCHIVESCENTERPickupGridView视图表格createAction参数未配置' });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: '错误', desc: 'ARCHIVESCENTERPickupGridView视图表格updateAction参数未配置' });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
                        if(item.pimarchivescenter){
                            Object.assign(this.context,{pimarchivescenter:item.pimarchivescenter});
                        }
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'ARCHIVESCENTERPickupGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
    ...
    public getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.pimarchivescenter,args.row.pimarchivescenter);
        });
