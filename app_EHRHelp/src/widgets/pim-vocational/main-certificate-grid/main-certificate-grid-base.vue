    ...
    <el-table v-if="isDisplay === true"
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('zgzsbh')">
                <el-table-column show-overflow-tooltip :prop="'zgzsbh'" :label="$t('entities.pimvocational.main_certificate_grid.columns.zgzsbh')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimvocational.main_certificate_grid.columns.zgzsbh')}}
                      </span>
                    </template>
            ...
            <template v-if="getColumnState('zsmc')">
                <el-table-column show-overflow-tooltip :prop="'zsmc'" :label="$t('entities.pimvocational.main_certificate_grid.columns.zsmc')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimvocational.main_certificate_grid.columns.zsmc')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.zsmc}}</span>
            ...
            <template v-if="getColumnState('zslx')">
                <el-table-column show-overflow-tooltip :prop="'zslx'" :label="$t('entities.pimvocational.main_certificate_grid.columns.zslx')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimvocational.main_certificate_grid.columns.zslx')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
            <codelist :value="row.zslx" tag='EhrCodeList0109' codelistType='STATIC' renderMode="STR" valueSeparator=";" textSeparator="、" ></codelist>
                        </template>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('zghqrq')">
                <el-table-column show-overflow-tooltip :prop="'zghqrq'" :label="$t('entities.pimvocational.main_certificate_grid.columns.zghqrq')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimvocational.main_certificate_grid.columns.zghqrq')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('zgsydw')">
                <el-table-column show-overflow-tooltip :prop="'zgsydw'" :label="$t('entities.pimvocational.main_certificate_grid.columns.zgsydw')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimvocational.main_certificate_grid.columns.zgsydw')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('fzyxq')">
                <el-table-column show-overflow-tooltip :prop="'fzyxq'" :label="$t('entities.pimvocational.main_certificate_grid.columns.fzyxq')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimvocational.main_certificate_grid.columns.fzyxq')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.fzyxq}}</span>
            ...
            <template v-if="getColumnState('jlspzt')">
                <el-table-column show-overflow-tooltip :prop="'jlspzt'" :label="$t('entities.pimvocational.main_certificate_grid.columns.jlspzt')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimvocational.main_certificate_grid.columns.jlspzt')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
...
import Main_CertificateService from './main-certificate-grid-service';
...
export default class Main_CertificateBase extends Vue implements ControlInterface {

    /**
     * 名称
     *
     * @type {string}
     * @memberof Main_CertificateBase
     */
     ...
     * 建构部件服务对象
     *
     * @type {Main_CertificateService}
     * @memberof Main_CertificateBase
     */
    public service: Main_CertificateService = new Main_CertificateService({ $store: this.$store });
     ...
     * @memberof Main_CertificateBase
     */
    public minorSortPSDEF: string = 'zghqrq';
    ...
    */
    ...
    public allColumns: any[] = [
        {
            name: 'zgzsbh',
            label: '证书编号',
            langtag: 'entities.pimvocational.main_certificate_grid.columns.zgzsbh',
            show: true,
            util: 'px'
        },
        {
            name: 'zsmc',
            label: '证书名称',
            langtag: 'entities.pimvocational.main_certificate_grid.columns.zsmc',
            show: true,
            util: 'px'
        },
        {
            name: 'zslx',
            label: '证书类型',
            langtag: 'entities.pimvocational.main_certificate_grid.columns.zslx',
            show: true,
            util: 'px'
        },
        {
            name: 'zghqrq',
            label: '签发日期',
            langtag: 'entities.pimvocational.main_certificate_grid.columns.zghqrq',
            show: true,
            util: 'px'
        },
        {
            name: 'zgsydw',
            label: '签发机构',
            langtag: 'entities.pimvocational.main_certificate_grid.columns.zgsydw',
            show: true,
            util: 'px'
        },
        {
            name: 'fzyxq',
            label: '发证有效期',
            langtag: 'entities.pimvocational.main_certificate_grid.columns.fzyxq',
            show: true,
            util: 'px'
        },
        {
            name: 'jlspzt',
            label: '审批状态',
            langtag: 'entities.pimvocational.main_certificate_grid.columns.jlspzt',
            show: true,
            util: 'PX'
        },
    ]
    ...
    public rules: any = {
        srfkey: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '执（职）业资格信息标识 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '执（职）业资格信息标识 值不能为空', trigger: 'blur' },
        ],
    }
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'PIMVOCATIONALGRPimZigGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'PIMVOCATIONALGRPimZigGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ pimvocational: _keys }),Object.assign({ pimvocational: _keys },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    public rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.pimvocational,$event.pimvocational);
        });
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('pimvocational_main_certificate_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('pimvocational_main_certificate_grid', JSON.stringify(this.allColumns));
    }
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: '错误', desc: 'PIMVOCATIONALGRPimZigGridView视图表格createAction参数未配置' });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: '错误', desc: 'PIMVOCATIONALGRPimZigGridView视图表格updateAction参数未配置' });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
                        if(item.pimvocational){
                            Object.assign(this.context,{pimvocational:item.pimvocational});
                        }
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'PIMVOCATIONALGRPimZigGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
...
<style lang='less'>
@import './main-certificate-grid.less';
</style>