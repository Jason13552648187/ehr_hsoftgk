    ...
    <i-col span="20" class="form-content">
      <row>
          ...
          <i-col v-show="detailsModel.n_pimpersonname_like.visible" :style="{}"  :lg="{ span: 6, offset: 0 }">
              <app-form-item name='n_pimpersonname_like' :itemRules="this.rules.n_pimpersonname_like" class='' :caption="$t('entities.pimexitandentry.default_searchform.details.n_pimpersonname_like')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.n_pimpersonname_like.error" :isEmptyCaption="false" labelPos="LEFT"> 
...
export default class DefaultBase extends Vue implements ControlInterface {
    ...
    public rules: any = {
        ...
        n_workflowstate_eq: [
            { type: 'string', message: '流程状态(=) 值必须为字符串类型', trigger: 'change' },
            { type: 'string', message: '流程状态(=) 值必须为字符串类型', trigger: 'blur' },
            { required: false, type: 'string', message: '流程状态(=) 值不能为空', trigger: 'change' },
            { required: false, type: 'string', message: '流程状态(=) 值不能为空', trigger: 'blur' },
        ],
    }
...
, 
        n_lx_eq: new FormItemModel({ caption: '类型(=)', detailType: 'FORMITEM', name: 'n_lx_eq', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 3 })
, 
        n_workflowstate_eq: new FormItemModel({ caption: '流程状态(=)', detailType: 'FORMITEM', name: 'n_workflowstate_eq', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 3 })
, 
    };
     ...
     */
    @Watch('data.n_lx_eq')
    onN_lx_eqChange(newVal: any, oldVal: any) {
        this.formDataChange({ name: 'n_lx_eq', newVal: newVal, oldVal: oldVal });
    }

    /**
     * 监控表单属性 n_workflowstate_eq 值
     *
     * @param {*} newVal
     * @param {*} oldVal
     * @memberof DefaultBase
     */
    @Watch('data.n_workflowstate_eq')
    onN_workflowstate_eqChange(newVal: any, oldVal: any) {
        this.formDataChange({ name: 'n_workflowstate_eq', newVal: newVal, oldVal: oldVal });
    }
    ...
    public load(opt: any = {}): void {
        if(!this.loadAction){
            this.$Notice.error({ title: '错误', desc: 'PIMEXITANDENTRYYGZZGridView视图搜索表单loadAction参数未配置' });
            return;
        }
        const arg: any = { ...opt };
    ...
    public loadDraft(opt: any = {},mode?:string): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'PIMEXITANDENTRYYGZZGridView视图搜索表单loaddraftAction参数未配置' });
            return;
        }
        const arg: any = { ...opt } ;
        Object.assign(arg,{viewparams:this.viewparams});
