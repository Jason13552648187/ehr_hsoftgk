    ...
    <el-table v-if="isDisplay === true"
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('ormsignorgname')">
                <el-table-column show-overflow-tooltip :prop="'ormsignorgname'" :label="$t('entities.ormsignorg.main_grid.columns.ormsignorgname')" :width="400"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.ormsignorg.main_grid.columns.ormsignorgname')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
                                <app-span name='ormsignorgname' editorType="TEXTBOX" :value="row.ormsignorgname"></app-span>
                        </template>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('orgcode')">
                <el-table-column show-overflow-tooltip :prop="'orgcode'" :label="$t('entities.ormsignorg.main_grid.columns.orgcode')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.ormsignorg.main_grid.columns.orgcode')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
...
export default class MainBase extends Vue implements ControlInterface {
     ...
     * @memberof MainBase
     */
    public appEntityService: OrmSignOrgService = new OrmSignOrgService({ $store: this.$store });
    ...
    */
    ...
    public allColumns: any[] = [
        {
            name: 'ormsignorgname',
            label: '法人主体组织',
            langtag: 'entities.ormsignorg.main_grid.columns.ormsignorgname',
            show: true,
            util: 'PX'
        },
        {
            name: 'orgcode',
            label: '组织编号',
            langtag: 'entities.ormsignorg.main_grid.columns.orgcode',
            show: true,
            util: 'PX'
        },
    ]
    ...
    public rules: any = {
        ...
        ormsignorgname: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '法人主体组织 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '法人主体组织 值不能为空', trigger: 'blur' },
        ],
        srfkey: [
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'OrmSignOrgBDWPickupGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'OrmSignOrgBDWPickupGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        _datas.forEach((record: any, index: number) => {
            let srfmajortext = record.ormsignorgname;
            if (index < 5) {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ ormsignorg: _keys }),Object.assign({ ormsignorg: _keys },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    public rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.ormsignorg,$event.ormsignorg);
        });
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('ormsignorg_main_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('ormsignorg_main_grid', JSON.stringify(this.allColumns));
    }
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: '错误', desc: 'OrmSignOrgBDWPickupGridView视图表格createAction参数未配置' });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: '错误', desc: 'OrmSignOrgBDWPickupGridView视图表格updateAction参数未配置' });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
                        if(item.ormsignorg){
                            Object.assign(this.context,{ormsignorg:item.ormsignorg});
                        }
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'OrmSignOrgBDWPickupGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
    ...
    public getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.ormsignorg,args.row.ormsignorg);
        });
