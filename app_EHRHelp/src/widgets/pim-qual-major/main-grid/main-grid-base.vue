        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
                ...
                <el-table-column show-overflow-tooltip :prop="'zyzgzy'" :label="$t('entities.pimqualmajor.main_grid.columns.zyzgzy')" :width="300"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimqualmajor.main_grid.columns.zyzgzy')}}
                      </span>
                    </template>
            ...
            <template v-if="getColumnState('qualevel')">
                <el-table-column show-overflow-tooltip :prop="'qualevel'" :label="$t('entities.pimqualmajor.main_grid.columns.qualevel')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimqualmajor.main_grid.columns.qualevel')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
            ...
            <template v-if="getColumnState('pimqualmajorname')">
                <el-table-column show-overflow-tooltip :prop="'pimqualmajorname'" :label="$t('entities.pimqualmajor.main_grid.columns.pimqualmajorname')" :width="200"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimqualmajor.main_grid.columns.pimqualmajorname')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
            ...
            <template v-if="getColumnState('quamajor')">
                <el-table-column show-overflow-tooltip :prop="'quamajor'" :label="$t('entities.pimqualmajor.main_grid.columns.quamajor')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimqualmajor.main_grid.columns.quamajor')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
...
export default class MainBase extends Vue implements ControlInterface {
     ...
     * @memberof MainBase
     */
    public appEntityService: PimQualMajorService = new PimQualMajorService({ $store: this.$store });
    ...
    */
    ...
    public allColumns: any[] = [
        {
            name: 'zyzgzy',
            label: '执业资格名称',
            langtag: 'entities.pimqualmajor.main_grid.columns.zyzgzy',
            show: true,
            util: 'PX'
        },
        {
    ...
    public getGridRowModel(){
        return {
          quamajor: new FormItemModel(),
          qualevel: new FormItemModel(),
          pimqualmajorname: new FormItemModel(),
    ...
    public rules: any = {
        ...
        pimqualmajorname: [
            ...
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '执业资格 值不能为空', trigger: 'blur' },
        ],
        srfkey: [
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'PIMQUALMAJORPickupGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'PIMQUALMAJORPickupGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        _datas.forEach((record: any, index: number) => {
            let srfmajortext = record.zyzgzy;
            if (index < 5) {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ pimqualmajor: _keys }),Object.assign({ pimqualmajor: _keys },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    public rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.pimqualmajor,$event.pimqualmajor);
        });
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('pimqualmajor_main_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('pimqualmajor_main_grid', JSON.stringify(this.allColumns));
    }
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: '错误', desc: 'PIMQUALMAJORPickupGridView视图表格createAction参数未配置' });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: '错误', desc: 'PIMQUALMAJORPickupGridView视图表格updateAction参数未配置' });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
                        if(item.pimqualmajor){
                            Object.assign(this.context,{pimqualmajor:item.pimqualmajor});
                        }
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'PIMQUALMAJORPickupGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
    ...
    public getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.pimqualmajor,args.row.pimqualmajor);
        });
