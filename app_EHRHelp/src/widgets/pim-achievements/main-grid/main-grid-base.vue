        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
                ...
                <el-table-column show-overflow-tooltip :prop="'khlx'" :label="$t('entities.pimachievements.main_grid.columns.khlx')" :width="120"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimachievements.main_grid.columns.khlx')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
            <codelist :value="row.khlx" tag='EhrCodeList0092' codelistType='STATIC' ></codelist>
                        </template>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('pimachievementsname')">
                <el-table-column show-overflow-tooltip :prop="'pimachievementsname'" :label="$t('entities.pimachievements.main_grid.columns.pimachievementsname')" :width="130"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimachievements.main_grid.columns.pimachievementsname')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
            <codelist :value="row.pimachievementsname" tag='EhrCodeList0115' codelistType='STATIC' renderMode="STR" valueSeparator=";" textSeparator="、" ></codelist>
                        </template>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('jd')">
                <el-table-column show-overflow-tooltip :prop="'jd'" :label="$t('entities.pimachievements.main_grid.columns.jd')" :width="130"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimachievements.main_grid.columns.jd')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
            ...
            <template v-if="getColumnState('yd')">
                <el-table-column show-overflow-tooltip :prop="'yd'" :label="$t('entities.pimachievements.main_grid.columns.yd')" :width="110"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimachievements.main_grid.columns.yd')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
            ...
            <template v-if="getColumnState('khjl')">
                <el-table-column show-overflow-tooltip :prop="'khjl'" :label="$t('entities.pimachievements.main_grid.columns.khjl')" :width="120"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimachievements.main_grid.columns.khjl')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
            <codelist :value="row.khjl" tag='EhrCodeList0142' codelistType='DYNAMIC' renderMode="STR" valueSeparator=";" textSeparator="、" ></codelist>
                        </template>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('evaluationunit')">
                <el-table-column show-overflow-tooltip :prop="'evaluationunit'" :label="$t('entities.pimachievements.main_grid.columns.evaluationunit')" :width="200"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimachievements.main_grid.columns.evaluationunit')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
...
export default class MainBase extends Vue implements ControlInterface {
     ...
     * @memberof MainBase
     */
    public appEntityService: PimAchievementsService = new PimAchievementsService({ $store: this.$store });
    ...
    */
    ...
    public allColumns: any[] = [
        {
            name: 'khlx',
            label: '考核类型',
            langtag: 'entities.pimachievements.main_grid.columns.khlx',
            show: true,
            util: 'PX'
        },
        {
            ...
            label: '年度',
            langtag: 'entities.pimachievements.main_grid.columns.pimachievementsname',
            show: true,
            util: 'PX'
        },
        {
            name: 'jd',
            label: '季度',
            langtag: 'entities.pimachievements.main_grid.columns.jd',
            show: true,
            util: 'PX'
        },
        {
            name: 'yd',
            label: '月度',
            langtag: 'entities.pimachievements.main_grid.columns.yd',
            show: true,
            util: 'PX'
        },
        {
            name: 'khjl',
            label: '考核结论',
            langtag: 'entities.pimachievements.main_grid.columns.khjl',
            show: true,
            util: 'PX'
        },
        {
            name: 'evaluationunit',
            label: '考评单位',
            langtag: 'entities.pimachievements.main_grid.columns.evaluationunit',
            show: true,
            util: 'PX'
        },
    ]
    ...
    public rules: any = {
        khjl: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '考核结论 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '考核结论 值不能为空', trigger: 'blur' },
        ],
        pimpersonid: [
        ...
        srfkey: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '绩效信息标识 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '绩效信息标识 值不能为空', trigger: 'blur' },
        ],
    }
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'PIMACHIEVEMENTSGRPimJixGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'PIMACHIEVEMENTSGRPimJixGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        _datas.forEach((record: any, index: number) => {
            let srfmajortext = record.pimachievementsname;
            if (index < 5) {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ pimachievements: _keys }),Object.assign({ pimachievements: _keys },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    public rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.pimachievements,$event.pimachievements);
        });
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('pimachievements_main_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('pimachievements_main_grid', JSON.stringify(this.allColumns));
    }
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: '错误', desc: 'PIMACHIEVEMENTSGRPimJixGridView视图表格createAction参数未配置' });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: '错误', desc: 'PIMACHIEVEMENTSGRPimJixGridView视图表格updateAction参数未配置' });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
                        if(item.pimachievements){
                            Object.assign(this.context,{pimachievements:item.pimachievements});
                        }
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'PIMACHIEVEMENTSGRPimJixGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
    ...
    public getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.pimachievements,args.row.pimachievements);
        });
