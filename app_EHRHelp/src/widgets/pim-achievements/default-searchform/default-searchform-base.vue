...
export default class DefaultBase extends Vue implements ControlInterface {
    ...
    public rules: any = {
        ...
        n_khjl_eq: [
            { type: 'string', message: '考核结论(=) 值必须为字符串类型', trigger: 'change' },
            { type: 'string', message: '考核结论(=) 值必须为字符串类型', trigger: 'blur' },
            { required: false, type: 'string', message: '考核结论(=) 值不能为空', trigger: 'change' },
            { required: false, type: 'string', message: '考核结论(=) 值不能为空', trigger: 'blur' },
        ],
        n_xmjb_eq: [
            { type: 'string', message: '项目级别(=) 值必须为字符串类型', trigger: 'change' },
            { type: 'string', message: '项目级别(=) 值必须为字符串类型', trigger: 'blur' },
            { required: false, type: 'string', message: '项目级别(=) 值不能为空', trigger: 'change' },
            { required: false, type: 'string', message: '项目级别(=) 值不能为空', trigger: 'blur' },
        ],
        n_dj_eq: [
            { type: 'string', message: '档级(=) 值必须为字符串类型', trigger: 'change' },
            { type: 'string', message: '档级(=) 值必须为字符串类型', trigger: 'blur' },
            { required: false, type: 'string', message: '档级(=) 值不能为空', trigger: 'change' },
            { required: false, type: 'string', message: '档级(=) 值不能为空', trigger: 'blur' },
        ],
    }
...
, 
        n_khjl_eq: new FormItemModel({ caption: '考核结论(=)', detailType: 'FORMITEM', name: 'n_khjl_eq', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 3 })
, 
        n_xmjb_eq: new FormItemModel({ caption: '项目级别(=)', detailType: 'FORMITEM', name: 'n_xmjb_eq', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 3 })
, 
        n_dj_eq: new FormItemModel({ caption: '档级(=)', detailType: 'FORMITEM', name: 'n_dj_eq', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 3 })
, 
    };
     ...
     * @memberof DefaultBase
     */
    @Watch('data.n_khjl_eq')
    onN_khjl_eqChange(newVal: any, oldVal: any) {
        this.formDataChange({ name: 'n_khjl_eq', newVal: newVal, oldVal: oldVal });
    }
     ...
     * @memberof DefaultBase
     */
    @Watch('data.n_xmjb_eq')
    onN_xmjb_eqChange(newVal: any, oldVal: any) {
        this.formDataChange({ name: 'n_xmjb_eq', newVal: newVal, oldVal: oldVal });
    }
     ...
     */
    @Watch('data.n_dj_eq')
    onN_dj_eqChange(newVal: any, oldVal: any) {
        this.formDataChange({ name: 'n_dj_eq', newVal: newVal, oldVal: oldVal });
    }
    ...
    public load(opt: any = {}): void {
        if(!this.loadAction){
            this.$Notice.error({ title: '错误', desc: 'PIMACHIEVEMENTSGRPimJixGridView视图搜索表单loadAction参数未配置' });
            return;
        }
        const arg: any = { ...opt };
    ...
    public loadDraft(opt: any = {},mode?:string): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'PIMACHIEVEMENTSGRPimJixGridView视图搜索表单loaddraftAction参数未配置' });
            return;
        }
        const arg: any = { ...opt } ;
        Object.assign(arg,{viewparams:this.viewparams});
