...
<div class='grid' style="height:100%;">
    ...
    <el-table v-if="isDisplay === true"
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('seialno')">
                <el-table-column show-overflow-tooltip :prop="'seialno'" :label="$t('entities.pimqualtype.main_grid.columns.seialno')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimqualtype.main_grid.columns.seialno')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
                                <app-span name='seialno' editorType="TEXTBOX" :value="row.seialno"></app-span>
                        </template>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('pimqualtypename')">
                <el-table-column show-overflow-tooltip :prop="'pimqualtypename'" :label="$t('entities.pimqualtype.main_grid.columns.pimqualtypename')" :width="200"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimqualtype.main_grid.columns.pimqualtypename')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
                                <app-span name='pimqualtypename' editorType="TEXTBOX" :value="row.pimqualtypename"></app-span>
                        </template>
                    </template>
                </el-table-column>
            </template>
...
export default class MainBase extends Vue implements ControlInterface {
     ...
     * @memberof MainBase
     */
    public appEntityService: PimQualTypeService = new PimQualTypeService({ $store: this.$store });
     ...
     * @memberof MainBase
     */
    public minorSortPSDEF: string = 'seialno';
    ...
    */
    ...
    public allColumns: any[] = [
        {
            name: 'seialno',
            label: '排序号',
            langtag: 'entities.pimqualtype.main_grid.columns.seialno',
            show: true,
            util: 'PX'
        },
        {
            ...
            label: '岗位（技能）证书',
            langtag: 'entities.pimqualtype.main_grid.columns.pimqualtypename',
            show: true,
            util: 'PX'
        },
    ]
    ...
    public getGridRowModel(){
        return {
          seialno: new FormItemModel(),
          pimqualtypename: new FormItemModel(),
    ...
    public rules: any = {
        ...
        pimqualtypename: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '岗位（技能）证书 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '岗位（技能）证书 值不能为空', trigger: 'blur' },
        ],
        srfkey: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '岗位（技能）证书目录ID 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '岗位（技能）证书目录ID 值不能为空', trigger: 'blur' },
        ],
    }
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'PIMQUALTYPEPickupGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'PIMQUALTYPEPickupGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        _datas.forEach((record: any, index: number) => {
            let srfmajortext = record.pimqualtypename;
            if (index < 5) {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ pimqualtype: _keys }),Object.assign({ pimqualtype: _keys },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    public rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.pimqualtype,$event.pimqualtype);
        });
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('pimqualtype_main_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('pimqualtype_main_grid', JSON.stringify(this.allColumns));
    }
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: '错误', desc: 'PIMQUALTYPEPickupGridView视图表格createAction参数未配置' });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: '错误', desc: 'PIMQUALTYPEPickupGridView视图表格updateAction参数未配置' });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
                        if(item.pimqualtype){
                            Object.assign(this.context,{pimqualtype:item.pimqualtype});
                        }
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'PIMQUALTYPEPickupGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
    ...
    public getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.pimqualtype,args.row.pimqualtype);
        });
