...
<div class='grid' style="height:100%;">
    ...
    <el-table v-if="isDisplay === true"
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('wyyz')">
                <el-table-column show-overflow-tooltip :prop="'wyyz'" :label="$t('entities.pimlanguageability.main_language_grid.columns.wyyz')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimlanguageability.main_language_grid.columns.wyyz')}}
                      </span>
                    </template>
             ...
             <dropdown-list 
              ...
              @change="($event)=>{gridEditItemChange(row, column.property, $event, $index)}">
                        ...
                        <template v-if="!actualIsOpenEdit">
            <codelist :value="row.wyyz" tag='EhrCodeList0122' codelistType='DYNAMIC' ></codelist>
                        </template>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('wydj')">
                <el-table-column show-overflow-tooltip :prop="'wydj'" :label="$t('entities.pimlanguageability.main_language_grid.columns.wydj')" :width="300"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimlanguageability.main_language_grid.columns.wydj')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template v-if="actualIsOpenEdit">
             ...
             <dropdown-list 
              ...
              @change="($event)=>{gridEditItemChange(row, column.property, $event, $index)}">
                        ...
                        <template v-if="!actualIsOpenEdit">
            <codelist :value="row.wydj" tag='EhrCodeList0146' codelistType='DYNAMIC' ></codelist>
                        </template>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('wydjhqsj')">
                <el-table-column show-overflow-tooltip :prop="'wydjhqsj'" :label="$t('entities.pimlanguageability.main_language_grid.columns.wydjhqsj')" :width="200"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimlanguageability.main_language_grid.columns.wydjhqsj')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template v-if="actualIsOpenEdit">
                        ...
                        <template v-if="!actualIsOpenEdit">
                                <app-span name='wydjhqsj' editorType="DATEPICKER" :value="row.wydjhqsj"></app-span>
                        </template>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('fj')">
                <el-table-column show-overflow-tooltip :prop="'fj'" :label="$t('entities.pimlanguageability.main_language_grid.columns.fj')" :width="200"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimlanguageability.main_language_grid.columns.fj')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template v-if="actualIsOpenEdit">
            ...
            <template v-if="getColumnState('jlspzt')">
                <el-table-column show-overflow-tooltip :prop="'jlspzt'" :label="$t('entities.pimlanguageability.main_language_grid.columns.jlspzt')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimlanguageability.main_language_grid.columns.jlspzt')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
...
import Main_LanguageService from './main-language-grid-service';
...
export default class Main_LanguageBase extends Vue implements ControlInterface {

    /**
     * 名称
     *
     * @type {string}
     * @memberof Main_LanguageBase
     */
     ...
     * 建构部件服务对象
     *
     * @type {Main_LanguageService}
     * @memberof Main_LanguageBase
     */
    public service: Main_LanguageService = new Main_LanguageService({ $store: this.$store });
     ...
     * @memberof Main_LanguageBase
     */
    public appEntityService: PimLanguageAbilityService = new PimLanguageAbilityService({ $store: this.$store });
     ...
     * @memberof Main_LanguageBase
     */
    public minorSortPSDEF: string = 'wydjhqsj';
    ...
    */
    ...
    public allColumns: any[] = [
        {
            name: 'wyyz',
            label: '语种',
            langtag: 'entities.pimlanguageability.main_language_grid.columns.wyyz',
            show: true,
            util: 'PX'
        },
        {
            name: 'wydj',
            label: '外语等级',
            langtag: 'entities.pimlanguageability.main_language_grid.columns.wydj',
            show: true,
            util: 'PX'
        },
        {
            name: 'wydjhqsj',
            label: '外语等级获取时间',
            langtag: 'entities.pimlanguageability.main_language_grid.columns.wydjhqsj',
            show: true,
            util: 'PX'
        },
        {
            name: 'fj',
            label: '附件',
            langtag: 'entities.pimlanguageability.main_language_grid.columns.fj',
            show: true,
            util: 'PX'
        },
        {
            name: 'jlspzt',
            label: '审批状态',
            langtag: 'entities.pimlanguageability.main_language_grid.columns.jlspzt',
            show: true,
            util: 'PX'
        },
    ]
    ...
    public getGridRowModel(){
        return {
          ...
          wydjhqsj: new FormItemModel(),
          wyyz: new FormItemModel(),
          srfkey: new FormItemModel(),
          jlss: new FormItemModel(),
          wydj: new FormItemModel(),
        }
    }
    ...
    public rules: any = {
        fj: [
             { required: true, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '附件 值不能为空', trigger: 'change' },
            { required: true, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '附件 值不能为空', trigger: 'blur' },
        ],
        pimpersonid: [
        ...
        wydjhqsj: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '外语等级获取时间 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '外语等级获取时间 值不能为空', trigger: 'blur' },
        ],
        wyyz: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '语种 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '语种 值不能为空', trigger: 'blur' },
        ],
        srfkey: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '语言能力标识 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '语言能力标识 值不能为空', trigger: 'blur' },
        ],
        jlss: [
        ...
        wydj: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '外语等级 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '外语等级 值不能为空', trigger: 'blur' },
        ],
    }
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'PIMLANGUAGEABILITYGRYYNLGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'PIMLANGUAGEABILITYGRYYNLGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        _datas.forEach((record: any, index: number) => {
            let srfmajortext = record.pimlanguageabilityname;
            if (index < 5) {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ pimlanguageability: _keys }),Object.assign({ pimlanguageability: _keys },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    public rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.pimlanguageability,$event.pimlanguageability);
        });
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('pimlanguageability_main_language_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('pimlanguageability_main_language_grid', JSON.stringify(this.allColumns));
    }
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: '错误', desc: 'PIMLANGUAGEABILITYGRYYNLGridView视图表格createAction参数未配置' });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: '错误', desc: 'PIMLANGUAGEABILITYGRYYNLGridView视图表格updateAction参数未配置' });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
                        if(item.pimlanguageability){
                            Object.assign(this.context,{pimlanguageability:item.pimlanguageability});
                        }
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'PIMLANGUAGEABILITYGRYYNLGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
    ...
    public getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.pimlanguageability,args.row.pimlanguageability);
        });
...
<style lang='less'>
@import './main-language-grid.less';
</style>