<template>
  ...
  <row>
    <i-col span="20" class="form-content">
      <row>
                    <i-col v-show="detailsModel.n_ygbh_like.visible" :style="{}"  :lg="{ span: 8, offset: 0 }">
              <app-form-item name='n_ygbh_like' :itemRules="this.rules.n_ygbh_like" class='' :caption="$t('entities.pimarchives.default_searchform.details.n_ygbh_like')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.n_ygbh_like.error" :isEmptyCaption="false" labelPos="LEFT"> 
          ...
          <i-col v-show="detailsModel.n_pimpersonname_like.visible" :style="{}"  :lg="{ span: 8, offset: 0 }">
              <app-form-item name='n_pimpersonname_like' :itemRules="this.rules.n_pimpersonname_like" class='' :caption="$t('entities.pimarchives.default_searchform.details.n_pimpersonname_like')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.n_pimpersonname_like.error" :isEmptyCaption="false" labelPos="LEFT"> 
          ...
          <i-col v-show="detailsModel.n_dabh_like.visible" :style="{}"  :lg="{ span: 8, offset: 0 }">
              <app-form-item name='n_dabh_like' :itemRules="this.rules.n_dabh_like" class='' :caption="$t('entities.pimarchives.default_searchform.details.n_dabh_like')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.n_dabh_like.error" :isEmptyCaption="false" labelPos="LEFT"> 
              <input-box v-model="data.n_dabh_like"  @enter="onEnter($event)"    :disabled="detailsModel.n_dabh_like.disabled" type='text'  style=""></input-box>
          </app-form-item>
...
export default class DefaultBase extends Vue implements ControlInterface {
    ...
    public rules: any = {
        ...
        n_dabh_like: [
            { type: 'string', message: '档案编号 值必须为字符串类型', trigger: 'change' },
            { type: 'string', message: '档案编号 值必须为字符串类型', trigger: 'blur' },
            { required: false, type: 'string', message: '档案编号 值不能为空', trigger: 'change' },
            { required: false, type: 'string', message: '档案编号 值不能为空', trigger: 'blur' },
        ],
        n_ygzt_eq: [
        ...
        n_dastate_eq: [
            { type: 'string', message: '档案状态 值必须为字符串类型', trigger: 'change' },
            { type: 'string', message: '档案状态 值必须为字符串类型', trigger: 'blur' },
            { required: false, type: 'string', message: '档案状态 值不能为空', trigger: 'change' },
            { required: false, type: 'string', message: '档案状态 值不能为空', trigger: 'blur' },
        ],
        n_zt_eq: [
            { type: 'string', message: '档案借阅状态 值必须为字符串类型', trigger: 'change' },
            { type: 'string', message: '档案借阅状态 值必须为字符串类型', trigger: 'blur' },
            { required: false, type: 'string', message: '档案借阅状态 值不能为空', trigger: 'change' },
            { required: false, type: 'string', message: '档案借阅状态 值不能为空', trigger: 'blur' },
        ],
    }
...
, 
        n_dabh_like: new FormItemModel({ caption: '档案编号', detailType: 'FORMITEM', name: 'n_dabh_like', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 3 })
, 
...
, 
        n_dastate_eq: new FormItemModel({ caption: '档案状态', detailType: 'FORMITEM', name: 'n_dastate_eq', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 3 })
, 
        n_zt_eq: new FormItemModel({ caption: '档案借阅状态', detailType: 'FORMITEM', name: 'n_zt_eq', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 3 })
, 
    };
    ...
    /**
     * 监控表单属性 n_dabh_like 值
     *
     * @param {*} newVal
     * @param {*} oldVal
     * @memberof DefaultBase
     */
    @Watch('data.n_dabh_like')
    onN_dabh_likeChange(newVal: any, oldVal: any) {
        this.formDataChange({ name: 'n_dabh_like', newVal: newVal, oldVal: oldVal });
    }
    ...
    /**
     * 监控表单属性 n_dastate_eq 值
     *
     * @param {*} newVal
     * @param {*} oldVal
     * @memberof DefaultBase
     */
    @Watch('data.n_dastate_eq')
    onN_dastate_eqChange(newVal: any, oldVal: any) {
        this.formDataChange({ name: 'n_dastate_eq', newVal: newVal, oldVal: oldVal });
    }
    ...
    public load(opt: any = {}): void {
        if(!this.loadAction){
            this.$Notice.error({ title: '错误', desc: 'PIMARCHIVESGRPimDangaGridView视图搜索表单loadAction参数未配置' });
            return;
        }
        const arg: any = { ...opt };
    ...
    public loadDraft(opt: any = {},mode?:string): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'PIMARCHIVESGRPimDangaGridView视图搜索表单loaddraftAction参数未配置' });
            return;
        }
        const arg: any = { ...opt } ;
        Object.assign(arg,{viewparams:this.viewparams});
