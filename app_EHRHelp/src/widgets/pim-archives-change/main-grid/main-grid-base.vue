        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
                ...
                <el-table-column show-overflow-tooltip :prop="'ddjltype'" :label="$t('entities.pimarchiveschange.main_grid.columns.ddjltype')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimarchiveschange.main_grid.columns.ddjltype')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
            <codelist :value="row.ddjltype" tag='EhrCodeList0227' codelistType='STATIC' renderMode="STR" valueSeparator=";" textSeparator="、" ></codelist>
                        </template>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('ormorgname')">
                <el-table-column show-overflow-tooltip :prop="'ormorgname'" :label="$t('entities.pimarchiveschange.main_grid.columns.ormorgname')" :width="250"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimarchiveschange.main_grid.columns.ormorgname')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('ormorgname2')">
                <el-table-column show-overflow-tooltip :prop="'ormorgname2'" :label="$t('entities.pimarchiveschange.main_grid.columns.ormorgname2')" :width="250"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimarchiveschange.main_grid.columns.ormorgname2')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('jlcssj')">
                <el-table-column show-overflow-tooltip :prop="'jlcssj'" :label="$t('entities.pimarchiveschange.main_grid.columns.jlcssj')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimarchiveschange.main_grid.columns.jlcssj')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('bz')">
                <el-table-column show-overflow-tooltip :prop="'bz'" :label="$t('entities.pimarchiveschange.main_grid.columns.bz')" :min-width="300"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimarchiveschange.main_grid.columns.bz')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.bz}}</span>
...
export default class MainBase extends Vue implements ControlInterface {
    ...
    */
    ...
    public allColumns: any[] = [
        {
            name: 'ddjltype',
            label: '调档记录类型',
            langtag: 'entities.pimarchiveschange.main_grid.columns.ddjltype',
            show: true,
            util: 'px'
        },
        {
            name: 'ormorgname',
            label: '调出单位',
            langtag: 'entities.pimarchiveschange.main_grid.columns.ormorgname',
            show: true,
            util: 'PX'
        },
        {
            name: 'ormorgname2',
            label: '调入单位',
            langtag: 'entities.pimarchiveschange.main_grid.columns.ormorgname2',
            show: true,
            util: 'PX'
        },
        {
            name: 'jlcssj',
            label: '调动时间',
            langtag: 'entities.pimarchiveschange.main_grid.columns.jlcssj',
            show: true,
            util: 'PX'
        },
        {
            name: 'bz',
            label: '备注',
            langtag: 'entities.pimarchiveschange.main_grid.columns.bz',
            show: true,
            util: 'STAR'
        },
    ]
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'PIMARCHIVESCHANGEGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'PIMARCHIVESCHANGEGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('pimarchiveschange_main_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('pimarchiveschange_main_grid', JSON.stringify(this.allColumns));
    }
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: '错误', desc: 'PIMARCHIVESCHANGEGridView视图表格createAction参数未配置' });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: '错误', desc: 'PIMARCHIVESCHANGEGridView视图表格updateAction参数未配置' });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'PIMARCHIVESCHANGEGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
