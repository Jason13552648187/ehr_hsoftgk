    ...
    <el-table v-if="isDisplay === true"
        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
            ...
            <template v-if="getColumnState('pimresearchfindingsname')">
                <el-table-column show-overflow-tooltip :prop="'pimresearchfindingsname'" :label="$t('entities.pimresearchfindings.main_2_grid.columns.pimresearchfindingsname')" :width="200"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimresearchfindings.main_2_grid.columns.pimresearchfindingsname')}}
                      </span>
                    </template>
              ...
              @change="($event)=>{gridEditItemChange(row, column.property, $event, $index)}">
                        ...
                        <template v-if="!actualIsOpenEdit">
                                <app-span name='pimresearchfindingsname' editorType="TEXTBOX" :value="row.pimresearchfindingsname"></app-span>
                        </template>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('hqsj')">
                <el-table-column show-overflow-tooltip :prop="'hqsj'" :label="$t('entities.pimresearchfindings.main_2_grid.columns.hqsj')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimresearchfindings.main_2_grid.columns.hqsj')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template v-if="actualIsOpenEdit">
                        ...
                        <template v-if="!actualIsOpenEdit">
                                <app-span name='hqsj' editorType="DATEPICKER" :value="row.hqsj"></app-span>
                        </template>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('fj')">
                <el-table-column show-overflow-tooltip :prop="'fj'" :label="$t('entities.pimresearchfindings.main_2_grid.columns.fj')" :width="200"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimresearchfindings.main_2_grid.columns.fj')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template v-if="actualIsOpenEdit">
            ...
            <template v-if="getColumnState('jlspzt')">
                <el-table-column show-overflow-tooltip :prop="'jlspzt'" :label="$t('entities.pimresearchfindings.main_2_grid.columns.jlspzt')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimresearchfindings.main_2_grid.columns.jlspzt')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
...
export default class Main_2Base extends Vue implements ControlInterface {
     ...
     * @memberof Main_2Base
     */
    public minorSortPSDEF: string = 'hqsj';
    ...
    */
    ...
    public allColumns: any[] = [
        {
            ...
            label: '科研成果名称(*)',
            langtag: 'entities.pimresearchfindings.main_2_grid.columns.pimresearchfindingsname',
            show: true,
            util: 'PX'
        },
        {
            name: 'hqsj',
            label: '获取时间(*)',
            langtag: 'entities.pimresearchfindings.main_2_grid.columns.hqsj',
            show: true,
            util: 'PX'
        },
        {
            name: 'fj',
            label: '附件',
            langtag: 'entities.pimresearchfindings.main_2_grid.columns.fj',
            show: true,
            util: 'PX'
        },
        {
            name: 'jlspzt',
            label: '审批状态',
            langtag: 'entities.pimresearchfindings.main_2_grid.columns.jlspzt',
            show: true,
            util: 'PX'
        },
    ]
    ...
    public getGridRowModel(){
        return {
          ...
          hqsj: new FormItemModel(),
          pimresearchfindingsname: new FormItemModel(),
    ...
    public rules: any = {
        ...
        hqsj: [
             { required: true, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '获取时间(*) 值不能为空', trigger: 'change' },
            { required: true, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '获取时间(*) 值不能为空', trigger: 'blur' },
        ],
        pimresearchfindingsname: [
             { required: true, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '科研成果名称(*) 值不能为空', trigger: 'change' },
            { required: true, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '科研成果名称(*) 值不能为空', trigger: 'blur' },
        ],
        pimpersonid: [
        ...
        srfkey: [
             { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '科研成果标识 值不能为空', trigger: 'change' },
            { required: false, validator: (rule:any, value:any, callback:any) => { return (rule.required && (value === null || value === undefined || value === "")) ? false : true;}, message: '科研成果标识 值不能为空', trigger: 'blur' },
        ],
        jlss: [
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'PIMRESEARCHFINDINGSGRRESEARCHGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'PIMRESEARCHFINDINGSGRRESEARCHGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        _datas.forEach((record: any, index: number) => {
            let srfmajortext = record.pimresearchfindingsname;
            if (index < 5) {
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ pimresearchfindings: _keys }),Object.assign({ pimresearchfindings: _keys },{viewparams:this.viewparams}), this.showBusyIndicator);
    ...
    public rowClick($event: any, ifAlways: boolean = false): void {
        ...
        let selectIndex = this.selections.findIndex((item:any)=>{
            return Object.is(item.pimresearchfindings,$event.pimresearchfindings);
        });
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('pimresearchfindings_main_2_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('pimresearchfindings_main_2_grid', JSON.stringify(this.allColumns));
    }
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: '错误', desc: 'PIMRESEARCHFINDINGSGRRESEARCHGridView视图表格createAction参数未配置' });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: '错误', desc: 'PIMRESEARCHFINDINGSGRRESEARCHGridView视图表格updateAction参数未配置' });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
                        if(item.pimresearchfindings){
                            Object.assign(this.context,{pimresearchfindings:item.pimresearchfindings});
                        }
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'PIMRESEARCHFINDINGSGRRESEARCHGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
    ...
    public getRowClassName(args:{row: any,rowIndex: number}){
        let isSelected = this.selections.some((item:any)=>{
            return Object.is(item.pimresearchfindings,args.row.pimresearchfindings);
        });
