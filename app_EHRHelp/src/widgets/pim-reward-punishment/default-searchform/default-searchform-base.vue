  ...
  <row>
    <i-col span="20" class="form-content">
      <row>
              ...
              <app-form-item name='n_pimpersonname_like' :itemRules="this.rules.n_pimpersonname_like" class='' :caption="$t('entities.pimrewardpunishment.default_searchform.details.n_pimpersonname_like')" uiStyle="DEFAULT" :labelWidth="130" :isShowCaption="true" :error="detailsModel.n_pimpersonname_like.error" :isEmptyCaption="false" labelPos="LEFT"> 
...
export default class DefaultBase extends Vue implements ControlInterface {
    ...
    public rules: any = {
        ...
        n_lx_eq: [
            { type: 'string', message: '奖惩分类(=) 值必须为字符串类型', trigger: 'change' },
            { type: 'string', message: '奖惩分类(=) 值必须为字符串类型', trigger: 'blur' },
            { required: false, type: 'string', message: '奖惩分类(=) 值不能为空', trigger: 'change' },
            { required: false, type: 'string', message: '奖惩分类(=) 值不能为空', trigger: 'blur' },
        ],
    }
...
, 
        n_lx_eq: new FormItemModel({ caption: '奖惩分类(=)', detailType: 'FORMITEM', name: 'n_lx_eq', visible: true, isShowCaption: true, form: this, disabled: false, enableCond: 3 })
, 
    };
    ...
    public load(opt: any = {}): void {
        if(!this.loadAction){
            this.$Notice.error({ title: '错误', desc: 'PIMREWARDPUNISHMENTGRPimJanglGridView视图搜索表单loadAction参数未配置' });
            return;
        }
        const arg: any = { ...opt };
    ...
    public loadDraft(opt: any = {},mode?:string): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'PIMREWARDPUNISHMENTGRPimJanglGridView视图搜索表单loaddraftAction参数未配置' });
            return;
        }
        const arg: any = { ...opt } ;
        Object.assign(arg,{viewparams:this.viewparams});
