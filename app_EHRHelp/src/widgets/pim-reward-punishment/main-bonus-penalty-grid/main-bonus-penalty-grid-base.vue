        ...
        ref='multipleTable' :data="items" :show-header="!isHideHeader">
                ...
                <el-table-column show-overflow-tooltip :prop="'lx'" :label="$t('entities.pimrewardpunishment.main_bonuspenalty_grid.columns.lx')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimrewardpunishment.main_bonuspenalty_grid.columns.lx')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
            <codelist :value="row.lx" tag='EhrCodeList0124' codelistType='STATIC' renderMode="STR" valueSeparator=";" textSeparator="、" ></codelist>
                        </template>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('jcjb')">
                <el-table-column show-overflow-tooltip :prop="'jcjb'" :label="$t('entities.pimrewardpunishment.main_bonuspenalty_grid.columns.jcjb')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimrewardpunishment.main_bonuspenalty_grid.columns.jcjb')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
            <codelist :value="row.jcjb" tag='EhrCodeList0159' codelistType='DYNAMIC' renderMode="STR" valueSeparator=";" textSeparator="、" ></codelist>
                        </template>
                    </template>
                </el-table-column>
            </template>
            <template v-if="getColumnState('jlsj')">
                <el-table-column show-overflow-tooltip :prop="'jlsj'" :label="$t('entities.pimrewardpunishment.main_bonuspenalty_grid.columns.jlsj')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimrewardpunishment.main_bonuspenalty_grid.columns.jlsj')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
            ...
            <template v-if="getColumnState('jcsj')">
                <el-table-column show-overflow-tooltip :prop="'jcsj'" :label="$t('entities.pimrewardpunishment.main_bonuspenalty_grid.columns.jcsj')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimrewardpunishment.main_bonuspenalty_grid.columns.jcsj')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <span>{{row.jcsj}}</span>
            ...
            <template v-if="getColumnState('jlspzt')">
                <el-table-column show-overflow-tooltip :prop="'jlspzt'" :label="$t('entities.pimrewardpunishment.main_bonuspenalty_grid.columns.jlspzt')" :width="150"  :align="'left'" :sortable="'custom'">
                    <template v-slot:header="{column}">
                      <span class="column-header ">
                        {{$t('entities.pimrewardpunishment.main_bonuspenalty_grid.columns.jlspzt')}}
                      </span>
                    </template>
                    <template v-slot="{row,column,$index}">
                        <template >
...
import Main_BonusPenaltyService from './main-bonus-penalty-grid-service';
...
export default class Main_BonusPenaltyBase extends Vue implements ControlInterface {

    /**
     * 名称
     *
     * @type {string}
     * @memberof Main_BonusPenaltyBase
     */
     ...
     * 建构部件服务对象
     *
     * @type {Main_BonusPenaltyService}
     * @memberof Main_BonusPenaltyBase
     */
    public service: Main_BonusPenaltyService = new Main_BonusPenaltyService({ $store: this.$store });
    ...
    */
    ...
    public allColumns: any[] = [
        {
            name: 'lx',
            label: '奖惩分类',
            langtag: 'entities.pimrewardpunishment.main_bonuspenalty_grid.columns.lx',
            show: true,
            util: 'px'
        },
        {
            name: 'jcjb',
            label: '奖惩级别',
            langtag: 'entities.pimrewardpunishment.main_bonuspenalty_grid.columns.jcjb',
            show: true,
            util: 'px'
        },
        {
            name: 'jlsj',
            label: '奖惩时间',
            langtag: 'entities.pimrewardpunishment.main_bonuspenalty_grid.columns.jlsj',
            show: true,
            util: 'px'
        },
        {
            name: 'jcsj',
            label: '奖惩事件',
            langtag: 'entities.pimrewardpunishment.main_bonuspenalty_grid.columns.jcsj',
            show: true,
            util: 'px'
        },
        {
            name: 'jlspzt',
            label: '审批状态',
            langtag: 'entities.pimrewardpunishment.main_bonuspenalty_grid.columns.jlspzt',
            show: true,
            util: 'PX'
        },
    ]
    ...
    public load(opt: any = {}, pageReset: boolean = false): void {
        if(!this.fetchAction){
            this.$Notice.error({ title: '错误', desc: 'PIMREWARDPUNISHMENTGRPimJanglGridView视图表格fetchAction参数未配置' });
            return;
        }
        if(pageReset){
            this.curPage = 1;
    ...
    public async remove(datas: any[]): Promise<any> {
        if(!this.removeAction){
            this.$Notice.error({ title: '错误', desc: 'PIMREWARDPUNISHMENTGRPimJanglGridView视图表格removeAction参数未配置' });
            return;
        }
        let _datas:any[] = [];
        ...
        const removeData = () => {
            ...
            const post: Promise<any> = this.service.delete(_removeAction,Object.assign(context,{ pimrewardpunishment: _keys }),Object.assign({ pimrewardpunishment: _keys },{viewparams:this.viewparams}), this.showBusyIndicator);
     ...
     */
    public setColState() {
		const _data: any = localStorage.getItem('pimrewardpunishment_main_bonuspenalty_grid');
		if (_data) {
     ...
     */
    public onColChange() {
        localStorage.setItem('pimrewardpunishment_main_bonuspenalty_grid', JSON.stringify(this.allColumns));
    }
            ...
            try {
                if(Object.is(item.rowDataState, 'create')){
                    if(!this.createAction){
                        this.$Notice.error({ title: '错误', desc: 'PIMREWARDPUNISHMENTGRPimJanglGridView视图表格createAction参数未配置' });
                    }else{
                    ...
                    if(!this.updateAction){
                        this.$Notice.error({ title: '错误', desc: 'PIMREWARDPUNISHMENTGRPimJanglGridView视图表格updateAction参数未配置' });
                    }else{
                        Object.assign(item,{viewparams:this.viewparams});
    ...
    public newRow(args: any[], params?: any, $event?: any, xData?: any): void {
        if(!this.loaddraftAction){
            this.$Notice.error({ title: '错误', desc: 'PIMREWARDPUNISHMENTGRPimJanglGridView视图表格loaddraftAction参数未配置' });
            return;
        }
        let _this = this;
...
<style lang='less'>
@import './main-bonus-penalty-grid.less';
</style>